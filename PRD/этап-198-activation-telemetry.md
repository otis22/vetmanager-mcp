...
- `reason_class` mapping: `AuthError` → `auth_error`,
  ...
  `VetmanagerError` → `vetmanager_error`, `ValueError` validation paths →
  `validation_error`, CSRF validation path → `csrf_error`, anything else →
  `unknown`.
...
- Cardinality budget for `vetmanager_activation_event_accounts`: event names
  (3) × device classes (3) × auth modes (3) × reasons (6) = maximum 162 series.
...
- event metric cardinality budget is 162 series.
