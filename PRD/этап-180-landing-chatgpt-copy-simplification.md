...
- Заменить copy в существующей секции `#chatgpt-connector` и кратком hero-note,
  который ведёт к этой секции.
...
- Hero-note остаётся коротким и не дублирует подробную инструкцию подключения.
- В ChatGPT landing copy не появляется ручной bearer token language.
...
5. Hero-note использует короткую формулировку `Работает прямо в ChatGPT` и не
   содержит старую инструкционную фразу `права выбираются при подключении`.
6. В ChatGPT landing copy нет manual credential language: `Bearer`, `API key`,
   `service token`, `service bearer`.
7. Regression test закрепляет новый hero-note, заголовок, оба предложения, CTA
   и отсутствие manual credential language в ChatGPT-секции.
8. Тесты лендинга проходят.
