...
4. В `tools/client.py` использовать узкий denylist только для
   ...
   `get_clients` и `get_client_by_id`. Allowlist здесь намеренно не применяется:
   клиентские поля широки и нужны агентам, а решение касается одного
   подтверждённого паспортного поля.
5. Сохранить transport envelope целиком независимо от `success` и всегда
   ...
   Неизвестная форма `data`
   не меняет контракт ответа, но фиксируется штатным runtime logger; её
   найденные user records всё равно проходят allowlist.
