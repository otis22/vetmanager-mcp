...
- Установить приоритет free-text PII redaction: сначала найти e-mail и phone
  ...
  PII candidate. Pure date/date-time candidates не считаются телефоном.
...
3. Сначала собрать e-mail и phone candidates; сохранить date/time только если
   они не перекрыты PII. Candidate, составленный только из date tokens и
   разделителей, не считать телефоном.
...
косметическая помеха, открытый телефон или e-mail — privacy leak. Добавить
табличные регрессии на оба наблюдённых формата даты, ISO `T`/timezone, локальную
дату, телефон непосредственно после даты, phone с четырёхзначным кодом города
и phone-like e-mail. Runner должен отклонять evidence path внутри repo.

### Инварианты

- Настоящий телефон в free text по-прежнему заменяется на `[redacted-phone]`.
- Date/time сохраняется, если не пересекается с найденным PII.
