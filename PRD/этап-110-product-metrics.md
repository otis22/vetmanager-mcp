...
CLI:
- 30-day window hardcoded (stage 116.1 removed the earlier `--window-days` flag — оно было half-wired)
...
- `rate_limited`, `revoked`, `expired`, `ip_denied`, `no_scopes`, `no_connection` (stage 116.3: `disabled` убран — нет соответствующего event'а в auth_audit)
