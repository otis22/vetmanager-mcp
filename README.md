...
- `SUPPORT_EMAIL` — адрес поддержки. Показывается **только в кабинете** вошедшему пользователю; на публичный лендинг не попадает никогда (этап 284) — иначе адрес собирают спам-боты, парсящие HTML. Не задан или задан с мусором — в кабинете остаётся только ссылка на GitHub Issues. В коде адреса нет: репозиторий открытый, и вписанная в исходник почта осталась бы в истории git навсегда.
- `CUSTOM_CLINIC_HOSTS` — клиники, открывающие Vetmanager на собственном домене, в формате `ключ_аккаунта=хост` через запятую (`clinic=vm.clinic.example`). Ключ аккаунта — тот, по которому клиника заведена в billing-api; из адреса он не выводится. Проверка **добавляется** к публичным зонам `vetmanager.cloud` / `vetmanager2.ru`, а не заменяет их, и привязывает адрес к конкретному аккаунту. Пустая переменная = поведение до этапа 292. Значения задаются только в `.env` на сервере: это чужие адреса, и в открытом репозитории им не место — в коде остаётся имя переменной.
...
- `PERSISTENT_LOG_PATH` — каталог persistent structured application logs; Compose использует named volume. Файл режется по 10 МиБ, хранение — **14 UTC-дней**, объём ограничен отдельным бюджетом в 2 ГиБ. Раньше рядом стоял ещё плоский лимит «не больше 14 файлов», и он бил первым: под нагрузкой файлов за сутки выходит полтора десятка, поэтому две недели молча превращались в один день (этап 302). Срок хранения и бюджет объёма — разные ограничения, и второе не должно подменять первое. Не задавайте путь вне управляемого volume.
...
- service metrics:
  ...
  - `vetmanager_report_ai_jobs_total{outcome}`, `vetmanager_report_ai_job_transitions_total{from_stage,to_stage}`, `vetmanager_report_ai_job_terminal_outcomes_total{outcome}` и `vetmanager_report_ai_job_{stage_,}duration_seconds_{count,sum,max}` — process-local наблюдение создания и lifecycle Report AI. Эти series считают локальные наблюдения, а не уникальные задания: `abandoned_wait` означает отсутствие следующего MCP poll до local TTL, не состояние очереди Ветменеджера; если polling возобновится после TTL, то же задание начнёт новое observation и может дать второй sample;
  - `vetmanager_report_ai_exports_total{operation,outcome}` + `vetmanager_report_ai_export_duration_seconds_{count,sum,max}` — отдельно наблюдаемые start/poll export attempts; duration начинается только после успешного `StartReport` и не содержит URLs, IDs или clinic data;
...
#### Тестовый контур: какой стенд считается нашим

**Наш тестовый стенд один — `devtr6`.** Решение владельца 02.09.2026: держать
два контура значит проверять не то. На `devtr6` проходят живые пробы, включая
запись медкарты, и именно он записан выделенным тестовым стендом с этапа 5.
`TEST_DOMAIN` / `TEST_API_KEY` указывают на него.

**Расхождение, которое пока сохраняется.** Bearer-токен тестового MCP-клиента
(тот, которым ходят инструменты из Cursor или Claude) указывает на **другой**
стенд — `devslon67` (`devslon67.vetmanager2.ru`). Сборки разные: одна и та же
запись медкарты проходит на `devtr6` и падает с 500 на `devslon67`. Перевод
токена требует действия на стороне Ветменеджера — контур за токеном выдаём
не мы (этап 255.4).

Отсюда правило до перевода: **фраза «проверил на devtr6» неполна**. Указывать,
чем именно проверял:

| Чем проверяю | Какой стенд отвечает |
|---|---|
| сырой REST с `TEST_API_KEY` | `devtr6` |
| MCP-инструмент под bearer-токеном | `devslon67` |

Проверить, где находишься, можно по числу клиник: на `devtr6` их две, под
bearer-токеном — одна. Обнаружилось это неприятным способом: медкарта с `id = 6`
на двух стендах разная, и проверка едва не пошла по чужим данным.
