...
**106 инструментов** по 13 группам сущностей:
...
| Finance | `get_payments`, `get_payment_by_id`, `create_payment` ⚠️, `get_invoice_documents`, `get_invoice_document_by_id`, `add_invoice_document`, `delete_invoice_document`, `get_closing_of_invoices`, `get_closing_of_invoice_by_id`, `get_cassas`, `get_cassa_by_id`, `get_cassa_closes`, `get_cassa_close_by_id` | 13 |
...
⚠️ `create_payment` существует как MCP-tool, но Vetmanager REST API Payment entity разрешает только `restList`/`restView` (см. таблицу ниже и `artifacts/api_crud_permissions-ru.md`). Фактическое поведение wrapper'а на проде не подтверждено — в будущих релизах инструмент может быть убран или помечен deprecated.
