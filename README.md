...
- service metrics:
  ...
  - `vetmanager_report_ai_jobs_total{outcome}`, `vetmanager_report_ai_job_transitions_total{from_stage,to_stage}`, `vetmanager_report_ai_job_terminal_outcomes_total{outcome}` и `vetmanager_report_ai_job_{stage_,}duration_seconds_{count,sum,max}` — process-local наблюдение создания и lifecycle Report AI; stage vocabulary ограничен, а `abandoned_wait` означает только отсутствие следующего MCP poll до local TTL, не состояние очереди Ветменеджера;
