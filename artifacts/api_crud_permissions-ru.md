...
> **Оговорка этапа 268 (31.08.2026).** «API разрешает» ниже означает только то,
> что REST-путь отвечает, а не то, что операция делает правильную вещь.
> `delete_invoice` и `delete_invoice_document` были добавлены по этой таблице и
> сняты обратно: generic REST DELETE не выполняет правил Ветменеджера для
> удаления счёта. Прежде чем выводить write-инструмент из этой таблицы,
> проверяй, совпадает ли generic-путь с предметной операцией.
...
| **Invoice** | update | + (полный CRUD явно разрешён; **delete снят этапом 268 — не добавлять**) |
| **InvoiceDocument** | create, update | + (полный CRUD через наследование; **delete снят этапом 268 — не добавлять**) |
