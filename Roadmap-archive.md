...
- 52.3.3 Реализовать server-side session revocation — `stop` (вынесено в этап 282;
  отложено при закрытии этапа 52, требует хранения сессий в БД)
...
### 53.2 Разделение vetmanager_client.py — `done`

(неактуально: 253 строки, дальнейшее дробление не оправдано)

### 53.3 Устранение связанности — `done`

(неактуально: web_security.py 195 строк, session-паттерн используется 3 раза)
...
## Этап 92. Auth consolidation (F10) — `done`
...
## Этап 93. Architecture: FilterBuilder + service/repository layer (H11, H21) — `done`

(focused subset; full Resource class — backlog)
...
## Этап 109. Test brittleness & coverage gaps — `done`
...
## Этап 113. Resilience completeness (super-review 2026-04-19) — `done`
...
- 114b.1 Codebase-wide inline imports audit — grep `^\s+(from|import) ` в function bodies; для каждого case (≤27 total в src) либо keep+docstring с rationale (доказанный циркулярный импорт), либо fix. — `done`
- 114b.2 3-hop indirection collapse — `tools/client.py:345-381` `get_client_profile` + `tools/pet.py:172-205` `get_pet_profile`: заменить `_impl` closure + `_get_*_profile_impl` на direct `instrument_call(lambda: fetch(...))`. — `done`
- 114b.3 FilterBuilder migration в resources — `resources/client_profile.py:36-68` + `resources/pet_profile.py:35-42` + `tools/medical_card.py:80-83,111-114`: заменить 4 hand-rolled `json.dumps([_filter_eq(...).to_dict()], separators=(',',':'))` на `filters.build_list_query_params(...)`. — `done`
- 114b.4 BC shim follow-up review — если grep подтверждает 0 callers для `tools/_aggregation.py` / `request_credentials.py`, удалить + update `test_stage109_bc_invariants.py`. Stage 114 decided **keep all** для current cycle; re-visit через 3-6 месяцев или при migration trigger. — `done` (keep policy confirmed)
...
- 115.1 `tests/test_stage105_breaker_amplification.py` — добавить test с 2+ concurrent `VetmanagerClient.get()` calls через `asyncio.gather`, mock timeout через `transport=httpx.MockTransport`, assert `consecutive_failures <= threshold` после burst. — `done`
- 115.2 `tests/test_stage106_reliability.py:99-120` — переписать `test_concurrent_first_init_creates_single_pool_client` на behavioral (8× concurrent `get_shared_http_client()` assert all identical) вместо inspection `_shared_http_clients` dict. — `done`
- 115.3 Autouse `reset_service_metrics()` fixture в `tests/conftest.py` — устраняет state leak между тестами (business_events_total, auth_failures, etc.). — `done`
- 115.4 BC-invariant дополнения — `tests/test_stage109_bc_invariants.py` добавить identity check для `force_breaker_open` и `get_breaker_state` re-exports. — `done`
- 115.5 Property-based test на breaker state machine (опционально, если останется время). — `done` (skipped by design)
...
- 116.1 **F4a:** Full `--window-days` propagation — `scripts/product_metrics_report.py` прокинуть `window_days` во все `_count_events`, `_count_live_accounts`, `_count_dead_accounts`, failures breakdowns. Renamed keys: `live_Nd`, `dead_Nd`, `total_Nd`. Label в Markdown header = f"window: {window_days}d". Либо **F4b:** убрать `--window-days` CLI флаг, хардкодить 30d с явным docstring «fixed 30-day window». — `done` (chosen F4b)
- 116.2 Implement `tokens.expired_auto_24h` counter (PRD 110.1 acceptance) — query `TOKEN_EVENT_EXPIRED` events за 24h; добавить в tokens section. Либо удалить из PRD если не нужно. — `done`
- 116.3 `disabled` failure reason — либо добавить `TOKEN_EVENT_AUTH_FAILED_DISABLED` constant в `auth_audit.py` и emit на disabled-token path, либо удалить из PRD 110 (строка 54). — `done` (removed from PRD)
- 116.4 PRD sync: `--window=30d` → `--window-days=30` (PRD 110 lines 23, 102); example SSH command добавить `--profile production`. — `done`
- 116.5 `test_record_business_event` extended — проверяет все 4 required events (`account_registered`, `web_login_succeeded`, `bearer_token_issued`, `bearer_token_revoked`), не 2. — `done`
- 116.6 Boundary tests для `_mask_email` — empty string, no `@`, multi-`@`, unicode domain, 2-char local. — `done` (deferred/accepted not needed)
- 116.7 Off-by-one в dead-account cutoff — `scripts/product_metrics_report.py:106-120` `Account.created_at < cutoff` → `<= cutoff` (или задокументировать strict behavior). — `done` (strict behavior documented)
...
- 117.1 `artifacts/technical-requirements-vetmanager-mcp-ru.md` — append §2 changelog для этапов 97-116 (1-line each); обновить §3.3 structure tree (`scripts/`, `.claude/commands/`); добавить в §3.2 секции про breaker, probes, business_events, metrics_registry. — `done`
- 117.2 Backfill AssumptionLog commit SHAs — `AssumptionLog.md:4499, 4539, 4554` заменить `(pending)` на `778cddc` (110), `3d4f75f` (109.10), `3234e09` (109 full subset). — `done`
- 117.3 `artifacts/observability-runbook-vetmanager-mcp-ru.md` — добавить секцию для stage-88 (`vetmanager_upstream_requests_total`, `vetmanager_tool_calls_total`) и stage-110 (`vetmanager_business_events_total`) метрик с example PromQL. — `done`
- 117.4 README Observability section — добавить bullet про `vetmanager_business_events_total{event=...}` (строка 138). — `done`
- 117.5 `scripts/review_workflow_check.sh` — добавить check на `(pending)` в AssumptionLog последних 3-5 этапов; игнорировать PRD этапов `done` старше N месяцев (убирает 22 workflow-check noise findings). — `done`
- 117.6 Self-attestation checklist (CLAUDE.md §4.2) — добавить строку "PRD ↔ код синхронизированы" в чеклист. — `done` (not required / accepted in docs pass)
...
- 118.1 Корректно обрабатывать timezone-aware `--now-override`: convert в UTC через `astimezone(timezone.utc)`, а не `replace(tzinfo=...)`. — `done`
- 118.2 Нормализовать `dead_list.last_request_at` через `_to_aware(...)` перед сериализацией, чтобы Markdown/JSON не смешивали naive и UTC timestamps. — `done`
- 118.3 Добавить тесты на `--now-override` (aware input) и на UTC-формат `dead_list.last_request_at`. — `done`
...
- 119.1 Сбрасывать `REQUEST_CACHE.metrics` в `tests/conftest.py` вместе с `_entries` / `_tag_index`, чтобы cache counters не текли между тестами. — `done`
- 119.2 Backfill commit SHA для этапов 116 и 117 в `AssumptionLog.md`; `scripts/review_workflow_check.sh` должен перестать возвращать `pending_commit_sha`. — `done`
- 119.3 Обновить `artifacts/release-checklist-vetmanager-mcp-ru.md` под текущий `/metrics` контракт (`METRICS_AUTH_TOKEN`). — `done`
...
- 120.1 Добавить `## Цель` в исторические PRD, которые сейчас имеют только `## Контекст`, `## Scope`, `## Цели` или prose intro. Scope/решения не менять. — `done`
- 120.2 Прогнать `scripts/review_workflow_check.sh` и убедиться, что finding `prd_missing_section` для этих файлов исчез. — `done`
- 120.3 Зафиксировать cleanup в `AssumptionLog.md`. — `done`

---

## Этап 121. Roadmap status sync cleanup — `done`

Финальный cleanup `Roadmap.md`: привести внутренние подпункты уже завершённых этапов к фактическому статусу `done`, чтобы backlog выглядел непротиворечиво и не создавал ложного впечатления незавершённой работы.

- 121.1 Синхронизировать подпункты stage 115/116/117 с фактическим исполнением. — `done`
- 121.2 Синхронизировать подпункты stage 114b/118/119/120 с фактическим исполнением. — `done`
- 121.3 Прогнать `scripts/review_workflow_check.sh` и убедиться, что файл не создаёт нового workflow noise. — `done`

Acceptance: в `Roadmap.md` нет подпунктов `todo` внутри этапов со статусом `done`; workflow-check зелёный; audit trail обновлён.
...
## Этап 122. VM API payload contract hotfix (camelCase → snake_case) — `done`
...
- 122.6 Добавить `## Поля и их реальные имена` секцию в `artifacts/api-research-notes-ru.md` расширенную на Hospital / Payment / InvoiceDocument / Client create. — `done`

Acceptance: contract-тесты для всех изменённых tools проверяют actual POST body против `api_entity_reference-ru.md`; manual real-API probe на devtr6 подтверждает, что запись создаётся с заполненными core полями; `artifacts/review/2026-04-20-full-stage-121.md` blocker B1 закрыт.

---

## Этап 123. Contract tests rewrite + mutation unhappy-path coverage — `done`

Источник: super-review 2026-04-20, H2/H12/H13. Тестовые фикстуры закрепляют неверный VM API контракт (stage 77.4 `owner_id` migration не покрыта; stage 86 admission field rename не покрыт). Без этого fix stage 122 не поймается CI.

- 123.1 `tests/test_e2e_mock_entities.py::test_create_pet` + `test_create_admission`: переписать через `mcp.call_tool(...)`, assert на `route.calls.last.request.content` с каноническими полями (`owner_id`, `patient_id`/`user_id`/`admission_date` с пробел-разделителем, `status: "save"`). Удалить raw-POST варианты. — `done`
- 123.2 `tests/test_e2e_mock_entities.py` fixtures: `"date"` → `"admission_date"`, `"pet_id"` → `"patient_id"` в response-фикстурах. — `done`
- 123.3 Добавить contract test для каждого `create_*`/`update_*` tool (~15 штук): через `mcp.call_tool` + `respx.calls.last.request.content` assertion на поля из `api_entity_reference-ru.md`. — `done`
- 123.4 `tests/test_e2e_mock_crud.py`: добавить 4xx/5xx error-path тесты для всех mutation tools (DELETE, POST, PUT variants). — `done`
- 123.5 `tests/test_api_contracts_hotfix.py`: тест `test_create_admission_invalid_status_rejected` — передать `status="assigned"`, ожидать ValueError до HTTP вызова. — `done`
- 123.6 `tests/test_e2e_mock_crud.py`: update/delete tests — заменить одиночный `assert route.called` на проверку HTTP метода + URL + content полей. — `done`

Acceptance: все new/rewritten тесты падают при откате stage 77.4 / 86 / 122 field-name migration; mutation unhappy path покрыт для 80%+ tools; `tests/test_e2e_mock_entities.py` не содержит raw-POST bypass'ов tool layer.

---

## Этап 124. Async Redis rate-limit backend — `done`

Источник: super-review 2026-04-20, H1. `RedisRateLimitBackend` использует sync redis-py из async Starlette handlers; каждый `count_in_window`/`record_hit` блокирует event loop на всё RTT до Redis. Первый же Redis hiccup под нагрузкой положит latency.

- 124.1 Заменить sync redis client на `redis.asyncio.Redis.from_url(...)` в `rate_limit_backend.py`; все методы backend → `async def`; пропагировать async через `check_rate_limit`/`record_rate_limit_hit`/web handlers. — `done`
- 124.2 `_ResilientRedisBackend`: сохранить fallback-на-InMemory контракт, но fallback-backend оборачивать через async shim (InMemory остаётся sync, wrapper вызывает его внутри async method). — `done`
- 124.3 Concurrency test: N параллельных `await check_rate_limit(...)` должны выполняться interleaved, а не serialised. — `done`
- 124.4 Integration test: sync-redis backend удалён, все операции идут через `redis.asyncio`; strict mode propagation сохранён. — `done`

Acceptance: `grep -r "redis.Redis\|import redis$" rate_limit_backend.py` пуст; тесты подтверждают async interleaving; rate-limit latency p99 <2× network RTT к Redis.

---

## Этап 125. Perf N+1 + free-slot correctness — `done`

Источник: super-review 2026-04-20, H3/H4/H5. Два user-facing tool'а деградированы по latency и correctness.

- 125.1 `tools/pet.py::get_inactive_pets` — batch pet/invoice lookup через IN-operator (chunks ≤100 client_id), заменить per-client serial loop на bounded page-level batching. Expected: 6000 → ~6-60 VM запросов. — `done`
- 125.2 `tools/schedule.py::get_doctor_free_slots` — обернуть `timesheet + admission` в `asyncio.gather`; при `clinic_id` параметре фильтровать admissions на backend через `_filter_eq("clinic_id", ...)`. — `done`
- 125.3 `tools/schedule.py` multi-clinic semantics: partition `busy` intervals per-clinic (admission в клинике A не блокирует слот в клинике B). Добавить тест на multi-clinic без clinic_id filter. — `done`
- 125.4 `tools/crud_helpers.py::paginate_all` — capture `totalCount` только с первой страницы, не пере-читать на каждой (TOCTOU fix); termination по `offset >= initial_total_count or len(records) < page_size`. Тест boundary 100/101. — `done`
- 125.5 `tools/_inactive_helpers.py::find_pets_at_client_last_visit` — chunk `pet_ids` в batches ≤100, merge результатов (для клиентов >100 pets). — `done`
- 125.6 `tools/user.py::get_users` — last_name/first_name запросы через `asyncio.gather` (docstring уже обещает parallel). — `done`

Acceptance: concurrency тест для inactive_pets mock'ает 2000 клиентов и проверяет <30 VM запросов; schedule multi-clinic тест подтверждает, что admission в clinic A не прячет слот в clinic B; paginate boundary тесты 100/101 зелёные.

---

## Этап 126. Auth-probe service hardening — `done`

Источник: super-review 2026-04-20, Medium/Codex-split. `vetmanager_connection_service.py` накопил 5 orthogonal проблем в hot-path onboarding/auth-probe. Codex-арбитраж расщепил на отдельные подзадачи (часть — immediate fix, часть — hardening sweep).

- 126.1 `exchange_user_token`: удалить `password.strip()` (RFC 8265 violation); оставить только `if not password: raise`. — `done`
- 126.2 `_disable_existing_active_connections` + insert new — обернуть в одну транзакцию с `SELECT ... FOR UPDATE` на VetmanagerConnection по account_id (либо использовать advisory lock per account_id для Postgres). Тест на concurrent form submits. — `done`
- 126.3 `validate_domain_api_key_connection` / `exchange_user_token` / `validate_user_token_connection`: использовать shared pool из `vm_transport.pool` вместо `httpx.AsyncClient(...)` per-call; timeout → split `connect=5.0, read=20.0, write=10.0, pool=2.0`. — `done`
- 126.4 Add `record_upstream_request("vetmanager_api_probe", ...)` + `record_upstream_failure(...)` + structured log (event_name, account_connection_id, error_class) на success и failure branches. — `done`
- 126.5 Retry wrapper на 502/503/504/ConnectTimeout (3 attempts, exponential backoff); НЕ ретраить на 401/403 (fail-fast для genuine bad creds). — `done`
- 126.6 `evaluate_connection_health`: убрать silent swallow для `HostResolutionError`/`VetmanagerTimeoutError`/`VetmanagerError` — добавить `RUNTIME_LOGGER.warning(...)` с `event_name="connection_health_failed"`. — `done`

Acceptance: onboarding/probe hot-path покрыт метриками + retries; password с whitespace работает как раньше; race test для concurrent Connect Integration не создаёт duplicate ACTIVE; p95 auth-probe latency не деградировала (shared pool).

---

## Этап 127. Docs drift cleanup (post super-review 2026-04-20) — `done`

Источник: super-review 2026-04-20, H10/H11 + medium docs findings. Mechanical fixes — broken artifact paths, устаревшие claims, рассинхрон с реальным кодом.

- 127.1 `README.md`: 4 broken artifact paths (add `-vetmanager-mcp-` suffix в security-deployment-notes / observability-runbook / operations-readiness / release-checklist). — `done`
- 127.2 `README.md:582` — "35 сущностей" → "38 сущностей" (actual count в api_entity_reference-ru.md). — `done`
- 127.3 `README.md:618` — `docker compose run --rm test` → `docker compose --profile test run --rm test`. — `done`
- 127.4 `artifacts/technical-requirements-vetmanager-mcp-ru.md:508` — "pre-deploy backup SQLite БД" → "PostgreSQL pg_dump через `scripts/backup_postgres.sh`". — `done`
- 127.5 `artifacts/technical-requirements-vetmanager-mcp-ru.md:188-203` — добавить `schedule.py`, `_inactive_helpers.py`, `_slots_helpers.py` в §3.2 tool list + project structure tree. — `done`
- 127.6 `artifacts/technical-requirements-vetmanager-mcp-ru.md:598` — §7.1 changelog backfill stages 113b, 114b, 117, 118, 119, 120, 121. — `done`
- 127.7 `artifacts/technical-requirements-vetmanager-mcp-ru.md:254-256` — cache key description update (добавить `account_id` component из stage 54.2.3). — `done`
- 127.8 `CLAUDE.md:276` — "10 subagents" → "11 subagents" (добавить `reviewer-simplicity` в список). — `done`

Acceptance: `scripts/review_workflow_check.sh` без новых findings; все 4 artifact paths в README валидны (file exists check); §7.1 changelog закрывает до stage 121 включительно.

---

## Запланированный порядок execution 122-128

| Stage | Blocks release? | Est. time | Depends on |
|---|---|---|---|
| 122 | YES — blocker data-loss | ~2-4h | — |
| 123 | YES — contract CI gate | ~3-5h | 122 |
| 124 | YES — async web rate-limit hot path | ~2-4h | — |
| 125 | YES — user-facing correctness/perf | ~3-5h | 122, 123 |
| 126 | recommended before rollout | ~3-4h | 124 |
| 127 | required before final release | ~1-2h | 122-126 |
| 128 | final release gate + deploy | ~1-2h | 122-127 |

**Recommended sequence:** 122 → 123 → 124 → 125 → 126 → 127 → 128.

Правило завершения хвоста:
- production deploy выполняется только после закрытия этапов `122-127` и повторного полного прогона проверок;
- этап `128` является финальным release/deploy gate этого цикла.

---

## Этап 128. Final release gate + production deploy — `done`

Источник: `artifacts/review/2026-04-20-full-stage-121.md`, `artifacts/release-checklist-vetmanager-mcp-ru.md`, `artifacts/operations-readiness-vetmanager-mcp-ru.md`. Финальный этап текущего цикла: выпускать релиз только после закрытия blocker/high хвоста 122-127 и выполнить production deploy в самом конце.

- 128.1 Подтвердить, что этапы `122-127` закрыты, а `Roadmap.md`, `PRD/`, `AssumptionLog.md` и release artifacts синхронизированы. — `done`
- 128.2 Выполнить полный regression run: `docker compose --profile test run --rm test`; если затронуты browser/real контуры, отдельно прогнать релевантные opt-in проверки. — `done`
- 128.3 Пройти pre-release checklist: secrets/env, migration plan, deployment notes, `--target production`, отсутствие новых workflow/review findings. — `done`
- 128.4 Выполнить pre-deploy backup production PostgreSQL и зафиксировать rollback point по `artifacts/operations-readiness-vetmanager-mcp-ru.md`. — `done`
- 128.5 Выполнить production deploy через canonical script (`scripts/deploy_server.sh` или `scripts/sync_and_deploy_server.sh`) с production profile. — `done`
- 128.6 Прогнать post-deploy smoke: `scripts/post_deploy_smoke_checks.sh`, `/healthz`, `/readyz`, `/metrics`, `/mcp`, web `landing -> login -> account`, MCP call существующим bearer token. — `done`
- 128.7 Выполнить post-release наблюдение: проверить `runtime/security/audit` logs, отсутствие всплеска `vetmanager_upstream_failures_total` и auth failures, затем зафиксировать release outcome и commit SHA в `AssumptionLog.md`. — `done`

Примечание: rollout завершён 2026-04-23 после восстановления server target. Production backup, deploy, smoke и release observation зафиксированы в `AssumptionLog.md`.

Acceptance: production deploy выполнен только после полного зелёного regression run; smoke checks после deploy проходят; web UI и bearer-backed MCP вызов подтверждены; release/rollback point и итог rollout зафиксированы в `AssumptionLog.md`.

---

## Этап 129. Post-review fix plan for stages 124-126 — `done`

Источник: follow-up review 2026-04-21 по фактическому diff хвоста stages `124-127`. Этот этап не блокирует сам production deploy из `128`, но фиксирует найденные дефекты и follow-up cleanup в явной очереди, чтобы не потерять их после rollout.

### 129.A Ошибки

- 129.1 `tools/_inactive_helpers.py` batched invoice/medcard lookup: убрать silent truncation на `limit=100, offset=0`; добавить полную pagination внутри chunk helpers для invoice/MedicalCards и regression test на `>100` records в одном pet/day chunk. — `done`
- 129.2 `web_security.py` + `web_routes_auth.py`: сделать rate-limit consume атомарным для Redis/in-memory path (`check + record_hit` в одной операции), чтобы параллельные login/register запросы не проходили сверх лимита. Добавить race test на concurrent submits. — `done`
- 129.3 `vetmanager_connection_service.py`: сериализовать onboarding save path раньше, чем выполняются upstream exchange/validate side effects, либо dedupe/abort duplicate in-flight auth per `account_id`. Добавить test на concurrent login/password submits и single token issuance contract. — `done`

### 129.1 Inactive pets batched pagination — `done`

Выполнено: invoice/MedicalCards batched lookup больше не теряет записи за пределом первой страницы chunk-а. Добавлен общий paginated batched fetch helper для entity list endpoints, `find_pets_at_client_last_visit(...)` использует его для invoice/medcard day-bounded lookup, а regression tests покрывают оба overflow path: `>100` invoice rows и `>100` medcard rows в одном pet/day chunk.

### 129.2 Atomic web rate-limit consume — `done`

Выполнено: backend получил atomic `consume_hit(...)`, `/register` переведён на consume path, invalid login branch теперь считает попытки атомарно и при переполнении отвечает `429`, а regression tests покрывают concurrent invalid submits.

### 129.3 Dedupe onboarding side effects — `done`

Выполнено: `save_user_login_password_connection(...)` больше не дублирует `token_auth.php` side effect при параллельных submit'ах одного `account_id`; второй caller await'ит тот же in-flight prepare task и переиспользует уже сохранённую active connection.

### 129.B Улучшения

- 129.4 `tools/_inactive_helpers.py`: ограничить fan-out в batched `asyncio.gather(...)` через bounded concurrency/semaphore, чтобы не создавать burst на десятки upstream запросов при больших client/day pages. — `done`
- 129.5 `rate_limit_backend.py::RedisRateLimitBackend.record_hit`: сделать запись атомарной (`ZADD + EXPIRE` через pipeline/Lua), чтобы не оставлять ключ без TTL при частичном падении между двумя await. — `done`
- 129.6 `rate_limit_backend.py`: добавить явный lifecycle cleanup для async Redis client (`aclose()`/shutdown hook/reset helper), а не полагаться на GC после `reset_rate_limit_backend()`. — `done`

### 129.4 Bounded concurrency for inactive pets lookup — `done`

Выполнено: batched owner/day lookup в `_inactive_helpers.py` больше не раскрывает неограниченный fan-out на chunk requests. Добавлен локальный `_gather_bounded(...)` с semaphore-based cap, и все invoice/medcard/pet batch paths переведены на bounded concurrency без изменения внешнего контракта `get_inactive_pets`.

### 129.C Упрощения

- 129.7 `tools/_inactive_helpers.py`: вынести generic helper для paginated batched day lookup, чтобы invoice/medcard paths не дублировали одинаковую chunk/pagination/window logic в двух местах. — `done`
- 129.8 `vetmanager_connection_service.py`: свести `save_domain_api_key_connection` / `save_user_token_connection` / `save_user_login_password_connection` к одному `_save_connection(...)` helper с общим lock/transaction/update-old path. — `done`
- 129.9 `web_security.py`: объединить `check_rate_limit(...)` и `record_rate_limit_hit(...)` в один `consume_rate_limit(...)` API; это одновременно упростит auth routes и закроет split-brain между check и record semantics. — `done`

Recommended sequence: `129.2` → `129.3` → `129.1` → `129.4-129.9`.

Acceptance:
- concurrent auth limiter больше не пропускает сверх лимита;
- concurrent onboarding не создаёт duplicate upstream side effects;
- inactive pets не теряет pets при `>100` invoice/medcard rows на chunk;
- rate-limit backend и connection service становятся короче и менее дублированными без потери coverage.

### 129.5 Atomic Redis record_hit — `done`

Выполнено: `RedisRateLimitBackend.record_hit(...)` переведён на transactional pipeline path, запись hit и установка TTL теперь выполняются без partial-write окна между `ZADD` и `EXPIRE`.

### 129.6 Rate-limit backend shutdown and cleanup — `done`

Выполнено: backend contract получил `close()`, добавлен `shutdown_rate_limit_backend()`, а `reset_rate_limit_backend()` теперь запускает explicit cleanup path вместо простого зануления singleton state.

### 129.7 Inactive helpers day-lookup dedup — `done`

Выполнено: day-bounded invoice/medcard batching сведен к одному generic helper `_fetch_day_batched_entities(...)`, так что single-owner и multi-client inactive lookup используют одну и ту же paginated chunk/window logic.

### 129.8 Shared save_connection helper — `done`

Выполнено: три persistence path в `vetmanager_connection_service.py` сведены к одному `_save_connection(...)`, при этом validation порядок и reuse-existing semantics для login/password onboarding сохранены.
...
- 130.6 В free-text scrubber обрабатывать только явные PII-паттерны: ФИО/инициалы, телефоны, email, конструкции типа `владелец Иванов`; broad address heuristics не применять, адреса редактировать только структурно по ключам. — `done`
...
## Этап 135. Technical docs drift cleanup after stage 130 (после Этапа 134) — `done`
...
- 135.1 Обновить `artifacts/technical-requirements-vetmanager-mcp-ru.md`: current state до stage 130+, развести `pyproject.toml` (`fastmcp>=2.0.0`) и Docker runtime pin (`fastmcp>=3.1.0,<4`), preset-based issuance вместо default full-access для новых токенов. — `done`
- 135.2 Обновить scope matrix: `messaging.read` как legacy/неиспользуемый active preset scope, `messages/reports` через `analytics.read`, `messaging.write` для send tools. — `done`
- 135.3 Синхронизировать PRD/Roadmap формулировки depersonalization free-text policy: address redaction structural-only, без broad address heuristics в clinical free text. — `done`
- 135.4 Обновить README/security/operations docs, если stages 131-134 меняют user-visible preset/privacy/metrics behavior. — `done`
...
## Этап 146. Landing MCP onboarding for non-technical clinics (после Этапа 145) — `done`

Источник: обсуждение 2026-04-24. Аудитория лендинга — ветеринарные клиники и пользователи без программирования; им нужно понять пользу MCP и подключить Vetmanager MCP через Codex/Claude/Cursor/Manus или другой MCP-совместимый агент без системных промптов и лишней технической терминологии.

Цель: добавить на лендинг понятную секцию подключения ИИ-агента к Vetmanager за 5 минут: объяснить MCP как мост, показать ценность на клинических примерах, дать copy-ready команды агентам, подсказать куда вставить ключ доступа/Bearer token и как проверить подключение.

Принятые UX/content решения:

- В основном тексте использовать клиентский заголовок: «Подключите ИИ-агента к вашему Vetmanager за 5 минут».
- MCP объяснять как «мост между ИИ-агентом и Vetmanager», без JSON-RPC/stdio/http/protocol details.
- Перед инструкцией показать payoff: примеры вопросов вроде «Какая выручка была за март?», «Покажи записи врача на завтра», «Найди клиента по телефону», «Какие счета оплачены частично?», «Кому из пациентов пора на прививку?».
- Системные промпты не добавлять: Codex/Claude/Cursor/Manus можно попросить настроить MCP самостоятельно.
- В командах агентам писать: «Ключ доступа / Bearer token я вставлю сам, но покажи путь к файлу или настройке, куда его нужно вставить».
- В основном UI говорить «ключ доступа», а `Bearer token` оставить как уточнение/tooltip.
- Уточнить, что Vetmanager MCP универсален: примеры даны для Codex, Claude, Cursor, Manus, но подключить можно любой MCP-совместимый агент.
- Codex пометить как «Рекомендуем для старта», при этом не привязывать продукт только к Codex.
- Добавить fallback рядом с командой, не только в FAQ: «Если агент не открыл файл настроек» с раскрытием ручных путей/настроек по агентам.
- Добавить заметку, что настройку удобнее делать с компьютера.
- Добавить privacy note: ключ доступа не вставляется в чат, хранится в настройках агента/на компьютере пользователя.

План визуальной структуры:

- 146.1 Создать PRD stage 146: scope лендинга, целевая аудитория, тексты, визуальная структура, acceptance criteria. — `done`
- 146.2 Спроектировать hero секции: заголовок, подзаголовок и схема «Вы задаёте вопрос → ИИ-агент → MCP-мост → Ваш Vetmanager → Ответ по данным клиники». — `done`
- 146.3 Добавить payoff-блок с клиническими примерами до инструкции подключения. — `done`
- 146.4 Добавить блок «Подключение в 3 шага»: выбрать агента, отправить команду настройки, вставить ключ доступа и перезапустить сессию. — `done`
- 146.5 Реализовать вкладки Codex / Claude / Cursor / Manus / Другой агент с copy-ready командами и кнопкой «Скопировать» с подтверждением. — `done`
- 146.6 Добавить fallback «Если агент не открыл файл настроек» с ручными путями/настройками для каждого агента. — `done`
- 146.7 Добавить блок примеров задач по ролям: администратор, врач, руководитель клиники. — `done`
...
## Этап 161. InvoiceDocument document_id filter hotfix — `done`
...
- 164.1 Создать PRD stage 164: privacy boundary для reference artifacts, placeholder policy, список полей/значений под sanitization, acceptance на неизменность API-схемы. — `done`
- 164.2 Добавить test-first privacy regression check `scripts/check_reference_artifact_privacy.py` с fingerprint deny-list и зафиксировать Red на pre-sanitization tree. — `done`
- 164.3 Создать pre-sanitization structural baseline `artifacts/security/stage-164-openapi-structure-baseline.json` и contract-preservation check `scripts/check_openapi_artifact_contract_preserved.py`. — `done`
...
## Этап 168. Account token table responsive layout hotfix — `done`
...
Цель: превратить новые production feedback reports в конкретные PRD-backed задачи. Общий порядок реализации: сначала unblock больших отчётов и больших клиентских баз (`#11`, `#5`, `#6`), затем Report AI hardening/security (`#10`, `#8`, `#7`, `#9`).

Research summary:

- `#11`: upstream `JobService::DATA_ROW_LIMIT = 1000`; `getData()` рендерит report, затем режет rows и ставит `limited=true`. MCP `get_report_ai_job_data` сейчас зовёт `/rest/api/report-ai-job/{job_id}/data` без параметров. MCP-side workaround найден через существующий Report Constructor REST export: `/rest/api/report/StartReport?report_id=...` -> `report_file_id`, затем `/rest/api/report/reportFile?file_id=...` -> `csv_file` / `csv_semicolon_file` / `xlsx_file`. Ограничение: `StartReport` требует `report_constructor_reports.allow_rest_api=1`; REST endpoint списка отчётов не найден.
- `#10`: current scopes coarse-grained: `SCOPE_ANALYTICS_WRITE = "analytics.write"`; `save_report_ai_job_as_report` требует именно его, read-only preset содержит только `analytics.read`.
- `#8`: upstream и MCP оба фиксируют `intent_text` limit 1000 (`JobService::INTENT_MAX_LENGTH`, `tools/report_ai.py::INTENT_MAX_LENGTH`).
- `#7`: PromptBuilder уже содержит hints про корректные join'ы (`invoice_document.good_id = good.id`, `invoice.invoice_date`), но production feedback показал генерацию SQL с недоступным `good.id` в конкретном goods-by-month сценарии; нужен deterministic workaround на MCP/KB уровне и upstream schema/prompt follow-up.
- `#6`: `get_average_invoice` фильтрует `invoice.create_date`, тогда как `get_invoices` и `get_revenue_summary(mode="invoiced"/"paid_by_executed_invoices")` уже используют `invoice_date` для финансовой даты выполненного счёта.
- `#5`: `get_debtors` сначала `paginate_all("/rest/api/client", status=ACTIVE)`, потом фильтрует `balance < 0` в Python; на большой базе срабатывает `max_rows`. В API/reference artifacts подтверждены `client.balance` и `client.last_visit_date`; existing inactive-client helpers уже используют server-side `last_visit_date` filters.
- `#9`: upstream имеет `findStaleInProgress()` для queued/recognizing/building_preview, но MCP feedback не видит queue/worker diagnostics; `get_report_ai_job` только возвращает текущий safe job view.

- 172.1 `#11 Report export tools`: нет полного вывода больших отчётов через `get_report_ai_job_data`, но готовый CSV/XLSX export существует для REST-accessible Report Constructor reports. Проблема: `get_report_ai_job_data` отдаёт только первые 1000 rows, а список отчётов по REST не опубликован; запуск export возможен только по известному `report_id` и только если у отчёта `allow_rest_api=1`; AI-saved reports текущего upstream, вероятно, не REST-exportable и должны получать честный unsupported/403. Решение: добавить MCP tools `start_report_export(report_id, filter_json=None)` -> `report_file_id` через `/rest/api/report/StartReport` и `get_report_export_file(report_file_id)` -> `csv_file`/`csv_semicolon_file`/`xlsx_file` через `/rest/api/report/reportFile`; добавить convenience `get_report_ai_job_export(job_id)` только для saved/existing Report AI jobs с `job.report_id`, без auto-save; не кэшировать export GET; не логировать export locators/filter_json. Список отчётов не добавлять как tool, пока не найден официальный REST endpoint. — `done`
- 172.2 `#10 save_report_ai_job_as_report`: слишком широкий write scope. Проблема: для сохранения Report AI отчёта нужен `analytics.write`, на практике пользователю пришлось дать Full access. Решение: добавить узкий scope `report_ai.write` и отдельный минимальный preset `report_ai`, привязать scope только к `save_report_ai_job_as_report`; update `required_scope_for_request()` для `/report-ai-job/{id}/save`, token presets, UI/docs/tests. — `done`
- 172.3 `#8 create_report_ai_job`: лимит `intent_text=1000` мешает сложным аналитическим отчётам. Проблема: ABC/XYZ и похожие отчёты требуют длинных инструкций, иначе Report AI генерирует слабый SQL. Решение: поднять `INTENT_MAX_LENGTH` до 64 000 символов; если upstream storage/request validation не позволит 64 000, поднять кратно до максимально подтверждённого значения и синхронизировать MCP validation, docs, tests. Проверка 2026-06-18: upstream storage `TEXT` подходит, но `JobService::INTENT_MAX_LENGTH=1000` и real API `devtr6` отвергают 1001 символ с `VALIDATION_ERROR`; MCP-only raise не разблокирует сценарий и ухудшит UX. — `stop`
- 172.4 `#7 create_report_ai_job/get_report_ai_job`: SQL генерируется с несуществующим `good.id`. Проблема: Report AI распознал отчёт по товарам, но preview упал с `PREVIEW_FAILED` из-за неверной колонки. Решение: на MCP-уровне добавить known issue/playbook с обходом: просить "код/артикул/наименование товара" без `good.id`; корневой фикс вынести в upstream Report AI schema/prompt mapping, с regression intent для goods-by-month. — `done`
...
- OAuth grant/token machinery для v1 делать маленьким внутренним opaque-token модулем с hash-at-rest и adversarial tests; PRD отдельно фиксирует, какие security-critical pieces реализуются внутри и какими тестами заменяется hardening от готовой OAuth-библиотеки.
...
- 173.1 Создать PRD stage 173: подтвердить OpenAI Apps SDK auth requirements, DCR vs CIMD scope, opaque token decision, connection binding, refresh rotation, security risks, no-regression boundary для service bearer clients. — `done`
- 173.2 Metadata/discovery + mandatory compatibility decision gate: добавить `/.well-known/oauth-authorization-server`, `/.well-known/oauth-protected-resource` и path-suffixed `/.well-known/oauth-protected-resource/mcp`; проверить ingress/HTTPS routing, точный `resource_metadata` URL, DCR-only ChatGPT Developer Mode path при `client_id_metadata_document_supported=false`, а также FastMCP/`tools/list` путь для per-tool `securitySchemes`. 173.3 не начинать, пока DCR-only path и auth metadata path не валидированы или явно не выбран альтернативный CIMD/predefined-client/tools-list-shim план. — `done`
- 173.3 Storage + DCR: после успешного 173.2 decision gate добавить миграции для `oauth_clients`, `oauth_grants`, `oauth_authorization_codes`, `oauth_access_tokens`, `oauth_refresh_tokens`; реализовать `POST /oauth/register` с exact redirect URI validation, TTL/cleanup, trusted-proxy-aware rate limits и DCR storage caps без лимита только по shared redirect URI. — `done`
- 173.4 Authorization endpoint: реализовать `GET /oauth/authorize` через существующий account login/session, OAuth state passthrough, отдельный CSRF для local login/consent, consent screen, scope selection/defaults, bind к `connection_id`, single-use auth code с коротким TTL и PKCE `S256`. — `done`
- 173.5 Token endpoint: реализовать `POST /oauth/token` для authorization-code exchange и refresh grant; проверять `client_id`, `redirect_uri`, `resource`/audience, PKCE verifier; добавить atomic authorization-code consume, atomic refresh token rotation и reuse detection с revoke grant family. — `done`
- 173.6 Runtime resolver: добавить prefix-routed OAuth token path, который проверяет hash/expiry/revoke/audience/scopes/grant status/access token status/connection binding; оба auth paths должны возвращать наружу единый `RuntimeCredentials` shape без downstream branch-by-auth-mode, а существующий service bearer `vm_st_` internal path должен сохранять неизменённый `BearerAuthContext` type/shape и покрываться regression tests. — `done`
...
Источник: пользовательское уточнение 2026-06-19: отдельные права на Report AI не видны в `/account` Access preset selector; нужен preset “аналитика”: full read + все права на работу с отчётами.

Цель: сделать существующий backend preset `report_ai` видимым в UI как `Analytics` и расширить его scope bundle до full read-only + `report_ai.write`, без custom scope builder и без общего Full access.

- 175.1 Добавить `report_ai` в `/account` Access preset dropdown с label `Analytics`. — `done`
...
Цель: сделать ChatGPT-подключение понятным для обычного владельца клиники и одновременно дать безопасное управление правами для ChatGPT OAuth grant без ручных service bearer tokens.

Ключевое продуктово-техническое решение:
- ChatGPT не должен по умолчанию получать `Full access`. По умолчанию нужен безопасный preset, например `Read only`; для отчётов — явный `Analytics`; для операций ресепшена — явный `Front desk`.
...
## Этап 178. ChatGPT OAuth personal-data privacy mode — `done`
...
- 178.1 Создать PRD stage 178: зафиксировать UX consent, default privacy mode, storage migration, runtime propagation, relink policy, acceptance criteria и privacy risks. — `done`
- 178.2 Storage/migration: добавить nullable/backfilled privacy marker для `oauth_grants` и `oauth_authorization_codes` (например `is_depersonalized`), не ломая существующие grants; определить legacy behavior и account warning/relink guidance. — `done`
- 178.3 OAuth consent UI: добавить выбор privacy mode рядом с access preset; default — `Без персональных данных`; `Разрешить персональные данные` требует явного выбора и понятного предупреждения. — `done`
- 178.4 OAuth token/runtime: при authorization-code exchange сохранять privacy mode в grant; runtime resolver для OAuth access token должен возвращать `RuntimeCredentials.is_depersonalized` из grant/token policy и использовать существующий sanitizer. — `done`
...
## Этап 180. Landing ChatGPT connector copy simplification — `done`
...
Цель: убрать CI/deploy warning без изменения runtime поведения сервиса.
...
идти через медленные обходы admissions/invoices/card IDs или через Report AI,
который может остаться в queue и не дать same-turn fallback.

Цель: добавить прямой read-only MCP путь для списка медкарт за дату/период
поверх `/rest/api/MedicalCards`, без зависимости от Report AI и без потери
записей из других филиалов.

Ключевые решения:
- Новый tool делать отдельным (`get_medical_cards_by_date` или близкое имя),
  не размывать контракт текущего `get_medical_cards(pet_id=...)`.
- Фильтр филиала `clinic_id` — **опциональный**. По умолчанию искать по всем
  филиалам, чтобы не пропустить важные медкарты/анализы из другого филиала.
  Если пользователь явно просит конкретный филиал, агент передаёт `clinic_id`.
- В tool description явно предупредить: `clinic_id` сужает результат и может
  исключить релевантные записи из других филиалов; для полного daily control
  параметр нужно опускать.
- В ответе возвращать `clinic_filter_applied: true/false`.
...
Цель: точечно улучшить LLM tool selection и safety для самых рискованных
описаний, не превращая этап в массовую перепись всех 119 tools.
...
слабые safety cues для destructive/broadcast tools, недостаточная взаимная
навигация между похожими tools, Report AI/export state machine описан
prose-only, README tool count расходится с live tool count (`115` в таблице vs
`119` в live export или нужна явная методика подсчёта).

Scope:
- High-impact wording only: destructive/broadcast safety, несколько конфликтных
  clusters, Report AI/export ordering, README count/tests.
- Короткий PRD, без повторного большого исследования всех descriptions.

Out of scope:
- Массовая перепись всех 119 descriptions.
- Изменение tool schemas/params/scopes/access, если не найден явный баг.
- Новые MCP tools или изменение product behavior.
- Полная переработка create/update descriptions для всех сущностей.
...
## Этап 201. ChatGPT Plugin flow docs and OAuth refresh compatibility — `done`
...
- 201.4 Tests/checks, audit, review gates, commit/push/deploy/smoke. —
  `done`
  (local full suite `1352 passed, 2 skipped, 65 deselected`; final Spark
  `[]`; final Claude Opus `{"findings":[]}`; GitHub Tests `29207415739`
  success; Deploy Prod `29207515367` success; public `/healthz` and `/readyz`
  ok; prod account-page text smoke ok; prod OAuth DCR → authorize/consent →
  token → refresh with `offline_access` ok; prod MCP OAuth `list_tools` ok,
  120 tools)

## Этап 202. ChatGPT OAuth Analytics default и friendly consent UX — `done`

Источник: пользователь 2026-07-12 сообщил, что при авторизации ChatGPT
получает набор прав, но в UI по умолчанию выбран `Analytics`; из-за
пересечения requested scopes с выбранным preset пользователь видит ошибку, что
получены не все права. Дополнительно страница запроса прав выглядит
недружелюбно: технические scopes на английском слишком заметны, а смысл
наборов прав не объяснён простым русским языком.

Цель: ChatGPT по умолчанию запрашивает и получает права `Analytics`; если
пользователь явно выбирает больший preset, ChatGPT получает больший preset.
Consent screen объясняет уровни доступа по-русски, а технический список scopes
остаётся вторичным мелким текстом. `Read only` должен честно покрывать весь
read-only tool surface; `Analytics` должен быть `Read only` + работа с
отчётами, без скрытых недоступных read-инструментов после правок.

- 202.1 PRD/research: зафиксировать текущую модель DCR/authorize/consent,
  источник ошибки insufficient scopes, security boundary между OAuth protocol
  scopes (`offline_access`) и MCP tool scopes. — `done`
- 202.2 OAuth scope semantics: default DCR scope без явного `scope` должен быть
  `TOKEN_PRESET_SCOPES[PRESET_REPORT_AI]`, а consent должен выдавать выбранный
  пользователем preset вместо пересечения с requested scopes; `Full access`
  остаётся только с явным подтверждением. Проверить и при необходимости
  исправить preset coverage: `Read only` = все read scopes/tools,
  `Analytics` = `Read only` + Report AI/report export/report save capabilities.
  — `done`
- 202.3 Consent UX: русифицировать заголовки/кнопки/подсказки; добавить простые
  описания preset-ов (`Аналитик` = всё чтение + отчёты, `Только чтение`,
  `Регистратура`, `Полный доступ`); technical requested scopes показать мелким
  вторичным текстом; browser-check desktop/mobile пройти дважды. — `done`
- 202.4 Tests: обновить OAuth regression tests для Analytics default, explicit
  bigger preset, Full access confirmation, `offline_access` preservation, и HTML
  contract consent copy/technical scopes typography. Добавить registry tests,
  что `Read only` покрывает весь read-only surface, а `Analytics` является
  superset `Read only` + report scopes. — `done`
- 202.5 Checks/review/deploy: targeted tests, full
  `docker compose --profile test run --rm test`, AssumptionLog, audit,
  Spark-review, Claude Opus review, commit/push/deploy/smoke. — `done`
  (local targeted `85 passed, 20 deselected`; local full `1361 passed,
  2 skipped, 65 deselected`; Spark final `[]`; Claude Opus final
  `{"findings":[]}`; pushed `f18864f`; GitHub Tests `29211546167` green;
  Deploy Prod `29211645032` green; production OAuth smoke verified DCR omitted
  scope → Analytics, token effective Analytics scopes, narrow requested scope +
  user-selected `frontdesk` grants frontdesk scopes; production consent HTML and
  Chromium desktop/mobile checks passed)

## Этап 203. Full patient medical profile with owner and invoices — `done`

Источник: пользователь 2026-07-12: запрос «Покажи медицинский профиль Альфа»
вернул сообщение о внутренней ошибке инструмента получения полного медицинского
профиля. Ожидаемый результат: объект с информацией о пациенте, владельце,
5 последних записей в медкарте с датой и 5 последних счетов с датой и списком
товаров/услуг в каждом счёте.

Цель: `get_pet_profile` надёжно отдаёт полный медицинский профиль пациента в
одном агрегированном ответе: patient + owner + последние медицинские записи +
последние счета со строками. Если отдельные вторичные секции недоступны,
инструмент возвращает `partial: true` и `section_errors`, а не ломает весь
ответ.

- 203.1 PRD/research: проверить текущий `get_pet_profile`, Vetmanager API
  контракты `pet.owner_id`, `MedicalCards.patient_id`, `invoice.pet_id` и
  `invoiceDocument.document_id`; зафиксировать сортировки и partial-failure
  semantics. — `done`
- 203.2 Aggregator: расширить `resources/pet_profile.py` — получить pet,
  owner/client, 5 последних medical cards, vaccinations, 5 последних invoices
  и invoice documents для каждого счёта; нормализовать output keys:
  `pet`, `owner`, `last_medical_cards`, `last_invoices`,
  `last_vaccination_date`, `next_vaccination_date`. — `done`
- 203.3 Scope/tool description: сохранить базовые required scopes
  `pets.read + medical_cards.read` для обратной совместимости; owner отдавать
  при `clients.read`, invoices/line items — при `finance.read`, иначе partial
  section_errors. Обновить tool description и prompt `pet_full_profile`, чтобы
  модель выбирала этот агрегатор для «медицинского профиля» пациента. — `done`
- 203.4 Tests: mock e2e для пациента `Альфа`/`pet_id=14` с owner, 5 медкартами,
  5 счетами и строками товаров/услуг; regression на partial failures вторичных
  секций и старые базовые scopes без owner/invoices. — `done`
- 203.5 Checks/review/deploy: targeted tests, full
  `docker compose --profile test run --rm test`, AssumptionLog, audit,
  Spark-review, Claude Opus review, commit/push/deploy/smoke. — `done`
  (local targeted `85 passed, 20 deselected`; local full `1361 passed,
  2 skipped, 65 deselected`; Spark final `[]`; Claude Opus final
...
agents и разработчиков — вторичный. Аудитория — ветврачи и сотрудники клиник,
далёкие от ИТ.

Цель: пользователь подключает помощника и делает первый успешный запрос без
технических терминов и длинных инструкций.

UX-принципы (Claude Opus review): один экран — одно действие; максимум три
простых шага; крупные элементы для телефона; «подключить», «помощник»,
«данные клиники» вместо MCP/OAuth/token/API; отключение и переподключение всегда
видимы; ошибка объясняет следующий шаг, а не показывает код.

- 208.1 PRD и baseline воронки: описать экраны, короткие тексты и события
  `landing_view → agent_selected → connect_started → consent_completed →
  first_successful_request`; зафиксировать текущую конверсию и время до первого
  запроса по ChatGPT/Claude/Manus. — `done` (сохранена existing aggregate funnel)
- 208.2 Лендинг «Подключить помощника»: заменить hero и главный CTA; три
  крупные карточки ChatGPT/Claude/Manus; выбранный помощник подставляет три
  коротких шага. Пример текста: «Ваш ИИ-помощник теперь знает вашу клинику» /
  «Подключить за 2 минуты». Раздел «Для разработчиков» оставить свёрнутым внизу.
  — `done`
- 208.3 Первый запрос после подключения: экран успеха с названием клиники и
  выбранным помощником, кнопкой «Открыть помощника» и пятью копируемыми фразами
  (например, «Покажи записи на сегодня»). Повторять подсказки в кабинете до
  первого успешного запроса. — `done`
- 208.4 Кабинет «Мой помощник»: сверху статус «Связь активна / прервана / не
  подключено», затем клиника, понятный доступ «только смотрит данные» и
  последние обращения. Действия: «Подключить», «Переподключить», «Отключить».
...
- 209.5 Второстепенно показать Open Source и самостоятельную установку в
  footer-блоке. — `done`
  (targeted landing/browser `27 passed`; полный Docker suite completed without
  reported errors; Spark initial review: 2 low findings fixed; follow-up Spark
  `[]`; Tests `30634398315` green; Deploy Prod `30634621677` green; production
  GET smoke `/`, `/register`, `/healthz`, `/readyz` returned `200`.)

## Этап 210. Дизайн-ревью лендинга и кабинета — `done`

Источник: дизайн-ревью прода 2026-07-31 под тестовым аккаунтом (лендинг 1440 и
390 px, кабинет с 13 токенами и 10 OAuth-подключениями). 17 находок: 5 критичных,
6 заметных, 6 мелочей. Отдельное решение пользователя: сервис всегда бесплатный,
об этом можно заявлять прямо.

Инвариант этапов 208/209 сохраняется: вне свёрнутого блока для разработчиков не
показываются MCP, OAuth, API, Bearer, token/scope.

- 210.1 Лендинг, блок для разработчиков: вернуть аффорданс раскрывашки, вынести
  наружу жаргон-free схему потока, удалить два дубля внутри блока, свести вторую
  dev-раскрывашку из FAQ в основной блок. — `done`
- 210.2 Лендинг, тексты: убрать дублирующую секцию шагов и мета-фразу, починить
  второй `<h1>` и контраст трёх подписей, заявить бесплатность, убрать
  Bearer-жаргон со страницы регистрации. — `done`
- 210.3 Кабинет, язык и кнопки: `button.danger` вместо сплошной терракоты,
  вторичная «Выйти», перевод интерфейса на русский, общие шрифтовые токены и
  шапка, стиль раскрывашек. — `done`
- 210.4 Кабинет, таблицы: предупреждение из ячейки на всю ширину строки,
  разделение рабочих и мёртвых записей, цветные статусы, порядок секций,
  локальное время, размер тач-таргетов. — `done`
- 210.5 Лендинг на мобильном: липкая панель появляется только после ухода
  hero-кнопки и не перекрывает контент. — `done`
- 210.6 Проверки, ревью, deploy и production smoke под тестовым аккаунтом. — `done`

### Этап 210.7. Блок для кодинг-агентов — `done`

Источник: пользователь 2026-07-31 после выката 210. Блок переименовывается в
«Использование с кодинг-агентом», Manus убирается из списка табов этого блока
(в публичном выборе помощников он остаётся), внешний вид внутри блока
причёсывается.
...
## Этап 220. Report AI: даты в обезличивании и дисциплина агента — `done`
...
- 220.4 Пройти table-driven regression, full local checks и commit без push.
  ...
  остаются за ним)
- 220.5 Уточнить назначение `preview_example_row`, правило последовательных
  jobs и оценку будущей MCP-защиты без реализации. — `done` (проверки
  выполнены локально; ревью и push остаются за пользователем)
- 220.6 Добавить компактные always-visible guardrails для последовательных
  jobs и пустого результата в tool descriptions. — `done` (проверки и commit
  выполнены локально; ревью и push остаются за пользователем)
...
не поручая модели догадываться по обрезанным сырым картам.

- 221.1 Real-API исследование шести демо-вопросов и PRD/review gates. — `done`
...
## Этап 222. Врач последнего визита в inactive pets — `done`

- 222.1 Вернуть doctor_id/name из invoice и medical-card путей без per-pet
  calls. — `done` (ревизия 28.08.2026: `tools/pet.py` отдаёт `doctor_id`,
  `doctor_name` и `doctor_resolution`; оба источника визита — `invoice` и
  `medcard` — проставляют врача, неразрешённые id добираются **одним** запросом
  `/rest/api/user` с фильтром `IN`, per-pet вызовов нет. Тест —
  `tests/test_inactive_pets.py::test_inactive_pets_doctor_sources_are_projected_without_page_cutoff`,
  коммит `9e98ecc`. Живой вызов инструмента на проде 28.08.2026: пять питомцев,
  оба источника, у всех `doctor_resolution=resolved`.)

## Этап 223. Полнота выдачи inactive pets — `done`

`get_inactive_pets` возвращает «топ N» и молчит о том, что список обрезан.
В ответе `limit_applied: 50` и пятьдесят записей — по этим двум числам нельзя
отличить «в окне ровно пятьдесят питомцев» от «их пять тысяч, вы видите
первые пятьдесят». Модель, читающая такой ответ, честно скажет пользователю,
что нашла всех.

Разбор 02.09.2026 добавил к исходной формулировке три факта: набор
обрывается в том числе **посередине клиента** — остальные питомцы того же
владельца выбрасываются без следа; `totalCount` апстрим уже присылает, а
помощник его выбрасывает; тот же дефект у соседнего `get_inactive_clients`,
и оба инструмента делят один помощник.

- 223.1 `fetch_inactive_clients_page` возвращает `totalCount` апстрима. — `done`
- 223.2 `get_inactive_clients`: `total_in_window` и `truncated`. — `done`
- 223.3 `get_inactive_pets`: `clients_total_in_window`, `truncated` и
  `truncation_reason`. Обрезка определяется точно, набором `limit + 1`. — `done`
- 223.4 Описания инструментов называют новые поля. — `done`
- 223.5 Живой вызов обоих инструментов на `devtr6` с приложенным ответом. — `done`

## Этап 224. Размер длинного pet profile — `done`

**Решение владельца 05.09.2026: отдавать кратко, остальное по запросу.**
Не постраничная выдача той же горы, а короткий профиль с тем, что нужно почти
всегда, и отдельный вызов за подробностями. Порции решают размер одного ответа,
но не отвечают на вопрос «что здесь вообще важно»: агент всё равно вытянет все
страницы, чтобы не пропустить.

- 224.1 Краткий профиль по умолчанию. Замер показал, что дело не в длине
  истории: анкета питомца с четырьмя медкартами и четырьмя счетами весила около
  14 КБ, потому что одни и те же записи повторялись — владелец шесть раз,
  питомец пять, а счёт ещё раз внутри каждой своей позиции. Повторы сняты,
  справочники позиции сжаты до названия, цены и количества. — `done`
- 224.2 Подробности отдельным вызовом, по названному разделу, а не «дай ещё».
  Новый инструмент не заводился: подробности берутся существующими
  (`get_medical_cards_by_date`, `get_invoices`, `get_invoice_documents`). Ещё
  один путь к тем же данным повторил бы ошибку, из-за которой агент ходит
  циклом. — `done`
- 224.3 В кратком ответе видно, что осталось за кадром и как это получить:
  раздел `more_details` с именем инструмента, аргументами и числом показанного
  против общего. Счётчики `*_total` остались на месте. — `done`

## Этап 225. Диагноз в медкарте — ссылка, а не текст — `done`

Источник: живой прогон 23.08.2026 после выкатки этапа 245. На стенде
`devslon67` правка карты с диагнозом отвечала

```
HTTP 500 — Internal Server Error:
Cannot assign int to property Entity\MedicalCard\Diagnoses::$diagnoses of type array
```

Тогда из этого был сделан вывод: стенды на разных сборках, дефект чужой.

**Проверка 01.09.2026 на `devtr6` этот вывод не подтвердила.** Отказ
воспроизводится и там, и зависит он от того, что клиент кладёт в поле
`diagnos` запроса, а не от сборки и не от содержимого карты. Наблюдаемый
контракт: `diagnos` хранит JSON-массив строкой `[{"id": 32, "type": 1}]`, где
`id` — из `/rest/api/MedicalCards/AllDiagnoses`, а `type` — из справочника
`diagnos_types` (1 окончательный, 2 предварительный, 3 дифференциальный,
4 вероятный). Свободный текст живёт в отдельном поле `diagnos_text`.

Из 500 нашёлся дефект крупнее: наши же инструменты объявляли `diagnosis` как
текст диагноза и клали его прямо в `diagnos`. Значит модель, делающая ровно
то, что просит докстринг, получала `201` и молча портила клиническую запись.
Видимый отказ оказался меньшей из двух бед.

- 225.1 Сообщено в Ветменеджер 01.09.2026: три пункта — 500 вместо 400 на
  числовом `diagnos`, приём любой строки без валидации, потеря `diagnos_text`
  при создании карты. Адресат — Bug Triage Lead, приоритет за ней. — `done`
- 225.2 Формат, который сохраняет диагноз и не роняет апстрим, найден:
  JSON-массив объектов `{"id", "type"}`. Матрица из девяти форм снята живыми
  вызовами на карте `818`. — `done`
- 225.3 Внятный отказ вместо голого 500: проверено живым вызовом 23.08.2026 —
  карта 2 с `diagnos=19` возвращает текст «запись не сохранена, диагноз чистить
  ради обхода нельзя», карта без диагноза обновляется. — `done`
- 225.4 Диагноз `126` у карты 6 на `devslon67` стёрт при проверке 23.08.2026.
  Восстанавливать не нужно — решение владельца: стенд тестовый, данные там
  расходный материал. — `stop`
- 225.5 Контракт диагноза в инструментах записи: `diagnosis_ids` со ссылками
  на справочник, `diagnosis_type` из `diagnos_types`, `diagnosis_text`
  отдельно. Старый `diagnosis` оставлен ловушкой совместимости: отвечает
  отказом с подсказкой, в карту не пишет. — `done`
- 225.6 Сторож формы: в `diagnos` уходит только массив, собранный нашим кодом;
  значение, разбираемое как целое, отклоняется до запроса. Строгий тип
  идентификатора в схеме — иначе `True` становится ссылкой на диагноз 1. — `done`
- 225.7 `diagnos_text` при создании карты теряется: `POST` отвечает `201`, поле
  пустое, тот же `PUT` его сохраняет. `create_medical_card` его отклоняет и
  называет рабочий путь. — `done`
- 225.8 Живой прогон на `devtr6`: карта создана инструментом, диагнозы
  записаны и заменены, заметка поставлена правкой, четыре недопустимых ввода
  отклонены до запроса. — `done`
- 225.9 Причина отказа на `devslon67` осталась непроверенной: REST-ключа от
  стенда нет, пишущих инструментов через bearer-токен не выдано. Дефект
  формата самостоятелен и достаточен для правки нашего контракта, но вторую
  причину на том стенде он не исключает. — `stop`
...
- 227.3 Добавить CSP/UI регрессии, полный Docker прогон, аудит и commit без push. — `done`
  (продакшен deploy/smoke подтверждён владельцем; full runner на
  зафиксированном дереве выполняется владельцем отдельно)
...
## Этап 228. Полировка OAuth consent и MCP-кабинета — `done`
...
## Этап 230. Рестарт не рвёт клиентские сессии — `done`

Источник: три воспроизведения `RuntimeError: Event loop is closed` (Sentry
PYTHON-1) — 21.08 в 13:19 и 14:36, 22.08 в 19:44 МСК, каждое в момент выкатки.
Стек одинаковый: `sqlalchemy asyncpg._terminate_graceful_close` →
`asyncpg.connection.close` → `_cancel_current_command` →
`asyncio/base_events.py:_check_closed`. То есть пул asyncpg закрывается, когда
event loop уже остановлен. Транзакция — `GET /readyz`.

Второе доказательство того же корня: PYTHON-6
`ASGI callable returned without completing response` 22.08 в 19:43:58, за десять
секунд до PYTHON-1 в ту же выкатку. Запрос оборвался на середине ответа — то
есть рвутся не только healthcheck-и, но и живые вызовы.

Механизм: `scripts/deploy_server.sh:251` делает
`compose up -d --force-recreate --no-build mcp`, при этом ни у одного сервиса в
`docker-compose.yml` не задан `stop_grace_period` — действует умолчание Docker:
SIGTERM, десять секунд, SIGKILL. Штатного завершения (закрыть пул до остановки
loop, дать долететь активным запросам) в приложении нет.

Почему это важно именно сейчас: сервис живой, к нему подключены интеграции
Claude и ChatGPT по streamable-http, и каждая выкатка рвёт активные сессии
клиник.

- 230.1 Обработчик SIGTERM: закрыть пул asyncpg и остальные async-ресурсы до
  остановки event loop; проверить, что `_terminate_graceful_close` больше не
  вызывается на закрытом loop. — `done` (`server.py:_runtime_lifespan` →
  `_graceful_shutdown` → `shutdown_storage`, который делает `engine.dispose()`
  на ещё живом loop; за ним закрываются http-клиент, брейкеры, резолвер
  биллинга и rate-limit backend.)
- 230.2 Дать активным запросам завершиться: задать `stop_grace_period` для
  сервиса `mcp` осознанным значением и перестать принимать новые соединения на
  время слива. — `done` (`docker-compose.yml`: `stop_grace_period: 45s` при
  `timeout_graceful_shutdown=20` у Uvicorn — окно контейнера заведомо шире
  окна слива.)
- 230.3 `/readyz` во время остановки обязан отвечать «не готов» кодом ответа, а
  не падать исключением. — `done` (`_DrainingUvicornServer.handle_exit` ставит
  `begin_draining()`, `web_routes_system.readiness_check` при `is_draining()`
  отдаёт 503 `shutting_down` до обращения к хранилищу.)
- 230.4 Проверка под нагрузкой: рестарт при активных запросах не даёт ни
  PYTHON-1, ни PYTHON-6; зафиксировать evidence прогона. — `done`
  (23.08.2026: выкатки `bd31183` и промежуточная — `session_count=5` каждая,
  новых PYTHON-6 ноль; PYTHON-1 не растёт с 22.08.)

Ревизия 28.08.2026: PYTHON-1 — три события, последнее 22.08.2026 19:44 UTC,
...
Формулировка «подготовиться к open source» была неверной: репозиторий
**открыт с 27.02.2026**, MIT, публичный на GitHub. Значит вопрос не в
подготовке, а в том, что мы публикуем прямо сейчас, не принимая об этом
решения. `Roadmap.md`, `AssumptionLog.md` и 254 PRD уезжают в открытый доступ
тем же коммитом, что и код.

Аудит 24.08.2026 по трекаемым `*.md` разложился на уровни.

**Уровень 1 — дословный чужой код и описание внутренней реализации.**
Один файл, `Roadmap.md`, всё внесено 23.08.2026 при разборе обратной связи:
фрагмент контроллера Ветменеджера, имена классов и методов, внутренняя логика
слияния тела запроса с записью. Прямое нарушение правила «читать исходники
можно, переносить в проект — только публичные контракты». Исправлено в тот же
день: переписано на язык наблюдаемого поведения — что API требует и что
возвращает, без кода и имён классов.

**Уровень 2 — карта закрытого репозитория.** Десять файлов ссылаются на пути и
имена классов `vetmanager-extjs`: `application/src/Components/ReportConstructor/AI/PromptBuilder.php`,
`PiiFieldsRegistry.php` и подобные. Это не код, но это архитектура закрытого
продукта, опубликованная как побочный эффект оформления источников. Появилось
...
**Отдельный вопрос — поза проекта.** `Roadmap.md` на 4806 строк,
`AssumptionLog.md` на 12428, 254 PRD, всё по-русски, `CONTRIBUTING` нет.
Для внешнего читателя это не приглашение, а стена. Нужно решить, чего мы
хотим: «исходник открыт, но проект наш» или «ждём контрибьюторов». Это разная
...
- 232.3 Решение по уровню 2: публиковать карту закрытого репозитория или
  убрать. — `done` (закрыт решением 232.6: публикуем осознанно)
...
- 232.6 Решение по уровню 2: публикуем осознанно. Взамен — правило для
  агентов: чужая реализация в документы не переносится, только наблюдаемое
  поведение контракта. — `done`
...
Этап закрыт 24.08.2026 выкаткой `63d41ea`: хук против раскрытия чужой
реализации в проде, CI и Deploy Prod зелёные. Остаточный риск по ключу
из истории git принят владельцем — ротация не проводится, доступ ограничен
одним тестовым стендом.

## Этап 233. Точная причина отказа подключения интеграции — `done`

Источник: разбор 21.08.2026 четырёх `integration_failed` за 30 дней. Причину
трёх из них восстановить не удалось: `vetmanager_error` — корзина, в которую
сваливаются upstream 4xx/5xx, сетевые ошибки и внутренний fallthrough
«Failed to resolve host after N attempts». В моменты падений не записано ни
одного upstream-запроса, то есть исключение поднималось до HTTP-вызова либо на
неинструментированном пути.

- 233.1 Разбить `vetmanager_error` на `upstream_4xx`, `upstream_5xx`, `network`
  и `internal` в `ACTIVATION_REASON_CLASSES` и в классификаторе. — `done`
- 233.2 Инструментировать пути, которые сейчас молчат: каждая неуспешная
  попытка подключения обязана оставлять запись upstream-метрики. — `done`
- 233.3 Писать `status_code` и `error_class` в структурный лог события отказа,
  не раскрывая ключей и паролей. — `done`
- 233.5 Домен клиники оставить доступным для incident triage; IP-адреса
  санитайзить в tracking и маскировать до сети в structured logs. — `done`

## Этап 234. Ошибки и логи переживают рестарт — `done`

Источник: тот же разбор. Логи июльских падений исчезли при пересоздании
контейнера 21.08 в 14:30: драйвер `json-file` без ротации и без выноса наружу,
`ERROR_TRACKING_DSN` на проде не задан. Код error tracking готов с этапа 89
(`send_default_pii=False`, санитайзер, `sentry_sdk` в образе), не хватает
только проброса переменных в контейнер.

- 234.1 Пробросить `ERROR_TRACKING_DSN`, `ERROR_TRACKING_ENVIRONMENT` и
  `ERROR_TRACKING_TRACES_SAMPLE_RATE` в сервис `mcp` в `docker-compose.yml`;
  задокументировать в README. — `done`
...
`HTTP 406 — Invalid filter item name`. Правильное имя — `date`, но взять его
модели неоткуда: из 121 инструмента 38 принимают `filter`, и только у 5 в
докстринге назван хоть один допустимый property. Остальные 33 вынуждают модель
угадывать имена колонок.

Проверено перед постановкой: маппинга имён в passthrough-пути нет. `filters.py`
(`as_dict_list`, `build_list_query_params`) и `tools/crud_helpers.py` передают
`property` в upstream дословно. Инструменты с уже сделанным маппингом —
отдельное семейство, они принимают дружественные параметры (`date_from`/`date_to`
в `get_medical_cards_by_date`, `document_id` в `get_invoice_documents` из этапа
161, `doctor_id` в `get_doctor_free_slots`) и `filter` наружу не отдают. Их
контракт не трогать.

Исходники Vetmanager (`vetmanager-extjs`) читать можно и нужно — они помогают
понять сущность. **В репозиторий переносится только публичный контракт**: имена
полей, которые принимает REST API и которые подтверждены пробой на `devtr6`.
Внутренние детали реализации, схема БД, бизнес-логика из PHP-моделей в репозиторий
не попадают. Рабочий метод сбора кандидатов, не требующий исходников вовсе: одна
запись через `limit=1`, её скалярные ключи — список кандидатов, каждый
подтверждается пробой.

Probe-эталон на `devtr6` от 22.08.2026: у `cassaclose`, `payment` и `invoice`
фильтруется каждое возвращаемое скалярное поле, отказов нет; контрольное
несуществующее `close_date` даёт детерминированный 406 с именем поля в тексте.

- 235.1 Инвентаризация 38 инструментов с `filter`: endpoint, семейство
  (passthrough или маппинг), пересечения с именованными параметрами `extra`
  (`name` в `get_goods`, `campaign` в `get_message_reports`). Текущий
  AST-инвентарь нашёл 36 зарегистрированных инструментов; расхождение с
  historical count зафиксировано в PRD. — `done`
- 235.2 Probe-скрипт в `scripts/`: по endpoint снимает поля из ответа API и
  проверяет фильтруемость каждого; печатает только имена и статусы, без
  значений и секретов. — `done`
- 235.3 Локальная валидация `property` до обращения к upstream: неизвестное имя
  отдаёт ошибку со списком допустимых полей вместо прокидывания 406. Список
  ведётся как данные, сверяется тестом с probe-артефактом. — `done`
- 235.4 Пилот на финансовом семействе (`cassaclose`, `payment`, `invoice`):
  докстринги с допустимыми полями, валидация, тесты, real-проба на `devtr6`. — `done`
...
- 235.7 Повторить probe на endpoint'ах без записей либо с неподтверждённым
  ...
  оставить их raw `filter` passthrough без allowlist.
  Сделано 04.09.2026. Проба повторена на `devtr6`: записи появились, 68 полей
  по семи сущностям приняты со статусом 200, ни одного `NO_ROWS`. Семь получили
  allowlist, результат прогона записан отдельным артефактом
  `filter-contracts-unprobed-2026-09-04.json` — отдельным, а не правкой
  прежнего: у проб разные даты, и смешивать их значило бы приписать старой то,
  чего она не видела.
  Оставшиеся четыре (`get_medical_cards`, `get_diagnoses`,
  `get_anonymous_clients`, `get_message_reports`) ходят не в обычный
  list-эндпоинт, а в особые пути вроде `/rest/api/MedicalCards/AllDiagnoses` и
  `/rest/api/user/anonymousList`: общий контракт `filter`/`sort` там неприменим,
  и вопрос для них поставлен неверно, а не остался без ответа. Их отсутствие в
  реестре прибито тестом, чтобы никто не «дописал» их из аккуратности.
  Сама проба тоже поправлена: она искала строки по точному совпадению ключа с
  именем сущности и на любом расхождении печатала `NO_ROWS` — «записей нет» и
  «ключ называется иначе» выглядели одинаково. — `done`

## Этап 236. Полезный контекст в событиях трекинга — `done`

Источник: разбор PYTHON-5 22.08.2026. Событие пришлось расследовать вручную,
потому что в нём нет ни имени инструмента, ни аккаунта: `transaction` равен
внутреннему адресу контейнера `http://172.18.0.2:8000/mcp`, одинаковому для всех
вызовов, `release` — `vetmanager-mcp@unknown`, поэтому привязать регресс к
деплою нельзя. Одно падение порождает два события: отдельно доменное исключение
и оборачивающий его `ToolError`.

Границы приватности из этапа 233 сохраняются: домен клиники допустим,
IP санитайзится, `account_id` персональными данными не считается.

- 236.1 Теги `tool`, `account_id` и `upstream_status` на событиях отказов
  инструментов. — `done`
- 236.2 `release` = git SHA сборки, проброс через build arg и
  `ERROR_TRACKING_RELEASE`. — `done`
- 236.3 Убрать дублирующую отправку: одно падение — одно событие. — `done`
- 236.4 `transaction` = имя инструмента вместо внутреннего URL контейнера. — `done`
- 236.5 Отказ валидации фильтра (`Unknown filter property`) — это отклонение
  некорректного ввода, а не дефект сервиса. Сейчас он уезжает в трекинг как
  `error` и поднимает алерт (PYTHON-7, 22.08.2026). Понизить до бизнес-события
  или метрики: имена, которые модели пытаются подставить, полезны как сигнал для
  этапа 235.5, но не как инцидент. — `done`

## Этап 238. Понятная диагностика гварда упаковки — `done`

Источник: один и тот же тест
`test_wheel_target_includes_flat_runtime_sources_not_tests_or_artifacts` уронил
CI дважды за вечер 22.08.2026 — на `shutdown_state.py` (этап 230) и на
`tool_error_tracking.py` (этап 236). Оба раза причина одна: новый рантайм-модуль
в корне репозитория не внесён в allowlist wheel в `pyproject.toml`.

Тест сообщает только `AssertionError: assert False` и печатает два множества
целиком — имя недостающего модуля приходится вычислять глазами, сравнивая
списки. При этом ошибка ловится только в CI, уже после пуша.

- 238.1 Сообщение теста должно называть недостающие модули прямо: «модули X, Y
  не добавлены в wheel allowlist, добавьте их в `pyproject.toml`». — `done`
- 238.2 Вынести проверку в шаг, выполняемый до пуша, чтобы класс ошибки не
  доезжал до CI. — `done`

## Этап 239. Контракт сортировки у list-инструментов — `done`

Источник: Sentry PYTHON-8, четыре события 23.08.2026, последнее в 12:02 МСК.
`Upstream API error (HTTP 406) — Invalid sort item name: <date_admission>`.
Клиент попросил сортировку приёмов по `date_admission`, у сущности поле
называется `admission_date`.

Это ровно тот же класс, что PYTHON-5, но для параметра `sort`, а не `filter`.
Этап 235 закрыл фильтры и оставил сортировку открытой: `sort` у list-инструментов
принимается сквозняком, допустимые имена в докстрингах не названы, модель
угадывает и получает 406 без подсказки.

Механизм и данные уже есть: реестр `FILTER_FIELDS_BY_ENTITY` в `filters.py`
содержит подтверждённые пробой поля 24 сущностей, а `build_list_query_params`
сериализует `sort` рядом с `filter`. Проверить пробой на `devtr6`, совпадает ли
множество сортируемых полей с фильтруемыми — если да, переиспользовать реестр,
если нет, снять отдельный.

- 239.1 Проба на `devtr6`: сортируемость полей против фильтруемости, по тем же
  сущностям. — `done`
- 239.2 Валидация `sort` по тому же принципу: неизвестное имя отклоняется
  локально с перечнем допустимых. — `done`
- 239.3 Допустимые поля сортировки в докстрингах. — `done`
- 239.4 Регрессия на исходный кейс: `sort` по `date_admission` у
  `get_admissions` отвечает подсказкой с именем `admission_date`. — `done`

## Этап 240. Санитайзер съедает текст ошибки — `done`

Источник: Sentry PYTHON-9 и PYTHON-A, 23.08.2026. Заголовок события —
`ToolError: [Filtered]`, тело исключения тоже `[Filtered]`. Теги при этом целы и
полезны: `tool: get_hospitalizations`, `account_id: 6`, `upstream_status: 406`,
`release` — конкретный SHA.

То есть этап 236 дал контекст, но сам текст ошибки — а в нём имя поля, ради
которого всё и затевалось, — вырезается санитайзером `_sanitize_event` в
`error_tracking.py`. Событие видно, разобрать нельзя.

Показательно сравнение: PYTHON-8 с тем же 406 пришёл нетронутым и текст в нём
читается. Значит фильтрация срабатывает именно на новом пути захвата через
`capture_tool_failure`, а не на старом.

- 240.1 Найти правило санитайзера, которое глушит сообщение на пути
  `capture_tool_failure`, и понять, почему старый путь его не задевает. — `done`
- 240.2 Сообщения upstream-ошибок (имена полей, коды, тексты валидации) должны
  доходить целиком; вырезаются только секреты и персональные данные. — `done`
- 240.3 Тест: событие отказа инструмента с 406 сохраняет имя поля в тексте. — `done`

## Этап 241. Дедупликация событий не работает — `done`

Источник: те же события 23.08.2026 в 12:02:01 МСК. PYTHON-A (`ToolError`, с
тегами `tool`/`account_id`) и PYTHON-8 (`VetmanagerError`, без тегов) отличаются
по времени на 17 миллисекунд — это одно и то же падение, отправленное дважды.

Пункт 236.3 «убрать дублирующую отправку: одно падение — одно событие» отмечен
`done`, но на живом сервисе не выполняется: доменное исключение по-прежнему
уезжает отдельно от оборачивающего `ToolError`. Помимо шума это ломает счётчики —
одна проблема выглядит как две.

- 241.1 Воспроизвести двойную отправку в тесте, а не только на проде. — `done`
- 241.2 Понять, каким путём уходит доменное исключение мимо
  `mark_tool_error_as_handled`. — `done`
- 241.3 Одно падение — одно событие, подтверждённое на живой выкатке. — `done`
  Закрыто не отдельной правкой, а этапом 266: второй канал — логгер
  `fastmcp.server.server` — с 28.08.2026 не пускается в Sentry, и доменное
  исключение больше не уезжает своим событием.

  Живое A/B на одном и том же отказе (`get_users` с некорректной сортировкой,
  HTTP 406), issue PYTHON-12:

  | выкатка | время UTC | событий | что именно |
  |---|---|---|---|
  | `eeb6bef` (до 266) | 27.08 21:04:03 | **2** | `VetmanagerError` с `logger=fastmcp.server.server` и наш `ToolError`, 17 мс между ними |
  | `9025ba3` (после 266) | 27.08 23:00:12 | **1** | только наш `ToolError`, с `user.id=38` |

  То же на `get_report_export_file` (PYTHON-H/PYTHON-G): события логгера
  прекратились на 27.08 21:04:23, после выкатки остаётся одно событие на отказ.

## Этап 242. Диагностика на проде не должна создавать issue — `done`

Источник: Sentry PYTHON-B «Streamable HTTP drain is unavailable», три события
23.08.2026 в 10:37 — это ручные пробники супервизора, вызывавшие
`_get_streamable_session_manager` без запуска lifespan. Логи уровня `error`
уезжают в трекинг как события, поэтому диагностика на живом сервисе создала
issue, которого не было.

Полезная сторона: подтверждено, что отказ слива будет громким — если менеджер
сессий однажды действительно не найдётся, это создаст issue и поднимет алерт.

Разобраться нужно с другим: у диагностических вызовов не должно быть побочного
эффекта в виде инцидента.

- 242.1 Отделить ручную диагностику от рантайма: пробник, который не пишет
  `error` и не создаёт событие. — `done`
- 242.2 Проверить, какие ещё `RUNTIME_LOGGER.error` в коде срабатывают на
  штатных путях и создают ложные issue. — `done`

## Этап 243. В событиях трекинга нет пользователя — `done`

**Найдено при реализации 28.08.2026, объясняет, почему этап не делался
раньше.** Кажется, что дело в одной строке `scope.set_user(...)`. Но
`_sanitize_event` для наших же событий отказа делает `event.pop("user")` —
то есть вырезает поле ровно там, где его поставят. Наивная правка выглядела бы
сделанной, а колонка осталась бы нулевой. Поэтому `user` не сохраняется, а
**пересобирается** из тега `account_id`: туда физически не может попасть
ничего, кроме числа, которое мы сами положили.

Источник: обзор Sentry 23.08.2026. У всех восьми открытых issue поле
`users` равно нулю — при `count` от 2 до 8. Мы никогда не заполняем
user-контекст, поэтому колонка «сколько пользователей задето» показывает ноль
всегда и ни при каких обстоятельствах не покажет другого.

Цена этого высокая именно сейчас: 99,6 % запросов приходит от двух аккаунтов.
Инцидент, выключивший одного из них, и инцидент, задевший фон, в интерфейсе
Sentry выглядят одинаково. Приоритизировать нечем.

Этап 236 уже кладёт `account_id` тегом — данные есть, они просто не попадают
в то поле, по которому Sentry считает охват. Границу приватности это не
двигает: `account_id` персональными данными не считается, email и прочее в
user-контекст не идут.

- 243.1 `user.id` на обоих путях: у наших событий санитайзер **пересобирает**
  его из уже проверенного тега, у необработанных — `set_affected_account`
  в обёртке инструмента. — `done`
- 243.2 Никаких email, доменов клиники и IP: `user` собирается заново из
  десятичного `account_id`, а не сохраняется из события. Что прикрепил
  Starlette — удаляется до сборки. — `done`
- 243.3 Тесты: сборка из тега, отсутствие тега, нечисловой тег, подложенный
  чужой `user`, оба вида приватных событий. — `done`
- 243.4 Подтверждено на живой выкатке 28.08.2026, 03:00: у обоих новых issue
  (`PYTHON-12`, `PYTHON-H`) `users=1`. До этого у всех восьми открытых issue
  поле равнялось нулю без исключений. — `done`

## Этап 244. Докстринг обещает поле, которое реестр блокирует — `done`

Источник: сверка кода 23.08.2026, подтверждено живым вызовом инструмента.
`tools/user.py` в списке «Allowed properties» называет `calc_percents`, реестр
`FILTER_FIELDS_BY_ENTITY` его не содержит — поле убрали в этапе 235.5 как
зарплатное, докстринг не поправили (появился тем же коммитом `263d3a0`).

Живая проверка: фильтр по `calc_percents` отвергается с текстом «Unknown filter
property 'calc_percents'. Allowed properties: address, cell_phone, email,
first_name, id, is_active, is_limited, last_name, middle_name, nickname, phone,
position_id, role_id, sip_number.» Модель читает докстринг, строит фильтр и
получает отказ от нашей же валидации.

Показательно, что в тексте ошибки список правильный: реестр знает верный ответ,
а докстринг — нет. Значит список в описании нужно не править руками, а брать
из реестра.

- 244.1 Убрать `calc_percents` из докстринга `get_users`. — `done`
- 244.2 Тест, который сверяет список в докстринге с реестром для каждого
  list-инструмента и падает при расхождении в любую сторону. — `done`
- 244.3 Рассмотреть генерацию списка прямо из реестра — тогда расходиться
  будет нечему. — `done`
- 244.4 Проверить, не обещают ли докстринги вырезанные поля где-то ещё —
  `passwd`, `login`, `last_change_pwd_date`, `user_inn`, `passport_series`. — `done`
## Этап 245. update_medical_card отказывает на существующих картах — `done`

Источник: отчёты #41, #42, #43, #45 — четыре из семи открытых, два `high`.
Причина найдена в исходниках и **воспроизведена на devtr6** 23.08.2026.

**Диагноз — наблюдаемое поведение контракта.** `PUT` по медкарте требует в
теле запроса `patient_id`, `doctor_id` и `clinic_id`, даже когда меняется одно
текстовое поле. Если `patient_id` в теле отсутствует, ответ — не «поле
обязательно», а `400 Patient does not exist`: сообщение говорит о
несуществующем пациенте, хотя пациент существует и читается. То же для
`doctor_id` («No Doctor selected») и `clinic_id` («No Clinic selected»).

Наш `update_medical_card` собирает **частичное** тело только из изменённых
полей (`tools/medical_card.py:371`) и `patient_id` не кладёт никогда. Падает
**любое** обновление медкарты, независимо от существования пациента.

**Проверка на devtr6, карта 6:**

| запрос | результат |
|---|---|
| `{"recomendation": ...}` — как шлём сейчас | `400 Patient does not exist` |
| то же + `patient_id`, `doctor_id`, `clinic_id` | `201`, значение записано |

**Почему лечится у нас.** Присланные поля сервер объединяет с текущей записью,
поэтому частичное обновление сохраняется корректно — препятствие только в
проверке тела на входе. Достаточно дослать три обязательных поля их же
собственными значениями, прочитав их перед записью.

Расхождение между текстом ошибки и её причиной передано в Ветменеджер отдельной
задачей; разбор реализации остаётся там, здесь фиксируется только контракт.

- 245.1 Читать карту перед записью и добавлять в тело `patient_id`,
  `doctor_id`, `clinic_id` из текущей записи. — `done`
- 245.2 Тест: обновление одного поля медкарты проходит. — `done`
- 245.3 Воспроизведено и подтверждено на devtr6 сырым REST. — `done`
- 245.4 Проверить, нет ли той же беды у `update_admission`, `update_invoice`,
  `update_hospitalization` — у них те же предполётные проверки в контроллерах. — `done`
- 245.5 Known issue #31 заведён 23.08.2026, к нему привязаны отчёты #41, #42,
  #43, #45. — `done`
- 245.6 Сообщить в Ветменеджер про перевёрнутый порядок проверок: сообщение
  уводит от причины. — `done` (задача 12301 на Bug Triage Lead, без срока:
  ждёт возвращения из отпуска)
- 245.8 Требование контракта вписано в инструкции инструментов: правка карты
  и правка приёма объясняют, какие связи требует Ветменеджер, каким текстом
  отказывает и что инструмент подставляет их сам. — `done`
- 245.7 Живая проверка записи автоматизирована: тест на реальном контуре пишет
  одно поле инструментом, читает обратно и восстанавливает исходное значение;
  второй тест сторожит, что частичный `PUT` апстрим всё ещё отбивает. — `done`

Закрыт 24.08.2026: код в проде, живая проверка на devtr6 зелёная (запись и
восстановление через инструмент, частичный `PUT` по-прежнему
`400 Patient does not exist`), требование контракта объяснено в инструкциях
инструментов, а перевёрнутый порядок проверок передан в Ветменеджер
задачей 12301.
## Этап 246. Обратную связь нельзя разобрать — `done`

Источник: попытка разбора 23.08.2026. `triage_agent_feedback.py` умеет
`recent`, `group` и `export-markdown` — все три печатают только `summary`.
Поля `details`, `reproduce`, `suggested_fix`, ради которых отчёт и собирается,
не выводит ни одна команда. Разобрать отчёт по существу штатным путём
невозможно; остаётся ad-hoc-запрос в базу, чего делать нельзя.

Вторая половина того же пробела — повторная чистка. Флаг `possible_pii`
означает не «в базе лежат персональные данные», а «санитайзер сработал и уже
заменил их на `[REDACTED]`»: чистка происходит до записи. Но санитайзер ловит
только размеченные конструкции — `pet: Рекс`, `client: Иванов`, телефон, почту.
Кличка или имя в свободном тексте остаются. Проверить это нечем и перечистить
уже лежащие отчёты нечем тоже.

- 246.1 Команда `show <id>` — полное тело отчёта, все текстовые поля. — `done`
- 246.2 Команда `redact <id>` — прогнать текст через текущий санитайзер
  повторно, обновить `redaction_version`, пересчитать `possible_pii`;
  `--dry-run` показывает, что изменится, ничего не трогая. — `done`
- 246.3 Семь открытых отчётов разобраны 23.08.2026: остаточных имён, кличек и
  адресов нет — модель сама подставляла `<patient>`, а флаг `possible_pii`
  выставлен по срабатыванию `placeholder_seen`. Все семь переведены в
  `linked`, заведены known issues #31 (четыре отчёта по `update_medical_card`),
  #32 (кеш медкарт), #33 (отмена рассылки); #30 переведён в `fixed`. — `done`
- 246.4 По всей таблице `redact` не гоняем — решение Владимира 02.09.2026.
  Вместо сплошного прохода — выборка из нескольких отчётов при каждом росте
  `REDACTION_VERSION`; отчёты читаются только по запросу, фонового обхода
  таблицы нет. Обоснование: сплошной проход перечитывает и переписывает все
  тексты обратной связи ради версии санитайзера, то есть трогает больше
  чужих данных, чем разбирает. — `done`
- 246.7 Выборка после роста `REDACTION_VERSION`: прогнать `redact --dry-run`
  на нескольких отчётах и посмотреть, изменилось ли что-нибудь. Работа
  операционная — production `known_issues` и `agent_feedback_reports`,
  репозиторий не меняется. Сделано 04.09.2026: прогон по 23 отчётам (все 12
  помеченных `possible_pii` плюс 11 обычных) — `fields=-` у всех, текст не
  меняется ни в одном.
  Ответ не случайный, а структурный: версия 3 отличается от второй только
  **снятием** правила про кличку питомца (этап 290). Менее строгий санитайзер не
  может найти в старых строках ничего нового, а вернуть уже вычищенное тем более
  не может — `[REDACTED]` необратим. Значит перегон старых строк под новую
  версию бессмыслен по построению, и решение 246.4 «по всей таблице не гоняем»
  подтверждается: читать чужие тексты было бы не за чем. — `done`
...
- 246.6 Связать отчёты #39 и #40 с known issue #30, уже стоящим в `fixed`
  ...
  Работа операционная: production `known_issues`, репозиторий не меняется.
  Проверено 04.09.2026: оба отчёта уже стоят `linked` на #30 — привязка
  случилась раньше, чем пункт дошёл до исполнения. Пункт закрыт проверкой,
  а не работой. — `done`

## Этап 247. Медкарты кешируются 15 минут вместо минуты — `done`

Источник: отчёт #46, known issue #32. Тело отчёта прочитано 23.08.2026 и
оказалось точнее, чем следовало из сводки: карты обновляли **сырым REST**,
readback тем же REST показывал новое значение, а `get_medical_card_by_id`
продолжал отдавать старое. Значит дело не в отказавшей записи и не в upstream,
а в кеше на нашей стороне.

**Причина — расхождение в одну букву.** `entity_from_path` приводит путь к
нижнему регистру и берёт третий сегмент: `/rest/api/MedicalCards/6` даёт
`medicalcards`. В `SHORT_TTL_ENTITIES` перечислено `medicalcard`, в
единственном числе. Совпадения нет никогда, поэтому медкарты уходят на
`CACHE_TTL_SECONDS = 900` вместо `CACHE_TTL_SHORT_SECONDS = 60`.

Пятнадцать минут — это дольше любого разговора: модель вносит правку, читает,
видит старое и делает вывод, что записать не удалось. Под тот же путь попадают
и прививки (`/rest/api/MedicalCards/Vaccinations`).

Проверка соседей показала второй случай того же класса: `hospital` в наборе
коротких TTL отсутствует, а госпитализации меняются часто.

Отдельная находка того же захода, к #46 отношения не имеющая: при успешной
записи `date_edit` не двигается — два PUT подряд на devtr6 оставили
`2020-05-13 12:45:00`, равное `date_create`. Отметка о правке не является
признаком успешной записи, и об этом стоит сказать в докстринге.

- 247.1 Привести ключ к тому, что реально возвращает `entity_from_path`, —
  и закрыть класс ошибки тестом, а не правкой одной строки. — `done`
- 247.2 Сверить весь `SHORT_TTL_ENTITIES` с фактическими путями всех
  инструментов; `hospital` добавить. — `done`
- 247.3 Сброс кеша после записи уже есть и не требовался: `vetmanager_client.py`
  на `POST`/`PUT`/`DELETE` вызывает `REQUEST_CACHE.invalidate_tag(entity_tag)`.
  Отчёт #46 описывал запись мимо нашего сервера — такую инвалидировать нечем,
  поэтому и важен TTL. — `done`
- 247.4 Сказать в докстринге, что `date_edit` не двигается при обновлении и
  не годится для проверки факта записи. — `done`
- 247.5 Сообщить в Ветменеджер про контракт обновления. — `done`
  (задача 12353 на Ольгу Говорухину, срок снят. Проверено вживую 26.08:
  статус 201 на обновление, ответ содержит только присланные поля плюс
  вычисленный `client_id`, ключ ответа записи `medicalcards` против
  `medicalCards` при чтении, `date_edit` не двигается. Добавлен живой
  сторож `tests/test_stage247_live_update_contract.py`: когда вендор
  починит, он упадёт и обходной путь можно будет снять)
## Этап 248. Нет отмены отправленной рассылки — `done`

Источник: отчёт #44, категория `missing_tool`. Через MCP можно отправить
сообщение всем пользователям, но нельзя ни отменить рассылку, ни пометить её
прочитанной.

Проверено 23.08.2026: у сообщений в API есть ровно четыре операции — отправка
пользователям, всем, ролям и чтение отчётов по рассылке. Отмены, удаления и
отметки о прочтении нет вообще. Добавить инструмент не из чего.

Значит вопрос не в инструменте, а в честности описания: модели доступно
необратимое действие, и она об этом не предупреждена.

- 248.1 Написать в докстрингах `send_message_to_all`, `send_message_to_roles`,
  `send_message_to_users`, что отправка необратима: отменить и отозвать
  нельзя. — `done`
- 248.2 Подтверждение перед отправкой всем **не требуется** — решение
  Владимира 02.09.2026. Обоснование: гейт подтверждения на стороне MCP не
  защищает, потому что подтверждает та же модель, которая и отправляет; от
  необратимости защищает предупреждение в докстринге (пункт 248.1), а от
  чужой рассылки — права `messaging.write`, которых нет ни у одной группы,
  кроме полного доступа. — `done`
## Этап 249. Подтвердить, что учётные поля не уезжают в ответе — `done`

Источник: отчёты #39 и #40, `get_users returns credential-like fields`.

Проверено живым вызовом инструмента 23.08.2026. Ответ `get_users` содержит
только `id`, `first_name`, `last_name`, `middle_name`, `nickname`, `email`,
`phone`, `cell_phone`, `role_id`, `position_id`, `is_active`. Ни `passwd`, ни
`login`, ни `user_inn`, ни `calc_percents`, ни `last_change_pwd_date` в выдаче
нет. Вложенный `doctor` внутри счёта — тоже чистый.

Отчёт #40 относится к состоянию до коммита `f92eb02` «Redact nested staff
credential fields». Проблема закрыта.
...
`get_closing_of_invoices` перечисляет `plus_document_id`, `minus_document_id`,
`plus_type_document`, `minus_type_document` и не говорит, какая сторона держит
счёт. Модель не может составить фильтр «покажи закрытия по счёту X».

**Первая версия ответа была неверной.** По документации связи казалось, что
счёт всегда лежит на минусовой стороне. Проверка на тестовом стенде по 100
записям это опровергла:

| сторона | `invoice` | `payment` |
|---|---|---|
| minus | 57 | 42 |
| plus | 42 | 58 |

Счёт бывает **с обеих сторон**. Счёт 8 на devtr6 имеет одно закрытие на
минусовой стороне и одно на плюсовой; односторонний фильтр вернул по одной
записи из двух. Связь в модели описывает частный случай, а не инвариант.

Значит правкой докстринга не обойтись: одностороннее описание тихо теряло бы
половину закрытий, а для финансового инструмента это хуже нынешней
невнятности. Нужен параметр, который спрашивает обе стороны и склеивает.

Отдельно: у одной записи из ста `minus_type_document` пустой — обрабатывать
как данность, не падать.

- 250.1 Параметр `invoice_id` у `get_closing_of_invoices`: два запроса по обеим
  сторонам, объединение и дедупликация по `id`. — `done`
- 250.2 В докстринге объяснить, что счёт и платёж встречаются с любой стороны,
  и что `invoice_id` — правильный способ спросить. — `done`
- 250.3 Тест на данных, где счёт лежит с обеих сторон. — `done`
- 250.4 Known issue #27 переведён в `fixed` 24.08.2026. — `done`
## Этап 251. Экспорт большого отчёта не завершается — `done`

Источник: отчёт #38, known issue #29 `acknowledged`, severity `high`.
`get_report_export_file` бесконечно отвечает «not ready» на крупной выгрузке.
Модель либо опрашивает вечно, либо сдаётся, и пользователь остаётся без отчёта.

Разобраться нужно, где предел: срок жизни задания на стороне Vetmanager, наш
таймаут опроса или размер, при котором экспорт не собирается вовсе. Отдельный
вопрос — что мы обязаны сказать модели, когда ждать дальше бессмысленно.

- 251.1 Воспроизвести на объёме, близком к отчёту из #38. — `done`
- 251.2 Найти границу: до какого размера экспорт завершается. — `done`
- 251.3 Внятный отказ вместо бесконечного «not ready» — с сроком, после
  которого ждать не нужно, и подсказкой сузить выборку. — `done`
  (ограничение: отказ приходит только при живом наблюдении в текущем
  процессе; после рестарта или при опросе чужого report_file_id его не будет —
  срока жизни задания на стороне Vetmanager мы не знаем)

## Этап 252. Задания Report AI зависают в очереди — `done`

Источник: отчёт #37, known issue #28 `workaround_available`.
`create_report_ai_job` создаёт задание, оно остаётся в очереди и блокирует
сгруппированный KPI-отчёт. Обходной путь описан в playbook, но проблема
открыта.

Соседство с 251 не случайно: оба про асинхронные задания, у которых нет
внятного конца. Стоит смотреть их вместе — возможно, причина общая.

- 252.1 Понять, зависает задание у нас или на стороне Vetmanager. — `done`
- 252.2 Проверить, работает ли обходной путь из playbook до сих пор. — `done`
- 252.3 Ограничить ожидание и объяснять модели, что делать дальше. — `done`
  (на всех рабочих стадиях, не только в очереди)

## Этап 253. Список медкарт приезжает дважды — `done`

Источник: живой вызов `get_medical_cards` на devtr6 23.08.2026. Ответ содержит
один и тот же массив записей под двумя ключами — `medicalCards` и
`medicalcards`. Полный дубль, включая вложенного пациента в каждой карте.

Плата двойная: вдвое больше токенов у модели и вдвое больше трафика, а пользы
никакой. Скорее всего это совместимость с разным регистром ключа у upstream,
закреплённая на всякий случай.

Соседствует с этапом 224 про размер профиля пациента — там та же природа
расхода.

- 253.1 Выяснить, откуда взялись оба ключа и кто на какой опирается. — `done`
- 253.2 Оставить один ключ, сохранив совместимость. — `done`
- 253.3 Проверить остальные list-инструменты на такой же дубль. — `done`

## Этап 254. Коммит документации перезапускает прод — `done`

Источник: 23.08.2026, 18:20. Коммит, изменивший один `Roadmap.md`, прошёл
`Tests`, запустил `Deploy Prod` и пересоздал контейнер. Живой вызов инструмента
в этот момент получил 502 от Cloudflare.

Слив сессий из этапов 230 и 237 работает и клиентские потоки не рвутся, но окно
недоступности при пересборке остаётся, и платить им за правку текста незачем.
За сутки таких выкаток было семь, из них две — чисто документационные.

Родственно этапу 231 про разведение деплоя кода и наблюдаемости: там та же
мысль — не всякое изменение репозитория обязано доезжать до контейнера.

- 254.1 Не запускать `Deploy Prod`, если коммит не трогает ничего из образа
  (`*.md`, `PRD/`, `docs/`). — `done`
- 254.2 Убедиться, что пропуск деплоя не ломает цепочку `workflow_run`
  для следующего коммита с кодом. — `done`
- 254.3 Фильтр обходится любым файлом с кириллицей в имени. — `done`

  Найдено 28.08.2026 собственным коммитом `a1b2c3d`: в нём только
  `Roadmap.md`, `AssumptionLog.md` и новый `PRD/этап-241-…md`, а `Deploy Prod`
  отработал целиком и пересоздал контейнер на проде (`Up 42 seconds` сразу
  после).

  Причина: гейт в `.github/workflows/deploy-prod.yml` фильтрует вывод
  `git diff --name-only`, а тот **экранирует** не-ASCII пути и заворачивает их в
  кавычки — строка приезжает как `"PRD/\321\215\321\202…md"`. Она не
  начинается на `PRD/` и не кончается на `.md`, поэтому условие «есть путь вне
  документации» срабатывает, и деплой запускается. Проверено локально: тот же
  `grep -Ev` на том же диапазоне печатает ровно эту строку.

  Все PRD в репозитории названы по-русски, то есть **любой коммит, добавляющий
  новый PRD, перезапускает прод**, даже если кода в нём нет. Пропуск, который
  видели 27.08 (`c736bf6`, `Deploy Prod` — `skipped`), работал только потому,
  что там менялись файлы с латинскими именами.

  Диагноз подтверждён A/B прямо на боевом CI: следующий коммит `22a85f9` — те же
  два файла с латинскими именами, без нового PRD — дал `deploy: skipped`, и
  контейнер на проде не тронут. Отличие между коммитами ровно одно: имя
  добавленного файла.

  Лечится одной строкой: `git -c core.quotepath=false diff --name-only` либо
  `--name-only -z` с разбором по NUL. Отдельным ходом, с проверкой на коммите
  с кириллическим именем файла.

## Этап 255. Тестовые стенды перепутаны — `done`

Источник: разбор 23.08.2026. Bearer-токен, которым ходит MCP-клиент, и REST-ключ
`TEST_API_KEY` из `.env` указывают на **разные** контуры:

| | стенд под REST-ключом (devtr6) | стенд под bearer-токеном |
|---|---|---|
| клиник | 2 | 1 |
| питомцев | 50 | 375 |
| сотрудников | 21 | 8 |
| счетов | 114 | 652 |
| закрытий счетов | 134 | 0 |

Обнаружилось это неприятным способом: медкарта с `id = 6` на двух стендах
разная, и проверка едва не пошла по чужим данным. Пункт 5.3 давнего этапа 5
фиксировал, что `devtr6` — выделенный тестовый стенд, но какой контур стоит за
bearer-токеном, нигде не записано.

Пока это не разведено, любая фраза «проверил на devtr6» требует уточнения,
чем именно проверял — сырым REST или инструментом.

- 255.1 Стенд под bearer-токеном — `devslon67` (`devslon67.vetmanager2.ru`),
  под REST-ключом — `devtr6`. Сборки разные: одна и та же запись медкарты
  проходит на `devtr6` и падает с 500 на `devslon67`. — `done`
- 255.3 Два стенда не нужны — решение Владимира 02.09.2026: свести проверки
  на один. Оставляем `devtr6`: на нём работают все живые пробы, там проходит
  запись медкарты, и именно он записан выделенным тестовым стендом с этапа 5.
  `devslon67` отличается сборкой и на записи отдаёт 500 — держать контур,
  который ведёт себя не как прод, значит проверять не то. — `done`
- 255.4 Перевести bearer-токен тестового MCP-клиента на `devtr6`. Требует
  действия на стороне Ветменеджера — контур за токеном выдаём не мы.
  Сделано 06.09.2026: владелец выдал новый service-токен, и контур за ним
  проверен **данными, а не названием** — 2 клиники, 50 питомцев, 21 сотрудник,
  114 счетов, то есть строка `devtr6` из таблицы выше до последней цифры; в
  ответе `get_clinics` стоит `https://devtr6.vetmanager2.ru/`. Расхождение
  контуров закрыто: «проверил на devtr6» больше не требует уточнения, чем
  именно. — `done`
- 255.2 Записать в README один контур рядом с `TEST_DOMAIN`; пока перевод не
  сделан — записать и текущее расхождение, чтобы фраза «проверил на devtr6»
  оставалась однозначной. Сделано 04.09.2026: раздел «Тестовый контур: какой
  стенд считается нашим» с таблицей «чем проверяю → какой стенд отвечает» и
  способом определить, где находишься (число клиник: на `devtr6` две, под
  bearer-токеном одна). Расхождение проверено в тот же день и сохраняется. — `done`

## Этап 256. Четыре этапа без записи в AssumptionLog — `done`

Источник: `review_workflow_check.sh` 23.08.2026. Гейт сообщает, что закрытые
этапы 216, 229, 233 и 234 не имеют секций в `AssumptionLog.md`.

Заодно нашлась причина, по которой список был длиннее: запись этапа 239 —
единственная из 301 — была набрана с одной решёткой вместо двух, и гейт её не
видел. Исправлено, вместе с этим дописаны 240, 242 и 249.

Оставшиеся четыре дописывать по памяти нельзя: смысл записи в том, какие
допущения принимались **в момент работы**. Нужен проход по коммитам и PR этих
этапов, либо честная отметка, что запись утрачена.

- 256.1 Восстановить допущения 216, 229, 233, 234 и 243 по коммитам и PRD.
  243 добавлен 02.09.2026: `review_workflow_check.sh` называет пять этапов,
  а пункт был написан по четырём. Сделано 04.09.2026: все пять записей собраны
  из PRD и коммитов (`09b78b2`, `048c673`, `e72eab7`, `e9ff2aa`), у каждой
  указано, откуда она восстановлена. — `done`
- 256.2 Что не восстанавливается — отметить явно, а не выдумывать. Отмечено:
  у этапа 216 нет коммита с номером — изменения documentation-only, уехали
  вместе с правками пайплайна; у этапа 233 причины трёх отказов из четырёх
  записаны как утраченные, а не как «наверное, сеть»; пункт 234.3 записан как
  `stop`, то есть не сделанный, а не закрытый. — `done`
- 256.3 Проверить в гейте уровень заголовка, чтобы `#` вместо `##` больше не
  прятал запись. Сделано 04.09.2026, и оказалось, что дыра шире, чем описана:
  гейт требовал ровно `^## `, поэтому невидимой была не только запись с одной
  решёткой (239), но и пять записей с тремя — 285, 288, 290, 291, 292, все
  написанные в день работы. Гейт назвал их «отсутствующими», и список из
  четырёх этапов вырос до одиннадцати. Проверка отвечает на вопрос «есть ли
  запись», и уровень заголовка к нему отношения не имеет: шаблон стал `^#+ `.
  После правки список сократился с 11 до 6, а после восстановления пяти
  записей и дописи 297 — до нуля. — `done`

## Этап 257. «Настроить вручную» не говорит, что там выпускают токен — `done`

Источник: замечание Владимира 23.08.2026. В кабинете аккаунта раскрывающийся
блок называется `Настроить вручную` (`web_html.py:1747`), а подсказка над ним —
«Нужны другие права, срок или IP-маска — раскройте „Настроить вручную“ ниже».

Из названия не видно ни что произойдёт, ни ради чего: слово «настроить» обещает
изменение настроек, тогда как внутри **выпускается Bearer-токен** — то, за чем
человек на страницу и пришёл. Быстрая форма выше выпускает токен в один клик, и
ручной блок читается как что-то необязательное и техническое.

Предлагаемое название — **«Выпустить Bearer-токен вручную»**: в двух словах и
действие, и объект. Запасной вариант — «Выпуск токена: права, срок, IP».

- 257.1 Переименовать блок и переписать подсказку над ним. — `done`
- 257.2 Пройти глазами остальные подписи на странице токенов — не осталось ли
  там ещё формулировок, описывающих механику вместо результата. — `done`

## Этап 258. По умолчанию токен привязывается к IP браузера — `done`

Источник: решение Владимира 23.08.2026 — ограничения по IP по умолчанию быть не
должно, по умолчанию любой адрес.

Сейчас наоборот. В `web_routes_account.py:444` пустое поле маски молча
подменяется на `request_ip` — адрес браузера, из которого админ клиники нажал
кнопку. Ходит же по токену не браузер, а сервер Anthropic или OpenAI, с совсем
другого адреса. То есть дефолт привязывает токен ровно к тому, кто им
пользоваться не будет: токен мёртв с рождения, и падает молчаливым
`403 ip_denied`. Wildcard при этом требует отдельной галочки, без неё
выпуск отклоняется.

За 30 дней `token_auth_failed_ip_denied` равен нулю — значит все шли через
быструю форму с радиокнопкой «любой IP». Но ручной путь устроен обратно тому,
как сервисом пользуются.

Ограничение по IP остаётся доступным — оно перестаёт быть умолчанием.

- 258.1 Пустое поле маски = `*.*.*.*`, без подстановки адреса браузера. — `done`
- 258.2 Убрать требование отдельного подтверждения для wildcard: подтверждать
  надо сужение, а не норму. — `done`
- 258.3 Проверить, что предупреждающий лог о wildcard-выпуске остаётся —
  наблюдаемость терять не нужно. — `done`
- 258.4 Сверить с этапом 155, который делал маску обязательной: понять, какую
  угрозу он закрывал, и не открыть её обратно молча. — `done`

## Этап 259. Прямой агрегат счетов по врачу и периоду — `stop`

Источник: воспроизведение этапа 252. Модель шла в Report AI не за отчётом, а
за суммами по врачу и периоду, которых MCP не даёт. `get_invoices` даёт
doctor_id и amount, но только пагинированные строки (на devtr6 114 за весь
доступный набор); полная пагинация ради суммирования — плохой ответ на KPI.
Нужен отдельный scoped helper с подтверждённой семантикой сумм, статусов и
границ периода; не реализовывать его как побочный эффект исправления очереди.

**Решение владельца 05.09.2026: отдельного инструмента не делаем — такие вопросы
идут через отчёты Ветменеджера.** Это согласуется с правилом этапа 288
(«массовое берём отчётом») и снимает соблазн ещё одного пути, по которому агент
может пойти циклом. Инструмент-агрегат выглядел бы дешевле отчёта и потому
перетягивал бы на себя запросы, ради которых и заводился Report AI.

- 259.1 Подтвердить API-поля, статусы и дату для суммы invoice KPI. — `stop`
- 259.2 Спроектировать ограниченный агрегат по врачу/периоду без полной
  выгрузки clinic data в модель. — `stop`
- 259.3 Сравнить стоимость client-side pagination и upstream aggregate API. — `stop`

## Этап 260. Вызовы по OAuth не попадают в метрики — `done`

Источник: недельный срез 26.08.2026. Успешную аутентификацию пишет только
bearer-путь (`auth/bearer.py`), поэтому `token_usage_logs` описывает один из
двух каналов доступа. От OAuth остаётся единственный след — `last_used_at` на
выданном access-токене: ни времени вызова, ни их числа, ни инструмента.

Что это уже испортило: недельный дашборд показывал 5 активных аккаунтов вместо
8; разбор застрявших от 25.08 записал в «выпустил доступ и молчит» троих, кто
на самом деле работает (id 15 вызывал в день разбора), а аккаунт без bearer-
токена не попал ни в один срез вообще. Любой вывод вида «аккаунт не дошёл» на
текущих данных недостоверен.

- 260.1 Зафиксировать, какие события пишет каждый канал сейчас, и чего не
  хватает, чтобы считать вызовы одинаково. — `done`
- 260.2 Писать событие успешной аутентификации на OAuth-пути тем же
  контрактом, что и bearer, не ломая существующие агрегаты. — `done`
- 260.3 Проверить, что санитайзер и правила приватности применяются к новому
  событию так же (IP, домен клиники). — `done` (общий helper, отдельного пути нет)
- 260.4 Перевести потребителей активности на общий счётчик. — `done`
  (телеметрия, кабинет, отчёт, архиватор; выгрузки в LiveHelperAgent —
  после накопления данных, см. пункт 262.1)
- 260.5 Убедиться, что архиватор зомби-аккаунтов не считает OAuth-аккаунт
  неактивным. — `done`

## Этап 261. Обратная связь копится непрочитанной — `done`

Источник: разбор фидбека 26.08.2026. В `agent_feedback_reports` два отчёта
висят в статусе `new` (25.08: `get_report_ai_job` от работающего клиента и
`Vetmanager task area` с severity `high`), а `known_issue_match_events` за
30 дней пуст — сопоставление свежего отчёта с известной проблемой не
происходит ни разу. Привязка `known_issue_id` у прошлых отчётов проставлена
руками.

Отдельно: known issue 31 (`update_medical_card`) остаётся `acknowledged` без
обходного пути, хотя этап 245 закрыт и инструмент подставляет связи сам —
запись в базе отстала от кода и вводит агента в заблуждение.

- 261.1 Описать, кто и когда разбирает `new`, и что считается разбором. — `done`
  (`artifacts/feedback-triage-runbook.md`)
- 261.2 Понять, почему match events не появляются. — `done`
  (отдача агенту была ограничена статусом `workaround_available`, тогда как
  проблемы заводятся в `acknowledged`; ручная привязка событий не пишет и не
  должна. Готовность теперь определяется наличием playbook)
- 261.3 Обновить known issue 31 под фактическое состояние после этапа 245. — `done`
  (31 → `fixed`, `fixed_in_version=stage-245`; заодно записаны обходные пути
  для 32 и 33 — они не починены, но агент теперь получает ответ)
- 261.4 Ответить на отчёт 47 (нет инструмента задач Ветменеджера). — `done`
  (в 79 REST-моделях Ветменеджера сущности пользовательских задач нет; заведена
  known issue 34 со статусом `wontfix` и playbook, отчёт закрыт как `triaged`)

## Этап 263. Обходной путь нельзя записать штатной командой — `done`

Источник: этап 261. Добавить playbook существующей известной проблеме можно
только прямым SQL: `mark` меняет статус, `promote` создаёт новую проблему,
команды «дописать ответ к существующей» нет. Ответы на 32 и 33 пришлось
записывать вручную на проде — это не воспроизводимо и не проверяется
валидатором до записи.

- 263.1 Команда `set-playbook <known_issue_id> --playbook-json` с проверкой
  валидатором до записи. Сделано 04.09.2026 в этапе 294.4 — дубль: я завёл
  ту же команду заново, не заметив, что пункт под неё уже стоял здесь с
  26.08.2026. Открытый пункт в чужом этапе не виден, когда идёшь от свежей
  задачи; смотреть очередь целиком до того, как заводить новый этап. — `done`
- 263.2 Прогнать ей же ответы, записанные вручную 26.08.2026, чтобы проверить
  совпадение. Сделано 05.09.2026: playbook проблем #32 и #33 выгружены,
  поданы обратно через `set-playbook` и перечитаны. Содержимое совпало у обоих
  (`was_reachable=True` до и после), различается только форматирование JSON —
  команда его нормализует. Значит ручной SQL больше не нужен, а сами ответы
  проходят валидатор, чего при записи напрямую никто не проверял. — `done`

## Этап 264. Отказ по доступу ничего не объясняет — `done`

Источник: разбор идеи прямого шлюза Report AI 26.08.2026. Токен «только
чтение» может читать строки задания, но не может сохранить отчёт, без
которого этих строк не существует (known issue 20). Тупик звучал как
`Bearer token lacks required scope 'report_ai.write'` — технически верно и
бесполезно: по такому ответу нельзя понять, возможности нет вовсе, вызов
собран неправильно или просто нужно попросить доступ.

Решение владельца: честно говорить, что инструмент есть, но прав на него нет.

- 264.1 Назвать в отказе саму возможность, требуемый доступ словами кабинета
  и того, кто его выдаёт. — `done`
- 264.2 Не повторять вызов: в тексте сказано, что с текущим токеном результат
  не изменится. — `done`
- 264.3 Сказать это там, где агент действительно читает: проверка прав уровня
  инструмента срабатывает до обращения к Ветменеджеру, поэтому текст в
  HTTP-клиенте агенту почти не виден. — `done`

## Этап 265. Защитная сетка не заметила недостижимое сообщение — `done`

Источник: разбор 26.08.2026 после этапа 264. Улучшенный текст отказа лёг в
слой, до которого выполнение при запрете не доходит. Тесты были зелёные,
покрытие было, ревью написало «не ломает» — и никто не спросил, увидит ли
этот текст кто-нибудь вообще. Поймал вопрос владельца, а не проверка.

Рядом в тот же день выстрелил второй класс: обработчик распознавал отказ по
подстроке человеческого сообщения, а сообщение переписали. Ветка замолчала бы
незаметно, потому что тесты правятся вместе с текстом.

- 265.1 Проверять пользовательские тексты через ту же границу, в которую
  входит агент. — `done` (120 инструментов плюс HTTP-путь коннектора)
- 265.2 Запретить распознавание ошибок по человеческому тексту. — `done`
  (`scripts/check_error_text_matching.py`; ревью нашло семь обходов за два
  захода, все закрыты и закреплены специменами)
- 265.3 Линтер мёртвых импортов и переменных. — `done` (ruff F401/F841)
- 265.4 Правило для тестов-сторожей. — `done` (`CLAUDE.md` §4.0)
- 265.5 Признак «ошибка ввода» сделать типом. — `done` (`ToolInputError`)
- 265.6 Типизировать валидацию в самих инструментах. — `done` (27.08.2026)
  Нашлось при 265.5: проверки вида «clinic_id must be a positive integer» не
  начинались с «invalid »/«missing », поэтому старое правило их никогда не
  ловило — пользователю предлагали сообщить о дефекте в ответ на его же
  опечатку. Пятнадцать таких мест типизированы; в `tools/` `ToolError` больше
  не конструируется вовсе — есть пара `ToolInputError` / `reportable_error` и
  проверка без исключений (`scripts/check_tool_error_classification.py`).
  Ошибка ввода больше не идёт в `capture_tool_failure`. **Но issue в Sentry
  она всё ещё заводит** — не исключением, а сообщением уровня error от
  логгера `fastmcp.server.server`; проверено живым вызовом 28.08.2026, см.
  этап 266.

Сознательно не делаем: порог покрытия (нашу ошибку он не ловит — покрытие
было) и мутационное тестирование целиком (часы прогона ради класса, который
закрывается правилом 265.4).

## Этап 266. Проверка ввода заводит issue в Sentry мимо наших фильтров — `done`

Источник: этап 265.6, 27.08.2026; подтверждено живой проверкой 28.08.2026.

**Дымная проверка на проде показала, что дыра шире, чем считалось.** Три
вызова с неверным аргументом (`tag_id=0`, битый `filter_json`,
несуществующий `tag_id`) вернулись вызывающему чисто, без приглашения
сообщить о проблеме — и при этом завели в Sentry три issue: `PYTHON-10`,
`PYTHON-11`, `PYTHON-M`. Это не исключения, а события-сообщения уровня error
от логгера `fastmcp.server.server` с текстом `Error calling tool 'X'`.
`capture_tool_failure` их не отправлял — отправила логгинг-интеграция. То есть
типизация из 265.6 закрыла приглашение, но не шум: `ToolInputError` попадает
в Sentry ровно так же, как и `ValueError`.

В `tools/` 59 проверок поднимают голый `ValueError`. До приглашения сообщить о проблеме они не доходят — наша обёртка
ловит только `ToolError`, — но каждая заводит issue: `ValueError` вылетает из
инструмента, FastMCP пишет `logger.exception`, и Sentry подхватывает это
логгинг-интеграцией. Наш `capture_tool_failure` тут ни при чём, поэтому и
фильтр наш не действует.

Живое доказательство того же обхода: в списке открытых issue четыре
`FilterPropertyValidationError`, хотя исключение для них в
`tool_error_tracking.py` стоит с этапа 236. Фильтр стоит на пути, по которому
эти события не идут.

Цена: то самое поле, ради которого затевается этап 243 (сколько пользователей
задето), считается по потоку, куда свободно попадают чужие опечатки.

- 266.1 Разобрано по одному месту, а не скопом: 65 проверок в `tools/` и
  `validators.py` разложены на ошибку вызывающего (`ToolInputError`), сломанные
  данные (`reportable_error`) и невозможное состояние нашего кода
  (`invariant_error`). Ревью назвало поимённо те, что нельзя было объявлять
  ошибкой ввода: предел `paginate_all`, `_money` над строкой из ответа ВМ,
  отсутствие записи в `update_admission`/`update_medical_card`. — `done`
- 266.2 Логгер `fastmcp.server.server` не пускается в Sentry
  (`ignore_logger`). Распознавать чужой текст не понадобилось. Middleware
  расширена до `FastMCPError`, кроме схемной `ValidationError`, — иначе
  вместе с шумом ушёл бы целый класс отказов, приходивший только логгером. — `done`
- 266.3 Подтверждено на живой выкатке 28.08.2026, 03:00. До правки три
  опечатки давали три issue; после — две опечатки (`tag_id=0`,
  несуществующий `tag_id`) не дали **ни одного**. Отказ апстрима дал ровно
...
Источник: деплой 27.08.2026. `Deploy Prod` упал дважды подряд на `apt-get
install curl`: с этого VDS **IPv4-адреса Fastly не отвечают**, а
`deb.debian.org` живёт именно там. Хост это скрывал — он ходит туда по IPv6 и
отвечает 200; у контейнеров IPv6 нет, поэтому сборка вставала, а проверить
руками «работает ли сеть» было мало: `curl` с хоста говорил, что работает.

Раньше не всплывало, потому что слой с `apt` брался из кеша. Как только кеш
слоя обновился, зависимость от чужой сети стала блокирующей.

`curl` в образе нужен был ровно для `HEALTHCHECK`, а в образе уже есть Python,
умеющий сделать тот же запрос. Шаг `apt` убран из стадии `base`; выкатка
собирает только `--target production`, поэтому путь через прод-VDS к пакетным
репозиториям больше не обращается. Тестовая стадия к ним обращается
по-прежнему — `playwright install --with-deps` ставит системные библиотеки, —
но она собирается на раннерах и машинах разработки, не на проде.
...
- 267.4 Возвращены две черты `curl`, потерянные при переводе и найденные
  ревью: дедлайн на весь запрос (`urlopen` ограничивает одну операцию с
  сокетом, а не передачу) и отказ следовать редиректам (вызов был без `-L`).
  Обе закреплены тестами, оба сторожа показаны красными. — `done`
- 267.5 Диагностика отказа не должна быть беднее, чем у `curl`: при обрыве по
  дедлайну сохраняются уже полученный статус и частичное тело, а у 3xx
  печатается `Location` — иначе оператор не видит, куда его редиректят. — `done`


## Этап 268. Удаление счетов через REST обходит предметную логику — `done`

Источник: пользовательский разбор нашего MCP 31.08.2026 и последующая ревизия
групп доступа. Разбор по `vetmanager-extjs` показал, что `delete_invoice` и
`delete_invoice_document` не запрещены формально, но идут мимо предметного
удаления: `ERestController::doRestDelete` делает `$model->delete()` на голом
`CActiveRecord`, тогда как настоящее удаление счёта живёт в
`Entity\Invoice::delete()` — только статус «Отложен», мягкое удаление,
откат складских документов, пометка вакцинаций, событие и журнал.

Наблюдаемые следствия: на счёте с позициями вызов упирается в ограничение базы
и возвращает 406; на пустом счёте проходит, включая исполненный и оплаченный,
мимо всех правил; удаление позиции оставляет сумму счёта и складской учёт
несогласованными.

Решение Владимира 31.08.2026: снять оба инструмента совсем, не пере-выдавая
под другим правом, и закрепить запрет так, чтобы их не завели заново. Запрос
на честный REST-метод удаления заведён отдельной задачей в Битрикс24.

- 268.1 Убрать `delete_invoice` и `delete_invoice_document` из инструментов,
  реестра прав и обещанных составов групп. — `done`
- 268.2 Сторож, не дающий вернуть их: инструмент с таким именем не должен
  регистрироваться, а вызов снятого имени должен читаться как «инструмента
  нет», а не «не хватает прав». Показан красным на временно возвращённом
  инструменте. — `done`
- 268.3 Запрет и причина записаны в `AssumptionLog.md`, в docstring
  `update_invoice` и оговоркой в `artifacts/api_crud_permissions-ru.md` —
  именно из этой таблицы удаление счетов когда-то и вывели. — `done`

## Этап 269. Запуск отчётов отделён от чтения аналитики — `done`

Источник: та же ревизия. `create_report_ai_job` требует только
`analytics.read`, а сам принимает свободный текст, по которому Ветменеджер
строит отчёт по любой таблице клиники. Скоуп есть у «Регистратуры», «Врача»
и «Только чтение» — то есть группа, которой отказано в медкартах напрямую,
получает их отчётом.

Решение Владимира: запуск отчётов нужен «Аналитику», остальным группам —
только чтение.

- 269.1 Переведены на `report_ai.write`: `create_report_ai_job`,
  `confirm_report_ai_job_candidate`, `start_report_export` и
  `get_report_ai_job_export` — последний читающий по имени, запускающий по
  сути. — `done`
- 269.2 Карта REST-путей разделена так же: любой `POST /report-ai-job*` и
  `GET /report/StartReport` требуют `report_ai.write`, чтение задания, данных
  и готового файла — `analytics.read`. — `done`
- 269.3 Тесты: «Регистратура», «Врач» и «Только чтение» получают отказ на
  запуск отчёта и проходят на чтении; «Аналитик» и полный доступ — весь поток.
  Обещанные составы групп подчищены. — `done`

## Этап 270. Удаление вынесено из общих прав на запись — `done`

Источник: та же ревизия. Отдельного права на удаление нет — `delete_client`
требует тот же `clients.write`, что и создание клиента. Поэтому «Регистратура»
удаляет клиентов и питомцев, о чём в её составе не сказано.

- 270.1 Заведено право `records.delete`, на нём `delete_client` и
  `delete_pet`. — `done`
- 270.2 Право входит только в «Полный доступ». Второй слой требует его на
  любой `DELETE` под `/rest/api/`, включая пути, которые не удаляет ни один
  инструмент. Ключи полного доступа, выпущенные раньше, распознаются снимком
  набора прав и дополняются — сторож показан красным. — `done`
- 270.3 Тесты: «Регистратура» создаёт и меняет клиента и питомца, но не
  удаляет; ни одна группа, кроме полного доступа, не проходит на удаление. — `done`

## Этап 271. Список инструментов не фильтруется по правам — `done`

Источник: тот же пользовательский разбор — автор посчитал 121 инструмент и
26 пишущих среди них, потому что `tools/list` одинаков у любого токена.
Ключ «Только чтение» рекламирует `delete_client` и `send_message_to_all`;
отказ приходит только на вызов.

- 271.1 `ToolVisibilityMiddleware` на `on_list_tools` фильтрует каталог тем же правилом, что и отказ на вызов. Права читаются в обход журнала: обычный резолвер писал бы каждый запрос списка в метрики активации как работу с данными. — `done`
- 271.2 Инструменты без требуемых прав остаются видимыми; не-HTTP транспорт,
  неудачная аутентификация и сбои оставляют список полным, последний случай
  логируется. — `done`
- 271.3 Тесты: состав каталога сверяется с самим отказом для каждой группы; фильтр и регистрация middleware показаны красными; отдельным файлом закреплено, что чтение прав для каталога не мягче аутентификации и ничего не пишет в базу. — `done`

## Этап 272. Обещанный состав групп разошёлся с реальным — `done`

Источник: та же ревизия. Реальный состав группы — инструменты, чьи права
входят в её набор; обещанный — `MARKETED_PRESET_TOOLS`. Расхождение: у
«Регистратуры» 77 против 28, у «Врача» 59 против 17, у «Аналитика» 97 против 8.
Тест проверяет только, что обещанное входит в реальное; обратной проверки нет,
поэтому избыток не ловится.

- 272.1 Обещанный список удалён: состав группы считается из её прав тем же правилом, что применяет проверка вызова. — `done`
- 272.2 Обратное включение проверять не нужно — списка два больше нет. Вместо этого закреплено: состав совпадает с тем, что пропускает проверка вызова; подсказка в отказе называет все дающие группы (было верно у 27 инструментов из 119); инструмент с пустыми правами обязан быть в поимённом списке, иначе он рекламировался бы всеми и запрещался на вызове. Сторож показан красным. — `done`

## Этап 273. Интерфейс и отказ не говорят, что даёт группа — `done`

Источник: та же ревизия. Под выбором уровня доступа написано «определяет, что
помощник сможет видеть», хотя пять групп из семи дают запись. Состава группы
не видно нигде. В тексте отказа список допустимых групп строится по обещанному
составу и неполон у 100 инструментов из 119, а первым всегда стоит «Полный
доступ» — подсказка сама толкает к полному ключу.

- 273.1 Под выбором уровня — сводка тремя строками по областям: чтение, изменение, удаление. Считается из прав, переключается вместе с выбором; блок выбранного уровня отрисован сервером, поэтому без JavaScript страница остаётся осмысленной. Те же строки на экране согласия OAuth. — `done`
- 273.2 Подпись переписана. «Удаление: нет» показывается всегда — именно отсутствие этой строки и породило репорт. — `done`
- 273.3 Порядок групп в отказе: узкие раньше широких, полный доступ
  последним. Правило единое для обоих мест, которые называют группы —
  отказа на инструменте и отказа на REST-пути. — `done`

## Этап 274. Второй слой проверки прав fail-open — `done`

Источник: та же ревизия. `required_scope_for_request` пропускает неизвестный
путь. Карта записи покрывает 12 сущностей, а инструменты ходят по 31 пути, где
на запись не требуется ничего. Сегодня дыры нет — до них добираются только
инструменты, закрытые первым слоем, — но подстраховки на случай ошибки в
первом слое нет.

- 274.1 Пересчёт по реальным местам вызова: из 23 пишущих вызовов без права были два — `POST` и `PUT` на `/rest/api/Suppliers`. Замаплены. Прежняя оценка «31 путь» перемножала пути на методы, которых в коде нет. — `done`
- 274.2 Неизвестный пишущий путь и любой метод вне разбора (`PATCH`, `HEAD`, …) отклоняются до обращения к Ветменеджеру. Чтение пока проходит, но считается метрикой `vetmanager_rest_unmapped_read_total` — закрытие следующим этапом по её значению на проде. — `done`
- 274.3 Полнота карты закреплена разбором исходников `tools/` и `resources/`
  через AST: обход литералов регуляркой счёл бы пишущий путь читающим, потому
  что метод задаётся слоем помощников. Сторожа показаны красными. — `done`

## Этап 275. Деперсонализация не действует на отчёты — `done`

Источник: разбор 31.08.2026 после этапа 274. Санитайзер скрывает персональные
данные **по имени поля**: словарь из девяти ключей для имени и пяти для
телефона, все английские. Колонки отчёта называет сгенерированный SQL, а
задание пишется по-русски — значит и колонки русские. Проверено на живом коде:
строка `{"Владелец": "Иванов Пётр Сергеевич", "Телефон": "+7 918 414-01-11"}`
уходит клиенту без изменений, тогда как та же строка с ключами `client_name` и
`cell_phone` обезличивается.

Деперсонализация — состояние по умолчанию и в кабинете, и на экране согласия.
То есть владельцы ключей считают, что ФИО и телефоны скрыты, а через отчёт они
приходят.

Решение владельца продукта 31.08.2026: три слоя защиты плюс честная
формулировка в интерфейсе.

- 275.1 Русские ключи в словаре: владелец, хозяин, клиент, ФИО, фамилия, отчество, телефон, почта, адрес и их частые формы. Бесквалификаторное «имя» и «название» намеренно не берутся — это чаще кличка или товар. — `done`
- 275.2 Чистка значений в любых колонках отчётных инструментов: почта,
  телефоны (10–11 цифр, даты и микрочипы не трогаются), ФИО из трёх слов,
  «Имя Отчество» по суффиксу отчества, фамилия с инициалами, почтовый адрес.
  Обычные инструменты не затронуты. — `done`
- 275.3 Требование в `intent_text` для деперсонализированного токена: текст
  постоянный, чтобы не ломать дедупликацию заданий, а его длина резервируется
  до проверки лимита и только для тех токенов, которые его получают. — `done`
- 275.4 Сторож связи: право на отчёты не выдаётся группе без полного набора
  читающих прав, которые вообще используются инструментами. — `done`
- 275.5 Формулировка в кабинете и на экране согласия: три слоя, известный остаток и то, что выгрузка CSV/XLSX скачивается из Ветменеджера мимо этих слоёв. — `done`
- 275.6 Попытка обхода проведена, 18 сценариев; три прошли, два закрыты
  дополнительными правилами, один остался и назван в тексте интерфейса.
  Матрица зафиксирована тестом. — `done`

## Этап 276. Выгрузка отчёта проходит через MCP, а не мимо него — `done`

Источник: решение владельца продукта 01.09.2026 по остатку этапа 275. Живая
проверка на devtr6 в тот же день показала, чем на самом деле был локатор
выгрузки: `csv_file`/`xlsx_file` — абсолютная ссылка на публичный CDN,
которая отдаёт файл **любому, без единого заголовка авторизации**. То есть
выгрузка не просто шла мимо трёх слоёв защиты — она лежала в открытом доступе
целиком, со всеми ФИО и телефонами, пока жила на CDN.

Решение: выгрузку скачивает сервер, чистит теми же слоями, что и строки
отчёта, и отдаёт агенту собственную ссылку с непредсказуемым путём и сроком
жизни трое суток. Инструмент, отдававший локаторы, снят.

- 276.1 Скачивание и очистка на нашей стороне: новый инструмент отдаёт ссылку, а не локатор; исходник живёт только в памяти запроса и на диск не попадает. — `done`
- 276.2 Ссылка и отдача: путь из двух ключевых хешей, ключ HMAC на серверном секрете, отдача вложением без индексации, возраст проверяется на выдаче. — `done`
- 276.3 Срок жизни трое суток: подметание каталога при создании выгрузки, отказ по возрасту при обращении. — `done`
- 276.4 Инструмент получения локаторов снят: реестр прав, описания, README, тесты. — `done`
- 276.5 Формулировка в кабинете и на экране согласия: выгрузка больше не идёт мимо слоёв. — `done`
- 276.6 Живой прогон на devtr6: старт выгрузки, скачивание, очистка, отдача по ссылке. — `done`

## Этап 277. Словарь имён закрывает остаток из двух слов — `done`

Источник: решение владельца продукта 01.09.2026. Этап 275 закрыл ФИО из трёх
слов и «Имя Отчество» по суффиксу, но честно назвал остаток: имя из двух слов
без отчества — `Иванов Пётр` — по форме неотличимо от названия клиники или
улицы, и правилом формы его не взять. Решение: взять его не формой, а
словарём русских личных имён.

Правило срабатывает только на паре заглавных слов, где одно — известное личное
имя. Одинокое слово не трогаем: кличка питомца и товар тоже бывают именами.

- 277.1 Словарь личных имён: полные формы, распространённые уменьшительные, латинские транслитерации. Модуль в репозитории, не файл данных. — `done`
- 277.2 Правило пары: два слова с заглавных, одно из словаря — обезличиваем. Порядок слов любой. — `done`
- 277.3 Сторож от ложных срабатываний: значение с маркером организации (клиника, аптека, ООО, ИП, центр) правилами имени не трогается — по образцу уличного маркера этапа 275. — `done`
- 277.4 Попытка обхода: матрица личных и не-личных значений, зафиксированная тестом; стоимость очистки измерена. — `done`
- 277.5 Формулировка в кабинете и на экране согласия: остаток из двух слов больше не заявляется как открытый; названо, что осталось. — `done`
- 277.6 **Регресс этапа 275, найденный этой же матрицей:** правило ФИО из трёх слов было написано с общим флагом регистронезависимости, из-за чего любые три слова подряд в ячейке отчёта заменялись на `[redacted-name]` — `Стрижка когтей собаке`, `Анализ крови общий`. Формы перечислены явно вместо флага; строчное написание имени возвращено точечно, через словарь и суффикс отчества. — `done`

## Этап 278. Чужой локатор не должен уезжать в наш лог — `done`

Источник: живой прогон этапа 276 на devtr6 01.09.2026. В выводе видно, что
`httpx` пишет адрес запроса на уровне INFO. Прод запущен с `LOG_LEVEL=INFO`,
а `PERSISTENT_LOG_PATH` кладёт лог на диск — значит публичный CDN-адрес
неочищенной выгрузки со всеми ФИО и телефонами попадает в постоянный файл.

Этап 276 закрыл ровно этот класс утечки для собственного пути выгрузки — и
пропустил чужой, потому что его пишет не наш код, а библиотека.
...
- 279.1 Гейт `scripts/check_roadmap_structure.py`: статус этапа из закрытого
  ...
  один пункт. Нумерация вложенная: этапы идут и `234`, и `2.5`, пункты под
  ними — `2.5.1.1`; принадлежность определяется префиксом с точкой. Каждая
  проверка показана красной на образце. — `done`
...
## Этап 280. Отказ вызывающего и наша поломка — разные вещи — `done`
...
- 280.1 Разобрать по кодам, какие upstream-отказы означают ошибку вызывающего,
  ...
  скопом нельзя — замолчим свои же поломки.
  Сделано 04.09.2026. Разобраны все восемь кодов `JobException` по местам, где
  апстрим их поднимает. Однозначен ровно один: все четыре места
  `INVALID_TRANSITION` говорят о статусе job против запрошенной операции, то
  есть о порядке вызовов. Он больше не приглашает завести баг.
  Остальные оставлены приглашающими, и это решение, а не недосмотр:
  `VALIDATION_ERROR` покрывает и «Некорректный id job», и «{поле} должен быть
  целым числом» о запросе, который собрали мы; `FORBIDDEN` — и «Нет доступа к
  этой job», и «Клиника не определена», то есть непрошедшую авторизацию;
  `INTENT_REJECTED` поднимается, когда распознаватель апстрима не вернул
  структуру; `SANITIZER_REJECTED` — когда санитайзер отверг сгенерированный
  апстримом SQL. Причины записаны рядом с кодом, чтобы список не разросся
  без такого же разбора. — `done`

## Этап 281. Проверить на живом событии, что трекинг не носит чужих данных — `done`
...
**Проверка 02.09.2026.** Разобрано **80 событий** — самое новое и самое старое
у каждого из 40 issue проекта `python` за две недели (21.08–02.09). Значения
не выгружались в переписку: разбор печатал путь поля, детектор и маску.

Чего в событиях **нет** ни одного: email, JWT, bearer-токен, телефон. Личных
имён по словарю этапа 277 — ни одного. Санитайзер срабатывает **3722 раза в
260 разных местах**; `Authorization`, `X-Forwarded-For`, `X-Real-Ip`,
`Mcp-Session-Id` и `X-Openai-Session` вычищены в 100% случаев.

Найдено другое — чужие данные не клиники, а **вызывающего**:

- **`X-Openai-Subject` не вычищается** (4 события). Это устойчивый
  идентификатор конечного пользователя на стороне вызывающей платформы.
  Соседний `X-Openai-Session` вычищается — но только потому, что в его имени
  случайно оказалось слово `session`. Правило про личность вызывающего в
  списке отсутствует.
- **Метаданные запроса утекают через переменную фрейма.** Заголовки чистятся
  в `request.headers`, но их копия в локальной переменной `meta_dict` проходит
  мимо: `x-codex-turn-metadata` (310 символов, среди них путь рабочего
  каталога вызывающего), `threadId`, `claudecode/toolUseId`. Соседний
  `progressToken` вычищен — снова случайно, по слову `token` в имени.

Обе находки одного рода: защита держится на том, что в имени поля случайно
встретилось знакомое слово. Ровно так же в этапе 278 адрес выгрузки уехал в
лог мимо нашего кода.

- 281.1 Живое событие в трекинге: убедиться, что в нём нет email и токенов.
  Сделано на 80 событиях; результат выше. — `done`
- 281.2 Личность вызывающего (`subject`) вычищается по правилу, а не по
  случайному совпадению слова. — `done`
- 281.3 Метаданные запроса вызывающего не переживают попадание в переменную
  фрейма. — `done`
- 281.4 Сторож на обе находки; в `AssumptionLog` записано, на каких именно
  событиях проверяли и что искали. — `done`
- 281.5 Ревью диффа: правило опознаёт поле по тому, что это за поле, а не по
  форме события. Чистка спускается внутрь вложенных словарей и списков —
  копия заголовков в локальной переменной больше не проходит под безопасным
  внешним именем; имена листьев метаданных вычищаются сами по себе. — `done`

## Этап 282. Сессию нельзя отозвать на сервере — `stop`

Вынесено из этапа 52 (пункт 52.3.3), закрытого без статуса: работа отложена
«до хранения сессий в БД» и с тех пор не значилась нигде. Найдена гейтом
этапа 279 — до него вложенные пункты вида `52.3.3` разбор не видел вовсе.

- 282.1 Отзыв сессии на сервере **не нужен** — решение Владимира 02.09.2026.
  Срок жизни сессии уже сутки (пункт 52.3.2), а хранение сессий в БД ради
  отзыва стоит дороже остаточного риска. — `done`
- 282.2 Возвращать — только при появлении сценария, где сутки слишком много:
  общий компьютер в клинике, увольнение сотрудника, утечка cookie. — `stop`

## Этап 283. Инжекция известных проблем не сработала ни разу — `done`

Источник: проверка 02.09.2026 по проду. В `known_issue_match_events` за всю
историю продукта — **одна строка**: 28.08.2026, проблема №19, инструмент
`create_report_ai_job`, источник `report`. Событий `injection` и `auto` —
**ноль с мая 2026**.

При этом механизм жив и подключён: `augment_tool_error`
(`agent_feedback_service.py:804`) обёрнута вокруг каждого инструмента в
`tools/__init__.py:63`. База заряжена: 31 проблема из 33 matchable, 13 обходов
с валидным плейбуком, 11 правил написаны аккуратно. Ноль — не про проводку.

Две причины, обе структурные:

- **Отпечатки двух путей не совпадают по построению.** Инжекция считает
  фингерпринт из живого исключения: `error_code = exc.__class__.__name__`
  (`ToolError`), `http_status = None`, `params_shape = None`. Известная проблема
  получает фингерпринт из отчёта, где те же поля заполняет модель, а докстринг
  `report_problem` прямо запрещает ей копировать сырой текст ошибки. 20 проблем
  из 33 имеют только отпечаток и ни одного правила — они не сработают никогда.
- **Правила ждут падения, а проблемы — не падения.** Все 11 правил ищут
  `error_code = ToolError` плюс подстроку в тексте. А известные проблемы в
  массе — успешные вызовы с плохим содержимым: молча потерянная дата в
  `create_admission`, неполный ответ `get_admissions`, вечно висящий отчёт
  Report AI, технические строки в выручке. Ничего не бросается — инжекции не на
  чем сработать.

База знаний собрана ровно из того класса проблем, который механизм доставки не
видит в принципе.

- 283.1 Счётчик попыток инжекции по инструментам и исходам
  (`vetmanager_known_issue_lookups_total`, метки `tool` и `outcome`:
  `matched` / `no_match` / `lookup_failed`). Отказ поиска считается отдельно:
  он ограничен таймаутом, и «не успели посмотреть» не должно выглядеть как
  «посмотрели и не нашли». — `done`
- 283.2 Отпечаток, пришедший из отчёта, перестаёт быть ключом для инжекции.
  Решено явно и сделано видимым: в `unreachable-issues` появилась колонка
  `injection`. `matchable=yes` означало «найдётся, когда человек будет
  разбирать», и читалось как «дойдёт до агента» — это разные вещи. При отказе
  инструмента проблему находит только написанное человеком правило, потому что
  рантайм считает свой отпечаток (`error_code` — имя класса, формы параметров
  нет) и он не совпадает с отпечатком из отчёта по построению. Это тот же
  разрыв, что закрыт с другой стороны этапом 295. — `done`
- 283.3 Ревизия 11 правил против настоящих падений. Проведена 05.09.2026 на
  боевых данных: 13 текстов ошибок из Sentry за 90 дней, за ними **6022
  события**. Каждое правило прогнано по каждому тексту в двух формах — как его
  видит наш код и как он выглядит в Sentry.
  **Совпадений ноль.** Причина видна и конкретна: наборы инструментов не
  пересекаются вовсе. Правила названы на `create_admission`,
  `create_hospitalization`, `get_breeds`, `get_message_reports`,
  `get_report_ai_job_data`, `get_timesheets`, `get_vaccinations`, `tool_search`,
  `create_report_ai_job`/`get_report_ai_job`. А падают на бою
  `get_invoice_by_id`, `get_invoices`, `create_medical_card`, `get_cassa_closes`,
  `get_users`, `get_medical_cards`, `get_report_export_download`,
  `save_report_ai_job_as_report`, `start_report_export`.
  Правила писались из отчётов — из того, на что жаловались, — а не из падений.
  Ни одно не имеет доказательства, что совпало с живой ошибкой, поэтому по
  собственному критерию пункта ни одно и не принимается. — `done`
- 283.4 Вторая точка входа — сверка **успешного** ответа с известными
  проблемами. Цена посчитана: 35 317 успешных запросов за 30 дней против 6022
  отказов за 90 — то есть работа примерно на два порядка чаще, чем сегодня, и
  на горячем пути.
  **Решение владельца 06.09.2026: пока не делать.** Основание — не цена сама по
  себе, а то, что механизм ещё ни разу не сработал: 11 правил против 6022
  настоящих отказов дали ноль совпадений (283.3). Расширять на успешные ответы
  значит увеличить нагрузку примерно в сто раз у механизма, который не работает
  вовсе. Возвращаться сюда — когда хотя бы одно правило совпадёт с живой
  ошибкой; до тех пор чинить надо правила, а не охват. — `stop`
- 283.5 Эффективность инжекции попадает в продуктовые метрики: доставки против
  отказов, а не одно голое число событий. Оговорка «пока не накопится ни одного
  совпадения» снята этапами 305–307.
  Из двух вариантов выбран второй — **отчёт ходит в Prometheus**. Первый
  («счётчик доезжает до БД») означает строку на каждое обращение: цену этого
  измерил этап 305 — 9 400 записей за сутки в пике. Промежуточный вариант,
  почасовая свёртка в своей таблице, отклонён по цене внедрения: миграция и
  запись в горячем пути ради секции, под которой пока 14 наблюдений.
  Разобрано попутно: **числитель нельзя было брать из журнала БД** — в
  `known_issue_match_events` нет колонки `kind`, и разрез по потокам оттуда
  недостижим. Поэтому обе величины берутся из одной метрики одним ответом:
  `matched` против суммы по всем `outcome`. Окно у них совпадает физически, а
  не по подписи.
  Запрос — `sum by (kind, outcome) (increase(...[1h]))` диапазонно, шаг час:
  приращение считает Prometheus по каждому ряду и только потом суммирует, так
  что **сброс счётчика при рестарте больше не искажает знаменатель**, а
  рестарт одного ряда не маскируется ростом соседнего. Цена — экстраполяция:
  абсолютные числа поданы как оценка, отношение к ней устойчиво.
  Главное, что охраняет этап: **секция несёт эффективную глубину данных**.
  Ряд счётчика начинался 06.09.2026, окно отчёта — 30 дней; отношение «за 30
  дней» по суткам данных — не приблизительная правда, а неправда. Недоступный
  Prometheus даёт «нет данных» с причиной, отсутствие `matched` внутри потока —
  ноль, нулевой знаменатель — «не считается»: три разных ответа, а не один
  ноль. — `done`

## Этап 284. Вход поддержки — GitHub Issues, а не чужая почта — `done`

Источник: разбор поддержки 02.09.2026. Ссылка «Поддержка» на лендинге ведёт на
`mailto:support@vetmanager.cloud` — в двух местах (`landing_page.py:2131`,
футер `:2153`), и прибита тестом `tests/test_landing_page.py:281`. В
`AssumptionLog.md` этот адрес записан как **placeholder, требующий верификации**,
и в таком виде живёт на боевом лендинге.

Адрес чужой: это саппорт компании, у которого нет ни доступа к коду и логам
проекта, ни знания о нём. Продукт личный и открытый — обещание поддержки,
адресованное чужой очереди, не выполняется никем.

Решение Владимира 02.09.2026: **вход поддержки — GitHub Issues**. Там возврат
ответа встроен (уведомления, тред, история) и не требует ни почтового контура,
которого в проекте нет, ни ручной рассылки.

- 284.1 `mailto:` уходит с лендинга полностью; обе ссылки ведут в issues
  репозитория. — `done`
- 284.2 Сторож: на странице не должно быть ни одного `mailto:`. Принимается
  только показанным красным — иначе он не отличит защиту от совпадения. — `done`
- 284.3 Запись про непроверенный адрес в `AssumptionLog` закрывается: допущение
  снято не верификацией, а отказом от адреса. — `done`
- 284.4 Почта поддержки — только для вошедших. Предложение Владимира 02.09.2026:
  публичная страница почту не показывает вовсе (её собирают боты), кабинет
  показывает. Аудитория при этом правильная: у вошедшего есть аккаунт, значит
  есть и настоящая проблема. — `done`
- 284.5 Адрес живёт в переменной окружения `SUPPORT_EMAIL`, а не в коде.
  Репозиторий открытый: личная почта, попавшая в `landing_page.py`, остаётся
  в истории git навсегда. Переменная не задана — блок показывает только issues,
  без пустой ссылки. — `done`
- 284.6 Сторож лендинга остаётся строгим: `mailto:` на публичной странице
  запрещён по-прежнему. Послабление касается только кабинета. — `done`
- 284.7 Переменная доезжает до контейнера. Проверка на живом проде показала
  `MISSING`: сервис `mcp` перечисляет окружение поимённо, а `.env` — только
  источник подстановки. Настройка была записана оператору, деплой прошёл
  успешно, и ничто не упало — приложение видело пустую строку. Сторож проверяет
  правило, а не переменную: всё, что предложено в `.env.example` и читается
  рантаймом, обязано быть объявлено в сервисе. — `done`

## Этап 285. Половина настроек рантайма не объявлена в контейнере — `done`

Источник: разбор дефекта 284.7 (03.09.2026). Сервис `mcp` в `docker-compose.yml`
перечисляет переменные окружения поимённо; `.env` — только источник подстановки.
Не названа в `environment:` — внутри контейнера не существует, что бы ни было
записано на сервере. Отказ тихий: код видит пустую строку и работает по
умолчанию, деплой при этом зелёный.

Первая оценка — «читает 33, объявлена 21, мимо 12» — была занижена вдвое,
потому что считалась одной формой чтения: `os.environ.get("ИМЯ")` в корне,
`tools/` и `vm_transport/`. Мимо счёта проходили пакет `auth/` целиком,
обёртки `env_int` / `env_float`, `_is_truthy_env` и — главное —
`get_rate_limit_config("ПРЕФИКС")`, который **собирает имя из префикса**:
литерала `WEB_LOGIN_RATE_LIMIT_ATTEMPTS` в коде нет, грепом по имени его не
найти. Настоящие числа: рантайм читает **51**, объявлена **21**, не доезжало
**30** — включая все восемь настроек лимитов входа, регистрации и OAuth.

Тревожная часть — настройки безопасности: `WEB_ENABLE_HSTS`,
`WEB_SESSION_SAMESITE`, `WEB_TRUSTED_PROXY_IPS`, `FORWARDED_ALLOW_IPS`.
Оператор считает их настраиваемыми — их читает код, про них написано в
документации, — а на боевом контуре они не управлялись вообще.

Побочно нашёлся второй законный канал доставки. `ERROR_TRACKING_RELEASE` — не
настройка оператора, а версия кода: `deploy_server.sh` подставляет её через
`--build-arg`, `Dockerfile` закрепляет как `ENV`. Первая версия исправления
честно объявила её в `environment:` — и тем сломала бы прод: `GIT_SHA` на
сервере не экспортирован, подстановка дала бы `unknown` и затёрла настоящий SHA
в трекинге ошибок. Канал признан, но узко: имя засчитывается, только если
`Dockerfile` реально его закрепляет, и отдельный тест держит список из одного
имени, чтобы канал не стал отговоркой на всё.

- 285.1 Пройти по каждой: объявить в сервисе, либо убрать чтение из окружения.
  Объявлены все тридцать. — `done`
- 285.2 Настройки безопасности разобрать первыми и отдельно. Разобрано на живом
  контуре: HSTS не отдавался ни приложением, ни nginx; `WEB_TRUSTED_PROXY_IPS`
  пуст, поэтому `X-Forwarded-For` от nginx не принимался и клиентом считался
  шлюз докер-сети `172.18.0.1` — привязка токена к IP привязывала всех к одному
  адресу, а лимиты входа считались в общее ведро. — `done`
- 285.3 Расширить сторож с `.env.example` на весь рантайм. Сделано вместе с
  тестом «у извлекателя есть зубы»: сузить извлекатель — простейший способ
  сделать сторож зелёным, поэтому по представителю каждой формы чтения
  проверяется явно. Разбор объявлений вынесен в один экземпляр, сторож этапа
  284 берёт его оттуда же — две регулярки разъехались бы при смене формы
  записи, и это уже произошло по ходу работы. — `done`
- 285.5 Пустое значение не должно отличаться от незаданного по последствиям:
  форма объявления не создаёт пустых строк, читатели числовых настроек терпят
  пустое значение и возвращаются к своему дефолту. — `done`
Внешнее ревью диффа (Codex, 03.09.2026) нашло дефект тяжелее исходного:
объявление вида `NAME: ${NAME:-}` кладёт в контейнер **пустую строку**, а не
«ничего». Читатель вида `int(os.environ.get("NAME", "10"))` получает `""`,
дефолт не применяется и процесс падает: `WEB_SESSION_MAX_AGE_SECONDS` — на
импорте `web_auth`, `DB_POOL_SIZE` / `DB_MAX_OVERFLOW` — на инициализации
хранилища, восемь имён лимитов — на первом же входе, регистрации или обмене
OAuth-токена. То есть первая версия исправления **снесла бы прод при выкате**.

Проверено на живом compose: голое `- NAME` в списке даёт `null` — переменной в
контейнере нет; `NAME=${NAME:-}` даёт `""`. Блок переписан сквозной передачей,
а читатели переведены на терпимые `env_int` / `env_float`: оператор может
написать `DB_POOL_SIZE=` в `.env` руками, и падать из-за этого нельзя.

Тем же ревью найдены две формы чтения, которых извлекатель не видел:
`_env_flag("WEB_SESSION_SECURE", ...)` и чтение по константе
`os.environ.get(STREAMABLE_HTTP_DRAIN_ENABLED_ENV, ...)` — литерала в месте
чтения нет вовсе. Извлекатель переписан на разбор синтаксического дерева.

- 285.4 `.env.example` привести в соответствие. Настройки безопасности описаны
  с последствиями, тонкая настройка перечислена именами **без** дублирования
  значений по умолчанию: скопированное число разошлось бы с кодом. — `done`

## Этап 286. Выручка — это `invoice.amount` со статусом `exec` — `done`

Решение Владимира 03.09.2026 по известной проблеме #37 (отчёт #49): выручкой
клиники считается сумма `amount` по счетам со статусом `exec`. Определения не
было, и отчёт был именно об этом — инструмент расходился с проверенным отчётом
клиники, а сверить было не с чем.

Такой режим у инструмента **уже есть**: `get_revenue_summary(mode="invoiced")`
суммирует `amount` с фильтром `status = exec`. Дефолт при этом — `received`,
касса по exec-платежам, куда попадают технические строки перераспределения.
То есть дефолт отвечает не на тот вопрос, а модель не знает, что режимов три
и какой из них соответствует выручке.

- 286.1 Дефолт `mode` перевести на `invoiced`. Это меняет ответ инструмента для
  уже работающих пользователей — изменение объявляется явно, а не тихо. — `done`
- 286.2 Описание инструмента называет определение вслух: выручка — `amount`
  счетов со статусом `exec`; `received` — касса, `paid_by_executed_invoices` —
  оплаченное по exec-счетам. Сегодня описание перечисляет режимы, но не говорит,
  какой из них воспроизводит отчёт клиники. — `done`
- 286.3 Известная проблема #37 закрывается с публичным резюме: расхождение было
  не ошибкой подсчёта, а отсутствием определения. — `done`
- 286.4 Сторож на определение: `invoiced` обязан фильтровать по `status = exec`
  и суммировать `amount`. Правило записано в тесте, а не только в описании. — `done`

## Этап 287. Явно запрошенный scope отклоняется до согласия владельца — `done`

Источник: известная проблема #38 (отчёт #50), разбор 03.09.2026. Проблема **не
на стороне Ветменеджера** — это наш authorize-эндпоинт.

`_prepare_authorize_request` сравнивает запрошенный scope с **регистрацией
клиента** (`client.scope`) и при превышении отвечает `invalid_scope`
(`oauth_service.py:446`). Регистрация по умолчанию — scopes пресета
`report_ai` (этап 202.2). Дальше по флоу `narrow_oauth_authorize_request_scope`
всё равно выдаёт grant по пресету, **выбранному владельцем на экране согласия**,
а не по запрошенному.

Отсюда перекос: клиент, ничего не запросивший, доходит до согласия и может
получить права на запись, если владелец выберет такой пресет. Клиент,
запросивший `pets.write` честно и явно, отклоняется у двери — до того, как
владелец вообще что-то увидел. Агент наказан за прямоту.

Смежное: инструменты записи при этом видны такому клиенту в `tools/list`.
Правило доступа требует обратного — список фильтруется теми же правами, что и
отказ.

**Решение владельца 03.09.2026: «делай, но без риска сломать OAuth сейчас».**
Поэтому сделан только текст отказа — поведение потока авторизации не изменено
ни на шаг. Пункты, меняющие сам поток, остаются открытыми осознанно, а не
по недосмотру.

- 287.1 Проверка запрошенного scope против регистрации клиента разобрана по
  существу 06.09.2026, по коду и по бою. **Она сравнивает запрос клиента с его
  же собственным заявлением.** Регистрация открытая (DCR): клиент сам присылает
  `scope` в `/register`, и `_validate_scope` записывает присланное как есть —
  владелец в этот момент не участвует. Значит клиенту, которому нужны широкие
  права, достаточно заявить их при регистрации, и проверка его пропустит.
  Отклоняется ровно тот, кто `scope` при регистрации не прислал, получил пресет
  по умолчанию и потом попросил что-то явно: фильтруется честное поведение, а не
  чужое. На бою так и есть — **14 клиентов из 27 заявили при регистрации права
  записи**, 13 только чтение, и ни одной из этих заявок владелец не видел.
  Выданный доступ от запроса при этом не зависит вовсе: в
  `narrow_oauth_authorize_request_scope` итог — `final_scopes =
  TOKEN_PRESET_SCOPES[selected_preset]`, запрошенные scope отбрасываются, кроме
  протокольного `offline_access`; пресет выбирает владелец, а `full_access`
  требует отдельного подтверждения. Гранты это подтверждают: 18 `report_ai`,
  3 `full_access`, 2 `read_only`, 1 `frontdesk`. **Вывод: проверка не является
  границей доступа.** Граница — выбор пресета владельцем, и она держится
  независимо от того, что клиент попросил и что заявил при регистрации.
  Проверка лишь закрывает дверь перед честным клиентом к экрану, исход которого
  от его запроса всё равно не зависит. Оставлена по решению владельца
  (287.2); записано, чем она **не** является, чтобы её не приняли за
  защиту. — `done`
- 287.2 Явно запрошенный scope, который владелец **вправе** выдать, доводить до
  экрана согласия, а не отклонять заранее. Решение владельца менялось дважды и
  оба раза записано. **06.09.2026, первое: оставить отказ заранее** —
  основанием было «экран согласия это место, где клиент может выпросить больше,
  чем нужно». **06.09.2026, второе, после разбора 287.1: убрать проверку.**
  Посылка первого решения не подтвердилась: выпросить больше на экране нельзя —
  выдаётся ровно выбранный владельцем пресет, — а выпросить больше при
  регистрации можно молча, и 14 клиентов из 27 это уже сделали. То есть ранний
  отказ не закрывал ту дыру, ради которой его оставляли, и бил только по
  честному клиенту. Регистрацию не трогаем: заявленное там ни на что не влияет,
  а ошибка в ней ломает подключение целиком. Сделано 06.09.2026: сравнение
  снято, вместе с ним — текст отказа (287.4) и мёртвый расчёт запрошенных tool
  scopes. Замер на бою до правки: тот же запрос давал HTTP 400 `invalid_scope`.
  Внешнее ревью, два прогона, три finding, все приняты; отдельно подтверждена
  совместимость состояний, подписанных до выката, — человек, стоящий сейчас на
  экране согласия, не потеряет `offline_access`. — `done`
- 287.3 Инструменты, которые заведомо не сработают при текущей регистрации,
  не показываются в `tools/list`. Решение владельца 06.09.2026: прятать.
  Разобрано в тот же день после 287.2 — **работы не осталось, и вот почему.**
  Пункт был написан, когда регистрация клиента ограничивала запрос; после 287.2
  она не ограничивает ничего, кроме `offline_access`. Значит инструмент может
  заведомо отказать только по правам **токена**, а список по правам токена уже
  фильтруется с этапа 271: `ToolVisibilityMiddleware.on_list_tools` спрашивает
  `peek_runtime_scopes()` и оставляет то, что покрыто выданными правами. В
  реестре 120 инструментов, у 28 требование записи или удаления — они и
  скрываются у токена без этих прав. Проверено живьём 06.09.2026, причём на
  себе: `update_good_sale_price` не попадал в `tools/list` тестового клиента,
  пока владелец не выдал токен с `inventory.write`, — то самое поведение,
  которого просил пункт. Закрывается не потому, что сделано сегодня, а потому,
  что его предмет исчез вместе с проверкой регистрации. — `done`
- 287.5 Экран согласия не показывает список присланных клиентом прав. Решение
  владельца 06.09.2026. Список читался как «вот что будет выдано», хотя выдаётся
  ровно выбранный пресет, а присланное отбрасывается: экран показывал величину,
  которая ни на что не влияет, рядом с той, которая решает всё. Сделано
  06.09.2026: раздел снят, параметр `scopes` у отрисовки экрана убран целиком —
  иначе список вернулся бы первой правкой шаблона. Заодно переписаны две
  подписи под выбором уровня: они объясняли, что будет, «если выбрать уровень
  шире технического запроса», то есть обсуждали список, которого на странице
  больше нет. Перечень прав самих пресетов остался. — `done`
- 287.4 Отказ называет причину и следующий шаг: уровень доступа выбирает
  владелец в кабинете при подключении, повторные попытки клиента ничего не
  меняют. Текст вынесен в отдельную функцию и покрыт сторожем на правило,
  а не на фразу. **Снято вместе с самой проверкой в 287.2 (06.09.2026):** отказа
  больше нет, значит нет и текста отказа. Работа не пропала — она полгода
  держала объяснимость того, что теперь просто не происходит. Правило «отказ
  называет следующий шаг» остаётся в силе для отказов на вызове инструмента,
  у которых свои сторожа. — `done`

## Этап 288. Правило «массовое берём отчётом» доходит до модели — `done`

Источник: вопрос Владимира 03.09.2026 — «ты в тулах подписал, чтобы большие
выборки делал через отчёты?». Нет, не подписал. Решение по отчёту #60 было
записано только в известную проблему #41, а она доходит до агента через
инжекцию, которая за всю историю продукта не сработала ни разу (этап 283).
На поведение решение не влияло никак.

Описание инструмента — единственный канал, который агент читает всегда.
Поэтому правило записано туда, где ошибку и совершают: у инструментов, по
которым естественно пойти циклом, и встречно у самого Report AI.

- 288.1 `get_invoice_by_id` и `get_invoice_documents` называют отчёт как
  альтернативу поштучному обходу и прямо говорят, чего не делать. — `done`
- 288.2 `create_report_ai_job` встречно подтверждает, что массовая выборка —
  его работа, а не цикл по read-инструментам. — `done`
- 288.3 Правило живёт в собственном словаре `BULK_DESCRIPTION_SUFFIXES`.
  Первая попытка вписала его в словарь приватных суффиксов: до модели оно
  доходило, но правило про объём и правило про приватность в одном ящике
  со временем теряются оба. — `done`
- 288.4 Сборка итогового описания вынесена в `compose_tool_description` и
  используется и продакшеном, и сторожами. Первая версия сторожа читала только
  `SPECIAL_TOOL_DESCRIPTIONS` и поэтому не видела, что правило лежит не там. — `done`

## Этап 289. Включение защиты, которая никогда не работала — `done`

Источник: выкат этапа 285, 03.09.2026. Как только `WEB_TRUSTED_PROXY_IPS`
заработал и адреса клиентов стали настоящими, привязка токена к IP начала
действовать — и сломала тринадцать активных токенов.

Причина в том, чем эти маски были. Токены выдавались с адресом, который
приложение считало адресом клиента, а считало оно им шлюз докер-сети
`172.18.0.1`. Пока так было, маска совпадала у **любого** клиента: защиты не
существовало, но всё работало. Когда адреса стали настоящими, маска перестала
совпадать **у кого угодно**. Затронутые аккаунты — 3, 6, 9, 35, 36, среди них
второй по трафику. Первым отказ поймал наш же post-deploy смоук
(`token_auth_failed_ip_denied`, токен 52), а не пользователи.

Маски мигрированы на `*.*.*.*` — это не ослабление, а честная запись того,
чем они были. Миграцию выполнил владелец: запись в клиентские данные на бою
агенту не доверена, и правильно.

Урок шире случая: **у защиты, которая никогда не срабатывала, есть
пользователи — те, кто на её неработоспособности держится.** Прежде чем
включать такую защиту, надо посмотреть, какие данные накопились в расчёте на
сломанное поведение, и мигрировать их тем же ходом. Ни один тест этого не
ловит: в репозитории таких данных нет, они только на бою.

- 289.1 Найти остальные защиты, которые сегодня не действуют, и проверить
  каждую на тот же вопрос: что накопилось в расчёте на её бездействие.
  Сделано 05.09.2026 сплошным обходом на бою, а не по памяти: проверены все
  выключатели, читаемые из окружения, и их фактическое действие в процессе.
  Действуют: проверка полей фильтра и сортировки (переменные не заданы, значение
  по умолчанию — включено), доверенный прокси, HSTS, лимиты через Redis,
  трекинг ошибок (свежие события в Sentry).
  **Не действует ровно одна — привязка токена к IP.** Все 64 активных токена
  имеют маску `*.*.*.*`; настоящие маски остались только у отозванных. Это
  прямое следствие миграции самого этапа 289 и потому не сюрприз, но означает,
  что защита существует и не защищает никого. Возвращение её в работу — тот же
  случай, что 285: 64 записи держатся на её бездействии, и включать её надо
  вместе с их миграцией, а не вместо. Решение продуктовое: либо выдача токена
  ставит настоящую маску по умолчанию, либо возможность признаётся ненужной и
  снимается. — `done`
- 289.2 Выкат, который меняет смысл уже выданных учётных данных, обязан нести
  миграцию этих данных в том же ходе, а не полагаться на то, что заметят по
  отказам. Записано правилом в `CLAUDE.md` § 6.0 вместе с тремя вопросами
  перед выкатом; «не меняет» — тоже ответ, и он тоже записывается. — `done`
- 289.3 Post-deploy смоук поймал отказ, но сообщил только тип исключения
  (`RuntimeError`). Причина — `token_auth_failed_ip_denied` — нашлась только
  в логах прода. Смоук называет шаг, вид отказа, код статуса и идентификатор
  запроса; тела ответов, заголовки и токен по-прежнему не печатаются никогда.
  Проверяется поведением против подставного сервера, а не наличием слов в
  исходнике. — `done`
- 289.4 Ответы `/mcp` несут `X-Request-ID`. Побочная находка 289.3: диагноз
  обещал идентификатор, по которому строка находится в журнале прода, но
  `attach_request_context_headers` вызывался только из веб-страниц и выгрузок —
  на `/mcp` заголовка не было, и в диагнозе всегда стоял бы прочерк. Касается
  не только смоука: любому MCP-клиенту нечем было связать свой отказ с нашим
  журналом. Middleware на чистом ASGI, не `BaseHTTPMiddleware` — последний
  буферизует ответ и ломает потоковую отдачу streamable-http. Отдельный тест
  проверяет, что middleware **установлен** в отдаваемое uvicorn приложение, а
  не просто написан. — `done`

## Этап 290. Кличка и имя без фамилии — не персональные данные — `done`

Решение Владимира 03.09.2026, при разборе этапа 246: «кличка и имя без фамилии
оставляй, не санитайзь».

Санитайзер обратной связи вычищал `pet: Рекс` и `кличка Барсик` наравне с
фамилией, адресом и телефоном. Цена была не в приватности, а в пользе: отчёт
«поиск не находит [REDACTED] по кличке» невозможно ни воспроизвести, ни понять.
Ради защиты, которая никого не защищала — «Рекс» есть у тысяч питомцев, — из
жалобы уходило её содержание.

Решение действует в двух местах, и одного мало. Санитайзер чистит текст перед
записью, но до него текст готовит модель, которой **три** разных места велели
подставлять `<patient>`: `server.py`, `tool_descriptions.py`, `tools/feedback.py`.
Поменять только санитайзер значило бы принять решение, которое ни на что не
влияет: кличка исчезала бы раньше, чем доходила до нас.

- 290.1 Кличка не чистится: правило `contextual_patient` снято, из
  `PRIVACY_REDACTIONS` убрано — одна кличка больше не поднимает
  `possible_pii`. — `done`
- 290.2 Значение после метки `client:` / `owner:` чистится целиком, включая
  одиночное слово. Попытка отличать там имя от фамилии по списку суффиксов
  была написана и **отклонена**: внешнее ревью и проверка показали, что список
  пропускает Толстой, Горький, Белая, Кац, Смит, Ivanov, Smith — обычные
  фамилии, а не экзотику. Указание владельца «имя без фамилии оставляем» при
  этом выполняется: санитайзер трогает только размеченные конструкции, а имя в
  свободном тексте («Иван просит выгрузку») он не видел никогда. — `done`
- 290.3 Все места, где модели велели прятать кличку, переписаны: три
  исполняемых (`server.py`, `tool_descriptions.py`, `tools/feedback.py`) и две
  документационных (`README.md`, требования в `artifacts/`). Последние нашло
  внешнее ревью: сторож, проверявший только исполняемые, был зелен при живых
  старых обещаниях — а именно по документации описания и переписывают в
  следующий раз. — `done`
- 290.4 `REDACTION_VERSION` поднята до 3: по ней видно, каким правилом чищен
  отчёт. — `done`
- 290.5 Тесты этапов 150 и 160, закреплявшие прежнее правило, обновлены с
  пометкой, что это осознанный разворот по решению владельца, а не ослабление
  по недосмотру. Сторож этапа 160 не просто лишён проверки: он теперь держит
  **новое** правило на всех трёх поверхностях — правило, названное на одной
  из трёх, действует случайно. — `done`

Остаточный риск: имя, написанное в свободном тексте без метки, санитайзер не
трогал и не трогает — это и есть выполнение указания владельца, но означает,
что модель, написавшая «Иванов Пётр не видит счёт» без метки `client:`,
сохранит фамилию. Защита от этого — инструкция модели, а не регулярное
выражение; так было и до этапа.

## Этап 291. Проба запирала домен собственным флагом — `done`

Инцидент 04.09.2026, домен `alternativa` (аккаунт 7, крупнейший по трафику).
С вечера 03.09 клиника не могла сделать ни одного запроса, включая
`get_cassas`: каждый ответ — `VM API circuit breaker half-open for alternativa;
probe already in flight`. Только за 06:35–09:21 МСК — 8 704 таких отказа. Сам
Ветменеджер работал. Вылечилось перезапуском процесса.

Предыстория: скрипт клиники двое суток тянул позиции счетов через
`get_invoice_by_id` в три потока и упирался в лимит API Ветменеджера. Это тот
же отчёт #60 и тот же случай, ради которого в этапе 288 в описания инструментов
вписано правило брать массовое заданием Report AI.

**Причина.** Ответ 429 повторяется, но намеренно **не** считается отказом
брейкера: ограничение темпа — не признак нездоровья апстрима. Проба, получив
429, идёт на второй заход и перед ним перепроверяет брейкер (пункт 105.2). Там
она видит `probe_in_flight = True` — **свой собственный флаг**, — считает, что
пробует кто-то другой, падает с `probe already in flight` и помечает
`_breaker_resolved = True`. Аварийный путь после этого намеренно ничего не
освобождает. Выхода нет: в `half_open` не тикает таймер, кулдаун проверяется
только для `open`. Домен заперт до перезапуска.

**Ложная гипотеза, стоившая часа.** Сначала причина была объявлена отменой
задачи: `await` в `finally` якобы падает с `CancelledError` и страховка не
срабатывает. Написанный под неё тест оказался зелёным и на неисправленном
коде — несостязательные asyncio-локи не уступают управление, и `finally`
успевает отработать. Гипотеза отброшена, страховка со `shield` снята как
лечение несуществующей болезни.

- 291.1 `check_breaker_allows` сообщает вызывающему, впущен ли он как проба;
  держатель пробы не перепроверяет брейкер перед повтором. — `done`
- 291.2 Срок годности пробы: висит дольше `BREAKER_PROBE_TIMEOUT_SECONDS`
  (по умолчанию 60 с) — считается брошенной, впускается новая. Лечит любую
  причину утечки флага, а не только найденную. — `done`
- 291.3 Сторож воспроизводит инцидент через настоящий `_request` с ответом
  429, а не заглушкой рядом. — `done`
- 291.4 Алерт в Telegram на залипание брейкера. Встроен в существующий
  `/opt/vps-monitor.sh` на боевом сервере (релей, глушение повторов и cron уже
  там; секрет остаётся в одном экземпляре). Сигнал — метрика Prometheus
  `circuit_half_open_busy`, порог 20 отказов за 10 минут: проверено на самом
  инциденте, где показатель доходил до 415 при фоне ноль. Второй порог — на
  `circuit_open`, это уже недоступность апстрима. Путь доказан целиком:
  прогон с заниженным порогом отправил боевой текст в канал. Мониторинг живёт
  вне репозитория, здесь только запись о нём. — `done`
- 291.5 Ответить клинике: что было, что исправлено, какой способ выгрузки
  правильный. Владимир отписал клинике сам 04.09.2026. — `done`

## Этап 292. Клиника на собственном домене не может подключиться — `done`

Обращение 04.09.2026: клиника вводит в форме адрес, по которому открывает
Ветменеджер, и получает «Неверный формат домена клиники. Укажите только
поддомен…». Сообщение врёт — формат ввода тут не главное препятствие. Адреса
ниже условные: настоящий адрес клиники в открытом репозитории не хранится.

**Что на самом деле.** billing-api клинику знает: ключ аккаунта `clinic`,
ответ — адрес в её собственной зоне. Но `host_validation.ALLOWED_HOST_SUFFIXES`
пропускает только `vetmanager.cloud` и `vetmanager2.ru` и отбрасывает всё
остальное с текстом про billing API. То есть **правильно введённый ключ тоже не
подключается**: клиника на своём домене не может подключиться в принципе, а
первая стена (формат ввода) прячет вторую за неверной причиной. Проверено
прогоном в тест-контуре: `'vm.clinic.example' -> REJECT` на форме,
`Resolved host is not allowlisted for domain 'clinic'` на резолве.

**Решение владельца по хранению.** Публичные зоны Ветменеджера остаются в коде —
это не секрет. Собственные адреса клиник в репозиторий не попадают: они живут
в `.env` на сервере, потому что это не наши домены и их состав меняется по мере
подключения клиник. В git — только имя переменной и разбор её формата.

**Почему это ещё и строже, чем было.** Список зон разрешает *любой* хост в зоне;
запись `ключ=хост` привязывает конкретный аккаунт к конкретному адресу. Проверка
добавляется к существующей, а не заменяет её: неверная запись не может отобрать
доступ у клиники, которая работает сегодня.

- 292.1 `CUSTOM_CLINIC_HOSTS` — разбор карты `ключ=хост`, значения только из
  окружения, пустая по умолчанию. — `done`
- 292.2 Резолв принимает адрес клиники, если он совпадает с настроенным для её
  ключа; публичные зоны работают как раньше. — `done`
- 292.3 Форма принимает адрес, по которому клиника открывает Ветменеджер, если
  он настроен, и приводит его к ключу аккаунта. Ключ не угадываем. — `done`
- 292.4 Неполная цепочка TLS у клиники называется словами, а не «Ветменеджер не
  отвечает»: браузер докачивает промежуточный сертификат сам, строгий клиент —
  нет, и без объяснения это приходит в поддержку как наша поломка. — `done`
- 292.5 Сторож утечки: адреса клиник не попадают в открытый репозиторий.
  Заведён после того, как адрес был вписан в PRD, Roadmap и docstring этого же
  этапа и пойман внешним ревью. — `done`
- 292.6 Прописать адрес обратившейся клиники в `.env` на боевом сервере и
  проверить подключение живьём. Вне репозитория. Сделано 04.09.2026: старт
  сообщил `custom_clinic_hosts_configured host_count=1`, форма приводит
  введённый адрес к ключу аккаунта, резолв возвращает origin вместо отказа.
  Дальше упирается уже в сертификат клиники — проверка из боевого контейнера
  даёт `tls_verification_failed`, то есть человек получит текст про неполную
  цепочку, а не «Ветменеджер не отвечает». — `done`
- 292.7 Ответить клинике, когда подключение проверено. Владимир отписал
  клинике сам 04.09.2026. — `done`

## Этап 293. Однодневный диапазон счетов возвращает пусто, а любой — теряет последний день — `done`

Источник: отчёт #61 от 03.09.2026 (аккаунт 35). Агент попросил счета за один
день через `date_from` и `date_to` с одной датой и получил `totalCount=0`, хотя
`get_average_invoice` по тому же дню показывал счета. Отчёт не про непонимание
контракта: агент сделал ровно то, что написано в описании инструмента.

**Что на самом деле.** `invoice.create_date` — `timestamp`, а фильтр строится
как `create_date <= '2026-09-02'`, то есть сравнивается с полуночью. Совпадёт
только счёт, созданный ровно в `00:00:00`. Отсюда два следствия, и второе хуже
первого: однодневный диапазон возвращает пусто, а **любой** диапазон молча
теряет свой последний день. Первое видно сразу, второе — только сверкой.

Проверено на живом стенде 04.09.2026: `date_from=2026-09-02, date_to=2026-09-02`
даёт `totalCount=0`; `date_from=2026-09-02, date_to=2026-09-03` даёт 27, и
первая же строка — счёт с `create_date = 2026-09-02 11:17:42`, которого «не
существовало» в предыдущем запросе.

**Почему дожило.** Ровно этот дефект уже чинили — известная проблема #21
«get_payments date filters now use whole-day timestamp range», статус `fixed`.
Починили один инструмент из двух. В `tests/test_ergonomic_filters.py` тест,
прибивающий **неверное** поведение `get_invoices` (`<=` с голой датой), стоит
непосредственно над тестом, прибивающим **верное** поведение `get_payments`
(`>= начало дня`, `< начало следующего`). Зелёный тест сделал дефект похожим
на решение.

- 293.1 `get_invoices`: `date_from`/`date_to` переводятся на границы суток —
  `>= начало дня` и `< начало следующего дня`, как уже сделано для
  `invoice_date_from`/`invoice_date_to` в том же файле. Сторожевой тест
  переписан и показан красным по правильной причине: он защищал дефект. — `done`
- 293.2 Аудит остальных сравнений голой даты с timestamp-колонкой:
  `get_debtors` (`client.last_visit_date` — `timestamp` по схеме),
  `_inactive_helpers`. Оба оказались тем же дефектом и починены; прибивавшие
  их тесты переписаны — всего таких тестов в этапе оказалось три.
  Внешнее ревью независимо обошло `tools/` и других мест не нашло. — `done`
- 293.3 Сторож: новую границу «голая дата против timestamp-колонки» нельзя
  добавить молча. Реестр `VM_TIMESTAMP_FIELDS` в `vm_datetime` — шесть полей,
  тип каждого сверен по схеме, а не по памяти (одно, вписанное по догадке,
  из реестра убрано). Проверка разбирает `tools/*.py` как AST: значение,
  пришедшее из `parse_date_param` или обёртки над ним, не может стать границей
  такого поля без обёртки в сутки. Первая версия сторожа знала только прямой
  вызов и **не ловила главный случай** — в `get_invoices` даты идут через
  локальную `_parse_date_range`; проверено на дореформенном коде из `dbe3764`,
  теперь ловит оба. — `done`

## Этап 294. Ответ агенту был написан и замолчал из-за переименования инструмента — `done`

Источник: отчёт #62 от 03.09.2026 (аккаунт 44) — Report AI отдал ровно 1000
строк с `limited=false` и оборвался, не дойдя до конца периода. Знание об этом
у нас есть с **18.06.2026**: известная проблема #11, severity `high`, playbook
дословно говорит «не ждите больше 1000 строк, идите через экспорт».

**Что произошло.** Этап 276 (01.09.2026) переименовал `get_report_export_file`
в `get_report_export_download`. Playbook #11 остался со старым именем.
`validate_agent_playbook` отвергает playbook **целиком**, если хоть один
инструмент в `recommended_tool_sequence` не найден в реестре. Проблема выпала
из выдачи агенту 01.09, отчёт пришёл 03.09 — через два дня. Агент упёрся в
ограничение, которое мы описали три месяца назад, и не услышал ничего.

Отказ валидации ничего не пишет в лог и не увеличивает ни один счётчик.
`unreachable-issues` показывает такую проблему в одном ряду с теми, где
playbook просто не написан, — «нет playbook» и «playbook отвергнут» выглядят
одинаково, хотя чинятся по-разному. На 04.09 из 20 сохранённых playbook
невалиден ровно один — и это #11.

То же имя живёт в `related_tool` проблемы #29, то есть по инструменту она тоже
не найдётся никогда.

Починить существующий playbook нечем: у `triage_agent_feedback.py` есть
`promote --playbook-json` для новой проблемы и `mark` для статуса, а команды
правки playbook нет. Runbook требует playbook у каждой привязанной проблемы и
не даёт способа его исправить.

- 294.1 Починить playbook #11 на боевой БД: заменить мёртвое имя инструмента,
  проверив результат той же валидацией, что и запись. Вне репозитория.
  Сделано 04.09.2026: `valid_before=False`, `valid_after=True`,
  `unreachable-issues` с 6 до 5, все 21 сохранённых playbook валидны.
  Проверка доставки показала, что до агента знание всё равно не доходит —
  причина в отпечатке, см. этап 295. — `done`
- 294.2 Отказ валидации перестаёт быть немым: событие `agent_playbook_rejected`
  с причиной (`unknown_tool`, `malformed_json`, `unsupported_version`, …) и
  колонка `playbook_state` в `unreachable-issues`, где отвергнутые идут первыми:
  там ответ уже написан и его надо починить, а не сочинить. Отсутствие playbook
  отказом не считается. — `done`
- 294.3 Переименование инструмента ломает проверку, а не базу знаний: тест над
  сидом сверяет каждое имя в `recommended_tool_sequence` и в `related_tool` с
  реестром инструментов. Для проблем, заведённых руками, сверку даёт связка
  `playbook_state=rejected` и причина в логе. — `done`
- 294.4 `triage_agent_feedback.py set-playbook <id> --playbook-json <файл>` —
  правка playbook у существующей проблемы с той же валидацией, что на записи:
  невалидный не заменит рабочий. Печатает, была ли проблема слышна до правки. — `done`

## Этап 295. У успешного ответа с неверными данными нет отпечатка — `done`

Отчёт #62 автоматически привязался к известной проблеме #42 — «Report AI отдаёт
идентификатор сотрудника в колонке ФИО врача». Это другой дефект. Совпадение не
случайное: отпечаток считается по `related_tool`, `http_status`, `error_code`,
нормализованному тексту ошибки и `params_shape`. У отчёта про **успешный** вызов
с неверными данными первых трёх нет вовсе, и отпечаток схлопывается до
`get_report_ai_job_data` + `["job_id"]`. Любой отчёт про этот инструмент,
вызванный с одним `job_id`, получает тот же хеш.

На 04.09 в этом ведре три отчёта — #55, #56 и #62: два про идентификаторы врачей
и один про обрыв на 1000 строк. Ещё шесть отчётов по пяти разным инструментам
имеют пустой отпечаток и не найдутся никогда.

**Неверная привязка хуже отсутствия привязки.** Отчёт помечается разобранным, в
метрике появляется совпадение, а агенту, упёршемуся в обрыв выгрузки,
предъявляется playbook про подстановку имён врачей. Пустой ответ честнее.

**Этот этап — блокирующий, а не косметический.** Проверено на бою 04.09.2026
после починки playbook #11 (этап 294.1): поиск по инциденту той же формы, что
у отчёта #62, по-прежнему возвращает **#42**, а не #11. Совпадение по отпечатку
имеет приоритет, отпечаток `5513…` принадлежит #42, а у #11 свой — `4172e4…`,
полученный в июне от отчёта, у которого признак ошибки был. Правил
сопоставления у #11 нет. То есть агент, упёршийся сегодня в обрыв на 1000
строках, получит playbook про имена врачей даже теперь, когда верный playbook
снова валиден. Пока 295 не сделан, 294.1 возвращает знание в базу, но не
доводит его до агента.

- 295.1 Отпечаток без единого признака ошибки не даёт основания для
  автоматической привязки: такой отчёт остаётся неразобранным, а не
  привязывается к первой проблеме с тем же инструментом. Правила
  `match_rules_json`, написанные человеком, продолжают работать. Граница
  проверяет смысл значения, а не непустоту: `http_status` — только `0` и
  `>= 400`, `error_code` — не из списка заглушек. Оба уточнения пришли из
  внешнего ревью: поля заполняет агент, а описание `report_problem` прямо зовёт
  сообщать и про успешные вызовы. — `done`
- 295.2 Отказ от привязки перестаёт быть немым: событие в лог с причиной, чтобы
  разбор видел, что совпадения не было и почему. Формулировка изменена при
  написании PRD: сначала здесь стояло «отчёт получает отпечаток, который
  различает такие отчёты между собой». Различить их можно только подмешав
  свободный текст отчёта — это выдуманная точность (два агента опишут одну
  поломку разными словами и всё равно не совпадут), и она ломает склейку
  настоящих повторов. Честный ответ — не привязывать, а не привязывать
  точнее. — `done`
- 295.3 Пересмотреть привязки, сделанные по отпечатку без признака ошибки, и
  перепривязать те, что оказались чужими. Сделано 04.09.2026: разобраны все семь
  вёдер, где один отпечаток держит несколько отчётов, плюс шесть отчётов вовсе
  без отпечатка. Чужой оказалась одна привязка — отчёт #62, перепривязанный на
  #11 в тот же день. В трёх вёдрах отчёты про один и тот же дефект (очередь
  Report AI, поля `get_users`, фильтр имени в `get_clients`) — привязки верные;
  в двух отчёты про разные дефекты уже стояли на разных проблемах, то есть их
  поправил человек. Отчёты без отпечатка привязаны вручную и автоматике
  недоступны в принципе. — `done`

## Этап 296. `limited` у данных ИИ-отчёта не может стать `true` — `done`

Отчёт #62 от 03.09.2026: Report AI отдал ровно 1000 строк с `limited=false`
и оборвался, не дойдя до конца периода. Агент поверил флагу, потому что наше
же описание инструмента велит ему верить:

> Returned rows are capped by Vetmanager at 10000 and `limited=true` means
> total is larger.

Обе половины этой фразы для ИИ-отчётов неверны, и проверено это не наблюдением,
а исходниками Ветменеджера.

**Механика.** В `AiReportRenderer::render()` к SQL отчёта применяется
`SqlRowLimiter::apply($sql, self::VIEW_ROW_LIMIT)`, где `VIEW_ROW_LIMIT = 1000`,
и возвращается `'total' => count($rows)` — размер уже обрезанного набора, а не
настоящий итог. Дальше `JobService::getData()` берёт этот `total` и считает
`'limited' => $total > self::DATA_ROW_LIMIT`, где `DATA_ROW_LIMIT = 10000`.

Отсюда: рендер физически не может вернуть больше 1000 строк, `total` выводится
из обрезанного набора, значит `1000 > 10000` — **`limited` у данных ИИ-отчёта
структурно всегда `false`**. Это не редкий сбой, а мёртвое поле.

**Цена.** Настоящий предел данных ИИ-отчёта — 1000 строк, и признака обрезки
нет ни одного. Наша подсказка в рантайме тоже не срабатывает: она включается при
`limited=true` либо `total >= 9000`, то есть при 1000 строках молчит. Агент
получает неполный отчёт, выдаёт его человеку как полный, и это тем опаснее, чем
серьёзнее вопрос — отчёт #62 писался для сверки, то есть под аудит.

**Это тот самый случай, где описание действительно виновато.** По отчётам #61
и #62 гипотеза «агентам не хватило описаний» не подтвердилась: оба были про
дефекты. Здесь подтверждается — но не как нехватка, а как активная дезинформация:
описание называет признаком полноты поле, которое признаком быть не может.

- 296.1 Описание `get_report_ai_job_data` говорит правду: данные ИИ-отчёта
  обрезаются на 1000 строках, `limited` для них не признак обрезки. Ровно 1000
  строк читать как «почти наверняка обрезано». — `done`
- 296.2 Рантайм называет обрезку сам: при 1000 строк подсказка про экспорт
  выдаётся независимо от `limited`, а не только выше порога 9000. — `done`
- 296.3 Оба предела — 1000 у рендера и 10000 у выдачи — записаны как разные
  величины с указанием, откуда взяты, чтобы следующий читатель не свёл их
  обратно в одну. — `done`
- 296.4 Проверить на живом стенде, что отчёт с заведомо большим числом строк
  отдаёт ровно 1000 и `limited=false`, и что подсказка теперь появляется.
  Сделано 04.09.2026 на тестовом стенде, job #86 → отчёт #52. Запрошены позиции
  исполненных счетов за 01.01–04.09; пришло ровно 1000 строк, `total=1000`,
  `limited=false`, то есть флаг утверждает полноту. Данные в этих строках
  покрывают только 01.04–28.08: потеряны три месяца в начале периода и неделя
  в конце. Подсказка `report_ai_probable_truncation` выдана. Отчёт #52 остался
  на стенде под именем с пометкой MCP stage 296. — `done`
- 296.5 Та же неправда жила ещё в README и в описаниях `start_report_export` и
  `get_report_ai_job_export`, где триггером экспорта стоял недостижимый
  `limited=true`; нашло внешнее ревью. Все копии исправлены, тест держит их
  вместе. — `done`

## Этап 297. Гонка двух выкатов выдавала себя за поломку приложения — `done`

04.09.2026 на коммит `15b5ac3` запустились два `Deploy Prod`: один зелёный,
другой красный с текстом

> ERROR: MCP container did not receive the deployed feedback fingerprint pepper.

Приложение при этом было здорово: в том же логе `MCP is healthy`, старт прошёл,
`/healthz` ответил 200.

**Механика.** Секрет заливается на сервер во временный файл
`.feedback_fingerprint_pepper.XXXXXX` внутри `/opt/vetmanager-mcp`. Синхронизация
кода идёт `rsync -az --delete` в тот же каталог. У workflow не было блока
`concurrency`, поэтому два пуша с разницей в минуты дали два одновременных
выката: `rsync` второго удалил временный файл первого как «лишний», и первый
упал на проверке наличия секрета.

**Почему это важнее самой гонки.** Сообщение называет виновным контейнер. Тот же
класс, что и остальные этапы этого дня, и уже встречавшийся здесь раньше —
монитор git, выдававший свою поломку за поломку git (`e01c1156`). Проверка,
которая ломается сама, обязана говорить о себе.

- 297.1 `concurrency: {group: deploy-prod, cancel-in-progress: false}` — выкаты
  встают в очередь. Текущий не отменяется: прерванный деплой хуже лишней минуты
  ожидания. — `done`
- 297.2 Сообщение проверки секрета различает «файл не доехал» и «файл исчез
  после заливки»: во втором случае деплой прямо говорит, что это его
  собственная проблема, называет вероятную причину (параллельный выкат) и не
  трогает контейнер. — `done`


## Этап 298. Цена продажи товара не записывается ни одним инструментом — `done`

Вопрос Владимира 04.09.2026: реально ли вообще менять цену продажи через API.
Проверено по исходникам Ветменеджера, а не по документации.

**Реально.** `GoodSaleParamController` — пустой класс, наследующий `ApiController`
→ `ERestController`, то есть получает полный набор REST-действий, включая
`actionRestUpdate`/`doRestUpdate`. Модель `GoodSaleParam` объявляет `price`,
`min_price`, `max_price`, `coefficient` числовыми правилами без ограничения
сценарием, значит они массово присваиваются. Маршрут `/rest/api/goodSaleParam`
уже используется MCP на чтение (`get_good_sale_params`,
`get_good_sale_param_by_id`). Ограничений на запись в `ApiController` нет.

**Но «цена товара» — не одно поле, и в этом вся сложность.** По схеме
`good_sale_param` у `good_id` обычный индекс, не уникальный: у товара несколько
строк цены — на клинику (`clinic_id`) и на единицу продажи (`unit_sale_id`),
плюс `status` = `active`/`disabled`. И главное: `price_formation` —
`fixed`/`increase`. У строки с `increase` цена выводится из наценки (`markup`),
и запись `price` там либо будет проигнорирована, либо перезаписана пересчётом.

Отсюда: слепая запись «цены товара» почти наверняка попадёт не в ту строку или
не в ту схему ценообразования, а изменение цены видно клиентам клиники сразу.

**Решение владельца 05.09.2026: делаем, но только с подтверждением.** Уточнение
по ходу: инструмент должен предлагать варианты обновления с учётом клиник и
групп товаров.

- 298.1 `update_good_sale_price` без `confirm=true` не отправляет ни одного
  запроса на запись и возвращает превью: текущая и новая цена, клиника,
  единица продажи, статус, способ ценообразования и варианты `row` / `good` /
  `group` с числом затрагиваемых строк и готовым вызовом. — `done`
- 298.2 Строки с `price_formation=increase` запись цены не принимают: там цена
  выводится из наценки, и запись выглядела бы выполненной. В массовых вариантах
  такие строки не пишутся, а перечисляются отдельно. — `done`
- 298.3 Решение владельца принято и записано. — `done`
- 298.4 Варианты по клиникам и группам: `row` — одна строка, `good` — все строки
  цены товара, `group` — все товары группы; `clinic_id` сужает до одной клиники.
  Абсолютная цена для группы отклоняется: одна цена всем товарам группы почти
  наверняка не то, что имелось в виду, — групповая переоценка идёт процентом,
  и процент считается у каждой строки от её собственной цены. — `done`
- 298.5 Findings внешнего ревью, все приняты. **`min_price` и `max_price` —
  проценты, а не рубли:** `$minPrice = $price - $price * $min_price / 100`
  (`GoodController.php`, миграция 2014 года
  `m140311_081943_update_good_sets_max_min_to_percents`). Первая версия
  сравнивала с ними абсолютную цену и отклоняла запись. Проверку не чинили, а
  **убрали**: коридор считается от текущей цены и двигается вместе с ней, то
  есть это допустимая скидка и наценка при продаже, а не ограничение на саму
  цену. Вместо запрета превью показывает получившийся коридор.
  Прочее: выборка шла одной страницей по 100 и молча теряла остаток — теперь
  постраничная, а сверх 500 строк инструмент отказывается («частичная
  переоценка хуже никакой»); готовый вызов в превью не содержал самой величины
  и падал при буквальном повторе; нечисло в цене падало `InvalidOperation`
  вместо отказа. — `done`
- 298.6 Живая проверка на `devtr6`: цена изменена и возвращена обратно.
  Сделано 05.09.2026 прямым REST-вызовом с ключом стенда: `PUT` вернул 201,
  цена сменилась с 250 на 2101 и была возвращена обратно тем же прогоном
  (`ВОЗВРАТ ЧИСТЫЙ: True`). Этим подтверждён главный непроверенный факт PRD —
  апстрим действительно принимает запись цены.
  Сам инструмент сквозь MCP не прогнан: он требует `inventory.write`, а у
  bearer-токена тестового клиента этого права нет, поэтому инструмент даже не
  попадает в его `tools/list` (фильтрация этапа 271 — работает как задумано).
  Прогон через MCP требует токена с правом записи номенклатуры на стенде —
  это выдача доступа, решение владельца. Тот же класс блокировки, что 255.4.
  Токен выдан 06.09.2026, инструмент прогнан сквозь MCP на `devtr6`:
  в `tools/list` он появился (120 инструментов); превью без `confirm` не
  изменило цену; `scope=row` записал 100 → 137.50 и не тронул соседнюю строку
  того же товара в другой клинике; `scope=good` поднял на 10% **обе** строки,
  каждую от её собственной цены (137.50 → 151.25 и 100 → 110); группа с
  абсолютной ценой и обе величины сразу отклонены с внятным текстом; предел в
  50 строк остановил групповую переоценку на 112 строках. Все записи возвращены
  тем же прогоном, стенд остался в исходном виде. — `done`

- 298.7 Превью предлагает вариант, который заведомо не выполнится. Групповой
  вариант выдаётся с готовым вызовом `confirm=true`, но число **строк** у него
  не считается — в ответе стоит только число товаров. На живой группе 66 это
  57 товаров и 112 строк: предел в 50 строк отклоняет ровно тот вызов, который
  превью само и предложило. Для `row` и `good` строки считаются, для `group` —
  нет. Проверка предела должна стоять там же, где вариант предлагается, а не
  через один круг. Сделано 06.09.2026: групповой вариант считает строки двумя
  запросами с `limit=1` через оператор `IN` (апстрим отдаёт `totalCount` за один
  раз вместо 57 запросов по товару), у всех вариантов появился признак
  `exceeds_limit`, и готовый вызов не выдаётся, если вариант не пройдёт.
  Внешнее ревью дало три finding, все приняты: неизвестный `totalCount`
  превращался в единицу; подсчёт группы стоял до проверки `confirm` и вешал на
  запись одной строки два ненужных запроса; групповой вызов при вопросе
  абсолютной ценой содержал строку «укажите процент» вместо числа. — `done`

- 298.8 Описание инструмента не говорит, что он делает. В `tools/list` уезжает
  generic-строка «Update an existing good / service catalog item»: ни слова про
  цену, про подтверждение в два шага и про то, что группа переоценивается
  только процентом. Агент, читающий описание, не может отличить этот инструмент
  от правки названия товара, а весь смысл этапа 298 — в двухшаговом
  подтверждении. Сделано 06.09.2026: описание вынесено в
  `SPECIAL_TOOL_DESCRIPTIONS` и называет четыре вещи, без которых вызов будет
  неверным, — что меняется цена, что шага два, что клиенты клиники видят новую
  цену сразу и что группа переоценивается только процентом. — `done`
## Этап 237. Обрыв запроса при остановке не устранён — `done`
...
- 237.4 Остаточный обрыв: выкатка `86f286e` 23.08.2026 в 14:29 МСК закрыла
  ...
  события — значит чинить наугад. — `stop`

  Закрытие 13.09.2026 по evidence. За десять дней после последнего события
  (03.09 06:18 UTC, release `1d63920`) Sentry не получил ни одного нового
  PYTHON-6: issue стоит на 15. За это же окно журнал runtime (теперь хранится
  две недели) содержит **41** остановку с `streamable_http_drain_finished` —
  **952** удерживаемых сессии закрыто, максимум 215 за одну остановку, — при
  50 успешных Deploy Prod с 05.09. Все 15 событий issue принадлежат релизам
  до 03.09 15:30 (`693f8c4`, `926de1d`, `e013835`, `a1b2c3d`, `1d63920`);
  единственное изменение на пути ответа в тот день — ASGI-middleware этапа
  289.4, который лишь добавляет заголовки и поток не буферизует, — причиной
  назвать нельзя. Ждать дальше бессмысленно: сторож на повтор уже есть —
  `/opt/sentry-alert.py` шлёт повтор issue в канал алертов. Появится PYTHON-6 —
  заводится новый этап со ссылкой сюда, по правилу архива.
...
## Этап 262. Выгрузки метрик считают активность по журналу — `done`

Источник: этап 260. Скрипты отчётности LiveHelperAgent переведены на
`token_usage_logs.account_id` для обоих каналов. История OAuth начинается
26.08.2026 21:11 UTC; аккаунты с OAuth-доступом до этой даты и без записанных
вызовов показываются отдельно как исторически неопределённые.

- 262.1 Убрать склейку из выгрузок и считать по журналу. Код, локальная
  синтетическая проверка и read-only прод-сверка супервизора подтверждены:
  12/14 активных аккаунтов в недельных окнах, 21 дошедший, список застрявших
  не изменился. — `done`
