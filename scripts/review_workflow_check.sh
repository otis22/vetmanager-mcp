...
if [ -n "$DONE_STAGES" ]; then
  ...
  for S in $DONE_STAGES; do
    ...
    #
    # Этап 256.3: уровень заголовка не проверяется. Раньше здесь стояло `^## `,
    # и запись, набранная тремя решётками вместо двух, была для гейта
    # несуществующей — так «пропали» пять этапов подряд (285, 288, 290, 291,
    # 292), написанные в тот же день, что и работа. До них так же пропадал 239,
    # набранный одной решёткой. Проверка отвечает на вопрос «есть ли запись»,
    # и уровень заголовка к этому вопросу отношения не имеет.
