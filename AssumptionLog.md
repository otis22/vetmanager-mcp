...
- **Follow-up живой проверки Report AI:** `preview_example_row` является
  намеренно выдуманным правдоподобным примером, а не строкой клиники. Его
  единственное назначение в helper — до чтения строк сверить ожидаемые колонки,
  типы и группировку; значения никогда не пересказываются пользователю.
  Наблюдение для иллюстрации: preview «Собаки — 1250» и реальные строки
  «Кошки — 48» могут различаться при полностью совпавшей структуре.
- Наблюдения пользователя на одном контуре уточнены как порядок величины, не
  SLA: один job — 60–90 секунд до `ready_to_save`; три созданных подряд —
  около 3–3,5 минут каждый; при плотной работе — до 4 минут 43 секунд. Helper
  требует создавать jobs строго последовательно и предлагать один отчёт с
  группировкой либо несколько отчётов по очереди.
- **Рекомендация по возможной MCP-защите:** пока не вводить жёсткий отказ
  второго незавершённого job. Контракт текущего вызова не отличает осознанное
  действие от ошибки агента, а локально отслеживаемый job может зависнуть или
  не охватить jobs вне MCP. Будущая отдельная contract-задача может добавить
  bounded active-job record и explicit user-approved bypass; отказ должен
  сообщать `job_id`, status, age и дальнейшее действие, не обещая полного
  tenant-wide обзора.
- Always-visible metadata дополняет helper, а не дублирует его: описание
...
- Privacy-contract добавляется суффиксом к исходному описанию tool, не заменяя
  его routing/filter guidance. Endpoint-level fixtures подтверждают redaction
  каждого описанного client/owner и staff context; `report_problem` исключает
  только конкретные privacy-restricted поля из релевантного описания, не
  подавляя сообщения о других неожиданно отсутствующих данных. Для
...
- Email поддержки `support@vetmanager.cloud` — placeholder, требует верификации
  реального адреса. **Снято этапом 284 (02.09.2026):** допущение закрыто не
  верификацией, а отказом от адреса — вход поддержки переведён на GitHub Issues.
...
## Этап 210. Дизайн-ревью лендинга и кабинета — 2026-07-31

- **Источник**: дизайн-ревью прода под тестовым аккаунтом (лендинг 1440 и
  390 px, кабинет с 13 ключами и 10 OAuth-подключениями). 17 находок: 5
  критичных, 6 заметных, 6 мелочей.

- **Новый продуктовый факт**: пользователь зафиксировал, что сервис всегда
  бесплатный и об этом можно заявлять прямо. В `artifacts/prd-vetmanager-mcp-ru.md`
  про цену не сказано ничего, поэтому факт внесён этим этапом. Формулировка —
  «Бесплатно», без «пока» и «сейчас»; тест запрещает на лендинге слова
  `тариф`, `пробный`, `триал`, `подписк`, `пока бесплатно`, `без карты`.

- **Инвариант 208/209 сохранён**. Исходная находка ревью предлагала вынести
  схему потока и расшифровку MCP из свёрнутого блока наружу. Это ломало
  принятое решение «вне свёрнутого блока для разработчиков не показываются
  MCP, OAuth, API, Bearer, token/scope». Принято изменённое решение: наружу
  вынесена жаргон-free схема («защищённое подключение» вместо «MCP-мост»),
  а фраза «MCP — это мост между ИИ-агентом и Vetmanager» осталась внутри
  блока. Ценность достигнута, инвариант цел, тест это охраняет.

- **Разделение agent-facing и user-facing меток**. `TOKEN_PRESET_LABELS`
  оставлен английским: через `get_token_preset_label` и
  `get_presets_allowing_tool` он попадает в
  `tool_scope_security.build_scope_denied_message` — текст ошибки MCP,
  который читает ИИ-агент, а не человек. Для кабинета введён отдельный
  `TOKEN_PRESET_DISPLAY_LABELS` и `display_access_label()` в `web_html.py`;
  тест фиксирует, что английский словарь не изменился.

- **Деградация без JS**. `<time datetime="{ISO}">` содержит серверную
  UTC-строку, которую пользователь видел и раньше; `Intl.DateTimeFormat`
  только заменяет `textContent`. Аналогично `IntersectionObserver` для липкой
  панели: без него панель просто остаётся видимой, как было.

- **Найдено при реализации**: перенесённый из FAQ JSON-конфиг содержал
  захардкоженный production URL. Заменён на тот же плейсхолдер
  `__MCP_SERVER_URL__`, что и в командах для агентов, — это чинит конфиг для
  self-hosted установок. Обнаружено тестом `test_stage146_mcp_url_uses_mcp_path`,
  который начал покрывать конфиг после переноса.

- **Тесты, чьи ожидания изменены осознанно**: `test_stage168_account_token_layout`
  (`<summary>Details</summary>`, `Revoke`, английские `access_label`),
  `test_landing_page::test_stage146_mcp_onboarding_core_copy_and_privacy`
  (пять фраз-примеров переехали из свёрнутого блока в публичную секцию),
  `test_stage173_oauth_metadata`, `test_stage189_activation_onboarding`,
  `test_stage199_activation_first`, `test_web_auth` — все на строки, которые
  этап переводит на русский. Смысл проверок сохранён.

- **PRD-review сторонней моделью**: Codex `gpt-5.5`, запуск 1/2. 1 critical,
  8 warning, 1 nit. Принято 9. Отклонён warning «резать этап на 210A/B/C»:
  этап уже декомпозирован на шесть подзадач по ≤2ч и ≤150 LOC, а разделение
  на отдельные Roadmap-этапы дало бы промежуточное состояние с полурусским
  кабинетом. Идея учтена в разбиении коммитов по blast radius.

- **Smoke**: `scripts/stage210_prod_smoke.py`, воспроизводимый, read-only.
  Требует `STAGE208_SMOKE_EMAIL` и `STAGE208_SMOKE_PASSWORD`. Прогон против
  прода ДО деплоя дал 19/39 — падали ровно те 20 проверок, которые чинит этап.

### Этап 210 — ревью сторонней моделью на committed diff и закрытие

- **Code/diff review сторонней моделью**: Codex `gpt-5.5`, оба запуска
  использованы (2/2). Запуск 1 — 0 critical, 2 warning, 2 nit. Запуск 2 ушёл
  без diff в теле промпта (`git show` не выполнился из-за сброса cwd), поэтому
  его замечания приняты как advisory и проверены самостоятельно через grep по
  production-модулям.

- **Принято из ревью (3)**: перевод `issued_token_privacy_label` (одноразовый
  блок после выпуска ключа показывал `Depersonalized`/`Standard`, и smoke это
  не ловил, потому что ключей не выпускает); мобильная карточная раскладка для
  строк `colspan` без `data-label`; возврат `data-label="Действия"` на
  action-ячейки. По второму пункту рендер на 390 px показал больше, чем было в
  замечании: имя помощника с бейджем ломалось по буквам («Claude» в столбик) —
  исправлено классом `.grant-name-cell`.

- **Отклонено (1 nit)**: «hero содержит "ни сейчас, ни потом", а PRD просил
  "Бесплатно" без "сейчас"». Запрет в PRD касается формулировок, создающих
  ожидание будущего биллинга (`пока бесплатно`, `тариф`, `пробный`,
  `без карты`). «Оплата не нужна ни сейчас, ни потом» — прямое отрицание
  будущего биллинга, оно усиливает обещание. Второй запуск ревью с отклонением
  согласился.

- **Найдено самостоятельно после ревью**: фолбэк `"No scopes"` в
  `web._summarize_scopes` — последняя английская строка в user-facing модели
  представления. И собственная регрессия: запись в таблице подключений теперь
  занимает до трёх строк (данные, «Подробнее», предупреждение), а разделитель
  остался после первой — блок деталей визуально относился к следующей записи.
  Разделитель перенесён на последнюю строку группы через `:has(+ ...)`.

- **Проверки**: Docker suite `1421 passed, 2 skipped, 65 deselected`.
  GitHub Tests и Deploy Prod зелёные.

- **Production smoke после деплоя**: `scripts/stage210_prod_smoke.py` →
  **39/39 passed** против `https://vetmanager-mcp.vromanichev.ru` под тестовым
  аккаунтом. До деплоя тот же скрипт давал 19/39. Проверено визуально на живом
  проде с реальными данными аккаунта (13 ключей, 10 подключений): 4 рабочих
  ключа видны, 9 завершённых свёрнуты за счётчиком, даты локальные, статусы
  цветные, предупреждения читаются во всю ширину. Высота страницы кабинета
  4896 → 3869 px на тех же данных.

- **Ограничение smoke**: скрипт read-only и не выпускает ключ, поэтому путь
  «создание ключа → одноразовый блок» на проде не проверяется автоматически.
  Он покрыт unit-тестом `test_issued_key_block_speaks_russian`.

## Этап 210.7. Блок для кодинг-агентов — 2026-08-01

- **Источник**: пользователь после выката 210. Два его пункта по контенту
  («Что можно спросить после подключения», «Примеры задач по ролям») уже были
  закрыты этапом 210 — он смотрел прод до обновления.

- **Решение пользователя**: вариант (а) — блок сужается до кодинг-агентов.
  Manus убран из табов и подсказок этого блока, потому что он не кодинг-агент;
  таб `Claude` переименован в `Claude Code`. Оба остаются доступны через
  публичный путь: карточки `ChatGPT / Claude / Manus` и `/register?agent=manus`
  не тронуты, тест это фиксирует.

- **Claude Desktop ≠ Claude Code**. Ревью сторонней моделью подняло это как
  critical: прежний текст описывал настройку через конфиг приложения и
  перезапуск Claude, что для Claude Code неверно. Команда и подсказка
  переписаны на `claude mcp add` и проверку через `/mcp`. Механическая замена
  слова дала бы работающую с виду, но ложную инструкцию.

- **Регрессии этапа 210, устранённые здесь**: (R-1) правило
  `.onboarding-head h2` осталось от прежней разметки, а тег стал `<h3>` —
  заголовок блока набирался дефолтным шрифтом и был мельче `.subhead` ниже;
  (R-2) `.onboarding-head` осталась двухколоночной сеткой после переноса схемы
  наружу, текст был ужат в ~500 px при доступных 1150; (R-3) перенесённый из
  FAQ `<pre>` с JSON лежал вне `.body` и потерял оформление.

- **Заголовок блока удалён совсем**, а не починен: он дословно повторял
  `.section-title` секции. Ассерт на него в
  `test_stage146_mcp_onboarding_core_copy_and_privacy` снят осознанно.

- **Фраза «Настройку удобнее делать с компьютера»** вынесена из плашки про
  ключ (она не про ключ) к «Подключению в 3 шага», но осталась внутри
  `data-testid="mcp-onboarding-main-copy"` — на неё завязан существующий тест.

- **Мёртвый CSS**: удалены `.prompts-wrap`, `.prompt-grid`, `.prompt-chip` —
  осиротели после удаления чипов в этапе 210, и `.onboarding-head`, `.grid-5`
  после этого подэтапа.

- **PRD-review сторонней моделью**: Codex `gpt-5.5`, запуск 1/2. 1 critical,
  6 warning, 1 nit. Принято всё, отклонённых нет.
...
- Финальный range-review передал два принятых finding'а. По подтверждённому
  ...
  меняя MCP contract.
- Следующий range-review выявил связанную fail-open ветку: error envelope с
  user payload обходил projection, а аномальный list payload терял
  `totalCount`. Envelope и `data` обработаны раздельно: верхний уровень всегда
  копируется целиком, user records рекурсивно проецируются независимо от
  `success`, `totalCount` сохраняется, а любая неузнанная форма `data` пишет
  единый runtime warning. `uv.lock` не отслеживается и не имеет истории в
  репозитории, но создаётся `uv run`; добавлен в `.gitignore` как локальный
  артефакт.
- Последний follow-up устранил оставшуюся эвристику под подтверждённым ключом
  `data.user`/`data.users`: записи там теперь проецируются безусловно, даже
...
- **Локальный ShellCheck gate:** CI дополнительно проверяет все `scripts/*.sh`
  с `shellcheck --severity=warning` и `bash -n`; эти команды теперь входят в
  обязательные локальные проверки при изменении shell-скриптов. В evidence
  runner создание отсутствующего parent выполняется под `umask 077`, поэтому
  каждый новый промежуточный каталог получает `0700`; это устраняет неверное
  предположение, что `mkdir -p -m 700` защищает все компоненты пути.
- **Исполнимость process commands:** прежняя локальная команда ShellCheck была
  фактически проверена только через распакованный в `/tmp` бинарник, а не в
  записанном виде. Она заменена на Docker-вызов официального образа; правило
  workflow теперь требует перед commit копировать любую новую команду из
  документации и проверять именно её exit code.
- **Проверка записанных команд:** точные строки из `AGENTS.md` для
  `koalaman/shellcheck:v0.9.0` и `bash -n` завершились с exit `0`; отсутствующий
...
- Этап 222: invoice уже содержит doctor context, medcard — doctor_id; для
  последнего используется один `user.id IN [...]` lookup. Неуказанный ID и
  реально отсутствующий сотрудник различаются как `not_specified`/`not_found`.
  `GET /rest/api/user` пагинируется и имеет `totalCount`, поэтому lookup
  строится по известному набору IDs, а не по первой странице: только ответ на
  этот exact `IN`-запрос даёт право поставить `not_found`. Реальный прогон
  devslon67 2026-08-18 (`months_min=6`, `months_max=120`, `limit=20`)
...
- 2026-08-24: ручная проверка 23.08 превращена в тест реального контура
  (`tests/test_stage245_live_update_medical_card.py`, маркер `real_api`).
  Первый тест пишет `recomendation` инструментом, читает значение обратно и
  восстанавливает исходное сырым PUT — восстановление проверяется отдельным
  чтением, потому что пустое значение инструментом не отправить (пустые поля
  трактуются как «не менять»). Второй тест сторожит апстрим-барьер: частичный
  PUT обязан падать с упоминанием пациента, иначе обходной путь пора снимать.
  Прогон на devtr6: `2 passed`. Значения и учётные данные не записывались.
  Список берётся с `/rest/api/MedicalCards`; путь в нижнем регистре отдаёт 404,
  а `call()` в `test_e2e_real.py` такую ошибку глушит в skip — поэтому
  собственный клиент здесь без обёртки.
- Code/diff review сторонней моделью, валидный запуск 1/2 (attempt 1/3), принял
  оба finding'а. Medium: восстановление бралось из списочной проекции — если в
  ней нет `recomendation`, тест затёр бы живой текст пустой строкой, а финальная
  проверка равенства этого не показала бы. Baseline теперь снимается одиночным
  чтением, и тест пропускается, если поле в ответе отсутствует. Low: у сторожа
  апстрима не было отката — ровно в том случае, который он ловит (частичный PUT
...
- 2026-08-24, отдельный ход: требование контракта вписано в инструкцию
  инструмента. Докстринг `update_medical_card` теперь говорит, что правка карты
  требует пациента, врача и клинику, что отказ приходит как
  `400 Patient does not exist` даже на существующем пациенте и что инструмент
  подставляет их сам — искать питомца заранее не нужно. Сообщение об отказе,
  когда у карты нет одной из связей, дополнено указанием, что чинится это в
  Ветменеджере. Формулировки держатся наблюдаемого контракта: текст ошибки
  доступен любому API-клиенту, разбор реализации в репозиторий не переносится.
- То же сделано для `update_admission`: инструкция называет требуемые связи
  (клиника и временное окно приёма), приводит наблюдаемые отказы
  `400 No clinic selected` и `400 Start time or End time parameter is not
  specified.`, объясняет, что инструмент подставляет их сам и сохраняет текущую
  длительность при смене даты. Отдельно названо состояние «приём принят»:
  отказ `400 Editing can not be accepted reception.` не лечится содержимым тела.
- Перевёрнутый порядок проверок передан в Ветменеджер задачей 12301 на Bug
  Triage Lead, без срока — получатель в отпуске, задача заведена, чтобы не
  потерялось. В задачу вынесен и второй симптом: `No Client selected` про поле,
  которое сервер вычисляет сам.
- Code/diff review сторонней моделью на этих правках инструкций: валидный
  запуск 1/2, пустой список findings. Evidence:
  `~/.local/share/vetmanager-mcp-review-evidence/2026-08-24T191233Z-git_range-origin_main__HEAD-attempt-1-of-3.6dFgaa/claude-review-attempt-1-of-3.envelope.json`.
...
- `possible_pii=true` не означает, что персональные данные лежат в базе:
  ...
  PII», а для случая, когда сам санитайзер поумнел: `REDACTION_VERSION`
  сдвинулся, а старые строки хранят то, что прежние шаблоны пропустили.
- Санитайзер ловит только размеченные конструкции (`pet: Рекс`,
  `client: Иванов`, телефон, почта). Кличка или имя в свободном тексте
  проходят мимо, и `redact` этого не исправит — он лишь применяет текущие
  правила. Оценка остаточного риска остаётся ручной (246.3).
- `redact` пересчитывает `possible_pii` от объединения redactions по всем
  четырём текстовым полям, а не по одному: иначе строка, где чистка сработала
  только в `reproduce`, осталась бы помеченной как чистая.
- `--dry-run` печатает те же строки, но не пишет и не коммитит — чтобы
  прогон по всей таблице можно было сперва посмотреть.
- `link` добавлен потому, что `promote` и `resolve-report` создают **новый**
  known issue каждый. Четыре отчёта по `update_medical_card` имеют четыре
  разных отпечатка и без `link` дали бы четыре записи об одном баге.
  Отпечаток остаётся у отчёта, поэтому runtime-матчинг по нему не ломается.
- `link` инкрементит `report_count` и двигает `last_seen_at`, повторяя
  поведение `promote`; защиты от повторной привязки того же отчёта нет —
  команда операторская, вызывается по списку из `show`.
- `redact` пропускает строку, у которой обязательное поле после чистки стало
  бы пустым, вместо того чтобы уронить всю пачку. Сегодня до нуля санитайзер
...
- Фильтрация значения исключения заменена точечной редакцией: вместо того
  чтобы глушить сообщение целиком при любом совпадении, `_redact_exception_value`
  вырезает только сам секрет, почту, IP и телефон, оставляя остальной текст.
  Иначе имя поля — ради которого этап 235 и делался — не доходило до Sentry.
- Домен клиники в сообщении сохранён осознанно: по решению 21.08.2026 он не
  считается персональными данными и нужен поддержке для разбора.
- Разница поведения старого и нового пути захвата объясняется тем, что
  `capture_tool_failure` санитайзил значение исключения, а необработанные
  исключения шли мимо; выравнено по новому пути.

## Этап 242. Диагностика на проде не должна создавать issue (2026-08-23)

- Три события PYTHON-B 23.08.2026 создал я сам, вызвав
  `_get_streamable_session_manager` без запущенного lifespan. Менеджер сессий
  в FastMCP 3.4.7 появляется только внутри Starlette lifespan, поэтому вне его
  прямой lookup обязан быть тихим.
- Разделение сделано параметром `report_missing`: рантайм-путь слива
  осознанно подписывается на `ERROR`, диагностический пробник — нет. Отказ
  слива остаётся громким, что подтверждено этими же тремя событиями.

## Этап 249. Учётные поля в ответе get_users (2026-08-23)

- Проверено живым вызовом инструмента: ответ `get_users` содержит только
  одобренные поля, `passwd`/`login`/`user_inn`/`calc_percents`/
  `last_change_pwd_date` отсутствуют и в основном ответе, и во вложенном
  `doctor` внутри счёта.
- Отчёты #39 и #40 относятся к состоянию до коммита `f92eb02`; защита
  реализована проекцией ответа, а не только редакцией, поэтому новые поля
  upstream не протекут по умолчанию.
...
- Evidence прошлого запуска писался в `/tmp` и стёрт перезагрузкой машины;
  ...
  ребут переживает.
- Этап 232 закрыт: заголовок переведён в `done`, пункт 232.3 закрыт решением
  232.6 (публикуем карту закрытого репозитория осознанно, взамен — правило для
  агентов и хук). Исторический долг по секциям AssumptionLog для этапов 216,
  229, 233, 234 сознательно оставлен как есть — решение владельца 24.08.2026.
...
### Дополнение 26.08.2026 — этап 260 закрыт

- Code/diff review сторонней моделью: два валидных запуска, бюджет исчерпан.
  Запуск 1/2 — три findings (2 medium, 1 low), все приняты: `top_accounts`
  считал через bearer-токен и терял OAuth-аккаунты; кабинет, опрос и датчик
  тишины принимали активный грант за доступ, тогда как рантайм требует живого
  токена; `account_id` оставался nullable, то есть строка без аккаунта была
  валидной для базы и выпадала из агрегатов. Запуск 2/2 — два medium, оба
  приняты: критерий живого доступа был слабее, чем реальный обмен refresh
  (нужны `used_at IS NULL` и активный клиент), и первичная отрисовка кабинета
  расходилась с опросом статуса.
- Правки после второго запуска сделаны без нового ревью и помечены
  `deferred after 2 external-review iterations`.
- Исторические OAuth-вызовы не восстанавливаются — их не записывал никто.
  Ряд по дням получит разрыв на дате выката; отмечено этапом 262.
- Полный прогон после всех правок: 1668 passed, 2 skipped, 69 deselected.

### Дополнение 26.08.2026 — этап 252.3 закрыт

- Дыра была ровно там, куда пришла жалоба: `_observe_report_ai_queue`
  возвращал `None` для всех статусов кроме `queued` и стирал накопленное
  наблюдение, поэтому лимит ожидания и подсказка существовали только для
  очереди. Отчёт 48 от 25.08 — задание висит в `recognizing`.
- Порог применяется к возрасту стадии, а не задания: иначе задание, честно
  простоявшее час в очереди, получило бы лимит на первой секунде
  распознавания. Для `queued` семантика сохранена — там вопрос другой.
- PRD-review сторонней моделью: запуск 1/2 — три findings (2 high, 1 medium),
  все приняты (противоречие `created_at` и возраста стадии; порядок вызова
  lifecycle относительно диагностики; переоценка готовности пункта 251.3).
  Запуск 2/2 — findings пуст, PRD принят.
- Code/diff review: запуск 1/2 — два findings (метрика long_queued молча
  меняла смысл; публичное описание в `tool_descriptions.py` осталось
  queued-only, а именно оно уходит в схему). Запуск 2/2 — два findings
  (тот же разрыв в контракте структурированных логов; operator runbook).
  Все приняты. Бюджет исчерпан; правка лог-контракта и runbook сделана после
  второго запуска — `deferred after 2 external-review iterations`.
- Расширение потребовало трёх разведённых контрактов, а не одного: payload,
  Prometheus-счётчик и event_name в логах. Каждый из них кто-то читает
  снаружи, и в каждом «long queued» означало очередь.
- Пункт 251.3 закрыт с явным ограничением: отказ экспорта приходит только при
  живом наблюдении в текущем процессе.
- Полный прогон: 1681 passed, 2 skipped, 69 deselected.

### Дополнение 26.08.2026 — этап 261, пункты 261.1 и 261.2

- Причина пустых событий сопоставления: `find_known_issue_match` отбирал
  кандидатов только со статусом `workaround_available`, а авто-события уже
  писались для `open`, `acknowledged`, `workaround_available`. Проблемы
  заводятся в `acknowledged` — значит агент не получал ответ, который для него
  уже был написан. Готовность переопределена как наличие валидного playbook.
- `known_issue_match_events` пуста за всю историю при 30 отчётах с
  `known_issue_id`: привязки делались руками через `link`, который событий не
  пишет и не должен — событие означает «автоматика узнала».
- Обнаружено при разборе: перевести отчёт можно было только в `linked`.
  Отчёт «не дефект» оставался `new` навсегда, поэтому счётчик непрочитанного
  не мог вернуться к нулю. Добавлена команда `mark-report`.
- Code/diff review: два валидных запуска, бюджет исчерпан. Запуск 1/2 — одна
  medium (команды регламента не соответствуют CLI). Запуск 2/2 — две medium и
  одна low: невалидные inline-команды в prose, слишком широкое утверждение о
  приватности вывода (`recent` и `export-markdown` печатают сохранённый текст),
  узкий guard-тест. Все приняты; правки после второго запуска —
  `deferred after 2 external-review iterations`.
- Guard-тест проверен на ложную зелёность: с намеренно испорченной inline-
  командой он падает, с исправленной проходит.
- Полный прогон: 1691 passed, 2 skipped, 69 deselected.

### Дополнение 26.08.2026 — этап 261 закрыт, пункты 261.3 и 261.4

- Владелец санкционировал запись в прод (до этого мандат был только на чтение).
  Выполнено: ki 31 → `fixed` (`stage-245`); ki 32 и 33 получили agent playbook;
  отчёт 47 → known issue 34 `wontfix` через штатный `promote`, сам отчёт
  `triaged`; отчёт 48 привязан к ki 28.
- Все четыре JSON прошли настоящий `validate_agent_playbook` /
  `validate_match_rules_json` до записи, а не после.
- Решение по отчёту 47: в 79 REST-моделях Ветменеджера сущности
  пользовательских задач нет — всё, что называется `task`, это фоновые воркеры.
  Инструмент заводить не из чего; агенту записан ответ, что возможности нет.
- Состояние после работы: `unreachable-issues` → `total=0`, отчётов в статусе
  `new` не осталось, доходящих до агента проблем 16.
- Обнаружено ограничение инструментария: добавить playbook существующей
  проблеме можно только прямым SQL. Заведён этап 263.

### Дополнение 26.08.2026 — этап 247 закрыт

- Пункты 247.1–247.4 были закрыты кодом ещё 23.08, заголовок этапа отставал.
- 247.5 подтверждён вживую на devtr6 26.08.2026, четыре наблюдения контракта:
  обновление отвечает `201`, тело ответа содержит только присланные поля плюс
  вычисленный сервером `client_id` (5 против 22 полей при чтении), ключ ответа
  записи — `medicalcards` при `medicalCards` в чтении, `date_edit` остаётся
  равным `date_create`. Запись при этом применяется — данные не теряются.
- Наш код уже поддерживает оба написания ключа
  (`tools/medical_card.py`), то есть двойная обработка существует именно
  из-за расхождения контракта. Это аргумент задачи 12353.
- Добавлен живой сторож с обратной логикой: он утверждает текущее неверное
  поведение, поэтому упадёт, когда вендор починит, — тогда обходной путь
  снимается, а не тащится вечно.
- Побочно: тест упаковки поймал два временных файла-пробника, оставшихся в
  рабочем дереве после ручной проверки. Удалены до коммита.
- Полный прогон: 1691 passed, 2 skipped, 70 deselected.

### Дополнение 26.08.2026 — этап 247, второй гейт ревью

- Запуск 2/2 дал два findings, оба приняты. **medium**: пробный PUT слал
  `str(recomendation or "")`, то есть превращал `NULL` в пустую строку прямо
  в живой карте, и readback-assert такой подмены не заметил бы. Проба теперь
  отправляет только контекстные поля: обновление мержится в сохранённую
  запись, поэтому отвергнутый кандидат остаётся нетронутым. **low**: на
  контуре с единственной пригодной картой сторож молча пропускался бы, и
  починка вендором осталась бы незамеченной — добавлен возврат к общей карте.
- Бюджет code/diff review исчерпан; правки после второго запуска —
  `deferred after 2 external-review iterations`.
- Живой прогон обоих сторожей вместе после правок: 3 passed.
- Полный контур: 1691 passed, 2 skipped, 70 deselected.

## Этап 264. Отказ по доступу ничего не объясняет — 2026-08-26

- Проверено по реестру: читать данные задания Report AI могут пресеты
  `read_only`, `frontdesk`, `report_ai`; сохранять отчёт — только `report_ai`.
  Значит владелец read-only токена заперт: читать разрешено, но читать нечего.
- Названия доступов берутся из тех же меток, что показывает кабинет
  («Full access», «Analytics»), чтобы просьба к администратору совпадала с
  тем, что он видит у себя.
- Помощник `get_presets_granting_scope` размещён в реестре доступов, а не в
  клиенте: устройство пресета — предмет реестра, клиенту нужны только слова.
  Вызов пока один; это граница модулей, а не удобство.
- Полный прогон: 1693 passed, 2 skipped, 70 deselected.

### Дополнение 26.08.2026 — этап 264, ревью

- Ревью нашло две low. Первая: `_safe_export_error` распознавал отказ по
  подстроке `lacks required scope`, и после смены формулировки эта ветка
  замолчала бы. Сейчас её прикрывает проверка скоупов на уровне инструмента,
  то есть поломка была бы незаметной — худший вид. Введён устойчивый маркер
  `SCOPE_DENIED_ERROR_CODE`, привязка к тексту убрана.
- Вторая: не было прямых тестов помощника. Добавлены края — скоуп, который
  даёт только полный доступ; неизвестный скоуп (пустой кортеж); порядок,
  в котором полный доступ идёт первым.
- Полный прогон: 1696 passed, 2 skipped, 70 deselected.

### Дополнение 26.08.2026 — этап 264.3, вопрос владельца

- Владимир спросил: как агент увидит запрос отчёта, если прав нет. Проверка
  показала, что первая правка легла не туда. Список инструментов **не**
  фильтруется по правам — агент видит инструмент и вызывает его, но получает
  отказ от проверки уровня инструмента (`tool_scope_security`), а улучшенный
  текст я положил слоем ниже, в HTTP-клиент, куда выполнение не доходит.
  Ревью отметило это как «не ломает», и я принял вывод, не заметив обратной
  стороны: значит и не помогает.
- Формулировка перенесена в `_format_scope_denied_message`. Там же убран
  повтор одной и той же фразы дважды подряд и снята осиротевшая константа
  `SCOPE_DENIED_MESSAGE`.
- Тесты этапа 132 запирали прежнюю формулировку. Обновлены осознанно: точное
  предложение заменено на проверку сути, потому что привязка к тексту и была
  причиной прошлой ошибки.
- Тест этапа 132 запрещает слово `clinic` в тексте отказа — защита от утечки
  домена клиники в сообщение, которое агент может пересказать пользователю.
  Первая формулировка «Ask the clinic administrator» её нарушала; заменено на
  «your account administrator», защита оставлена острой.
- Полный прогон: 1697 passed, 2 skipped, 70 deselected.

- Ревью 2/2 нашло последний край: тот же форматтер обслуживает базовые
  инструменты (`report_problem`, `get_report_ai_prompt_helper`), которым
  особый пресет не нужен вовсе. При пустом наборе прав им доставался текст
  «ни один доступ не даёт этот инструмент» — читатель шёл искать разрешение,
  которого не существует. Случаи разведены: «у токена нет прав вообще» и
  «такого доступа нет ни в одном пресете».
- Бюджет code/diff review исчерпан; правка после второго запуска —
  `deferred after 2 external-review iterations`.
- Полный прогон: 1698 passed, 2 skipped, 70 deselected.

## Этап 265. Защитная сетка не заметила недостижимое сообщение — 2026-08-26

- Класс ошибки: сообщение, адресат которого его не увидит. Проверено, что
  список инструментов правами не фильтруется — агент видит инструмент и
  вызывает его, но отказ формирует проверка уровня инструмента, а не клиент.
  Генеративный тест ходит через ту же обёртку и проверяет 120 инструментов;
  новый попадает туда без правки теста.
- Границ две, а не одна: обёртка `_wrap_tool_with_depersonalization` и
  `OAuthChallengeMiddleware.on_call_tool`. Текст общий, форма ответа разная —
  для HTTP-пути отдельный тест, иначе правка формата оставит коннектор с
  пустым сообщением при зелёных stdio-тестах.
- `should_skip_report_hint` отличал ошибку ввода по префиксам сообщения.
  Признак переведён в тип `ToolInputError`; шесть мест, порождавших такие
  сообщения, лежали в одном файле.
- Найдено при этом: девять валидаций в `tools/` формулируются иначе
  («clinic_id must be a positive integer») и под старый префикс не подходили
  никогда — на опечатку пользователя ему предлагается сообщить о дефекте.
  Вынесено пунктом 265.6 отдельно от защитной сетки.
- Проверка на текстовые сравнения: первая версия ловила любое чтение текста и
  дала девять ложных срабатываний на показе и санитизации, а область имени
  протекала между обработчиками одной функции. Сужена до решений (условие,
  булев return, assert) и ограничена телом обработчика.
- `AuthError` наследуется от `VetmanagerError`, поэтому проверяется раньше
  базового класса — иначе правило разрешило бы разбор наших же сообщений.
- Реэкспорты `vetmanager_client` объявлены явным `__all__`: их поимённо
  проверяет тест этапа 109, это фасад после разделения модуля, а не забытые
  импорты.
- ruff включён только на мёртвый код (F401, F841); `F841` отключён для тестов,
  где неиспользованная локальная часто документирует намерение. Стиль и
  форматирование остаются отдельным решением.
- Каждый сторож этапа проверен намеренной поломкой: генеративный тест дал
  119 падений на подмене одной фразы, проверка текста — на трёх известно
  плохих образцах, линтер — на синтетическом мёртвом импорте.
- Полный прогон: 1836 passed, 2 skipped, 70 deselected.

### Дополнение 26.08.2026 — этап 265, ревью 1/2

- Ревьюеру была поставлена задача **обойти** проверку, и он нашёл четыре
  способа, каждый из которых пишут без всякого умысла: присваивание в
  локальную переменную (`message = str(exc)`), f-строка, `%`-форматирование и
  условие внутри comprehension. Сторож, который не пробовали обмануть, ничего
  не гарантирует — это подтвердилось на первом же заходе.
- Закрыто дословным разделением двух вещей: имя, связанное с исключением, и
  имя, в котором лежит его текст. Первая попытка объединила их и дала шесть
  ложных срабатываний на местах, где `exc` просто упоминается в условии.
- Второй finding: `UPSTREAM_EXCEPTIONS` был объявлен и не использовался,
  поэтому inline-разбор текста Ветменеджера проходил, хотя решение этапа
  требует именованного классификатора. Реализовано.
- Полный прогон после правок: 1841 passed, 2 skipped, 70 deselected.

### Дополнение 26.08.2026 — этап 265, ревью 2/2 и закрытие

- Второй заход ревьюера нашёл ещё три обхода, из них один практичный до
  неприятного: вынести сравнение в helper и не поставить аннотацию параметру.
  Разработчик так и напишет, не думая о правиле. Плюс `getattr(exc, "args")`,
  присваивание через тернарник и строковая аннотация `exc: "ToolError"`.
- Helper, читающий текст своего параметра, теперь считается классификатором:
  передача ему нашего исключения — finding. Исключение — параметр,
  аннотированный чужой ошибкой: это и есть разрешённая форма, где зависимость
  от их формулировки объявлена в сигнатуре. Первая версия правила поймала
  законный `_is_retryable_export_file_error`; уточнено.
- Итого за два захода ревью нашло **семь** способов обойти проверку. Все
  закреплены специменами: тринадцать известно плохих образцов и шесть
  известно хороших.
- Бюджет code/diff review исчерпан; правки после второго запуска —
  `deferred after 2 external-review iterations`.
- Полный прогон: 1846 passed, 2 skipped, 70 deselected.

### Дополнение 27.08.2026 — этап 265.6, типизация валидации в инструментах

- Инвентаризация: в `tools/` 37 мест конструирования `ToolError` (мест `raise`
  меньше, 28 — три фабрики его возвращают). Пятнадцать из них — ошибка
  вызывающего, остальные двадцать два — отказ апстрима, его нечитаемый ответ
  или наш сбой.
- Живое подтверждение из Sentry (27.08, 14 дней, 28 открытых issue):
  `PYTHON-Y — ToolInputError: Invalid feedback severity.` Тип уже говорил
  «ошибка вызывающего», а issue всё равно заводился: тип читало только
  приглашение сообщить о проблеме, трекинг про него не знал. Исправлено.
- Найдено при инвентаризации: `_validate_positive_int` в `tools/report_ai.py`
  проверял и аргумент вызывающего, и `report_id` из ответа Ветменеджера.
  Типизация вслепую обвинила бы вызывающего в чужом сломанном payload —
  для чужих данных заведён `_upstream_positive_int`.
- Там же: обе ветки `_tool_error_from_vm` возвращали одно и то же —
  `if exc.error_code` ничего не решал. Ветка убрана.
- Сторож: `scripts/check_tool_error_classification.py` — в `tools/` `ToolError`
  не конструируется вовсе; разрешены `ToolInputError` и `reportable_error`.
  Правило без исключений, инвентаря нет.
- **Инвентарь отменён по итогам ревью PRD.** Первая редакция перечисляла места
  `raise ToolError`. Ревью показало два прокола: три фабрики в репозитории
  `ToolError` **возвращают** (`_tool_error_from_vm`, `_safe_export_error`,
  `_medical_card_update_error`), и ключ «функция плюс счётчики» остаётся
  зелёным при компенсирующей правке — одну проверку перетипизировали, рядом
  добавили новую. Оба прокола исчезают вместе с инвентарём.
- Ревью PRD сторонней моделью: 1/2 — три findings (классификация «не найдено»,
  обходы сторожа, перенос `ToolInputError` без правки импортёров), все приняты;
  2/2 — три findings (несогласованный счёт мест, вызов через модульный алиас,
  ложное зелено на компенсирующих правках), все приняты. Бюджет PRD-review
  исчерпан; замена инвентаря на правило без исключений сделана после второго
  запуска и помечена `deferred after 2 external-review iterations`.
- Сторожи приняты красными (§4.0): в `tools/good.py` возвращён `raise
  ToolError(...)` — проверка упала на `good.py:328` и снова позеленела после
  отката; та же строка, переведённая обратно в `reportable_error`, роняет
  поведенческий тест на границе.
- Вне рамок и вынесено в отдельный этап: 59 проверок на `ValueError` в
  `tools/`. До приглашения они не доходят (обёртка ловит только `ToolError`),
  но каждая заводит issue в Sentry — и не через наш `capture_tool_failure`, а
  через `logger.exception` внутри FastMCP, куда наш фильтр не достаёт. Это же
  объясняет четыре живых issue `FilterPropertyValidationError`, хотя
  исключение для них стоит с этапа 236.
- Полный прогон: 1876 passed, 2 skipped, 70 deselected.

#### Ревью на committed diff 27.08.2026, запуск 1/2 — три findings

- **Принято.** Сторожа обходил `import fastmcp as fm` с вызовом
  `fm.exceptions.ToolError(...)`: разбирались только явные алиасы модуля
  `fastmcp.exceptions`. Разбор обобщён — ведущее локальное имя дотированного
  пути раскрывается в модуль, поэтому любая глубина псевдонима видна.
  Специмены добавлены (десять известно плохих, шесть известно хороших).
- **Принято.** «Report is not accessible for REST» для `report_id`, который
  выбрал сам вызывающий, — отказ по предусловию, записанному в докстринге
  инструмента, а не дефект. Это живой issue `PYTHON-N`, заведённый на нас
  дважды. `_safe_export_error` теперь знает, чей это `report_id`: у
  `start_report_export` — вызывающего, у экспорта задания — наш.
- **Отклонено с обоснованием.** Предложение типизировать upstream
  `VALIDATION_ERROR` и `INVALID_TRANSITION` как ошибку ввода. Эти коды
  одинаково означают и неверные данные вызывающего, и **наш** неправильно
  собранный запрос. Типизация вслепую замолчала бы наши собственные дефекты —
  ровно то, ради чего этап и делается. Разбор по конкретным кодам вынесен в
  этап 266 отдельным пунктом.
- Полный прогон после правок: 1881 passed, 2 skipped, 70 deselected.

#### Ревью на committed diff 27.08.2026, запуск 2/2 — один finding, принят

- Сторожа обходило аннотированное присваивание
  `ErrorClass: type[Exception] = ToolError` — разбирался только `Assign`.
  Закрыто вместе с моржовым оператором, лежавшим рядом; оба закреплены
  специменами. Итого за два захода code review найдено **три** обхода, за два
  захода PRD-review — ещё два. Все пять — обычный способ написать код.
- По `report_id_from_caller` и по новым тестам findings нет.
- Бюджет code/diff review исчерпан. Правки после второго запуска —
  `deferred after 2 external-review iterations`.
- Полный прогон: **1883 passed, 2 skipped, 70 deselected**. В одном из
  промежуточных прогонов упал `test_stage199_activation_first.py::
  test_activation_first_page_does_not_overflow_common_viewports` — прогон шёл
  12 минут вместо пяти под параллельной нагрузкой; отдельным запуском тот же
  файл даёт 10 passed, и в чистом полном прогоне он зелёный. К изменениям
  этапа отношения не имеет.

## Этап 267. Сборка образа зависела от сети, которой у прода нет

- Деплой `4298c17` упал дважды на `apt-get install curl`. Диагностика на VDS:
  контейнеры не открывают ни один IPv4-адрес Fastly, хост открывает
  `deb.debian.org` только по IPv6 (`curl -4` — таймаут, `curl -6` — 200).
  То есть «сеть работает» с хоста было ложным сигналом.
- Живой контейнер при этом здоров и ходит в Ветменеджер: сломан не выход в
  интернет, а конкретно маршрут до Fastly по IPv4.
- Решение: убрать зависимость, а не чинить чужой маршрут. `curl` стоял в
  образе только ради `HEALTHCHECK`; Python в образе уже есть. Шаг `apt` убран
  целиком — сборка больше не ходит в пакетные репозитории вовсе.
- Обе ветки healthcheck проверены запуском в собранном образе: 0 при ответе
  200, ненулевой код при недоступном сервисе.
- Не выбрано и почему: включать IPv6 в докере (NAT66 ради одной команды),
  подменять зеркало apt на российское (сборка на runner'ах GitHub пошла бы
  через него же).
- Первая правка сломала CI: `post_deploy_smoke_checks.sh` ходил через `curl`,
  а гоняется он **внутри того же образа**. Локально это не всплыло — образ
  собирался из кеша, где `curl` ещё лежал; поймал только чистый прогон в CI.
  Скрипт переведён на Python, поле диагностики переименовано в `request_exit`.
- Заодно вскрылось, что тест `test_post_deploy_smoke_checks_fail_with_attempt_
  context` оставался зелёным на сломанном скрипте: он ждёт отказа, а `curl:
  command not found` — тоже отказ. Ровно случай из правила §4.0.
- Проверено в образе без `curl` (`command -v curl` пусто): 1883 passed,
  2 skipped, 70 deselected; shellcheck чист.

### Дополнение 28.08.2026 — дымная проверка на проде и опровергнутое утверждение

- Проверено снаружи: `healthz`, `readyz`, `/mcp` на `mcp.vetmanager.cloud` —
  200; чтение живыми вызовами через прод тестовым ключом работает
  (`get_clinics`, `get_users` — стенд `devslon67`, 8 сотрудников).
- **Ошибка ввода на проде отвечает чисто**: `calculate_good_combination_price`
  с `tag_id=0`, `start_report_export` с битым `filter_json`,
  `get_good_combination` с несуществующим `tag_id` — три отказа без
  приглашения сообщить о проблеме.
- **Обратная проверка сошлась**: `get_report_export_file` с несуществующим id
  вернул отказ апстрима **с** приглашением. То есть граница проведена там,
  где задумано.
- **Опровергнуто собственное утверждение.** В Roadmap стояло «ошибка ввода
  перестала заводить issue в Sentry». Неверно: три вызова выше завели
  `PYTHON-10`, `PYTHON-11`, `PYTHON-M`. Это события-сообщения уровня error от
  логгера `fastmcp.server.server` (`tag logger=fastmcp.server.server`,
  `has exception: False`), а не наши `capture_tool_failure`. Формулировка
  исправлена, факт перенесён в этап 266 как доказательство, что дыра шире:
  логгинг-путь одинаково пропускает и `ValueError`, и `ToolInputError`.
- Там же видно, чем 266 будет неудобен: в этом событии ошибку ввода от дефекта
  отличить нечем — текст один и тот же, исключения нет.

#### Ревью на committed diff 28.08.2026, запуск 1/2 — три findings, все приняты

- **Моё утверждение было шире правды.** «Сборка больше не ходит в пакетные
  репозитории» верно только для `--target production`. Тестовая стадия тянет
  их через `playwright install --with-deps chromium` — ревью проверило
  сборкой `--target test --no-cache`. Выкатка собирает только production
  (`deploy_server.sh:207`), поэтому прод не задет, но формулировка исправлена
  в PRD и Roadmap: граница проходит по стадиям образа, а не по репозиторию.
- **Потеряна семантика `--max-time`.** `urlopen(timeout=N)` ограничивает одну
  операцию с сокетом, а не передачу целиком: тело по байту раз в 0.6 с
  проходило односекундный лимит за десять секунд (замерено ревьюером в
  собранном образе). Добавлен сторожевой таймер на весь запрос.
- **Появилось следование редиректам.** `curl` вызывался без `-L`, то есть 3xx
  был ответом и валил проверку. После перевода `302` на здоровую страницу
  проходил smoke — редирект в никуда маскировался бы здоровьем. Возвращён
  отказ следовать редиректам через `HTTPRedirectHandler.redirect_request`.
- Оба новых сторожа приняты красными (§4.0): при возврате прежнего поведения
  падают именно они (`2 failed, 4 passed`), после отката поломки — `6 passed`.
- Побочно убран мёртвый слой совместимости: `SMOKE_CURL_MAX_TIME_SECONDS`
  задавали только мои же тесты, то есть новое имя не проверялось ничем.
  README рекламировал `SMOKE_CONNECT_TIMEOUT_SECONDS`, которого больше нет.
- Полный прогон: 1885 passed, 2 skipped, 70 deselected. Shellcheck чист.

#### Ревью на committed diff 28.08.2026, запуск 2/2 — два findings, оба приняты

- **Диагностика стала беднее `curl`.** При обрыве по дедлайну watchdog делал
  `os._exit` до записи, и лог показывал `request_exit=7 http_status=000
  body=` — медленный сервер выглядел ровно как недоступный. `curl --max-time`
  выходит с 28, но статус печатает и частичное тело сохраняет. Теперь так же:
  статус и накопленное тело пишутся перед выходом, а `bash` больше не затирает
  статус при ненулевом коде.
- **`Location` у 3xx съедался.** Отказ следовать редиректу без указания, куда
  он вёл, оставляет оператора без главного. Заголовок печатается в поток
  ошибки и попадает в строку диагностики.
- Оба сторожа приняты красными: при снятии этих двух правок падают именно
  новые проверки, и текст падения дословно повторяет жалобу ревью.
- Ревью подтвердило отдельной проверкой, что оба теста честные: на `eeb6bef`
  падают по нужным причинам, dribble-тест — по `elapsed 9.6s`, а не по
  `subprocess.run(timeout=15)`.
- Бюджет code/diff review исчерпан; эти правки — `deferred after 2
  external-review iterations`.
- Полный прогон: 1885 passed, 2 skipped, 70 deselected. Shellcheck чист.

## Этапы 266 и 243. Поток ошибок должен что-то значить

Два этапа вместе: 243 добавляет в события поле «сколько аккаунтов задето», 266
убирает из тех же событий чужие опечатки. Порознь каждый бессмыслен.

- **Канал, который никто не контролировал.** FastMCP пишет об отказе
  инструмента на своей границе сам, а логгинг-интеграция Sentry превращает
  запись уровня error в событие — раньше, чем наша middleware увидит ошибку, и
  независимо от её решения. Отсюда `PYTHON-10`, `PYTHON-11`, `PYTHON-M` от
  трёх опечаток 28.08 и четыре живых `FilterPropertyValidationError`, хотя
  исключение для них стоит с этапа 236: фильтр стоял на пути, по которому эти
  события не шли. Логгер закрыт через `ignore_logger`; распознавать чужой
  текст не понадобилось.
- **Чего нельзя было потерять вместе с шумом.** `FastMCPError`, не являющийся
  `ToolError` (`AuthorizationError`, `ResourceError`, `PromptError`), приходил
  **только** логгером. Middleware расширена до `FastMCPError`.
- **И чего нельзя было приобрести.** Схемная `fastmcp.exceptions.ValidationError`
  событий не создавала: FastMCP пишет её предупреждением. Расширение
  middleware вернуло бы её в Sentry и сломало приёмку собственного этапа —
  исключена явно, закреплено тестом (проверено: без исключения тест падает).
- **Почему 243 не был сделан раньше.** `_sanitize_event` для наших событий
  делает `event.pop("user")` — вырезает поле ровно там, где его поставят.
  Поэтому `user` не сохраняется, а пересобирается из тега `account_id`:
  только десятичное значение, только для двух наших видов приватных событий,
  сборка до удаления ключей, нет тега — нет `user`.
- **Разбор 65 проверок по одной.** Ревью PRD дважды назвало это блокером:
  «перевести 54 в `ToolInputError`» скрыло бы наши дефекты. Поимённо не ввод:
  предел `paginate_all` (защита сервиса), `invoice._money` (суммы из ответа
  ВМ), отсутствие записи и битый контекст в `update_admission` и
  `update_medical_card`. Отдельная корзина — `invariant_error` для состояний,
  которых наш код не должен допускать (`_slots_helpers`).
- **Спор с ревью, решённый в его пользу наполовину.** Ревью привело
  `calculate_inactive_window` как пример инварианта чистого хелпера. Проверка
  по коду показала, что `months_min` — аргумент инструмента (`get_inactive_pets`),
  то есть ввод; а вот `compute_free_slots` действительно инвариант: `schedule.py`
  валидирует те же значения выше по стеку.
- **Форма сторожа изменена по третьему finding'у.** Правило «нет `ValueError`
  в теле функции под `@mcp.tool`» обходится уже написанным кодом: валидации
  давно вынесены в хелперы рядом (`_parse_money_filter`, `_parse_date_range`,
  `_reject_payment_*`, `_validate_admission_status`). Поэтому правило про имя,
  как в 265.6: в `tools/` и `validators.py` голый `ValueError` не поднимается.
- Ревью PRD сторонней моделью: 1/2 — семь findings (два блокера), 2/2 — шесть
  (один блокер), все приняты. Бюджет PRD-review исчерпан.
- Все пять новых сторожей приняты красными; для схемной `ValidationError`
  проверено отдельно, что тест ловит именно свой регресс.
- Полный прогон: 1908 passed, 2 skipped, 70 deselected.
- Оба этапа оставлены `in_progress`: закрывать их можно только живой
  проверкой на проде (266.3 и 243.4), а не зелёным прогоном.

#### Ревью на committed diff 28.08.2026, запуск 1/2 — три findings, все приняты

- **Нарушен инвариант, который я сам записал.** `set_affected_account(None)`
  не очищал уже установленного пользователя: ревью измерило — после `101`,
  затем `None` следующее событие ушло с `user={'id': '101'}`. То есть на
  неаутентифицированный вызов встал бы чужой аккаунт. Теперь `None` очищает,
  а не пропускает.
- **`isdecimal()` принимает юникодные цифры.** `'１２３'` и `'٤٢'` доезжали до
  `user.id` как есть и считались бы отдельными людьми. Добавлена проверка
  `isascii()`; в параметризацию теста добавлены оба образца.
- **Сторож обходился `getattr(builtins, "ValueError")`.** Проверено запуском:
  до правки `exit_code=0`. Разбирается и прямой вызов результата `getattr`, и
  присваивание его имени; то же для `ToolError`. Специмены добавлены —
  восемнадцать известно плохих образцов против восьми хороших.
- Ревью подтвердило запуском то, что тестами не проверить: `ignore_logger`
  действительно перекрывает путь логгинг-интеграции (соседний логгер при этом
  отправляет событие), а через настоящий `FastMCP.call_tool` на `ToolError`,
  `ResourceError`, `PromptError` и голый `RuntimeError` приходит **по одному**
  событию — дублей расширение middleware не создало.
- По разбору 65 проверок ревью ошибок не нашло.
- Полный прогон после правок: 1913 passed, 2 skipped, 70 deselected.

#### Ревью на committed diff 28.08.2026, запуск 2/2 — один finding, принят

- `set_affected_account` не держал тот же инвариант, что путь тега: `-5`
  давал `user.id="-5"`, `True` давал `"True"` (в Python `bool` — это `int`).
  Теперь принимается только положительное целое, не булево.
- **Оставшиеся обходы сторожа названы, а не спрятаны.** Ревью проверило
  запуском четыре: `operator.attrgetter`, `vars(builtins)`, `__builtins__` и
  импорт алиаса из соседнего модуля. Само же оценило, что реально пишут только
  последний — он и закрыт: имена, которые модуль экспортирует как псевдоним
  класса, теперь разрешаются между сканируемыми файлами. Три остальных
  записаны в докстринг проверки как осознанный предел.
- **Побочно поймана врущая диагностика.** Первая версия межмодульного разбора
  сообщала «builds ToolError» про псевдоним `ValueError`: имя клалось в оба
  набора сразу, а проверка класса шла первой. Псевдоним теперь помнит, что
  именно он несёт.
- Ревью подтвердило запуском: `101 → None` даёт событие без `user`,
  `101 → 202` — с `202`, исключение из `set_user` вызова не роняет, юникодные
  цифры отрезаны, `ignore_logger` не сломан, дублей нет, схемная
  `ValidationError` даёт ноль событий.
- Бюджет code/diff review исчерпан; правки после второго запуска —
  `deferred after 2 external-review iterations`.
- Полный прогон: 1914 passed, 2 skipped, 70 deselected.

#### Живая приёмка 28.08.2026, 03:00 — оба этапа закрыты

Baseline снят до проверки: за предшествующий час issue не было ни одного.
Затем четыре вызова через прод тестовым ключом:

| вызов | что вернулось | issue |
|---|---|---|
| `calculate_good_combination_price(tag_id=0)` | ошибка ввода | нет |
| `get_good_combination(tag_id=424242)` | ошибка ввода | нет |
| `get_users(sort=[{"order":"ASC"}])` | 406 от апстрима | `PYTHON-12`, **одно** |
| `get_report_export_file(999999)` | 404 от апстрима | `PYTHON-H`, **одно** |

До правки те же две опечатки дали бы два issue, а каждый отказ апстрима — два
события вместо одного. У обоих новых issue **`users=1`**; до этого поле
равнялось нулю у всех восьми открытых без исключений.

Старые события от логгера FastMCP (`PYTHON-G`, `PYTHON-M` вида
«Error calling tool 'X'», 27.08 21:03–21:04) после выката не повторяются.

## Этап 222. Врач последнего визита в inactive pets — ревизия (2026-08-28)

Общее допущение ревизии пяти `in_progress` этапов: этап закрывается только тем,
что можно предъявить сегодня — код в `main`, тест в наборе, событие в Sentry или
живой вызов. Всё, что подтверждается одной лишь записью в Roadmap, считается
неподтверждённым. Правок кода в ревизии не делалось.

- `tools/pet.py` отдаёт `doctor_id`, `doctor_name` и `doctor_resolution`; врач
  берётся из обоих источников визита (`invoice` — сразу с именем, `medcard` —
  только id), неразрешённые id добираются **одним** запросом `/rest/api/user`
  с фильтром `IN`. Per-pet вызовов нет — это и было единственным требованием
  пункта 222.1. Тест
  `tests/test_inactive_pets.py::test_inactive_pets_doctor_sources_are_projected_without_page_cutoff`,
  коммит `9e98ecc`.
- Живой вызов инструмента на проде 28.08.2026 (limit=5, окно 1–60 месяцев):
  пять питомцев, оба источника визита, у всех `doctor_resolution=resolved`.
- Запись об этапе до сих пор жила пунктом внутри секции 221 — отсюда своя
  секция.

## Этап 230. Рестарт не рвёт клиентские сессии — ревизия (2026-08-28)

- Все три открытых пункта оказались сделанными в коде, но не отмеченными:
  `_runtime_lifespan` → `_graceful_shutdown` → `shutdown_storage` делает
  `engine.dispose()` на ещё живом loop (230.1); `stop_grace_period: 45s` против
  `timeout_graceful_shutdown=20` у Uvicorn (230.2);
  `_DrainingUvicornServer.handle_exit` ставит `begin_draining()`, а
  `readiness_check` при `is_draining()` отдаёт 503 `shutting_down` до похода в
  хранилище (230.3).
- Живое подтверждение: PYTHON-1 (`Event loop is closed`) — три события,
  последнее 22.08.2026 19:44 UTC. Ни одной выкатки после этапа он не пережил.
- Остаточный обрыв ASGI-ответа сознательно **не** закрывается здесь: он живёт
  пунктом 237.4, и ревизия 28.08 вернула его из наблюдения в работу. PYTHON-6
  вырос до 10 событий, два последних — 27.08.2026 20:59:14 UTC с разницей 8 мс,
  оба от останавливаемого контейнера (`release e013835`). Ограничение evidence:
  лог слива за то окно недоступен — контейнер пересоздан, драйвер `json-file`
  уехал вместе с ним, поэтому `session_count` той остановки неизвестен.

## Этап 241. Дедупликация событий — ревизия (2026-08-28)

- Закрыт не своей правкой, а этапом 266: второй канал — логгер
  `fastmcp.server.server` — перестал пускаться в Sentry, и доменное исключение
  больше не уезжает отдельным событием. Допущение, которое при этом принято:
  дедупликация держится не маркером на исключении, а закрытием чужого канала;
  маркер (`mark_tool_error_as_handled`) остаётся второй линией на случай, если
  событие придёт другим путём.
- Живое A/B на одном и том же отказе (`get_users` с некорректной сортировкой,
  HTTP 406, issue PYTHON-12): выкатка `eeb6bef` 27.08 21:04:03 — **два**
  события (`VetmanagerError` с `logger=fastmcp.server.server` и наш `ToolError`,
  17 мс между ними); выкатка `9025ba3` 27.08 23:00:12 — **одно** событие, наш
  `ToolError`, `user.id=38`. То же на `get_report_export_file`: события логгера
  прекратились на 27.08 21:04:23.
- PRD этапа не существовало; написан ретроактивный
  `PRD/этап-241-дедупликация-событий.md` с явной оговоркой о порядке.

### Побочная находка ревизии — про сам гейт

`review_workflow_check.sh` числит этап 243 среди «done без записи в
AssumptionLog», хотя запись есть: она называется «## Этапы 266 и 243», а гейт
ищет строго `^## Этап 243`. Это та же слепота, что и с одной решёткой у этапа
239; лечится пунктом 256.3. Зафиксировано, чтобы отчёт гейта не читали как
реальный пробел.

## Этап 254. Фильтр деплоя обходится кириллицей в имени файла (2026-08-28)

Найдено побочно, в ходе ревизии `in_progress`, собственным коммитом `a1b2c3d`:
документационный коммит (`Roadmap.md`, `AssumptionLog.md`, новый
`PRD/этап-241-…md`) прошёл гейт и пересоздал контейнер на проде.

- Гейт `deploy-prod.yml` фильтрует вывод `git diff --name-only`. Принятое при
  реализации допущение — «имена путей приезжают как есть» — неверно: git
  экранирует не-ASCII пути в октальные последовательности и заключает их в
  кавычки. Строка `"PRD/\321\215…md"` не начинается на `PRD/` и не кончается
  на `.md`, поэтому читается как «путь вне документации».
- Следствие сильнее единичного случая: все PRD репозитория названы
  по-русски, значит **каждый коммит с новым PRD** перезапускает прод. Пропуск
  27.08 (`c736bf6`) сработал только потому, что там менялись файлы с
  латинскими именами — то есть зелёный случай не доказывал работу фильтра.
- Проверка воспроизведения — локальный `grep -Ev` на диапазоне
  `e9ff2aa..a1b2c3d` печатает ровно эту строку.
- Диагноз подтверждён A/B на боевом CI: коммит `22a85f9` (те же два файла с
  латинскими именами, без нового PRD) дал `deploy: skipped`, контейнер не
  тронут. Единственное отличие от `a1b2c3d` — имя добавленного файла.
- Правка не делается этим ходом: она меняет CI и требует своей проверки на
  коммите с кириллическим именем файла. Пункт 254.3, этап возвращён в
  `in_progress`.
- Побочная польза для 237.4: та самая незапланированная остановка 07:19 UTC
  прошла без PYTHON-6 (счётчик остался на 10). Значит обрывается не каждая
  остановка — разбирать нужно разницу между ними.

### Пункт 254.3 — реализация (2026-08-28)

- Граница разделения выбрана по проверяемости: в `scripts/deploy_scope_check.sh`
  ушло решение о скоупе (список путей → вердикт), а поиск базового коммита
  остался в workflow — он ходит в GitHub API и в тестовом контейнере
  невоспроизводим. Тестовый образ вдобавок не содержит `git`, поэтому
  end-to-end прогон в CI молча пропускался бы; вместо него контракт производителя
  путей закреплён текстовым утверждением по workflow.
- Сопоставление ведётся `case`-шаблонами, а не регэкспом по склеенному тексту:
  экранирование и переводы строк в именах перестают быть фактором.
- Пустой список путей по-прежнему означает деплой. Допущение прежнее и
  осознанное: отсутствие списка — не доказательство того, что коммит
  документационный.
- Claude Opus code/diff review, валидный запуск 1/2, evidence
  `~/.local/share/vetmanager-mcp-review-evidence/2026-08-28T083810Z-git_range-HEAD1__HEAD-attempt-1-of-3.jOKM98/claude-review-attempt-1-of-3.envelope.json`,
  outcome=valid_verdict, output_tokens=3630, thinking_tokens=2584. Принят
  medium: ранний выход из цикла закрывает пайп под пишущим `git diff` — сегодня
  141 проглатывается, но под `pipefail` это уронило бы job именно для коммитов
  с кодом. Цикл дочитывает stdin; сторож показан красным на версии с ранним
  выходом.
- Claude Opus code/diff review, валидный запуск 2/2, evidence
  `~/.local/share/vetmanager-mcp-review-evidence/2026-08-28T085134Z-git_range-HEAD2__HEAD-attempt-2-of-3.ROBhWn/claude-review-attempt-2-of-3.envelope.json`,
  outcome=valid_verdict, output_tokens=4428, thinking_tokens=3912. Принят low:
  `process.wait(timeout=60)` внутри `with Popen(...)` при срабатывании таймаута
  уводил бы падение в бесконечное ожидание в `__exit__`. Добавлено убийство
  процесса, объём входа уменьшен с 200 000 путей до 20 000 (всё ещё несколько
  буферов пайпа). Бюджет ревью исчерпан, правка помечена
  `deferred after 2 external-review iterations`.
- Найденная попутно причина, по которой дефект дожил до прода: тест этапа 254
  закреплял форму реализации (точную строку вызова `git diff` и регэксп), а не
  поведение. Такой тест не мог покраснеть на ошибке внутри этой же строки.

### Дымная проверка после 254.3 и новая улика по 237.4 (2026-08-28, 12:10)

- Прод после выкаток: `/healthz`, `/readyz`, `/register`, `/login` — 200;
  OAuth-метаданные отдают issuer, authorize/token/register endpoint'ы;
  динамическая регистрация клиента на пустое тело отвечает 400, а не 404/500.
- Неаутентифицированный `tools/call` возвращает challenge
  (`_meta.www_authenticate` с `resource_metadata`), а не данные — слой доступа
  цел. Аутентифицированные вызовы через коннектор Claude отдают данные
  (`get_clinics`, `get_users`, `get_pets`).
- Правки 254.3 не касаются рантайма: изменены workflow, скрипт решения и тесты.
- Новая улика по 237.4: остановка при выкатке `799f46f` в 09:06:28 UTC дала
  **четыре** события PYTHON-6 за 24 мс (счётчик 10 → 14), при том что остановка
  в 07:19 не дала ни одного. Отличие — живые клиентские сессии: во второй к
  сервису была подключена интеграция Claude с идущими вызовами. Значит
  воспроизведение надо строить от удерживаемых сессий, а не от факта выкатки.

## Этап 268. Инструменты удаления счетов сняты

Дата: 2026-08-31.

**Что снято.** `delete_invoice` и `delete_invoice_document` убраны из
инструментов, из реестра прав `TOOL_REQUIRED_SCOPES`, из карты описаний
`TOOL_ENTITY_MAP` и из README. Право `finance.write` осталось — на нём висит
`update_invoice`.

**Почему.** Наблюдаемое поведение контракта: на счёте, у которого есть позиции,
удаление отклоняется на стороне базы и счёт остаётся; на счёте без позиций
удаление проходит независимо от его состояния, включая исполненный и
оплаченный; удаление позиции проходит всегда и не приводит к пересчёту суммы
счёта и связанных складских документов. Предметное удаление счёта в
Ветменеджере устроено иначе: разрешено только для состояния «Отложен», не
удаляет запись физически, откатывает связанные складские документы и пишет
запись в журнал действий. Инструмент выглядел как «удалить счёт», а делал не
то, и делал это непредсказуемо.

Решение владельца продукта 31.08.2026: снять совсем, не переносить под другое
право и не оставлять с предупреждением в описании.

**Условие возврата.** Только после того, как Ветменеджер даст REST-метод поверх
предметного удаления. Запрос заведён задачей 12497 в Битрикс24 (доска
«Подготовка задач до разработки», приоритет низкий, срок не ставился).

**Сторож.** `tests/test_stage268_invoice_delete_tools_removed.py` проверяет, что
ни один зарегистрированный инструмент так не называется и что имён нет в
реестре прав, карте описаний и обещанных составах групп. Сторож показан
красным: до снятия инструментов все три его проверки падали
(`3 failed`), после снятия — `3 passed`.

**Ревью PRD сторонней моделью.** Попытки 1/3 и 2/3 неуспешны
(infrastructure): Codex не отвечал ни через MCP-инструмент, ни через CLI —
«Your access token could not be refreshed because your refresh token was
already used». Отказ провайдера, а не разбираемый verdict, слот бюджета не
израсходован. После `codex login` владельца — валидный запуск 1/2, три medium,
все приняты:

- критерий приёмки «ни один тест не ссылается на снятые инструменты»
  противоречил стражу, который обязан хранить эти имена; критерий сужен до
  «нет старых ожиданий, кроме сторожа этапа»;
- сторож проверял только регистрацию, а обещанное поведение при вызове — нет.
  Добавлен тест: вызов снятого имени должен читаться как «инструмента нет», а
  не «не хватает прав», иначе агент пойдёт просить доступ, который не поможет.
  Показан красным на временно возвращённом инструменте
  (`2 failed, 3 passed`), после снятия заглушки — `5 passed`;
- `artifacts/api_crud_permissions-ru.md` продолжал советовать добавить
  `Invoice delete` и `InvoiceDocument delete` — из этой таблицы они когда-то и
  появились. Добавлена оговорка этапа: «API разрешает» означает лишь, что путь
  отвечает, а не что операция делает правильную вещь.

Второй запуск PRD-review не делался: правки точечные, структура PRD не менялась.

**Прогон.** `docker compose --profile test run --rm test` —
1920 passed, 2 skipped, 70 deselected.

## Этап 269. Запуск отчётов отделён от чтения аналитики

Дата: 2026-08-31.

**Что изменено.** Четыре инструмента переведены с `analytics.read` на
`report_ai.write`: `create_report_ai_job`, `confirm_report_ai_job_candidate`,
`start_report_export` и `get_report_ai_job_export`. Пятый пишущий,
`save_report_ai_job_as_report`, был на этом праве с этапа 172.2. Читающие три —
`get_report_ai_job`, `get_report_ai_job_data`, `get_report_export_file` —
остались на `analytics.read`.

Второй слой (`required_scope_for_request`) разделён так же: любой
`POST /rest/api/report-ai-job*` и `GET /rest/api/report/StartReport` требуют
`report_ai.write`; `GET` задания, его данных и `reportFile` — `analytics.read`.
Без этого слой пропускал бы запуск отчёта по праву на чтение.

**Почему.** `create_report_ai_job` принимает свободный текст, по которому
Ветменеджер строит отчёт по данным клиники. Пока инструмент жил на
`analytics.read`, группа «Регистратура», которой отказано в `get_medical_cards`,
получала те же медкарты отчётом. Ограничения по областям на сформированный
отчёт не переносятся — Ветменеджер про наши права не знает.

**Новое право не заводилось.** `report_ai.write` уже существовало, уже означало
«работа с отчётами» и уже входило ровно в две группы — «Аналитик» и «Полный
доступ». Это и есть требуемый результат, поэтому составы групп не менялись.

**Что сломается у пользователей.** Ключи групп «Только чтение», «Регистратура»
и «Врач» теряют запуск отчётов и выгрузок; чтение готовых отчётов у них
остаётся. Решение владельца продукта 31.08.2026, ломающий характер принят.

Отдельный случай — **старые OAuth-подключения без группы**
(`access_preset IS NULL`): для них рантайм берёт сохранённый набор прав как
есть, а `report_ai.write` в наборе тех времён не было. Такое подключение
перестанет запускать отчёты, пока владелец не переподключит интеграцию с
выбором группы. У bearer-ключей этого нет: старый набор из 19 прав
распознаётся `deserialize_token_scopes` как полный доступ и дополняется до
всех прав. Сколько таких подключений — по коду не видно, нужен читающий запрос
к базе прода.

**Обещанные составы групп** приведены в соответствие: из «Только чтение» и
«Регистратуры» убраны четыре переведённых инструмента, чтение отчётов
оставлено.

**Ревью PRD сторонней моделью — валидный запуск 1/2**, четыре finding, все
приняты: в PRD не был назван `save_report_ai_job_as_report` (уже на нужном
праве, но поток из восьми инструментов описывался как семь); не был описан
эффект для OAuth и старых снимков прав; в списке ожидаемых соответствий
REST-путей отсутствовал `/save`; не был назван состав задетых тестов.

**Прогон.** `docker compose --profile test run --rm test` —
1978 passed, 2 skipped, 70 deselected.

## Этап 270. Удаление вынесено из общих прав на запись

Дата: 2026-08-31.

**Что изменено.** Заведено право `records.delete`. `delete_client` и
`delete_pet` требуют его вместо `clients.write` / `pets.write`. Право входит
только в группу «Полный доступ».

Второй слой: **любой `DELETE` под `/rest/api/` требует `records.delete`**,
независимо от сущности. Раньше `POST`, `PUT` и `DELETE` разбирались одной
веткой по карте сущностей, и неизвестный удаляющий путь проходил без
требования прав вовсе.

**Почему одно право, а не `clients.delete` и `pets.delete`.** Удаляющих
инструментов осталось два (счета сняты этапом 268), и доступны они одной
группе. Два права различались бы только именем, а в интерфейсе показывались
бы одной строкой. Разделить позже, когда появится группа, которой нужно одно
без другого, дешевле, чем сейчас поддерживать различие, которое ничего не
различает.

**Совместимость.** У bearer-ключа права хранятся списком, поэтому новое право
не появилось бы у уже выпущенных ключей полного доступа — они перестали бы
быть полным доступом. Сегодняшний 20-элементный набор добавлен в
`LEGACY_FULL_ACCESS_SCOPE_SNAPSHOTS`, и такой ключ читается как полный доступ
и дополняется до текущего набора. Сторож показан красным: с удалённым снимком
`test_a_full_access_key_issued_before_this_stage_still_deletes` падает.

У OAuth иначе: гранты с выбранной группой пересчитываются от группы и
получают право автоматически; гранты без группы (`access_preset IS NULL`) его
не получат, но они и так отзываются при обновлении токена с требованием
переподключиться и выбрать уровень доступа. Отдельной миграции не нужно, окно
«полный доступ без удаления» ограничено временем жизни текущего
access-токена.

**Ревью PRD сторонней моделью — валидный запуск 1/2**, три finding, все
приняты:

- high: утверждение «снимки покрывают и OAuth» неверно для грантов без
  группы — рантайм возвращает сохранённый набор как есть, а обновление токена
  такой грант отзывает. Описано явно;
- medium: существующий тест строил «старый полный доступ» вычитанием
  `report_ai.write` из актуального набора. После этапа такой список содержал
  бы `records.delete` и перестал быть историческим. Переписан на явный
  19-элементный список, рядом заведён тест на 20-элементный снимок;
- low: не был назван `artifacts/technical-requirements-vetmanager-mcp-ru.md`
  с матрицей групп и описанием второго слоя. Обновлён.

**Прогон.** `docker compose --profile test run --rm test` —
2007 passed, 2 skipped, 70 deselected.

## Этап 271. Список инструментов фильтруется по правам токена

Дата: 2026-08-31.

**Что сделано.** `ToolVisibilityMiddleware` на `on_list_tools`: для
HTTP-запроса берутся права токена, и в каталоге остаются инструменты, чьи
требуемые права входят в набор. Правило то же, что у `_ensure_tool_scopes_allowed`,
выражено один раз (`visible_tools_for_scopes`) — каталог и отказ не могут
разойтись. Фильтруются сами объекты `Tool`, список не пересобирается, поэтому
метаданные `securitySchemes` сохраняются.

Middleware зарегистрирован последним и реализует только `on_list_tools`:
цепочка вызова `OAuthChallengeMiddleware → ToolErrorTrackingMiddleware → tool`
не изменилась.

**Права для каталога читаются в обход журнала.** `peek_runtime_scopes` в
`runtime_auth.py` находит токен по хешу и возвращает набор прав, не тратя
бюджет ограничителя частоты, не записывая событие «аутентификация удалась» и
не трогая `last_used_at`. Это не косметика: метрики активации считают именно
эти записи, чтобы отличить «выпустил ключ и не пользуется» от «работает», —
обычный резолвер превратил бы каждый запрос каталога в работу с данными.
Функция консервативна: при любой неуверенности возвращает `None`, и каталог
остаётся полным. Ошибка в эту сторону безопасна — доступ решает проверка
вызова.

**Три случая, когда список остаётся полным:** не-HTTP транспорт; неудачная
аутентификация (обычный discovery до авторизации); и любой другой сбой —
но последний пишется предупреждением `tool_visibility_scope_lookup_failed`,
чтобы поломка инфраструктуры не выглядела нормальным ответом.

**Чего этап не чинит:** клиент, закешировавший каталог до авторизации.
Своего кеша каталога у сервера нет, аутентифицированный запрос всегда
фильтруется; остаточный риск на стороне клиента записан в PRD.

**Ревью PRD сторонней моделью — валидный запуск 1/2**, шесть findings, все
приняты: побочные эффекты обычного резолвера (журнал, ограничитель,
`last_used_at`); слишком широкое понятие «аутентификация не прошла» —
инфраструктурный сбой нельзя маскировать полным каталогом молча; риск
отравления клиентского кеша; сохранение `securitySchemes` при фильтрации;
безопасный порядок регистрации middleware; операционное допущение о правах
ключа дымной проверки (`post_deploy_mcp_tool_smoke.py` ищет `get_users`,
значит ключу нужен `users.read`).

**Сторожа показаны красными.** Сломанный фильтр (возврат полного списка) —
падает `test_the_middleware_filters_an_authenticated_catalogue`; снятая
регистрация middleware — падает `test_the_middleware_is_actually_registered`.
Второй сторож заведён специально: фильтр, который никуда не подключён,
выглядит зелёным и при этом не работает — так этап 264 выпустил недостижимое
сообщение.

**Аудит.** Первая версия импортировала `peek_runtime_scopes` внутри функции;
`scripts/inline_imports_audit.py` это поймал. Цикла импорта нет, импорт поднят
на уровень модуля.

**Прогон.** `docker compose --profile test run --rm test` —
2031 passed, 2 skipped, 70 deselected.

### Этап 271 — findings ревью на дифф (запуск 1/2)

Три finding, все приняты.

**medium ×2 — чтение прав для каталога было мягче настоящей аутентификации.**
`peek_runtime_scopes` проверяла только хеш, отзыв, срок и статус токена. Обычный
путь отказывает ещё и при неактивном аккаунте, несовпадении IP-маски, пустом
наборе прав и отсутствии активного подключения; у OAuth — при неактивном
аккаунте и подключении. Значит токен, которому отказали бы, всё равно получал
**подогнанный под его права** каталог: украденный ключ, использованный с чужого
адреса, узнавал, что он мог бы делать. Все недостающие проверки добавлены в обе
ветки.

Сознательное расхождение осталось одно — ограничитель частоты: запрос каталога
не вызов и не должен тратить бюджет вызовов.

**low — тесты middleware подменяли `peek_runtime_scopes` и настоящую реализацию
не трогали**, то есть остались бы зелёными ровно при том расхождении, которое
описано выше. Заведён
`tests/test_stage271_peek_scopes_matches_auth.py`: на реальных строках базы для
каждого способа отказа проверяется, что каталог отвечает «прав не знаю», и
отдельно — что запрос каталога не пишет ни строки в журнал и не двигает
`last_used_at`. Сторож показан красным: со снятой проверкой IP падает случай
«другой адрес».

Случай «неактивный аккаунт» в матрицу не попал: `accounts.status` сегодня
принимает только `active`, такую строку не завести. Обе ветки читают одну и ту
же колонку.

**Прогон после правок.** 2038 passed, 2 skipped, 70 deselected.

### Этап 271 — findings ревью на дифф (запуск 2/2, бюджет исчерпан)

**medium — оставалось последнее расхождение:** обе настоящие ветки входа
заканчиваются расшифровкой учётных данных подключения, а она может упасть сама
по себе — сменённый ключ шифрования, недописанное подключение, неподдерживаемый
режим. `peek_runtime_scopes` этого шага не делала, поэтому аккаунт со сломанным
подключением получал подогнанный каталог. Добавлен
`_connection_credentials_resolve`; при отказе — «прав не знаю».

**low — тест равенства покрывал только bearer.** Добавлен случай OAuth
(отозванный грант) и случай с нерасшифровываемым подключением. У последнего
обычный путь падает не `AuthError`, а исключением из самой расшифровки —
в тесте это зафиксировано отдельно, потому что важно не как он отказывает,
а что учётных данных он не выдаёт.

Бюджет ревью на дифф исчерпан (2/2). Дальнейших правок по этому этапу нет.

**Прогон.** 2040 passed, 2 skipped, 70 deselected.

## Этап 272. Состав группы считается из прав

Дата: 2026-08-31.

**Что сделано.** `MARKETED_PRESET_TOOLS` удалён. Состав группы даёт
`tools_for_preset(preset)`, который повторяет правило проверки вызова.
`get_presets_allowing_tool` построена на нём.

**Почему не синхронизация двух списков.** Расхождение и появилось из снимка,
который однажды перестали обновлять: у «Аналитика» было 97 инструментов против
обещанных 8, у «Регистратуры» — 71 против 24. Ещё один снимок протух бы так же.
Один источник дешевле любого сторожа за двумя.

**Цена расхождения была не косметическая.** Из обещанного списка строится текст
отказа. У 92 инструментов из 119 он занижал перечень подходящих групп, причём
первым всегда называл «Полный доступ»: агент, упёршийся в отказ, читал
«попросите полный доступ» там, где хватило бы «Аналитика». Теперь перечень
полон.

**Ловушка, которую нашло ревью PRD.** Буквальная формула «требуемые права ⊆
права группы» истинна для пустого списка прав, а проверка вызова пропускает
только поимённый `BASELINE_ALLOWED_TOOLS`. Инструмент, которому забыли
проставить права, рекламировался бы всеми группами и запрещался на вызове —
ровно то расхождение, которое этап убирает. Вычисление повторяет правило
буквально, а инвариант «инструменты с пустыми правами = baseline» закреплён
тестом. Сторож показан красным: с ослабленной формулой и добавленным
инструментом без прав падает восемь проверок.

`BASELINE_ALLOWED_TOOLS` переехал из `tool_scope_security` в реестр: это данные
о правах, а не о проверке, и реестр не может импортировать модуль проверки
обратно.

**Сторожа, стоявшие на удалённом списке, перенесены**, а не удалены: снятые
инструменты удаления счетов (этап 268) не должны попасть в состав ни одной
группы; запуск отчётов (этап 269) не должен предлагаться группам без права на
отчёты. Курированный список из восьми инструментов «Аналитика» в
`test_stage130` заменён проверкой, что весь поток Report AI доступен.

**Ревью PRD сторонней моделью — валидный запуск 1/2**, две medium, обе приняты
(ловушка с пустыми правами; перечень сторожей, которые нужно перенести).

**Прогон.** 2057 passed, 2 skipped, 70 deselected.

### Этап 272 — finding ревью на дифф (запуск 1/2)

**low.** `tests/test_stage132_scope_enforcement.py` проверял строку отказа
подстрокой `"Allowed presets: Full access, Read only"`. После этапа полный
перечень для `get_inactive_pets` — «Full access, Read only, Analytics», и
подстрока осталась зелёной: сторож перестал закреплять то, ради чего написан.
Ожидание переписано на всю строку целиком. Это ровно тот класс теста, который
и позволил расхождению дожить до пользовательского разбора.
...
- `access_summary.py` — сводка прав тремя строками по областям: «Чтение»,
  «Изменение», «Удаление». Все три показываются всегда; «Удаление: нет» —
  главный смысл этапа, потому что тревога пользователя была ровно про то,
  чего на экране не было.
- Страница кабинета: сводка под выбором уровня, переключается вместе с ним.
  Блок выбранного уровня отрисован сервером, остальные скрыты, поэтому без
  JavaScript страница остаётся осмысленной. Подпись переписана: «определяет,
  что помощник сможет **видеть и менять**».
- Экран согласия OAuth: те же три строки для каждого из четырёх уровней.
- Порядок групп в отказе: сначала узкие, «Полный доступ» — последним. Правило
  применяется в обоих местах, которые называют группы: `get_presets_allowing_tool`
  (отказ на уровне инструмента) и `get_presets_granting_scope` (отказ на уровне
  REST-пути).

**Названия областей — по смыслу права, а не по имени скоупа.** `analytics.write`
— это смены (`create_timesheet`), а не отчёты; отчёты живут на `report_ai.write`
и названы отдельно. Таблица покрывает все поддерживаемые права, это закреплено
тестом: право, не попавшее ни в одну область, молча исчезло бы из сводки.

Строка удаления называет «клиенты и питомцы» и закреплена тестом против
реестра: появится третий удаляющий инструмент — тест упадёт, и текст придётся
обновить.

**Про внешний вид.** Пользователь просил, чтобы страница не стала хуже.
Проверяется, а не оценивается на глаз: сводка использует существующую
muted-типографику, карточка внутрь карточки не вкладывается, перечень областей
не обёрнут в `<code>` и переносится — это отдельный тест; браузерные тесты
(10 штук, Playwright) с проверками горизонтального скролла на 390 px зелёные.

**Ревью PRD сторонней моделью — валидный запуск 1/2**, пять findings, все
приняты: второй путь отказа `get_presets_granting_scope`; каноническая таблица
областей со всеми правами и оговоркой про `analytics.write`; требования к
безопасности разметки (экранирование, nonce, данные для скрипта через
`data-`-атрибуты); поимённый список задетых тестов вёрстки и браузерных;
операциональный критерий «внешний вид не хуже».

**Побочно найдено сторожем упаковки.** `test_packaging_metadata` поймал, что
новый корневой модуль не добавлен в allowlist колеса: без правки
`pyproject.toml` `access_summary.py` не попал бы в образ и прод упал бы на
импорте.

**Прогон.** 2081 passed, 2 skipped, 70 deselected (опциональные контуры);
отдельно `-m "browser and not real_browser"` — 10 passed.

### Этап 273 — finding ревью на дифф (запуск 1/2)

**low.** Сторож вёрстки резал HTML до первого `</div>`, а это конец первой
строки сводки, — и проверял отсутствие вложенных карточек, `<code>` и запрета
переноса на коротком начальном фрагменте. Остальные строки и все прочие
уровни не проверялись вовсе. Заменено разбором блока по балансу тегов; сторож
показан красным: подставленный `white-space: nowrap` в последний preset-блок
теперь ловится.

## Этап 274. Второй слой: запись закрыта, чтение под наблюдением

Дата: 2026-08-31.

**Пересчёт риска.** Прежняя оценка «31 путь без права» была посчитана неверно:
перемножены все известные пути на все методы, включая комбинации, которых в
коде нет. По реальным местам вызова: 56 путей в `tools/` и `resources/`,
`GET` без права — ноль, пишущих вызовов 23, из них без права два — `POST` и
`PUT` на `/rest/api/Suppliers`. Динамически собираемых сущностей нет;
подставляются идентификаторы и нормализованный телефон.

**Что сделано.**

- `suppliers → inventory.write` в карте записи. Для законного вызывающего
  ничего не меняется: инструменты и так требуют это право.
- Неизвестный `POST`/`PUT` и **любой метод, которого нет в разборе**
  (`PATCH`, `HEAD`, `OPTIONS`, …) возвращают `REQUEST_NOT_MAPPED`, и клиент
  отказывает до обращения к Ветменеджеру. `REQUEST_NOT_MAPPED` намеренно не
  скоуп: никакой токен его не держит, и в отказе не появляется совета
  «попросите такое право».
- Неизвестный `GET` по-прежнему проходит, но пишет
  `rest_scope_unmapped_read` и увеличивает
  `vetmanager_rest_unmapped_read_total`, отдаваемый в `/metrics`.

**Почему чтение не закрыто вместе с записью.** Неверно отклонённое чтение
ломает все выдачи разом, неверно отклонённая запись — один сценарий. Сегодня
незакрытых читающих путей ноль, но окно наблюдения нужно не для них, а для
пути, которого не видно в исходниках. Закрытие чтения — следующий этап, по
значению счётчика на проде.

**Полнота — тестом по синтаксису, а не регуляркой.** Метод задаётся слоем
помощников (`crud_create`/`crud_update`/`crud_delete` превращают один
`endpoint` в три метода, путь медкарты живёт в константе), поэтому обход
литералов легко счёл бы пишущий путь читающим. Тест разбирает исходники
`tools/` и `resources/` через AST, находит вызовы `crud_*`, `.get`, `.post`,
`.put`, `.delete`, разрешает имена модульных констант и требует право для
каждой пары «метод → путь». Отдельная проверка следит, что сам сканер
что-то находит: молча пустой сканер сделал бы зелёным всё остальное.

**Вне слоя намеренно.** `vetmanager_connection_service.py` ходит в Ветменеджер
напрямую при заведении подключения — от имени владельца аккаунта, а не
выданного ключа. Прав токена у этих запросов нет, `_require_scope` к ним
неприменим, тест полноты их не покрывает.

**Ревью PRD сторонней моделью — валидный запуск 1/2**, пять findings, все
приняты: `PATCH`/`HEAD`/`OPTIONS` проходили мимо закрытия (high); прямые
вызовы из сервиса подключений не учтены в инвентаризации; сканер литералов
слишком слаб для пары «метод → путь»; в пути подставляется не только числовой
идентификатор; наблюдение не было операциональным — не названы метрика и
способ её прочитать.

**Сторожа показаны красными.** Возврат fail-open на запись — три падения;
дополнительно снятое право `suppliers` — пять.

**Прогоны.** `docker compose --profile test run --rm test` — 2094 passed,
2 skipped, 70 deselected. Живой контур devtr6 (`-m real_api`) — 59 passed,
8 skipped: настоящие вызовы к Ветменеджеру, включая записи, отказов не
получили.

### Этап 274 — findings ревью на дифф (запуск 1/2)

**high — дыра в закрытии, которую я открыл сам.** Ранний выход для пути,
который функция не может разобрать (короче трёх сегментов, не под
`/rest/api/`, пустой), стоял **до** проверки метода. Поэтому `POST ""`,
`POST "/foo"` и `PATCH "/foo"` трактовались как незамапленное чтение и
проходили наверх — ровно то, что этап закрывает. Проверка метода поднята
в начало, неразбираемый путь на запись отклоняется.

**medium — сканер полноты не видел вызовы через обёртки.** `_vm_get(path)` в
`good.py` и `_call_vm(method, path)` в `report_ai.py` передают путь дальше, и
прямых вызовов там нет: тест «у каждого вызова есть право» оставался зелёным,
не зная об этих путях вовсе. Сканер научен обеим формам, а проверка
самопроверки дополнена именами, которые видны только через обёртки
(`productsDataForInvoice`, `report-ai-job`).

**medium — предупреждение писало путь целиком.** Незамапленный путь по
определению тот, на который никто не смотрел, а в его хвосте может оказаться
идентификатор записи или телефон. Пишется только сущность (первые три
сегмента); закреплено тестом с телефоном в пути.

**Прогоны после правок.** Полный — 2095 passed, 2 skipped, 70 deselected;
живой контур devtr6 — 59 passed, 8 skipped.

## Этап 275. Три слоя защиты персональных данных в отчётах

Дата: 2026-09-01.

**Находка.** Санитайзер скрывает персональные данные по имени поля, словарь
английский. Колонки отчёта называет сгенерированный SQL по русскому заданию.
Живая проверка: `{"Владелец": "Иванов Пётр Сергеевич", "Телефон": "+7 918
414-01-11"}` уходил деперсонализированному токену без изменений, тогда как та
же строка с ключами `client_name` и `cell_phone` обезличивалась.
Деперсонализация включена по умолчанию — владельцы ключей считали, что ФИО и
телефоны скрыты.

**Слой 1 — имя колонки.** Русские ключи: владелец, хозяин, клиент, ФИО,
фамилия, отчество, контактное лицо, телефон и его формы, почта, адрес.
Бесквалификаторные «имя», «название», «адрес клиники» намеренно не берутся:
в отчёте это чаще кличка, товар или адрес поставщика, и слой испортил бы
данные вместо защиты.

**Слой 2 — значение, в любой колонке.** Только для отчётных инструментов и
только при деперсонализации. Почта; телефоны с 10–11 цифрами (микрочип из 15
цифр, штрихкод из 13 и даты не трогаются); ФИО из трёх слов; «Имя Отчество» по
суффиксу отчества; фамилия с инициалами; почтовый адрес по своим признакам
(город, улица, дом, квартира). Обычные инструменты не затронуты: их поля
предсказуемы, а чистка значений там стоила бы реальных данных.

**Слой 3 — задание.** Деперсонализированному токену в `intent_text`
дописывается требование не включать персональные данные. Работает раньше
остальных — на генерации. Текст постоянный, иначе ломается дедупликация
заданий на стороне Ветменеджера; его длина резервируется до проверки лимита в
20 000 символов и только для тех токенов, которые требование получают.

**Сторож связи (275.4).** Право на отчёты не выдаётся группе без полного
набора читающих прав. Иначе обход области видимости, закрытый этапом 269,
вернулся бы молча при заведении, скажем, «Аналитика по финансам».
`messaging.read` из инварианта исключён: его не требует ни один инструмент.

### Попытка обхода (275.6)

18 сценариев. Прошли три:

- **имя из двух слов без отчества** в колонке с неожиданным названием
  («Иванов Пётр») и **одна фамилия** («Иванов») — по форме не отличимы от
  названия клиники или улицы; закрывать их значило бы вычищать из отчёта
  осмысленные данные;
- **почтовый адрес строкой** — это закрыто отдельным правилом, добавленным
  после попытки.

Дополнительно закрыто правилом отчества: «Пётр Сергеевич» ловится, хотя это
тоже два слова.

Проверено, что не ломается: товар, услуга, порода, кличка, название клиники,
город, микрочип, штрихкод, дата со временем. Матрица зафиксирована тестом,
чтобы утверждение на странице не перестало быть правдой молча.

**Формулировка в интерфейсе не та, которую просил владелец продукта.** Он
просил включить «сценария, который обошёл бы защиту, мы не нашли». Такой
сценарий нашёлся — имя из двух слов, — поэтому в тексте вместо этой фразы
названо, что именно остаётся незакрытым. Там же сказано, что выгрузка CSV и
XLSX скачивается из Ветменеджера напрямую и через эти слои не проходит:
поля `csv_file`, `xlsx_file` содержат локатор файла, а не его тело.

**Ревью PRD сторонней моделью — валидный запуск 1/2**, восемь findings, все
приняты: точка применения слоя 2 (нужен tool-aware санитайзер, иначе либо
сломаем обычные инструменты, либо пропустим метаданные задания); экспорт как
отдельный формат; данные не только в `rows`, но и в `preview_example_row`,
заголовках кандидатов и безопасном тексте ошибки; лимит `intent_text` и
дедупликация; ложные срабатывания на микрочипах, штрихкодах и ИНН; слишком
широкие русские ключи; матрица сценариев обхода; операциональная
формулировка инварианта 275.4.

**Прогоны.** Полный — 2136 passed, 2 skipped, 70 deselected. Живой контур
devtr6 — 59 passed, 8 skipped.

### Этап 275 — findings ревью на дифф (запуск 1/2)

Четыре finding, все приняты. Все — про качество самих правил, и два из них
показали, что моя собственная попытка обхода была слабой.

**medium — ФИО ловилось только в одном написании.** `иванов петр сергеевич`,
`ИВАНОВ ПЕТР СЕРГЕЕВИЧ`, `Ivanov Petr Sergeevich` и `Иванов И И` проходили
насквозь. В базе колонка держит имя в любом из этих видов. Мой собственный
прогон показал латиницу «закрытой» — потому что искал в результате кириллические
подстроки и на латинскую строку просто не срабатывал: проверка была неверной, а
не правило. Правило переписано на регистронезависимое и с латиницей, инициалы —
с точками и без.

**medium — список суффиксов отчества был неполон.** `Сергей Никитич`,
`Иван Кузьмич`, `Павел Фомич` не ловились. Добавлен суффикс `-ич`.

**low — адрес не ловился с заглавной буквы** (`Ул. Ленина, д. 5`,
`Проспект Мира, д. 10`).

**low — и он же перехватывал лишнее:** `улица Красных Партизан` и
`проспект Чекистов` — обычные аналитические разрезы, а не адреса.

Разрешено так: адрес требует номера дома, поэтому название улицы само по себе
остаётся; правила ФИО не применяются к значению, в котором после вычистки
адреса всё ещё виден уличный маркер. Порядок важен: сначала адрес, потом
имена — иначе `Иванов Пётр Сергеевич, ул. Ленина, д. 5` потеряло бы имя из-за
защиты улицы.

Побочно: регистронезависимость сначала испортила `Вялый паралич` — правило
отчества приняло его за «Имя Отчество». Обе части пары теперь обязаны
начинаться с заглавной, суффикс сверяется без учёта регистра.

**Стоимость.** Отчёт предельного размера — 10 000 строк по пять колонок —
чистится за 0.31 секунды.

**Повторная попытка обхода после правок:** 17 значений с персональными данными
скрыты, 14 осмысленных значений не пострадали, расхождений нет. Остаток
прежний: имя из двух слов без отчества и одна фамилия.

---

## Этап 276. Выгрузка отчёта проходит через MCP, а не мимо него — 01.09.2026

**Живая проверка контракта до всякого кода.** `devtr6`, отчёт `report_id=84`.
`StartReport` → `report_file_id`; `reportFile` пока сборка идёт отвечает
**HTTP 401** с телом `Error: build is not started yet.`, затем
`Error: build in progress.`, потом отдаёт четыре поля файлов. Главное, ради
чего проверка затевалась: каждое поле — **абсолютный https-адрес на публичный
CDN**, и запрос по нему **без единого заголовка авторизации** вернул `HTTP 200`,
`content-type: text/csv`, тело в UTF-8 без BOM. С ключом API — байт в байт то же
самое. Значения локаторов не печатались и в репозиторий не попали.

Отсюда оценка этапа 275 («выгрузка идёт мимо слоёв») оказалась мягче
действительности: файл со всеми ФИО и телефонами был доступен всякому, кто
знает адрес, всё время жизни на CDN.

**Что решено и почему.**

- Ссылка выдаётся своя, путь — два сегмента HMAC-SHA256 по 128 бит на
  серверном ключе, производном от `WEB_SESSION_SECRET`. Голый хеш не годится:
  домен клиники известен, идентификатор токена — небольшое целое, такой путь
  перебирается за секунды.
- Части подписываются с указанием длины. Склейка через двоеточие неоднозначна:
  домен с портом и субъект с двоеточием дали бы двум разным парам один прообраз.
- Имя файла содержит 16 случайных байт, поэтому повторная выгрузка того же
  отчёта не переписывает файл, на который уже выдана ссылка.
- Срок жизни — трое суток, и правило действует **на выдаче**: файл старше
  срока не отдаётся и удаляется тут же. Подметание каталога при создании новой
  выгрузки нужно для диска, а не для приватности. Фоновый планировщик не
  заводился: его в проекте нет, а ради удаления файлов это новый режим отказа.
- Рядом с файлом лежит спутник с идентификатором доступа, и перед выдачей
  проверяется, что доступ жив. Запрос в базу делается **только после того, как
  путь совпал с существующим файлом**: иначе публичный маршрут стал бы
  способом нагружать базу вслепую. Любая ошибка проверки, включая недоступную
  базу, даёт тот же `404`.
- Право на отчёты и режим деперсонализации при выдаче не проверяются
  осознанно: файл уже очищен по правилам момента выгрузки, а разбор прав на
  неаутентифицированном маршруте был бы второй, более слабой копией модели
  доступа.
- Исходник на диск не пишется никогда — он живёт только в памяти запроса,
  потоком, с пределом 25 МБ по **распакованным** байтам. Переадресации не
  выполняются, `content-type` проверяется по списку: страница ошибки CDN,
  разобранная как CSV, была бы сохранена и выдана как отчёт.
- Ячейки, которые Excel исполнит как формулу, экранируются по первому
  значимому символу. Единственное исключение — значение целиком по
  `^-?\d+(?:[.,]\d+)?$`, иначе отчёт лишился бы отрицательных сумм.
- XLSX не отдаём: собрать его без новой зависимости нельзя, а тянуть её в
  512-мегабайтный прод ради второго формата этот этап не стал. CSV пишется в
  UTF-8 с BOM и точкой с запятой — Excel в русской локали открывает такой файл
  двойным щелчком.

**Что нашлось в собственном коде при разборе приватности логов.**

- `uvicorn` запущен с `access_log=True`, а `PERSISTENT_LOG_PATH` кладёт лог на
  диск. Строка доступа содержит полный путь, то есть ключ от файла. Поставлен
  фильтр на логгер `uvicorn.access`.
- Скрабер Sentry чистит заголовки, cookies, query и тело, но `request.url` не
  трогал. Добавлена замена пути на шаблон.
- Собственный журнал маршрутов пишет шаблон, а не путь, — здесь было в порядке.

**Принятый остаток.** Журнал обратного прокси на VDS видит путь и пишет его;
настройка прокси живёт вне репозитория. Ссылка сама по себе является ключом:
она остаётся в истории браузера, её может скачать превью мессенджера, а
отозвать одну конкретную ссылку нечем — можно только сменить секрет и погасить
все сразу.

**Живая проверка после реализации.** `docker compose --env-file .env --profile
test run --rm -T test python -m pytest -m real_api
tests/test_stage276_live_export_download.py` — `1 passed`. Тест запускает
настоящую сборку на devtr6, дожидается её, скачивает через инструмент,
проверяет, что ответ содержит только наш адрес, что файл лёг на диск и что он
находится по выданной ссылке.

Побочно подтверждён контракт guard'а: повторный `StartReport` в том же
получасе получил `403 can not run a report more than 10 minutes` — tenant-wide
ограничение Ветменеджера, уже описанное в README, а не наша ошибка.

**Тест-сторожа показаны красными** (правило 4.0). Сломано по одному:
имя снятого инструмента возвращено в реестр прав; подстановка шаблона вместо
пути обращена в тождество; экранирование формул отключено; проверка возраста
файла обойдена; проверка живости доступа заменена на «всегда жив». Красными
стали 15 тестов из двух файлов; после восстановления — снова зелёные.

**Ревью диффа сторонней моделью, запуск 1/2** — два findings, оба приняты и
оба про послабления, которые я оставил себе сам:

- **medium:** список разрешённых `content-type` включал пустую строку, то есть
  ответ CDN без заголовка типа проходил проверку. Страница ошибки без ярлыка —
  та же страница ошибки. Пустая строка убрана.
- **medium:** «первый значимый символ» перед формулой Excel отбрасывал только
  пробелы и `\x00`. Значение `\x1b=1+1` или `​=1+1` уезжало бы
  неэкранированным, а Excel смотрит сквозь такие символы. Теперь отбрасываются
  все управляющие и форматирующие символы (`Cc`, `Cf`) и пробельные.

Оба сторожа показаны красными до правки: `\x1b=1+1`, `​=1+1` и ответ без
`content-type` падали, после правки зелены. Полный прогон: 2210 passed,
2 skipped, 71 deselected.

---

## Этап 277. Словарь имён закрывает остаток из двух слов — 01.09.2026

**Идея.** Форма не различает `Иванов Пётр` и `Клиника Вера` — оба суть две
заглавные подряд. Словарь различает: `Пётр` — личное имя, `Ленина` — нет.
Пара обезличивается, только если одно из двух слов есть в словаре личных имён.
Одинокое слово не трогается никогда: кличка `Марта` и товар `Роза` — это то,
ради чего отчёт и строится.

**Словарь** — 533 записи: полные мужские и женские имена, ходовые
уменьшительные и латинские написания частых имён, перечисленные вариантами
(`petr`, `peter`, `piotr`, `pyotr`), а не выведенные алгоритмом.
Правило поддержки: имя, которое одновременно обычное слово (`Вера`, `Роман`,
`Роза`), кладётся в словарь только вместе с отрицательным тестом.

**Сторож — местный, а не общий.** Уличный сторож этапа 275 гасит правила имени
на всём значении; для комментария `Оплата от Клиника Вера, врач Иванов Пётр`
это оставило бы ФИО нетронутым. Новый сторож смотрит на саму пару: пара не
чистится, если любое из её слов — маркер (юрформа, организация, место,
святое). Коммерческий хвост (`плюс`, `фарм`, `вет`) блокирует только вторым
словом: `Вера Плюс` — название, `Vet Petr` — человек, и различает их
исключительно позиция маркера. Маркеры места дополнительно закрывают пару,
идущую сразу за ними: улица действительно называется тем, что стоит после
слова «улица».

**Решение, которое стоит назвать вслух.** Правила контекста для организаций
нет намеренно. Соблазн «если перед парой юрформа — не трогать» спасает
`ООО Вера Иванова` как название поставщика, но той же рукой оставляет
`Ветклиника Иванов Пётр`, где живой человек. Одно правило не обслужит оба
случая: снаружи они одинаковы. Ошибаемся в сторону защиты — потерянное
название поставщика это испорченная строка, пропущенное ФИО это утечка.

По той же причине товары и услуги не взяты в маркеры: `Гель Виктория` уцелел
бы, но вместе с ним уцелел бы `Осмотр Иванов`.

## Регресс этапа 275, найденный матрицей этого этапа

**Правило ФИО из трёх слов уничтожало обычный текст.** 31.08 оно было
переписано с флагом `(?ui)`, чтобы ловить `IVANOV PETR SERGEEVICH`. Флаг
поймал латиницу и заодно превратил `[А-ЯЁA-Z]` в «любая буква»: требование
заглавных исчезло, и **любые три слова подряд** в ячейке отчёта стали именем.

Проверено на коде из прода:

```
'Стрижка когтей собаке'      -> '[redacted-name]'
'Анализ крови общий'         -> '[redacted-name]'
'Вакцинация против бешенства'-> '[redacted-name]'
```

Это работало в бою сутки для всех токенов с деперсонализацией. Тесты этапа 275
этого не увидели, потому что в наборе «должно уцелеть» не было ни одного
значения из трёх слов — все были из одного-двух.

**Починено** перечислением форм вместо флага: слово — это `Заглавная+строчные`
или `ЗАГЛАВНЫЕ`, в кириллице или латинице. Имя написано так; предложение — нет.

**Что фикс отнимал и как это возвращено.** Старое правило ловило и полностью
строчное `иванов петр сергеевич`; новое — нет. Возвращено точечно: строчная
пара «имя + отчество» чистится, только если первое слово есть в словаре.
`петр сергеевич` — человек, `вялый паралич` — диагноз, и отличает их словарь,
а не форма. Без этой оговорки суффикс `-ич` съел бы половину диагнозов.

**Урок для набора тестов.** «Должно уцелеть» надо строить не из удобных
коротких значений, а из тех, что длиннее и сложнее подозреваемых. Матрица
этапа 275 проверяла защиту на именах и забыла проверить её на обычной работе
клиники.

**Тест-сторожа показаны красными** (правило 4.0). Сломано по одному: правилу
трёх слов возвращён общий флаг регистронезависимости, отключено правило пары,
снят сторож маркеров, снята словарная оговорка у строчного отчества. Красными
стали 21 и 9 тестов соответственно; после восстановления — 118 passed.
Полный прогон: **2274 passed, 2 skipped, 71 deselected**.

**Ревью диффа сторонней моделью, запуск 1/2** — два high, оба про частичную
очистку, оба приняты:

- **high:** формы с дефисом, апострофом и внутренней заглавной чистились
  наполовину — `Петров-Водкин Пётр Сергеевич` возвращался как
  `Петров-[redacted-name]`. Остаток фамилии на виду и сломанное значение
  впридачу. Причина: определение «слова имени» у правила трёх слов не знало
  про дефис и апостроф, а у правила пары — знало. Теперь определение одно на
  оба правила, с внутренним дефисом, апострофом и внутренней заглавной
  (`МакДональд`), и с отдельной формой для `O'Connor`, где перед апострофом
  одна буква. Голые заглавные (`А Б В`) именем по-прежнему не считаются.
- **high:** `иванов петр сергеевич` возвращается как `иванов [redacted-name]`,
  а тест проверял только исчезновение имени. **Код оставлен как есть, тест
  ужесточён до точного равенства.** Проглатывать слово перед строчным именем
  нельзя: в нижнем регистре ничто не отличает фамилию от глагола, и правило
  съело бы `принимал петр сергеевич` целиком. Одинокая фамилия — это ровно тот
  остаток, который назван в тексте кабинета, а не новый.

Полный прогон после правок: **2280 passed, 2 skipped, 71 deselected**.

## Живое подтверждение этапа 276 — 01.09.2026, после закрытия окна guard'а

`StartReport` → `report_file_id`; первый опрос `reportFile` вернул
`HTTP 401 build in progress`, второй — `HTTP 200`. Инструмент скачал файл с
CDN, очистил и отдал:

```json
{
  "download_url": "https://vetmanager-mcp.vromanichev.ru/report-export/<32 hex>/<32 hex>.csv",
  "expires_at": "2026-09-04T06:57:09+00:00",
  "rows": 1,
  "columns": 1,
  "depersonalized": true
}
...
Файл на диске: 28 байт, права `600`, первая строка — `Количество`. Срок жизни
в ответе ровно трое суток от момента выгрузки. Адресов Ветменеджера и CDN в
ответе нет.

---

## Этап 278. Чужой локатор не должен уезжать в наш лог — 01.09.2026

**Найдено живым прогоном этапа 276, а не тестом.** В выводе того же прогона
видна строка библиотеки:

```
httpx: HTTP Request: GET https://<cdn>/vetmanager-public-user-files/<домен>/…csv "200 OK"
```

Этап 276 закрыл собственный подписанный путь в двух местах — фильтр
`uvicorn.access` и `request.url` в Sentry — и пропустил чужой адрес, потому
что его пишет не наш код. Прод работает с `LOG_LEVEL=INFO`, а
`PERSISTENT_LOG_PATH` кладёт журнал на диск, то есть публичная ссылка на
**неочищенную** выгрузку оседала в постоянном файле.

**Решение:** логгер `httpx` приглушён до `WARNING`. Информация не теряется —
каждый исходящий вызов уже проходит через собственную инструментовку с
метриками по эндпоинту, методу, статусу и длительности; строки библиотеки их
дублировали и вдобавок носили адрес.

**Остаток, который правкой репозитория не закрывается:** журналы, уже
написанные на VDS до этой правки, содержат такие строки. Их ротация и чистка —
операция на сервере.

**Урок тот же, что и в этапе 277:** правило «изменил инструмент — вызови его
вживую» окупилось второй раз за день. Ни один мок не показал бы строку
стороннего логгера.

**Сторож показан красным** (правило 4.0): приглушение снято — два теста из
трёх падают, третий («предупреждения видны») остаётся зелёным, как и должен.
Полный прогон: **2283 passed, 2 skipped, 71 deselected**.

**Ревью диффа сторонней моделью, запуск 1/2** — high и medium, оба приняты:

- **high:** приглушение логгера закрывает журнал, но не Sentry. При
  неожиданном исключении локальные переменные кадра — `locator`, `payload` с
  `csv_file` — уезжают в трейс, а скрабер чистит их только по именам ключей;
  адрес внутри строки-`payload` не ловился ничем. Добавлены имена ключей и,
  главное, вычистка **любого** `http(s)://…` в значениях локальных переменных.
  Адрес в локальной переменной почти никогда не объясняет падение, а здесь
  один из них — ключ от всей выгрузки клиники.
- **medium:** утверждение PRD «диагностика не теряется» было неверным именно
  для скачивания с CDN: собственная инструментовка покрывала только вызовы
  Ветменеджера, а строка httpx была единственным местом, где виден статус и
  время этого запроса. Скачивание обёрнуто своей записью
  (`target="report_export_storage"`, статус, длительность) — без адреса.
  Статус берётся сразу после получения ответа, поэтому отказ по типу тела или
  по размеру тоже измеряется.

Сторожа показаны красными: снятие вычистки URL и подмена записи метрики
роняют три теста. Полный прогон: **2286 passed, 2 skipped, 71 deselected**.

## Этап 225. Диагноз в медкарте — ссылка, а не текст (2026-09-01)

- Исходная посылка этапа («стенды на разных сборках, дефект чужой») проверена
  и снята. Живые вызовы на `devtr6` 01.09.2026, карта `818`: отказ 500 даёт
  ровно одна форма — значение `diagnos`, разбираемое как целое (`"32"`, `32`).
  Всё прочее принимается с `201` и сохраняется как есть: свободный текст,
  `[32]`, объект без `type`, ссылка на несуществующий диагноз. Пустая строка
  стирает диагноз в `0`. Хранимое испорченное значение последующим правкам не
  мешает — проверено по каждой строке матрицы.
- Наблюдаемый контракт поля: `diagnos` — строка с JSON-массивом
  `{"id", "type"}`; `id` из `/rest/api/MedicalCards/AllDiagnoses`, `type` из
  `ComboManualName` с именем `diagnos_types` (значения 1..4, по умолчанию 1 —
  `dop_param1: "true"`). Всё это снимается публичным API, чужие исходники в
  репозиторий не переносились.
- Допущение, которое пришлось принять: существование `id` в справочнике
  клиники инструмент не проверяет. Обещается форма ссылки, не её
  осмысленность; проверка стоила бы отдельного запроса на каждую запись, а
  апстрим неизвестный id принимает молча.
- `diagnosis_ids=[]` отклоняется, а не трактуется как «не менять»: у списка
  напрашивается чтение «очистить», а стирание диагноза в scope не входит.
  Молчаливое «не менять» оставило бы ловушку.
- Строгий тип идентификатора (`Field(strict=True)`) выбран потому, что
  pydantic без него принимает `True` как целое `1`, и булево значение стало бы
  ссылкой на реальный диагноз с id 1. Диапазон проверяется в коде, чтобы отказ
  называл справочник, откуда берутся значения.
- `diagnosis_text` при создании карты отклоняется: `POST` отвечает `201` и
  теряет поле, `PUT` то же значение сохраняет (карта `824`, 01.09.2026).
  Принять и потерять — та же молчаливая порча, что и исходный дефект.
- Причина отказа на `devslon67` осталась непроверенной: REST-ключа от стенда
  нет, пишущих инструментов через bearer-токен не выдано. Формат-дефект
  самостоятелен, но вторую причину не исключает.
- PRD-review сторонней моделью: два прогона (бюджет исчерпан), десять находок,
  все приняты, отклонённых нет. Ключевые: семантика `[]`, слабый сторож,
  `bool` как целое, недоказанный вывод про `devslon67`.
- Живой прогон (§6.1): карта `822` создана инструментом с
  `diagnosis_ids=[32,11]`, `diagnosis_type=2` — в карте
  `[{"id":32,"type":2},{"id":11,"type":2}]`; замена набора, заметка правкой,
  четыре недопустимых ввода отклонены до запроса, поле после отказов не
  тронуто. Полный прогон: `2314 passed, 2 skipped, 74 deselected`.

### Ревью диффа — 2026-09-01

- Два прогона, бюджет diff-review исчерпан. Шесть находок, все приняты,
  отклонённых нет.
- Прогон 1: `diagnosis_type` не strict (`True` проходит как `1`);
  `_diagnos_field` падает `TypeError` на не-списке вместо ответа; валидация
  диагноза в `update_medical_card` стоит после `crud_get_by_id`, то есть
  заведомо неверный ввод стоит запроса и может спрятаться за ошибкой чтения;
  мутация payload до возможного отказа; уточнить в докстринге, что пустой
  `diagnosis` игнорируется.
- Прогон 2: `diagnosis_type=1.0` проходит, потому что `1.0 in (1,2,3,4)`
  истинно, и в JSON уходит `"type":1.0`. Проверка заменена на
  `type(diagnosis_type) is not int`. Остальные пункты подтверждены закрытыми,
  блокеров нет.
- Все сторожи этапа показаны красными до приёма: отказ `diagnosis_text` на
  создании (снят флаг `on_create`), два отказа «до чтения карты», не-список,
  булев и дробный тип — на коде предыдущего коммита.
- Финальный полный прогон: `2323 passed, 2 skipped, 74 deselected`, exit 0.

## Этап 279. Роадмап теряет пункты молча

**Что проверялось.** `Roadmap.md` объявлен единственным источником очереди
работ, но никакой машинной проверки его структуры не было: существующие гейты
проверяют связку «этап → PRD → AssumptionLog», а не сам файл. Разбор 02.09.2026
по просьбе владельца нашёл пять структурных ошибок, каждая из которых делает
работу невидимой в очереди.

**Решения.**

- **Гейт судит структуру, а не содержание.** Пять проверок: статус этапа из
  словаря, статус у пункта, номер пункта против номера этапа, отсутствие
  незакрытых пунктов в закрытом этапе, непустота открытого этапа. Осмысленность
  формулировок и наличие источника не проверяются — это работа ревью, а не
  регулярного выражения.
- **Списка исключений нет.** На настоящем файле гейт сначала дал 23 находки.
  Девятнадцать из них — исторические формы записи (`done via 103a`,
  `**deferred** в stage 113b`, `N/A`), которые словарь не знает. Заводить под
  них allowlist отказались: исключение гасит гейт молча и навсегда. Вместо
  этого исторические строки приведены к словарю с сохранением смысла —
  `done via X` стал `` `done` (через X) ``, отложенные пункты 113 и 114 стали
  `stop` (они и правда унесены в 113b и 114b).
- **`supervisor_pending` признан статусом, а не опечаткой.** Он встречается
  один раз (пункт 234.3) и означает реальное состояние: работа записана, никто
  её не делает, она ждёт решения владельца. Внесён в словарь как **открытый** —
  именно поэтому гейт его и поймал внутри закрытого этапа 234.
- **Статус пункта ищется только в его собственном абзаце.** Комментарий под
  пунктом может цитировать любое слово словаря; если брать последний токен по
  всему блоку, комментарий переопределит статус. Закреплено тестом.
- **Остатки вынесены наружу, а не закрыты задним числом.** 266.4 → этап 280,
  234.3 → этап 281, 249.3 → пункт 246.6. Ни один не помечен `done`: работа не
  сделана, а только перестала прятаться.
- **Расхождение 249.3 и 246.3 разрешено в пользу 246.3.** Формулировка
  «перевести known issue #30 в `fixed`» была неверна: #30 переведён 23.08.2026,
  о чём и написано в 246.3. Осталась только связка отчётов #39 и #40, и она
  операционная — production `known_issues`, репозиторий не меняется (AGENTS.md,
  раздел про feedback triage).
- **Дубль, а не потеря.** Пункт 97.7 под этапом 104 оказался копией строки,
  которая и так стоит под своим этапом 97. Удалён, а не перенесён.
- **Этап 231 закрыт `stop` без работы.** Документационная половина сделана
  этапом 254, наблюдаемость деплоится вне репозитория. Собственного содержания
  не осталось; причина записана в самом этапе, чтобы его не завели заново.

**Живой вызов.** Не требуется: инструменты не менялись, изменения касаются
документов и инструмента разработки (CLAUDE.md 6.1 про изменённые инструменты).

**Сторожа показаны красными.** Проверки 3, 4 и 5 отключены в коде поочерёдно —
упало 9 тестов из 15. Проверка на настоящем файле показана красной подсаженным
этапом 999 с пунктом `todo` внутри `done`. Проверки 1 и 2 доказываются своими
образцами: каждый строит нарушение и требует ненулевого кода возврата, а
отдельный тест на корректном образце подтверждает, что гейт не падает всегда.

### Ревью диффа — 2026-09-02

- Один прогон, две находки, обе приняты; отклонённых нет.
- **high, принято.** Разбор не понимал вложенной нумерации: заголовки идут не
  только `## Этап 234`, но и `## Этап 2.5`, а пункты под ними — `2.5.1.1`.
  Старое выражение `^- (\d+)\.(\d+)\s` таких пунктов не видело **вовсе**: 169
  строк роадмапа проходили мимо всех пяти проверок, и тесты этого не ловили,
  потому что покрывали только простую форму `N.M`. Принадлежность пункта этапу
  теперь определяется префиксом с точкой (`2.5.` — родитель `2.5.1.1`, но не
  `2.51.1`), а не равенством первого числа.
- **low, принято.** Шапка `Roadmap.md` объявляла словарь из трёх статусов,
  тогда как файл использует пять. Исправлено там же.
- **Что вскрылось после починки.** Гейт, научившись видеть вложенные пункты,
  нашёл ещё три записи без статуса. Две (105.2 и 106.5) оказались ложной
  тревогой разбора: их статус стоит на отдельной строке **после** вложенного
  списка. Правило уточнено — вложенный маркированный подпункт не заканчивает
  пункт и не одалживает ему статус. Третья, 52.3.3 (server-side session
  revocation), — настоящая: работа отложена при закрытии этапа 52 и с тех пор
  не значилась нигде. Вынесена этапом 282.
- **Сторожа новых правил показаны красными:** на разборе из коммита `5602e88`
  падают четыре теста вложенной нумерации; на варианте, где подпункт
  заканчивает пункт, падают проверка настоящего файла и тест про статус после
  вложенного списка.

## Решения владельца — 2026-09-02

Четыре вопроса, висевшие в очереди как «оценить, стоит ли», сняты решением
Владимира. Записаны здесь, потому что каждый закрывает пункт роадмапа не
работой, а выбором.

- **248.2. Подтверждение перед массовой рассылкой не вводим.** Гейт
  подтверждения на стороне MCP подтверждала бы та же модель, которая и
  отправляет, — это не защита, а лишний ход. От необратимости защищает
  предупреждение в докстринге (248.1), от чужой рассылки — право
  `messaging.write`, которого нет ни у одной группы, кроме полного доступа.
- **255.3. Стенд остаётся один — `devtr6`.** На нём работают все живые пробы,
  проходит запись медкарты, и он записан выделенным тестовым стендом с этапа 5.
  `devslon67` собран иначе и на записи отдаёт 500: контур, который ведёт себя
  не как прод, проверяет не то. Перевод bearer-токена на `devtr6` — действие на
  стороне Ветменеджера, заведено пунктом 255.4; до него README обязан называть
  и текущее расхождение.
- **246.4. Сплошной `redact` по таблице не гоняем.** При росте
  `REDACTION_VERSION` — выборка из нескольких отчётов; отчёты читаются только
  по запросу, фонового обхода нет. Сплошной проход перечитывал бы и переписывал
  все тексты обратной связи ради версии санитайзера, то есть трогал бы больше
  чужих данных, чем разбирает.
- **282.1. Отзыв сессии на сервере не нужен.** Срок жизни уже сутки (52.3.2),
  хранение сессий в БД ради отзыва дороже остаточного риска. Возвращать — при
  появлении сценария, где сутки слишком много: общий компьютер в клинике,
  увольнение, утечка cookie.

## Этап 223. Полнота выдачи inactive pets

**Что проверялось.** `get_inactive_pets` отдавал `limit_applied: 50` и
пятьдесят записей, по которым нельзя отличить полную выдачу от обрезанной.

**Решения.**

- **Обрезка определяется точно, а не по длине списка.** Набор идёт до
  `limit + 1`; лишний питомец в ответ не попадает, но отвечает на вопрос,
  который иначе не имеет ответа: список ровно в лимит — это «все» или «первые»?
  Альтернатива (считать `truncated` по `len(result) == limit`) врёт ровно в
  том случае, ради которого поле и заводится.
- **Отдаём число клиентов в окне, а не питомцев.** `clients_total_in_window` —
  это `totalCount`, который апстрим уже присылал, а помощник выбрасывал. Число
  питомцев в окне узнать нельзя, не обойдя всех клиентов, — то есть не сделав
  ровно тот обход, от которого инструмент и защищает. Придумывать `total_pets`
  отказались: неизвестное лучше назвать неизвестным.
- **Нет счётчика — это `null`, а не ноль.** Ноль читается как «в окне никого»,
  и это другой ответ.
- **`truncation_reason` разделяет две разные ситуации.** `limit_reached`
  лечится большим `limit`, `client_scan_cap` — только сужением окна: там
  кончился бюджет обхода, а не место в выдаче. Обе сразу дают
  `limit_reached+client_scan_cap`.
- **Соседний `get_inactive_clients` починен тем же ходом.** Дефект тот же,
  помощник общий; чинить один и оставить другой было бы решением, которое
  нечем объяснить. Роадмап говорил только про питомцев — расширение записано
  здесь и в PRD, а не сделано молча.
- **Константы обхода подняты на уровень модуля.** Иначе поведение на упоре в
  потолок проверяется только моком двух тысяч клиентов, то есть не проверяется.

**Правка чужого теста.** `test_get_inactive_pets_batches_lookup_for_large_client_page`
требовал `medcard_route.call_count == 0`. Набор `limit + 1` дотягивается до
51-го питомца, у которого счёта нет, и один батчевый fallback срабатывает.
Ассерт ослаблен до `<= 1` с объяснением в самом тесте; смысл проверки —
«fallback остаётся батчевым и не превращается в вызов на клиента» — сохранён
и закреплён отдельным тестом `test_the_probe_costs_one_batched_fallback_not_one_per_client`,
показанным красным на `_BATCH_SIZE = 1` (20 клиентов → fallback на каждого).

**Живой вызов на `devtr6` 02.09.2026** (`pytest -m real_api
tests/test_stage223_live_inactive_completeness.py`, оба теста прошли):

- `get_inactive_clients` (`limit=2`, окно 1–240 месяцев):
  `"total_in_window": 16, "truncated": true`, в списке две записи.
- `get_inactive_pets` (`limit=1`, то же окно):
  `"truncated": true, "truncation_reason": "limit_reached",
  "clients_total_in_window": 16, "clients_scanned": 2,
  "safety_cap_reached": false`.

**Сторожа показаны красными.** Девять тестов этапа написаны до реализации и
падали с `KeyError` на отсутствующих полях; десятый — на `_BATCH_SIZE = 1`.

## Этап 281. Трекинг не носит чужих данных

**Что проверялось и на чём.** 80 событий проекта `python` организации
`home-xv3` — самое новое и самое старое у каждого из 40 issue за 21.08–02.09.2026.
Разбор печатал путь поля, сработавший детектор и маску значения; сами значения
в переписку не выгружались, выгрузка событий лежала вне репозитория и удалена
после этапа. Детекторы: email, JWT, bearer, телефон, длинный hex,
непрозрачная строка, домен клиники, а также личное имя по словарю этапа 277.

**Чего нет ни одного:** email, JWT, bearer-токен, телефон, личное имя.
`Authorization`, `X-Forwarded-For`, `X-Real-Ip`, `Mcp-Session-Id` и
`X-Openai-Session` вычищены в 100% случаев. Санитайзер срабатывает 3722 раза
в 260 разных местах.

**Ложные срабатывания, разобранные явно.** Детектор телефона сработал трижды —
на `Traceparent`, `Tracestate` и `X-Datadog-Tags`: длинные цифровые ряды
трассировки, не телефоны. Длинный hex — на идентификаторах самого Sentry,
версии релиза и путях модулей.

**Решения.**

- **Найдено не то, что искали, и это не повод считать проверку неудачной.**
  Искали email и токены клиники — не нашли. Нашли данные **вызывающего**:
  `X-Openai-Subject` (устойчивый идентификатор конечного пользователя на
  стороне вызывающей платформы) и метаданные запроса в переменной фрейма.
  Записано как находка, а не как «проверка прошла чисто».
- **Обе находки одного рода: защита держалась на совпадении слова.**
  `X-Openai-Session` рядом вычищен потому, что в имени случайно оказалось
  `session`; `progressToken` — потому что `token`. Это не правило, а удача.
  Тот же механизм отказа, что в этапе 278: адрес выгрузки уехал в лог мимо
  нашего кода, потому что писала его библиотека.
- **`subject` внесён подстрокой, `meta_dict`/`_meta` — точным совпадением.**
  Личность может приехать под разными именами, поэтому подстрока. А голые
  `meta` и `metadata` слишком часты: вычищать их подстрокой значит ослепить
  трекинг ровно там, где он объясняет собственные падения. Цена ошибки
  несимметрична только для идентификатора, и правило написано по этой
  несимметрии, а не единообразно.
- **Заголовки чистятся, копия заголовков в переменной — нет.** Ключевой урок
  этапа: защита, привязанная к месту (`request.headers`), не переносится на
  копию тех же данных в другом месте. Правило теперь привязано к имени поля и
  действует в переменных фреймов, breadcrumbs и контекстах одинаково.

**Проверка правки на настоящих значениях.** Утёкшие значения из выгрузки —
сам `X-Openai-Subject` и содержимое `meta_dict` — прогнаны через починенный
санитайзер: оба уходят в `[Filtered]`. Значения при этом не печатались,
выводился только вердикт.

**Повторная проверка на новом живом событии не делалась намеренно:** прод не
роняют ради проверки. Правка закрыта сторожами, следующее настоящее событие
покажет результат само.

**Сторожа показаны красными.** Три теста из пяти падали до правки; два
оставшихся — охранные («наблюдаемые заголовки остаются», «голое `meta` не
подметается») — зелёные и до, и после, что и требуется от них.

### Ревью диффа — 2026-09-02, этап 281

- Один прогон, две находки, обе приняты. Обе — про то, что правка закрыла
  **наблюдённую форму**, а не класс данных.
- **high, принято.** `_redact_mapping` смотрел только на внешнее имя и не
  спускался внутрь. `headers = dict(request.headers)` — обычная строка, одна
  такая живёт в `auth_audit.py`; попав во фрейм, она несла всю карту заголовков
  под безопасным именем `headers`, мимо чистки, которая те же значения только
  что вычистила уровнем выше. Ревьюер нашёл конкретное место в коде, а не
  предположил его.
- **medium, принято.** Точный набор `{"meta_dict", "_meta"}` защищал листья
  (`x-codex-turn-metadata`, `threadId`, `claudecode/toolUseId`) только пока они
  лежали внутри обёртки. Те же поля заголовком, в `extra` или отдельной
  переменной проходили насквозь — защита зависела от формы события, а не от
  того, что это за поле.
- **Что сделано.** `_redact_mapping` рекурсивный: спускается в словари и
  списки, правило имени действует на любой глубине. Глубина ограничена
  двенадцатью уровнями — санитайзер не должен сам стать падением, которое он
  описывает. Имена листьев внесены подстроками `turn-metadata`, `tooluseid`,
  `threadid`.
- **Урок этапа, записанный явно.** Оба круга правок — про одно: защита,
  привязанная к месту или к форме, не переносится. Сначала она держалась на
  случайном слове в имени, потом на том, что данные лежат в конкретном ключе.
  Правило должно опознавать поле по тому, что это за поле.
- **Сторожа показаны красными** на коде коммита `d8adec8`: три новых теста
  падают, шесть прежних остаются зелёными.


## Этап 284. Вход поддержки — GitHub Issues, а не чужая почта

- **Адрес `support@vetmanager.cloud` был чужим и непроверенным.** Он попал на
  лендинг как placeholder этапа 51 и прожил на бою до 02.09.2026, потому что
  тест `test_final_cta_prioritizes_clinic_help` сторожил его **наличие**, а не
  его уместность. Это второй случай той же формы отказа, что в этапах 278 и 281:
  проверка держалась на совпадении текста, а не на правиле.
- **Сторож намеренно шире правки.** Запрещён любой `mailto:` на странице, а не
  конкретный адрес: иначе следующий placeholder войдёт свободно. Показан красным
  до правки (`assert "mailto:" not in html` падал на обеих ссылках), зелёным —
  после.
- **Почему issues, а не своя почта.** Почтового контура в проекте нет вообще —
  ни `smtplib`, ни провайдера; заводить его ради поддержки означает новый контур
  с доставляемостью и отбивками. В issues возврат ответа встроен, обращение
  публично и ищется, а стоимость одного обращения не растёт с числом
  пользователей.
- **Личный адрес Владимира на лендинг не выносился.** Предложен им 02.09.2026
  голосом; точное написание голосом не восстанавливается, а вписывать угаданный
  адрес на боевую страницу нельзя. Решение отложено до письменного подтверждения;
  при добавлении меняется и сторож — с «нет `mailto:`» на «нет чужого адреса».
- **Не входило в этап:** поле ответа в `agent_feedback_reports` (отвечать в схеме
  сегодня некуда), раздел «Мои обращения» в кабинете, публичная страница известных
  проблем, формулировка про отсутствие SLA.

### Ревью диффа этап 284 (Codex gpt-5.5, 03.09.2026)

Critical/high нет. Утечки `SUPPORT_EMAIL` на публичные поверхности ревьюер не
нашёл: проверены лендинг, OG-мета, `.well-known`, health/readiness/metrics и
MCP-сниппеты — единственное чтение переменной остаётся страницей кабинета.

Принято четыре находки:

- **medium — валидация не покрывала URI-уровень.** Первая версия запрещала
  символы, ломающие HTML, и пропускала `?`, `&`, `=`, `%`: значение
  `owner@example.org?bcc=evil%40example.net` проходило и рендерило `mailto:`
  с полем `bcc`, а `owner%2Cevil@example.org` в почтовом клиенте разворачивалось
  в двух получателей. `escape()` тут не помогает — разметка безупречна, ломается
  смысл ссылки. Это тот же дефект, что этап описывает у себя же: запрет **сырой**
  запятой при разрешённом `%2C` — защита по совпадению символа. Чёрный список
  заменён белым: `^[A-Za-z0-9._+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)+$`.
- **low — сторожа не покрывали URI-уровень.** Добавлено шесть случаев:
  `?bcc=`, `&subject=`, `%2C`, `%0D%0A`, `mailto:` внутри значения,
  `javascript:`. Четыре из них показаны красными на прежнем валидаторе.
- **low — проверка «нет `mailto:`» слишком слаба.** Если невалидное значение
  однажды отрендерится видимым текстом, сторож остался бы зелёным. Добавлено
  утверждение, что сырое значение не встречается на странице вовсе.
- **low — этап 284 остался `todo` в закоммиченном дереве.** Следующий агент
  подобрал бы его как открытый. Статусы закрыты.

Отклонённых находок нет. Использован 1 валидный прогон внешнего ревью из 2.

### Этап 284, пункт 7 — настройка не доезжала до контейнера (03.09.2026)

Проверка на живом проде после успешного деплоя: `SUPPORT_EMAIL` внутри
контейнера — `MISSING`. Значение было записано в `.env`, деплой отработал,
код уехал, тесты зелёные — и почта в кабинете не появилась бы.

Причина: сервис `mcp` в `docker-compose.yml` перечисляет переменные окружения
поимённо. `.env` — источник подстановки, а не канал доставки: не названа в
`environment:` — не существует внутри контейнера. Отказ тихий: приложение
видит пустую строку и ведёт себя как при незаданной настройке, то есть
корректно и молча.

Это третий случай той же формы за сутки (278, 281, 284): защита или настройка
существует, но не соединена с местом, где нужна, и ничто об этом не сообщает.

Сторож написан на правило, а не на переменную: пересечение `.env.example` и
чтений `os.environ` в рантайм-модулях обязано быть объявлено в сервисе `mcp`.
Пересечение выбрано как критерий, потому что оно само отсекает ложные
срабатывания: `TEST_*` читаются тестами, `POSTGRES_*` — соседним сервисом,
`UID`/`GID` — сборкой, в рантайме ни одна не читается. Второй тест проверяет,
что множество непустое: сторож с пустым множеством зелен всегда и не значит
ничего. Показан красным — падал на `SUPPORT_EMAIL` до правки.

Полный аудит расхождений (33 переменные читаются рантаймом, 21 объявлена)
вынесен в отдельный этап: правило сейчас закрывает только то, что обещано
оператору.

### Этап 286. Выручка — определение, а не арифметика (03.09.2026)

- **Отчёт #49 был про отсутствие определения.** Режим `invoiced` уже суммировал
  `amount` по `status = exec` — ровно то, что владелец назвал выручкой. Расхождение
  давал дефолт `received` (касса), куда попадают технические строки перераспределения.
- **Сторож на описание сначала оказался зелёным по совпадению:** проверял подстроку
  `exec`, а она нашлась внутри слова `executed`. Переписан на целые формулировки
  (`invoice.amount`, `status='exec'`). Это тот же дефект, что в этапах 278, 281 и 284 —
  проверка держалась на случайном совпадении текста.
- **Шести существующим тестам проставлен явный `mode="received"`.** Они про усечение
  выдачи и битые суммы, а не про дефолт; их смысл сохранён, а не подогнан под зелёный.
- **Смена дефолта меняет ответ для действующих пользователей** и объявлена явно.
  Прежнее поведение доступно как `mode="received"`.

### Этап 287.4. Отказ по scope называет следующий шаг (03.09.2026)

- **Вчерашняя диагностика была неверной.** Известная проблема #38 записана как
  «правится на стороне Ветменеджера» — на деле это наш authorize-эндпоинт,
  сравнивающий запрошенный scope с регистрацией клиента.
- **Сделан только текст отказа.** Решение владельца — «без риска сломать OAuth
  сейчас», поэтому поток авторизации не изменён ни на шаг; существующие OAuth-тесты
  прошли без правок, что и есть доказательство неизменности.
- **Наблюдение, записанное для возврата к этапу:** запрошенный scope на итоговые
  права не влияет — грант собирается по пресету, выбранному владельцем на экране
  согласия. Проверка отклоняет то, что всё равно было бы заменено. Это аргумент за
  снятие проверки без расширения прав, но проверять отдельно.
- **Сторож проверяет правило, а не фразу:** назван следующий шаг, названы права,
  нет утверждения о выдаче. Переформулировка текста не ломает тест.

### Ревью диффа этапов 286 и 287.4 (Codex gpt-5.5, 03.09.2026)

Critical нет. Четыре находки, все приняты, отклонённых нет.

- **high — смена дефолта инструмента не дошла до промпта.** `daily_revenue()`
  говорил «Calculate the clinic revenue» и тут же требовал `mode="received"`.
  Самый прямой путь пользователя к выручке вёл ровно в то расхождение, ради
  которого делался этап. Промпт переведён на `invoiced`, определение названо
  внутри самого промпта, касса оставлена явным выбором с требованием сказать
  пользователю, что именно посчитано.
- **medium — живой артефакт помощника Report AI уводил в кассу.** Умолчание
  «Источник — оплаченные платежи (полученные деньги)» заменено на исполненные
  счета; уточняющий вопрос сохранён — спрашивать, когда есть у кого, правильно
  по-прежнему.
- **medium — существующий тест защищал устаревшее поведение.**
  `test_prompts_headers_only.py` требовал `mode="received"` в промпте про
  выручку: правильная правка уронила бы тесты, а противоречие оставалось
  зелёным. Тот же эффект был у `test_stage170_report_ai_tools.py`, пинившего
  старую строку артефакта. Оба обновлены с объяснением, почему прежнее
  утверждение было неверным.
- **medium — сторож 287.4 дёргал функцию рядом, а не достигаемый слой** (§4.0).
  Добавлен тест через настоящий `validate_oauth_authorize_request` с живым
  клиентом в базе. Сценарий ревьюера воспроизведён: старый литерал возвращён
  на место вызова при целом хелпере — три прежних теста остались зелёными,
  новый упал. Промежуточно он упал на дубликате ключа от прошлого прогона;
  красный по чужой причине ничего не доказывает, поэтому вставка клиента
  сделана идемпотентной и краснота перепроверена на утверждении.

- **Терпимость к пустому значению чуть не стала глухотой к нулю.** `env_int`
  по умолчанию отбрасывает значения `<= 0`; сырой `int()` их принимал.
  `DB_POOL_SIZE=0` у SQLAlchemy — «без ограничения», `DB_MAX_OVERFLOW=0` —
  «без запаса»: перевод на хелпер без `positive_only=False` молча подменил бы
  их дефолтами, то есть завёл бы ту же болезнь с другого конца. Найдено при
  перечитывании собственного диффа, не ревьюером.

- **Собственный сторож уронил двадцать пять чужих тестов.** Проверка
  `WEB_SESSION_MAX_AGE_SECONDS` делала `importlib.reload(web_auth)`:
  перезагруженный модуль отдаёт новые объекты, а `web_routes_auth` держит
  ссылки на старые — подпись сессии перестаёт сходиться. Полный прогон это
  поймал, точечный на двух файлах — нет. Переписано на отдельный процесс,
  который заодно воспроизводит настоящий сценарий дефекта: падение на импорте,
  то есть на старте сервиса. Красноту перепроверил возвратом сырого `int()` —
  процесс падает с тем самым `ValueError: invalid literal for int()`.

- **Сторож этапа 190 был зелёным при этом самом дефекте.**
  `test_release_is_a_build_arg_not_a_runtime_compose_override` проверял наличие
  строки в файле, но не различал блоки `build.args` и `environment` — имя,
  ошибочно вписанное в окружение, обе его проверки устраивало. Усилен: теперь
  проверяется отсутствие имени именно в блоке окружения; краснота показана
  возвратом строки. Название теста было верным, проверка — нет.

Использован 1 валидный прогон внешнего ревью из 2 на эту задачу.

### Этап 288. Правило про объём доходит до модели (03.09.2026)

- **Решение, записанное только в известную проблему, не действует.** Канал
  доставки — инжекция — не сработал ни разу за историю продукта (этап 283).
  Правило перенесено в описания инструментов, которые агент читает всегда.
- **Первая укладка правила была неверной:** оно попало в
  `PRIVACY_DESCRIPTION_SUFFIXES`. До модели доходило, но приватность и объём —
  разные правила, и в одном словаре они со временем теряются оба. Заведён
  `BULK_DESCRIPTION_SUFFIXES`.
- **Первая версия сторожа не увидела бы этой ошибки:** она читала только
  `SPECIAL_TOOL_DESCRIPTIONS`. Сборка вынесена в `compose_tool_description`,
  общую для продакшена и тестов, — проверка теперь физически не может
  разойтись с тем, что уезжает в `tools/list`.
- **Правило сформулировано с двух сторон** (запрет у зацикливаемого
  инструмента + подтверждение у Report AI): одной ссылки мало, без запрета
  цикл остаётся допустимым прочтением.
- **Внешнее ревью на этот дифф не запускалось:** правка описаний и разъезд
  словарей, поведение инструментов не менялось. Решение супервизора, бюджет
  ревью не расходован.

### Этап 285. Настройки доезжают до контейнера (03.09.2026)

- **Счёт дефекта был занижен вдвое, и занизил его собственный инструмент
  измерения.** Извлекатель знал одну форму чтения окружения, поэтому не видел
  пакет `auth/`, обёртки `env_int`/`env_float`, `_is_truthy_env` и имена,
  собираемые из префикса в `get_rate_limit_config`. Отсюда правило: у сторожа,
  который что-то извлекает, должен быть отдельный тест на полноту извлечения —
  иначе сузить извлекатель означает сделать сторож зелёным.
- **Первая версия исправления сломала бы прод.** `ERROR_TRACKING_RELEASE` была
  объявлена в `environment: ${GIT_SHA:-unknown}`; на сервере `GIT_SHA` не
  экспортирован, и подстановка затёрла бы настоящий SHA, вшитый в образ. Отсюда
  признание второго канала доставки — сборки образа — но с проверкой факта, а
  не на слово, и с тестом, удерживающим канал в одном имени.
- **Значения по умолчанию в `.env.example` намеренно не продублированы.**
  Скопированное число разойдётся с кодом и станет документацией, которая врёт;
  это ровно тот класс дефекта, который этап и закрывает.
- **HSTS включается с осознанным риском.** `max-age` год и `includeSubDomains`:
  протухший сертификат заблокирует вход без возможности продолжить. Certbot
  обновляет автоматически. Риск назван владельцу до включения; выбор
  делегирован ответом «решай и коммить».

- **Внешнее ревью нашло дефект тяжелее исходного, и он был мой.** Форма
  `NAME: ${NAME:-}` кладёт в контейнер пустую строку, а не «ничего»; читатели
  вида `int(os.environ.get("NAME", "10"))` падают на `int("")`. Первая версия
  исправления снесла бы прод при выкате — на импорте `web_auth`, на
  инициализации хранилища и на первом же входе. Отсюда два правила вместо
  одного: форма объявления не создаёт пустых строк (сквозная передача голым
  именем, проверено на живом compose), и читатель числовой настройки терпит
  пустое значение сам. Одной правильной формы записи в одном файле мало —
  это состояние файла, а не гарантия.
- **Извлекатель переписан с регулярок на разбор синтаксического дерева.**
  Ревью показало две формы, которых он не видел: `_env_flag("ИМЯ", ...)` и
  чтение по константе, где литерала в месте чтения нет вовсе. Перечислять
  хелперы поимённо — значит забыть следующий, поэтому распознаётся любой
  вызов, в имени которого есть `env`.
- **Разбор объявлений сведён в один экземпляр.** Сторожа этапов 284 и 285
  читали `docker-compose.yml` двумя разными регулярками; смена формы записи
  сломала вторую — ровно тот разъезд, который в этапе 288 уже чинили в
...
### Этап 289. Включение защиты, которая никогда не работала (03.09.2026)

- **Фикс сломал тех, кто держался на поломке.** Привязка токена к IP начала
  действовать — и тринадцать активных токенов, выданных с адресом шлюза
  докер-сети, перестали пускать кого бы то ни было. До этого они пускали всех.
  Проверить это тестом было нельзя: данных нет в репозитории, они только на
  бою.
- **Миграцию клиентских данных выполнил владелец.** Агенту запись в боевую
  базу заблокирована защитой среды; обходить не стал. Это верная граница:
  правка тринадцати учётных записей — не действие внутри рабочей задачи.
- **`172.18.0.1` → `*.*.*.*` — не ослабление.** Маска на адрес шлюза совпадала
  у любого клиента; wildcard записывает то же самое честно. Уровень защиты не
  изменился, изменилась достоверность записи.
- **Откат был вторым вариантом и хуже.** Он вернул бы вчерашнее: фиктивные
  адреса клиентов, неработающая привязка токенов, общий счётчик лимитов входа.

### Этап 289.3–289.4. Упавший смоук называет причину (03.09.2026)

- **Немота смоука была осторожностью, а не ленью.** Он намеренно не печатал ни
  тел ответов, ни заголовков, чтобы не унести токен или данные клиники в
  публичный журнал сборки. Между «сырое тело» и «имя класса исключения» есть
  четыре вещи, ни одна из которых не секрет: шаг, вид отказа, код статуса,
  идентификатор запроса. Осторожность сохранена — тесты проверяют, что токен
  не утекает ни на одном пути отказа.
- **Собственный тест был зелёным по совпадению.** Подставной сервер отдавал
  `x-request-id`, а прод на `/mcp` его не отдаёт — в бою в диагнозе всегда
  стоял бы прочерк. Поймано проверкой живого контура, а не тестом. Отсюда
  289.4 и отдельный тест на деградацию: диагноз обязан оставаться полезным
  без заголовка.
- **Middleware взят на чистом ASGI.** `BaseHTTPMiddleware` буферизует ответ и
  сломал бы потоковую отдачу streamable-http. В `_DrainingUvicornServer`
  по-прежнему передаётся необёрнутое приложение: слив сессий при остановке
  лезет во внутренности FastMCP и обёртку не переживёт.
- **Тест на установку middleware важнее теста на его работу.** Написанный, но
  не подключённый к приложению middleware — тот же дефект, ради которого
  затевался этап 285.
- **Внешнее ревью (1 прогон): одно замечание low, принято.** Путь разбора тела
  ответа бросал `ValueError` мимо `SmokeFailure` и печатал
  `step=response reason=unparsable status=- request_id=-` — то самое
  бесполезное сообщение, ради замены которого этап и делался. Сценарий
  реальный: промежуточный прокси отдаёт HTML с `content-type:
  application/json`. Исправлено, тест показан красным на этом сценарии.
  Ревьюер отдельно подтвердил, что необёрнутое приложение в
  `_DrainingUvicornServer` — верное решение: слив сессий смотрит на `.routes`,
  которых у обёртки нет.

### Этап 290. Кличка — не персональные данные (03.09.2026)

- **Решение владельца, а не техническое.** «Рекс» никого не опознаёт; правило
  стоило содержания жалоб и не давало защиты. Зафиксировано в роадмапе и PRD,
  чтобы через полгода не выглядело ослаблением по недосмотру.
- **Граница проведена по опознаваемости, а не по типу поля.** Фамилия
  опознаёт, личное имя — нет. Одно слово после метки клиента чистится только
  как фамилия, по списку суффиксов; два слова и инициалы — всегда.
- **Решение доведено до места, где действует.** Санитайзер — вторая половина;
  первая — три инструкции модели, которые велели прятать кличку до отправки.
  Изменить одну половину значило бы не изменить ничего. Отдельный тест держит
  все три файла.
- **Остаточный риск назван, а не спрятан:** одиночная нерусская фамилия
  (`owner: Кац`, `client: Smith`) останется. На латинице одно слово
  неотличимо от личного имени в принципе.
- **Старые отчёты не перечищаются** — чистка необратима. Версия санитайзера
  поднята до 3, по ней видно правило.
- **Эвристика «имя против фамилии» написана и отклонена.** Список русских
  суффиксов пропускал Толстой, Горький, Белая, Кац, Смит, Ivanov, Smith и ломал
  дефисные фамилии (`Петров-Водкин` → `-Водкин`). Найдено внешним ревью,
  подтверждено прямой проверкой. Значение после метки клиента чистится целиком;
  указание владельца выполняется тем, что имя в свободном тексте санитайзер не
  трогал никогда.
- **«Все места» оказались не всеми.** Сторож проверял три исполняемых файла и
  был зелен при живых обещаниях в `README.md` и требованиях в `artifacts/` —
  а именно по документации описания переписывают в следующий раз. Расширен.

Использован 1 валидный прогон внешнего ревью из 2 на эту задачу.

### Этап 291. Проба запирала домен собственным флагом (04.09.2026)

- **Первая гипотеза была неверной и стоила часа.** Причиной объявили отмену
  задачи; тест под неё оказался зелёным на неисправленном коде, потому что
  несостязательные asyncio-локи не уступают управление и `finally` успевает
  отработать. Правило подтвердилось на себе: тест, написанный под гипотезу,
  обязан быть показан красным до исправления — иначе он подтверждает не
  причину, а формулировку.
- **Чинится с двух сторон, и срок годности важнее точечной правки.** Держатель
  пробы больше не перепроверяет брейкер — это закрывает найденный путь.
  Брошенная проба протухает за минуту — это закрывает все остальные, включая
  ненайденные.
- **429 остаётся вне учёта отказов брейкера.** Ограничение темпа — не признак
  нездоровья апстрима; менять это значило бы открывать брейкер на здоровом
  сервере. Дефект был не в этом решении, а в том, что проба спотыкалась о
  собственный флаг.
- **Сторож этапа 285 поймал новую настройку в тот же день.** `BREAKER_PROBE_
  TIMEOUT_SECONDS` читалась рантаймом и не доезжала до контейнера; полный
  прогон это показал до выката.
- **Алерт встроен в существующий монитор, а не заведён вторым.** У
  `/opt/vps-monitor.sh` уже есть релей, глушение повторов и cron; отдельный
  скрипт означал бы вторую копию секрета релея и вторую схему дежурства.
- **Порог выбран по самому инциденту, а не на глаз.** В его окно
  `increase(circuit_half_open_busy[10m])` доходил до 415 при фоне ноль; порог
  20 ловит такое с запасом и не шумит.
- **Алерт проверен отправкой, а не рассуждением.** Прогон копии с заниженным
  порогом отправил боевой текст в канал. Непроверенный алерт — это тот же
  зелёный сторож, который ничего не сторожит.

### Этап 292. Клиника на собственном домене (04.09.2026)

- **Сообщение об ошибке называло не ту причину, и это стоило дороже самой
  поломки.** Клиника видела претензию к формату ввода, тогда как настоящая
  стена стояла на шаг дальше — в списке разрешённых зон. Обращение выглядело
  как вопрос про формат, хотя подключиться она не могла в принципе.
- **Чужие адреса живут в окружении, публичные зоны — в коде.** Решение
  владельца. Дело не в конфиденциальности домена (домен клиники не PII,
  решение 21.08.2026), а в том, что это чужие адреса и их состав меняется:
  открытому репозиторию они не принадлежат.
- **Проверка добавляется к зонам, а не заменяет их.** Заменяющая означала бы,
  что опечатка в `.env` отбирает доступ у работающей клиники. Добавляющая
  может только расширить — и при этом строже: адрес привязан к конкретному
  ключу аккаунта, тогда как зона разрешает любой хост внутри себя.
- **Первоначальный замысел был слабее и отвергнут.** Предлагалось выкинуть
  список зон и доверять ответу billing-api при общей проверке «публичный
  адрес». Это ослабление: авторитет ответа не отменяет того, что цель запроса
  задаёт посторонний.
- **Ключ аккаунта не выводится из адреса.** В разобранном обращении угадывание
  сработало бы — второй уровень домена совпал с ключом. Совпадение бренда с
  ключом не правило; на другой клинике такая угадка молча увела бы не туда.
- **Я сам занёс адрес клиники в репозиторий в этапе про то, как этого не
  делать.** Вычистил из формы и кода — и вписал в PRD, Roadmap и docstring.
  Поймал внешний ревьюер. Урок общий: запрет, сформулированный для кода, не
  распространяется на документы сам собой, и проверять надо там, где текст
  пишется свободно.
- **Сторож утечки сначала смотрел не туда.** Проверял кортеж
  `ALLOWED_HOST_SUFFIXES` и оставался зелёным, пока адрес лежал в трёх других
  файлах. Расширен на все файлы этапа; его границы (склейка строк, base64,
  остальной репозиторий) записаны в docstring, а не оставлены на догадку.
- **`tls_verification_failed` сужен до ошибки проверки сертификата.** Любая
  `ssl.SSLError` включала бы в этот класс несовпадение протокола — и человека
  отправляли бы чинить цепочку там, где дело в другом.
- **Значение живёт только на сервере, поэтому старт сообщает его размер.**
  Количество настроенных адресов в журнале — единственный способ отличить
  «карта пустая» от «карта не доехала после пересборки машины».

Использовано 2 валидных прогона внешнего ревью из 2 на эту задачу.
...
- **Неверная привязка хуже отсутствия привязки.** Отчёт помечается разобранным,
  в метрике появляется совпадение, а агенту выдаётся чужой playbook. Поэтому
  решение сформулировано как «не привязывать», а не «привязывать точнее».
- **Формула отпечатка не менялась.** В роадмапе 295.2 сначала стоял «отпечаток,
  который различает такие отчёты между собой». Различить их можно только
  подмешав свободный текст отчёта — это выдуманная точность: два агента опишут
  одну поломку разными словами и всё равно не совпадут, зато сломается склейка
  настоящих повторов. Формулировка изменена при написании PRD, причина записана
  в самом пункте.
- **Отпечаток остаётся в отчёте.** Он нужен для склейки повторов и для ручной
  привязки; меняется только его право быть основанием для автоматического матча.
- **Починка playbook #11 (этап 294.1) сама по себе отчёт #62 не закрыла.** После
  неё повтор поиска по инциденту той же формы всё равно возвращал #42: матч по
  отпечатку идёт раньше правил. Это и переставило 295 из «хорошо бы» в
  блокирующий — проверено вызовом, а не рассуждением.
- **Оба поля границы заполняет агент, а не рантайм.** Внешнее ревью нашло, что
  `http_status=200` и заглушка `-` в `error_code` проходили как признак ошибки,
  то есть запрет обходился штатным путём: описание `report_problem` прямо зовёт
  сообщать про успешные вызовы. Граница проверяет смысл значения, а не непустоту:
  статус — только `0` (обрыв связи) и `>= 400`, код — не из списка заглушек.
- **`http_status=0` намеренно остался признаком.** Этап 153 (F14) сохранил
  нулевой статус как обрыв соединения; проверка на истинность вместо явного
  сравнения тихо выключила бы подсказки для таких инцидентов.
- **Счётчик подавленных кандидатов — нижняя оценка.** Считаются только те, что и
  так выбраны по инструменту; отдельного запроса ради диагностики не делается.
  Записано в комментарии, чтобы число не приняли за точное.

Использован 1 валидный прогон внешнего ревью из 2 на эту задачу.

## Этап 293. Границы суток в фильтрах дат

- **Дефект уже чинили — на соседнем инструменте.** Известная проблема #21
  «get_payments date filters now use whole-day timestamp range» закрыта как
  `fixed`; `get_invoices` остался с голой датой. Починка одного инструмента из
  двух выглядела завершённой работой.
- **Три зелёных теста прибивали неверное поведение.** В `test_ergonomic_filters`
  тест на `get_invoices` стоял непосредственно над тестом на `get_payments`,
  требуя от первого полночь, а от второго — границу суток. Плюс
  `test_stage172_debtors_filters` и `test_inactive_clients`. Все три переписаны
  и показаны красными до правки кода: тест, прибивающий дефект, — часть дефекта.
- **Однодневный запрос — не главное следствие.** Пусто на один день видно сразу;
  потеря последнего дня у любого диапазона не видна ничем, кроме сверки с
  другим источником. Второе и было настоящей ценой.
- **Помощники вынесены в `vm_datetime`, а не скопированы в четвёртый раз.**
  В репозитории уже три локальные пары `_day_start`/`_next_day_start`.
  Существующие не трогал: они верны и покрыты тестами, а переписывание рабочего
  кода внутри починки живого дефекта смешивает два риска.
- **Окно неактивных клиентов стало правильнее, а не шире.** `cutoff_newest` по
  докстрингу был включающей границей дня; для timestamp это и выражается как
  `< начало следующих суток`. Докстринги приведены в соответствие — на них
  указало внешнее ревью, и это тот же класс, что весь этап: документ говорил
  одно, код делал другое.
- **Реестр timestamp-полей (293.3) сознательно оставлен отдельной работой.**
  Его цена выше, чем у самой починки; делать его внутри исправления живого
  дефекта — значит отложить исправление.

Использован 1 валидный прогон внешнего ревью из 2 на эту задачу.

## Этап 296. `limited` у данных ИИ-отчёта не может стать `true`

- **Разбирался по исходникам апстрима, а не по наблюдению.** Два предела в одной
  подсистеме: `AiReportRenderer::VIEW_ROW_LIMIT = 1000` режет SQL и возвращает
  `total` от уже обрезанного набора, `JobService::DATA_ROW_LIMIT = 10000` —
  предел выдачи, по которому считается `limited`. Отсюда `limited` структурно
  всегда `false`, а не «иногда врёт».
- **Это тот случай, где описание действительно виновато.** По отчётам #61 и #62
  гипотеза «агентам не хватило описаний» не подтвердилась — оба были про дефекты.
  Здесь подтверждается, но не как нехватка, а как активная дезинформация: мы
  называли признаком полноты поле, которое признаком быть не может.
- **Вердикт делается по числу строк, а не по `total`.** `total` приходит из того
  же обрезанного набора и потому менее прямой сигнал; `len(rows)` устойчивее к
  расхождению между ними.
- **Равенство строгое.** 999 строк — отчёт кончился сам; 1001 для ИИ-отчёта
  недостижима, и если такое придёт, значит наше понимание апстрима устарело —
  тогда вердикт «обрезано» был бы выдумкой.
- **Ложноположительный случай принят сознательно.** Отчёт ровно на 1000 строк
  получит предупреждение. Это дешевле обратной ошибки: агент не выдаст неполный
  аудит за полный.
- **Утверждение жило в четырёх местах.** Докстринг, `SPECIAL_TOOL_DESCRIPTIONS`
  (именно её видит модель), README и описания двух экспортных инструментов.
  Первую лишнюю копию поймал тест, две последние — внешнее ревью. Плюс два теста
  этапа 170 прибивали старый контракт. Правка одной копии оставила бы остальные
  учить неверному — поэтому тест теперь держит все.
- **Экспорт остаётся верным ответом.** Внешнее ревью подтвердило по исходникам,
  что `AiReportRenderer::getStatement()` предела рендера не применяет, то есть
  полные данные действительно доступны только через CSV/XLSX.

Использован 1 валидный прогон внешнего ревью из 2 на эту задачу.

## Этап 294. Playbook не должен молча выпадать из выдачи

- **Отказ валидации был неотличим от отсутствия playbook.** В `unreachable-issues`
  проблема #11 стояла в одном ряду с теми, для которых ответ не сочинили, — хотя
  её ответ был написан в июне и погас 01.09 от переименования инструмента.
  «Написать» и «починить одно имя» — разная работа, и теперь это видно.
- **Отвергнутые идут первыми.** Там ответ уже есть; это самая дешёвая работа с
  самым быстрым эффектом.
- **Отсутствие playbook отказом не считается.** У 21 из 42 проблем его просто нет;
  писать про это в лог значило бы утопить настоящие поломки в шуме.
- **Сторож над сидом, а не над боевой БД.** Проблемы, заведённые руками, живут
  только в БД, и тест до них не дотянется. Для них сверку даёт связка
  `playbook_state=rejected` плюс причина в логе — это операционный путь, а не
  тестовый, и так честнее, чем делать вид, что тест закрывает оба случая.
- **`set-playbook` появился из собственной боли.** 04.09.2026 замена одного
  устаревшего имени потребовала разового скрипта в боевом контейнере: `promote`
  заводит новую проблему, `mark` меняет статус, правки playbook не было вовсе.
  Валидация на входе та же, что на записи: невалидный playbook не заменит рабочий.

Внешнее ревью на эту задачу не запускалось: изменение диагностическое, боевой
путь выдачи подсказок им не затрагивается.

## Этап 293.3 и 297.2. Сторожа, которые говорят о себе

- **Первая версия сторожа не ловила дефект, ради которого написана.** Он знал
  только прямой `parse_date_param`, а `get_invoices` берёт даты через локальную
  обёртку `_parse_date_range` — на дореформенном коде сторож был зелёным.
  Проверено прогоном по `tools/invoice.py` из коммита `dbe3764`; после правки
  ловит оба настоящих случая. Второй раз за неделю сторож смотрел не туда
  (первый — утечка адреса в этапе 292): зелёный сторож надо проверять на
  настоящем сломанном коде, а не только на синтетике.
- **В реестр едва не попало выдуманное поле.** `payment_date` вписано по
  догадке и удалено после сверки со схемой: такого столбца нет. Реестр,
  собранный из памяти, хуже отсутствия реестра — он выглядит проверенным.
- **Разбор AST, а не grep.** Grep не отличит вызов от строки в докстринге и не
  увидит, откуда пришло значение; здесь важно именно происхождение.
- **297.2: проверка обязана называть свою поломку своей.** «MCP container did
  not receive the pepper» при живом контейнере — то же, что монитор git,
  выдававший свою поломку за поломку git (`e01c1156`). Теперь исчезнувший файл
  сообщается как проблема деплоя, с вероятной причиной и указанием не трогать
  приложение.
