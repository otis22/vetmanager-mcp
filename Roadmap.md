...
- 265.6 Типизировать валидацию в самих инструментах. — `done` (27.08.2026)
  ...
  Ошибка ввода перестала заводить issue в Sentry.
...
Источник: этап 265.6, 27.08.2026. В `tools/` 59 проверок поднимают голый
`ValueError`. До приглашения сообщить о проблеме они не доходят — наша обёртка
...
- 266.2 Закрыть путь `logger.exception` — фильтровать в `before_send`, а не
  только в middleware, иначе любой обход остаётся незамеченным. — `todo`
