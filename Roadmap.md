...
## Этап 255. Тестовые стенды перепутаны — `in_progress`
...
- 255.4 Перевести bearer-токен тестового MCP-клиента на `devtr6`. Требует
  действия на стороне Ветменеджера — контур за токеном выдаём не мы. — `todo`
...
- 298.6 Живая проверка на `devtr6`: цена изменена и возвращена обратно.
  ...
  это выдача доступа, решение владельца. Тот же класс блокировки, что
  255.4. — `supervisor_pending`
