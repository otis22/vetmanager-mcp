...
## Этап 285. Половина настроек рантайма не объявлена в контейнере — `todo`

Источник: разбор дефекта 284.7 (03.09.2026). Рантайм читает **33** переменные
окружения, в сервисе `mcp` объявлена **21**. Разница — двенадцать настроек,
которые внутри контейнера не существуют независимо от того, что записано в
`.env` на сервере. Отказ тихий: код видит пустую строку и работает по умолчанию.

Не объявлены: `DB_POOL_SIZE`, `DB_MAX_OVERFLOW`, `ERROR_TRACKING_BACKEND`,
`ERROR_TRACKING_RELEASE`, `FASTMCP_LOG_LEVEL`, `FILTER_CONTRACT_VALIDATION_ENABLED`,
`SORT_CONTRACT_VALIDATION_ENABLED`, `FORWARDED_ALLOW_IPS`, `LOG_FORMAT`,
`REPORT_EXPORT_DIR`, `SENTRY_DSN`, `VM_HTTP_CLIENT_CLOSE_GRACE_SECONDS`,
`WEB_ENABLE_HSTS`, `WEB_SESSION_MAX_AGE_SECONDS`, `WEB_SESSION_SAMESITE`,
`WEB_TRUSTED_PROXY_IPS`.

Часть из них безобидна — значение по умолчанию и есть рабочее. Часть тревожна:
`WEB_ENABLE_HSTS`, `WEB_SESSION_SAMESITE`, `WEB_TRUSTED_PROXY_IPS` и
`FORWARDED_ALLOW_IPS` — настройки безопасности, которые оператор считает
настраиваемыми, а изменить их на боевом контуре сегодня нельзя.

Сторож этапа 284 закрывает только пересечение с `.env.example`, то есть то,
что явно обещано оператору. Остальное он не видит.

- 285.1 Пройти по каждой из двенадцати: объявить в сервисе, либо убрать чтение
  из окружения и оставить константу. Третьего не давать — «читается, но задать
  нельзя» и есть источник дефекта. — `todo`
- 285.2 Настройки безопасности разобрать первыми и отдельно: если HSTS,
  `SameSite` и доверенные прокси на бою не управляются, надо знать, какое
  значение там сейчас действует. — `todo`
- 285.3 Расширить сторож с `.env.example` на весь рантайм после того, как
  расхождения разобраны: правило имеет смысл, только когда множество пустое. — `todo`
- 285.4 `.env.example` привести в соответствие: настройка, которую оператору
  предлагают задать, должна доезжать — сейчас это верно только по совпадению. — `todo`
