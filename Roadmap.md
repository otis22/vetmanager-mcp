...
## Этап 232. Ревью на соответствие цели open source — `todo`
...
**Диагноз.** `MedicalCardsController::doRestUpdate` начинается с проверок,
которые смотрят только на присланное тело:

```php
$pet = Pet::model()->findByPk($data['patient_id']);
if (empty($pet)) { throw new CHttpException(400, "Patient does not exist"); }
if (!isset($data['patient_id']) || empty($data['patient_id'])) {
    throw new CHttpException(400, "No Pet selected");
}
```

Порядок перевёрнут: `findByPk(null)` вызывается раньше проверки на пустоту,
поэтому отсутствующий `patient_id` даёт «Patient does not exist» вместо «No Pet
selected». Вторая проверка для этого случая — мёртвый код. Того же требуют
`doctor_id` и `clinic_id`.
...
**Почему лечится у нас.** Ниже по коду тот же метод делает
`$params = array_merge($medcards->get($id), $data)` — сервер сам дополняет
недостающие поля текущими значениями. Частичное обновление на уровне записи
работает; ломают его только предполётные проверки. Достаточно дослать три поля
их же собственными значениями.
...
Проверено в исходниках 23.08.2026: `MessagesController` содержит ровно четыре
действия — `doCustomRestPostUsers`, `doCustomRestPostAll`,
`doCustomRestPostRoles`, `doCustomRestGetReports`. Отмены, удаления и отметки
о прочтении в API нет вообще. Добавить инструмент не из чего.
...
## Этап 250. По счёту нельзя найти его закрытия — `todo`
...
**Первая версия ответа была неверной.** В `ClosingOfInvoices.php` связь
объявлена как `minus_document_id = invoice.id and minus_type_document =
'invoice'`, и по ней казалось, что счёт всегда на минусовой стороне. Проверка
на devtr6 по 100 записям это опровергла:
