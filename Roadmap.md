...
## Этап 308. Маска скрывает не только данные, но и то, о ком речь — `in_progress`
...
- 308.0 Разобраться, какой `name` означает человека. Найдено при разборе
  ...
  врача, а контакты филиала клиники, наоборот, прячутся зря. — `todo`
- 308.1 Адресная маска в санитайзере: таблица «ключ ответа → сущность и
  ...
  идентификатора рядом нет — остаётся необратимая маска. — `todo`
...
- 308.3 Согласовать обещание в интерфейсе: `REPORT_PRIVACY_NOTE` и экран
  ...
  модели, но возвращается человеку». — `todo`
