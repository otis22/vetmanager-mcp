...
## Этап 233. Точная причина отказа подключения интеграции — `in_progress`
...
- 233.1 Разбить `vetmanager_error` на `upstream_4xx`, `upstream_5xx`, `network`
  и `internal` в `ACTIVATION_REASON_CLASSES` и в классификаторе. — `in_progress`
- 233.2 Инструментировать пути, которые сейчас молчат: каждая неуспешная
  попытка подключения обязана оставлять запись upstream-метрики. — `todo`
- 233.3 Писать `status_code` и `error_class` в структурный лог события отказа,
  не раскрывая ключей и паролей. — `todo`
- 233.5 Домен клиники оставить доступным для incident triage; IP-адреса
  санитайзить в tracking и маскировать до сети в structured logs. — `in_progress`
