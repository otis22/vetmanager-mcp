...
## Этап 305. Правила инжекции написаны не на те инструменты — `todo`
...
- 305.2 Правила под настоящие падения — четыре класса из 28 событий: контракт
  фильтра и сортировки, клиника не выбрана, переход статуса Report AI, экспорт
  отчёта. Каждое с playbook, потому что без валидного playbook инжекция
...
- 305.4 Правило, мёртвое по статусу, видно разбору. Сейчас `unreachable-issues`
  показывает `injection=yes` у проблемы #34 в `wontfix`, хотя статус исключает
  её из выборки кандидатов. Колонка должна учитывать статус, иначе она обещает
  доставку, которой не будет. — `todo`
