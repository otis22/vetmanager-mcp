...
## Этап 283. Инжекция известных проблем не сработала ни разу — `todo`
...
- 283.5 Эффективность инжекции попадает в продуктовые метрики: доставки против
  отказов, а не одно голое число событий. Знаменатель теперь есть (283.1), но
  он живёт в Prometheus и обнуляется при рестарте, а продуктовый отчёт читает
  БД. Сводить их — отдельная работа: либо счётчик доезжает до БД, либо отчёт
  ходит в Prometheus. Делать это до того, как накопится хоть одно совпадение,
  незачем — считать отношение к нулю нечего. — `todo`
