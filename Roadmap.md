...
## Этап 233. Точная причина отказа подключения интеграции — `todo`
...
- 233.1 Разбить `vetmanager_error` на `upstream_4xx`, `upstream_5xx`, `network`
  и `internal` в `ACTIVATION_REASON_CLASSES` и в классификаторе. — `todo`
...
## Этап 234. Ошибки и логи переживают рестарт — `todo`
...
- 234.1 Пробросить `ERROR_TRACKING_DSN`, `ERROR_TRACKING_ENVIRONMENT` и
  ...
  задокументировать в README. — `todo`
