...
## Этап 265. Защитная сетка не заметила недостижимое сообщение — `todo`
...
- 265.1 Проверять пользовательские тексты через ту же границу, в которую
  входит агент: один генеративный тест по реестру доступов на все
  инструменты сразу. — `todo`
- 265.2 Запретить распознавание ошибок по человеческому тексту: причина
  отказа — код или тип, а не фрагмент предложения. — `todo`
- 265.3 Линтер мёртвых импортов и переменных: осиротевшая константа прожила
  бы долго. — `todo`
- 265.4 Правило для тестов-сторожей: сторож принимается, только если показан
  красным на намеренно испорченном входе. — `todo`
- 265.5 Признак «ошибка ввода» сделать типом, а не префиксом сообщения. — `todo`
- 265.6 Типизировать валидацию в самих инструментах. — `todo`
  Нашлось при 265.5: девять проверок вида «clinic_id must be a positive
  integer» не начинались с «invalid »/«missing », поэтому старое правило их
  никогда не ловило — пользователю до сих пор предлагают сообщить о дефекте
  в ответ на его же опечатку. Теперь, когда признак структурный, это чинится
  расстановкой типа; сделано будет отдельно, чтобы не смешивать с защитной
  сеткой.
