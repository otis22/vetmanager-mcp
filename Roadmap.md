...
## Этап 294. Ответ агенту был написан и замолчал из-за переименования инструмента — `todo`
...
- 294.2 Отказ валидации перестаёт быть немым: событие в лог с причиной и
  счётчик. `unreachable-issues` различает «playbook не написан» и «playbook
  отвергнут», потому что это разные поломки. — `todo`
- 294.3 Переименование инструмента ломает проверку, а не базу знаний: сверка,
  что каждое имя в `recommended_tool_sequence` и в `related_tool` существует
  в реестре инструментов. Тест — над сидом, операционная команда — над БД. — `todo`
- 294.4 Команда правки playbook у существующей проблемы в
  `triage_agent_feedback.py`, с той же валидацией на входе. — `todo`
