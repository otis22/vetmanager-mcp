...
## Этап 270. Удаление вынесено из общих прав на запись — `in_progress`
...
- 270.1 Ввести отдельное право на удаление и привязать к нему `delete_client`
  и `delete_pet`. — `todo`
- 270.2 Оставить это право только у полного доступа. — `todo`
- 270.3 Тесты: «Регистратура» создаёт и меняет, но не удаляет. — `todo`
