...
## Этап 299. Признак `possible_pii` срабатывает на правильном поведении — `todo`
...
- 299.1 `placeholder_seen` выводится из `PRIVACY_REDACTIONS`: увиденный
  плейсхолдер — свидетельство соблюдения контракта, а не риска. — `todo`
- 299.2 Отдельный признак «агент пользовался плейсхолдерами» сохранить, если он
  кому-то нужен, но не под именем и не в смысле `possible_pii`. — `todo`
- 299.3 Пересчитать флаг у существующих отчётов и проверить, сколько из 12
  останутся помеченными. Ожидание — ноль; если не ноль, значит есть настоящие
  случаи, которых мы до сих пор не видели за шумом. — `todo`
- 299.4 Показатель на дашборде метрик перечитать после пересчёта: цифра
  «возможные персональные данные» сейчас неверна. — `todo`
