...
## Этап 291. Проба запирала домен собственным флагом — `in_progress`
...
## Этап 292. Клиника на собственном домене не может подключиться — `in_progress`
...
- 292.7 Ответить клинике, когда подключение проверено. — `todo`

## Этап 293. Однодневный диапазон счетов возвращает пусто, а любой — теряет последний день — `in_progress`
...
## Этап 294. Ответ агенту был написан и замолчал из-за переименования инструмента — `in_progress`
...
## Этап 296. `limited` у данных ИИ-отчёта не может стать `true` — `in_progress`
...
- 296.4 Проверить на живом стенде, что отчёт с заведомо большим числом строк
  отдаёт ровно 1000 и `limited=false`, и что подсказка теперь появляется. — `todo`
