...
- 287.1 Проверка запрошенного scope против регистрации клиента разбирается по
  существу: она защищает от чужого клиента или мешает своему? Решение
  записывается, а не выводится из кода. — `todo`
- 287.2 Явно запрошенный scope, который владелец **вправе** выдать, доводить до
  экрана согласия, а не отклонять заранее. **Решение владельца 06.09.2026:
  оставить отказ заранее.** Поток авторизации не меняем: экран согласия — место,
  где клиент может выпросить больше, чем нужно для работы, и цена такой ошибки
  выше, чем неудобство от раннего отказа. Обратная сторона принята осознанно:
  человек не видит, почему подключение не даёт нужного инструмента, и приходит
  с вопросом к нам. Смягчает это 287.4 — отказ уже называет причину и
  следующий шаг. — `stop`
...
## Этап 298. Цена продажи товара не записывается ни одним инструментом — `in_progress`
