...
Источник: сверка кода 23.08.2026. `tools/user.py` в списке «Allowed properties»
называет `calc_percents`, а реестр `FILTER_FIELDS_BY_ENTITY` в `filters.py` это
поле не содержит — его убрали в этапе 235.5 как зарплатное. Докстринг остался
прежним.

Модель читает докстринг, строит фильтр по `calc_percents` и получает жёсткий
отказ «Unknown filter property» от нашей же валидации. Мы сами предложили поле
и сами его запретили.

Проверка остальных list-инструментов расхождений не нашла — случай единичный,
но он показывает, что связи между реестром и докстрингом нет никакой: они
совпадают, пока кто-то помнит поправить оба места.
...
- 244.3 Проверить, не обещают ли докстринги вырезанные поля где-то ещё —
...
Источник: обратная связь с прода, отчёты #41, #42, #43, #45 — четыре из семи
открытых, два с severity `high`, все по одному инструменту. Формулировки
сходятся: карта читается, пациент существует, запись возвращает «Patient does
not exist». Отчёт #41 уточняет, что вызов доходит до пути записи и падает
именно на проверке существования пациента.

Это самый громкий сигнал из всей обратной связи за месяц. Инструмент записи,
который отвергает валидные данные, для клиники хуже отсутствующего: модель
уверенно сообщает, что запись сделана, либо застревает в попытках.

Тела отчётов пока не прочитаны — в CLI разбора нет чтения полей `details` и
`reproduce` (этап 246). Начинать нужно с них: четыре независимых отпечатка
означают четыре разных набора параметров, и разница между ними и есть подсказка.

- 245.1 Прочитать тела #41, #42, #43, #45 и выписать, чем отличаются параметры
  вызовов при одинаковом симптоме. — `todo`
- 245.2 Воспроизвести на devtr6 на заведомо существующей карте. — `todo`
- 245.3 Сверить проверку существования пациента с реализацией в
  vetmanager-extjs — наша это проверка или upstream. — `todo`
- 245.4 Исправить и закрыть тест, который ловит именно этот отказ. — `todo`
...
Источник: отчёт #46, `get_medical_card_by_id returned stale fields after REST
update`. Модель обновила карту и тут же прочитала её — увидела прежние
значения.

Для сценария «внеси и проверь», а это основной способ, которым модель себя
контролирует, это разрушительно: она либо повторяет запись, либо сообщает
владельцу, что изменение не прошло.

- 247.1 Прочитать тело #46 и понять, какие именно поля отстали. — `todo`
- 247.2 Определить источник: наш кеш, кеш upstream или задержка репликации
  на стороне Vetmanager. — `todo`
- 247.3 Решить и описать контракт: либо чтение после записи консистентно,
  либо это явно сказано в докстринге инструмента записи. — `todo`
...
прочитанной. Инструменты отправки есть, обратного хода нет.

Асимметрия неприятная: необратимое действие доступно модели, а компенсирующее —
нет. Отдельный вопрос — существует ли отмена в самом Vetmanager; если нет,
честнее сказать об этом в докстринге отправки, чем молчать.

- 248.1 Проверить в vetmanager-extjs, поддерживает ли API отмену или отметку
  о прочтении уже отправленной рассылки. — `todo`
- 248.2 Если поддерживает — добавить инструмент; если нет — предупредить о
  необратимости в докстрингах `send_message_to_*`. — `todo`

## Этап 249. Подтвердить, что учётные поля не уезжают в ответе — `todo`

Источник: отчёты #39 и #40, `get_users returns credential-like fields`. #39
связан с known issue #30 в статусе `acknowledged`, #40 с тем же отпечатком
пришёл позже и лежит открытым.

В коде защита есть: `redact_sensitive_output_fields` в `privacy_utils.py`
вырезает `passwd`, `passport_series`, `last_change_pwd_date` глобально и
`login`, `user_inn`, `calc_percents` внутри подтверждённых контейнеров
сотрудников. Неясно одно — пришёл #40 до этой правки или после. Если после,
защита где-то не срабатывает.

- 249.1 Сверить дату #40 с датой коммита «Redact nested staff credential
  fields». — `todo`
- 249.2 Живой вызов `get_users` на devtr6: убедиться, что учётных полей в
  ответе нет. — `todo`
- 249.3 Закрыть known issue #30 либо переоткрыть с конкретикой. — `todo`

## Этап 250. Непонятно, какой стороной в closingOfInvoices лежит счёт — `todo`

Источник: отчёт #36, known issue #27 в статусе `acknowledged`, категория
`bad_description`. Докстринг `get_closing_of_invoices` перечисляет
`plus_document_id`, `minus_document_id`, `plus_type_document`,
`minus_type_document` и не говорит, какая сторона держит счёт, а какая платёж.
Модель не может составить фильтр «покажи закрытия по счёту X».

Ответ есть в `ClosingOfInvoices.php`, в объявлении связей: счёт всегда на
минусовой стороне — `minus_document_id = invoice.id and minus_type_document =
'invoice'`, платёж бывает с обеих (`plus_type_document = 'payment'` и
`minus_type_document = 'payment'`). Это публичная семантика контракта, её
можно перенести в докстринг.

- 250.1 Описать в докстринге, что счёт ищется по `minus_document_id` вместе
  с `minus_type_document = 'invoice'`, и привести пример фильтра. — `todo`
- 250.2 Проверить на devtr6, что фильтр из примера действительно возвращает
  закрытия по счёту. — `todo`
- 250.3 Закрыть known issue #27. — `todo`
