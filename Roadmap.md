...
## Этап 243. В событиях трекинга нет пользователя — `todo`
...
- 243.1 Заполнять `user.id` идентификатором аккаунта на всех путях захвата —
  и `capture_tool_failure`, и необработанные исключения. — `todo`
- 243.2 Никаких email, доменов клиники и IP в user-контексте: только
  `account_id`. — `todo`
- 243.3 Тест: событие отказа инструмента несёт непустой user-контекст. — `todo`
...
## Этап 266. Проверка ввода заводит issue в Sentry мимо наших фильтров — `todo`
...
- 266.1 Решить, что делать с 59 `ValueError`: типизировать как ошибку ввода
  или оставить, но перестать считать отказом сервиса. — `todo`
- 266.2 Закрыть путь логгера — фильтровать в `before_send`, а не только в
  middleware, иначе любой обход остаётся незамеченным. Отличить ошибку ввода
  от дефекта в этом событии нечем: текст один и тот же, `Error calling tool
  'X'`, исключения нет. Значит либо метить событие на нашей стороне до того,
  как FastMCP его залогирует, либо не пускать этот логгер в Sentry вовсе. — `todo`
- 266.3 Подтвердить на живой выкатке: опечатка в аргументе issue не создаёт. — `todo`
