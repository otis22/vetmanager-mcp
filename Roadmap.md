...
## Этап 287. Явно запрошенный scope отклоняется до согласия владельца — `in_progress`
...
- 287.3 Инструменты, которые заведомо не сработают при текущей регистрации,
  ...
  Основание — тот же приём уже работает для прав токена (этап 271) и себя
  оправдал: 06.09.2026 инструмент цены не показывался тестовому клиенту, пока у
  токена не было права записи, и это было верным поведением, а не помехой.
  Меняет видимый набор инструментов у живых подключений — значит выкат несёт
  миграцию выданных данных по правилу `CLAUDE.md` § 6.0: до выката посчитать по
  журналу, у скольких подключений и какие именно инструменты исчезнут, и по
  какой причине. — `todo`
