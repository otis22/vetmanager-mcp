...
## Этап 128. Final release gate + production deploy — `stop`
...
- 128.4 Выполнить pre-deploy backup production PostgreSQL и зафиксировать rollback point по `artifacts/operations-readiness-vetmanager-mcp-ru.md`. — `stop`
- 128.5 Выполнить production deploy через canonical script (`scripts/deploy_server.sh` или `scripts/sync_and_deploy_server.sh`) с production profile. — `stop`
- 128.6 Прогнать post-deploy smoke: `scripts/post_deploy_smoke_checks.sh`, `/healthz`, `/readyz`, `/metrics`, `/mcp`, web `landing -> login -> account`, MCP call существующим bearer token. — `stop`
- 128.7 Выполнить post-release наблюдение: проверить `runtime/security/audit` logs, отсутствие всплеска `vetmanager_upstream_failures_total` и auth failures, затем зафиксировать release outcome и commit SHA в `AssumptionLog.md`. — `stop`

Примечание: локально закрыта вся недеплойная часть release gate. Production rollout остановлен из-за недоступного server target; возвращаться к `128.4-128.7` имеет смысл только после восстановления сервера.
