...
## Этап 305. Правила инжекции написаны не на те инструменты — `in_progress`
...
- 305.2 Правила под настоящие падения — четыре класса, 41 событие за 90 дней:
  ...
  увидит — создаст вторую на тот же класс. — `todo`
- 305.3 Правила существующей проблемы становятся правимыми:
  ...
  правки без этого запрета делает дефект достижимее. — `todo`
...
- 305.5 Выкат на прод и живая проверка: первое совпадение правила с настоящим
  отказом. До него 283.5 остаётся без смысла — считать отношение к нулю
  нечего. — `todo`
- 305.6 Стоимость совпадения измеряется на живых цифрах: сколько запросов к БД
  добавляет матч и сколько строк появляется в `known_issue_match_events` на
  один отказ. Правило на предохранитель (5 994 события, 99.4% всех отказов) в
  ...
  скоупе не наступает, нельзя. Замер — вход в отдельный этап про
  предохранитель. — `todo`
