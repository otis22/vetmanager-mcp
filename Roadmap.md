...
## Этап 239. Контракт сортировки у list-инструментов — `todo`
...
- 239.2 Валидация `sort` по тому же принципу: неизвестное имя отклоняется
  локально с перечнем допустимых. — `todo`
- 239.3 Допустимые поля сортировки в докстрингах. — `todo`
- 239.4 Регрессия на исходный кейс: `sort` по `date_admission` у
  `get_admissions` отвечает подсказкой с именем `admission_date`. — `todo`
