...
## Этап 251. Экспорт большого отчёта не завершается — `in_progress`
...
- 251.3 Внятный отказ вместо бесконечного «not ready» — с сроком, после
  которого ждать не нужно, и подсказкой сузить выборку. — `todo`

## Этап 252. Задания Report AI зависают в очереди — `in_progress`
...
- 252.3 Ограничить ожидание и объяснять модели, что делать дальше. — `todo`
