...
- 225.4 Восстановить диагноз `126` у карты 6 на `devslon67` — стёрт при
  проверке 23.08.2026, через API вернуть нельзя из-за этой же ошибки. — `todo`
...
## Этап 244. Докстринг обещает поле, которое реестр блокирует — `todo`
...
- 245.5 Завести known issue и связать с ним все четыре отчёта. — `todo`
...
- 247.3 Решить, нужен ли сброс кеша сущности после записи через наши же
  инструменты записи. — `todo`
...
- 250.4 Закрыть known issue #27. — `todo`
...
## Этап 253. Список медкарт приезжает дважды — `todo`
...
- 253.1 Выяснить, откуда взялись оба ключа и кто на какой опирается. — `todo`
- 253.2 Оставить один ключ, сохранив совместимость. — `todo`
- 253.3 Проверить остальные list-инструменты на такой же дубль. — `todo`
