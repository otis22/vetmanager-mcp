...
## Этап 260. Вызовы по OAuth не попадают в метрики — `todo`
...
- 260.1 Зафиксировать, какие события пишет каждый канал сейчас, и чего не
  хватает, чтобы считать вызовы одинаково. — `todo`
- 260.2 Писать событие успешной аутентификации на OAuth-пути тем же
  контрактом, что и bearer, не ломая существующие агрегаты. — `todo`
- 260.3 Проверить, что санитайзер и правила приватности применяются к новому
  событию так же (IP, домен клиники). — `todo`
- 260.4 Перевести недельную выгрузку и разбор застрявших на общий счётчик,
  убрав нынешнюю склейку по `last_used_at`. — `todo`
- 260.5 Убедиться, что архиватор зомби-аккаунтов не считает OAuth-аккаунт
  неактивным (сейчас его спасает только проверка активного подключения). — `todo`
