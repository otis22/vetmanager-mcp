...
## Этап 295. У успешного ответа с неверными данными нет отпечатка — `todo`
...
- 295.1 Отпечаток без единого признака ошибки не даёт основания для
  ...
  привязывается к первой проблеме с тем же инструментом. — `todo`
- 295.2 Отчёт про «ответ пришёл, данные неверные» получает отпечаток, который
  различает такие отчёты между собой. — `todo`
