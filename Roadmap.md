...
## Этап 312. Запись о проблеме перестаёт врать — `in_progress`
...
- 312.1 Авто-репорт, заведённый с привязкой к известной проблеме, получает
  статус `linked`: разбор уже случился, человеку там делать нечего. — `todo`
- 312.2 `related_tool` известной проблемы обязан быть зарегистрированным
  ...
  правки поля — чинить такие записи руками в SQL нельзя. — `todo`
- 312.3 Гейт репозитория проверяет то же самое для seed-записей: имя
  инструмента в `seed_known_issues.py` обязано быть в реестре. — `todo`
- 312.4 Три активные боевые записи с именем вне реестра (#10, #19, #29)
  переведены на реальные имена; отказ экспорта на стенде отдаёт плейбук. — `todo`
