...
- 283.4 Вторая точка входа — сверка **успешного** ответа с известными
  ...
  на горячем пути. Решение продуктовое и владельцем не принималось —
  пункт остаётся открытым намеренно, а не забыт. — `supervisor_pending`
...
- 287.2 Явно запрошенный scope, который владелец **вправе** выдать, должен
  доводить до экрана согласия, а не отклоняться заранее. Меняет поток
  авторизации — отложено решением владельца. — `todo`
- 287.3 Инструменты, которые заведомо не сработают при текущей регистрации,
  не показываются в `tools/list`. Меняет видимый набор инструментов у живых
  подключений — отложено решением владельца. — `todo`
