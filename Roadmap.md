...
## Этап 238. Понятная диагностика гварда упаковки — `todo`
...
- 238.1 Сообщение теста должно называть недостающие модули прямо: «модули X, Y
  не добавлены в wheel allowlist, добавьте их в `pyproject.toml`». — `todo`
...
## Этап 242. Диагностика на проде не должна создавать issue — `todo`
...
- 242.1 Отделить ручную диагностику от рантайма: пробник, который не пишет
  `error` и не создаёт событие. — `todo`
- 242.2 Проверить, какие ещё `RUNTIME_LOGGER.error` в коде срабатывают на
  штатных путях и создают ложные issue. — `todo`
