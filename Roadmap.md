...
## Этап 245. update_medical_card отказывает на существующих картах — `in_progress`
...
Состояние на 24.08.2026: код в проде, живая проверка на devtr6 зелёная
(запись и восстановление через инструмент, частичный `PUT` по-прежнему
`400 Patient does not exist`). Открыт только 245.6 — сообщение в Ветменеджер
про перевёрнутый порядок проверок; это внешняя коммуникация, не код.
