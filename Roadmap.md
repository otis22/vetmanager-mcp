...
## Этап 293. Однодневный диапазон счетов возвращает пусто, а любой — теряет последний день — `todo`
...
- 293.1 `get_invoices`: `date_from`/`date_to` переводятся на границы суток —
  ...
  переписывается и показывается красным по правильной причине: он сейчас
  защищает дефект. — `todo`
- 293.2 Аудит остальных сравнений голой даты с timestamp-колонкой:
  `get_debtors` (`client.last_visit_date` — `timestamp` по схеме),
  `_inactive_helpers`. Либо починить, либо записать в docstring, почему
  граница здесь верна. — `todo`
