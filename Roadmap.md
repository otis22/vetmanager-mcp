...
## Этап 240. Санитайзер съедает текст ошибки — `todo`
...
- 240.1 Найти правило санитайзера, которое глушит сообщение на пути
  `capture_tool_failure`, и понять, почему старый путь его не задевает. — `todo`
- 240.2 Сообщения upstream-ошибок (имена полей, коды, тексты валидации) должны
  доходить целиком; вырезаются только секреты и персональные данные. — `todo`
- 240.3 Тест: событие отказа инструмента с 406 сохраняет имя поля в тексте. — `todo`

## Этап 241. Дедупликация событий не работает — `todo`
...
- 241.1 Воспроизвести двойную отправку в тесте, а не только на проде. — `todo`
- 241.2 Понять, каким путём уходит доменное исключение мимо
  `mark_tool_error_as_handled`. — `todo`
