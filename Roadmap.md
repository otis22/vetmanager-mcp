...
- 246.4 Оценить, стоит ли гонять `redact` по всей таблице при каждом росте
  `REDACTION_VERSION`. — `todo`
...
## Этап 248. Нет отмены отправленной рассылки — `todo`
...
- 248.1 Написать в докстрингах `send_message_to_all`, `send_message_to_roles`,
  ...
  нельзя. — `done`
- 248.2 Оценить, не стоит ли требовать явного подтверждения перед отправкой
  всем. — `todo`
...
## Этап 255. Тестовые стенды перепутаны — `todo`
...
- 255.2 Записать оба контура в README рядом с `TEST_DOMAIN`. — `todo`
- 255.3 Решить, нужны ли два стенда вообще, или проверки свести на один. — `todo`
...
## Этап 282. Сессию нельзя отозвать на сервере — `todo`
...
- 282.1 Оценить, нужен ли отзыв сессии сейчас: срок жизни уже сокращён до
  суток пунктом 52.3.2, и цена хранения сессий в БД может оказаться выше
  остаточного риска. — `todo`
- 282.2 Если нужен — хранение сессий и отзыв; если нет — записать решение
  и закрыть этап `stop` с причиной, а не молчанием. — `todo`
