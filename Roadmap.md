...
## Этап 298. Цена продажи товара не записывается ни одним инструментом — `todo`
...
- 298.1 Чтение перед записью: инструмент показывает все строки цены товара с
  клиникой, единицей продажи, статусом и способом ценообразования — чтобы было
  видно, что строк несколько и какая из них нужна. — `todo`
- 298.2 Запись цены отдельной строки с обязательным превью «было → станет» и
  явным подтверждением. Строки с `price_formation=increase` запись цены не
  принимают: там меняется наценка, а не цена. — `todo`
- 298.3 Решение владельца о самом факте записи цены из MCP: цена — то, что
  видит клиент клиники, и ошибка здесь дороже почти любой другой. — `todo`
