...
- 246.3 Разобрать семь открытых отчётов и убедиться, что остаточных имён,
  кличек и адресов в свободном тексте нет. — `todo`
...
## Этап 247. Чтение после записи отдаёт устаревшие поля — `todo`

Источник: отчёт #46. Причина найдена на devtr6 23.08.2026 и оказалась не
кешем.

**`date_edit` не обновляется при записи.** Два успешных PUT по карте 6 подряд
(`201`, значение действительно изменилось и читается) оставили
`date_edit = 2020-05-13 12:45:00` — ровно тем же, что и `date_create`.
Отметка о правке не двигается никогда.

Для модели это разрушительно: `date_edit` — естественный способ убедиться, что
запись прошла. Она видит старую отметку и делает единственный доступный вывод —
изменения не сохранились. Отсюда формулировка «stale fields after REST update».

Вторая, более мелкая находка того же захода: `doRestUpdate` возвращает
`'data' => ['medicalcards' => $data]`, то есть **только присланные поля**, а не
обновлённую запись. И отвечает `201 Created` на обновление существующей
записи вместо `200`.

- 247.1 Проверить, у каких ещё сущностей `date_edit` не двигается при
  записи. — `todo`
- 247.2 Решить, чинить ли на нашей стороне (дослать `date_edit` в теле, раз
  сервер делает `array_merge`) или это правка на стороне Ветменеджера. — `todo`
- 247.3 Пока не починено — сказать в докстринге, что `date_edit` не является
  признаком успешной записи. — `todo`
- 247.4 Рассмотреть возврат полной записи вместо присланных полей. — `todo`
