...
- 293.3 Сторож: новую границу «голая дата против timestamp-колонки» нельзя
  добавить молча. Одно место, которое знает, какие поля Ветменеджера —
  timestamp, и проверка, что фильтры по ним строятся сутками. — `todo`
...
## Этап 297. Гонка двух выкатов выдавала себя за поломку приложения — `in_progress`
...
- 297.1 `concurrency: {group: deploy-prod, cancel-in-progress: false}` — выкаты
  ...
  ожидания. — `done`
- 297.2 Сообщение проверки секрета должно различать «файл не доехал» и «файл
  исчез после заливки». Пока не сделано: очередь убирает наблюдавшуюся причину,
  но не делает текст честным. — `todo`
