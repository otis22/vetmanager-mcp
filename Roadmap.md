...
## Этап 268. Удаление счетов через REST обходит предметную логику — `in_progress`
...
- 268.1 Убрать `delete_invoice` и `delete_invoice_document` из инструментов,
  реестра прав и обещанных составов групп. — `todo`
- 268.2 Сторож, не дающий вернуть их: инструмент с таким именем не должен
  регистрироваться. Показать красным. — `todo`
- 268.3 Записать запрет и его причину в `AssumptionLog.md` и в docstring
  соседних инструментов счёта, чтобы причина не потерялась. — `todo`
