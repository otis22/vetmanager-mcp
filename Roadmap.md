...
## Этап 310. График работы врачей: своё право и полный набор операций — `todo`
...
- 310.1 Заведено право `schedule.write`; запись в `timesheet` перевешена на
  ...
  снимок прав старого ключа перестал бы читаться. — `todo`
- 310.2 `create_timesheet` перестаёт отклоняться апстримом: `type` обязателен и
  целый, `shedule_id` уходит явно. — `todo`
- 310.3 Появляются `update_timesheet` и `delete_timesheet`. — `todo`
- 310.4 Второй слой перестаёт требовать `records.delete` на удаление смены:
  сопоставление `DELETE` по сущности с прежним умолчанием для всех остальных
  путей. Ослабление точечное, сторож этапа 270 остаётся красным для них. — `todo`
...
- 310.6 Снимок набора прав и экран выдачи называют новое право своим именем —
  график работы сотрудников, а не аналитика. — `todo`
- 310.7 Появляется `get_timesheet_types`: обязательный `type` — идентификатор
  из справочника, а не свободное значение, и взять его сегодня неоткуда. — `todo`
- 310.8 Строка «Удаление» в снимке прав называет и смены. Сегодня она зависит
  ...
  стало бы ложью для «Регистратуры». — `todo`
