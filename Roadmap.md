...
- 283.1 Счётчик `ToolError` по инструментам: без знаменателя «инжекция 0» не
  читается — это либо «ошибок не было», либо «было N, совпало 0». В Sentry
  такие ошибки не летят, они обработаны. — `todo`
- 283.2 Отпечаток, пришедший из отчёта, перестаёт быть ключом для инжекции:
  он описывает другой объект. Что делать с 20 проблемами, у которых нет ничего
  кроме него, — решить явно, а не оставить их молча нерабочими. — `todo`
- 283.3 Ревизия 11 правил против настоящих падений: правило принимается
  только вместе с доказательством, что оно совпало с живой ошибкой. — `todo`
- 283.4 Вторая точка входа — сверка **успешного** ответа с известными
  проблемами. Это работа на каждом удачном вызове, а не на редком отказе;
  цена считается до решения, решение принимается отдельно. — `todo`
- 283.5 Эффективность инжекции попадает в продуктовые метрики: доставки против
  ошибок, а не одно голое число событий. — `todo`
