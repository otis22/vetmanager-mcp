...
## Этап 128. Final release gate + production deploy — `in_progress`
...
- 128.3 Пройти pre-release checklist: secrets/env, migration plan, deployment notes, `--target production`, отсутствие новых workflow/review findings. — `in_progress`
- 128.4 Выполнить pre-deploy backup production PostgreSQL и зафиксировать rollback point по `artifacts/operations-readiness-vetmanager-mcp-ru.md`. — `todo`
- 128.5 Выполнить production deploy через canonical script (`scripts/deploy_server.sh` или `scripts/sync_and_deploy_server.sh`) с production profile. — `todo`
- 128.6 Прогнать post-deploy smoke: `scripts/post_deploy_smoke_checks.sh`, `/healthz`, `/readyz`, `/metrics`, `/mcp`, web `landing -> login -> account`, MCP call существующим bearer token. — `todo`
- 128.7 Выполнить post-release наблюдение: проверить `runtime/security/audit` logs, отсутствие всплеска `vetmanager_upstream_failures_total` и auth failures, затем зафиксировать release outcome и commit SHA в `AssumptionLog.md`. — `todo`
...
## Этап 129. Post-review fix plan for stages 124-126 — `in_progress`
...
- 129.5 `rate_limit_backend.py::RedisRateLimitBackend.record_hit`: сделать запись атомарной (`ZADD + EXPIRE` через pipeline/Lua), чтобы не оставлять ключ без TTL при частичном падении между двумя await. — `todo`
- 129.6 `rate_limit_backend.py`: добавить явный lifecycle cleanup для async Redis client (`aclose()`/shutdown hook/reset helper), а не полагаться на GC после `reset_rate_limit_backend()`. — `todo`
...
- 129.7 `tools/_inactive_helpers.py`: вынести generic helper для paginated batched day lookup, чтобы invoice/medcard paths не дублировали одинаковую chunk/pagination/window logic в двух местах. — `todo`
- 129.8 `vetmanager_connection_service.py`: свести `save_domain_api_key_connection` / `save_user_token_connection` / `save_user_login_password_connection` к одному `_save_connection(...)` helper с общим lock/transaction/update-old path. — `todo`
- 129.9 `web_security.py`: объединить `check_rate_limit(...)` и `record_rate_limit_hit(...)` в один `consume_rate_limit(...)` API; это одновременно упростит auth routes и закроет split-brain между check и record semantics. — `todo`
