...
## Этап 236. Полезный контекст в событиях трекинга — `in_progress`
...
- 236.2 `release` = git SHA сборки, проброс через build arg и
  `ERROR_TRACKING_RELEASE`. — `todo`
- 236.3 Убрать дублирующую отправку: одно падение — одно событие. — `todo`
- 236.4 `transaction` = имя инструмента вместо внутреннего URL контейнера. — `todo`
- 236.5 Отказ валидации фильтра (`Unknown filter property`) — это отклонение
  ...
  этапа 235.5, но не как инцидент. — `todo`
