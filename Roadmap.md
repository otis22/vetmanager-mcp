...
- 244.1 Убрать `calc_percents` из докстринга `get_users`. — `todo`
- 244.2 Тест, который сверяет список в докстринге с реестром для каждого
  list-инструмента и падает при расхождении в любую сторону. — `todo`
...
- 244.4 Проверить, не обещают ли докстринги вырезанные поля где-то ещё —
  `passwd`, `login`, `last_change_pwd_date`, `user_inn`, `passport_series`. — `todo`
...
- 245.1 Читать карту перед записью и добавлять в тело `patient_id`,
  `doctor_id`, `clinic_id` из текущей записи. — `todo`
- 245.2 Тест: обновление одного поля медкарты проходит. — `todo`
...
- 245.4 Проверить, нет ли той же беды у `update_admission`, `update_invoice`,
  `update_hospitalization` — у них те же предполётные проверки в контроллерах. — `todo`
...
- 247.1 Привести ключ к тому, что реально возвращает `entity_from_path`, —
  и закрыть класс ошибки тестом, а не правкой одной строки. — `todo`
- 247.2 Сверить весь `SHORT_TTL_ENTITIES` с фактическими путями всех
  инструментов; `hospital` добавить. — `todo`
...
- 247.4 Сказать в докстринге, что `date_edit` не двигается при обновлении и
  не годится для проверки факта записи. — `todo`
...
- 250.1 Параметр `invoice_id` у `get_closing_of_invoices`: два запроса по обеим
  сторонам, объединение и дедупликация по `id`. — `todo`
- 250.2 В докстринге объяснить, что счёт и платёж встречаются с любой стороны,
  и что `invoice_id` — правильный способ спросить. — `todo`
- 250.3 Тест на данных, где счёт лежит с обеих сторон. — `todo`
...
- 255.1 Выяснить, какой аккаунт и домен стоит за bearer-токеном клиента. — `todo`
