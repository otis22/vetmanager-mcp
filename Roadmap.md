...
## Этап 230. Рестарт не рвёт клиентские сессии — `in_progress`
...
- 230.1 Обработчик SIGTERM: закрыть пул asyncpg и остальные async-ресурсы до
  ...
  вызывается на закрытом loop. — `todo`
...
- 237.4 Остаточный обрыв: выкатка `86f286e` 23.08.2026 в 14:29 МСК закрыла
  ...
  и остановкой listener. — `todo` (понижено до наблюдения: событие было
  однократным, две последующие остановки по 5 сессий прошли без обрывов;
  разбирать при повторении).
...
## Этап 241. Дедупликация событий не работает — `in_progress`
...
- 241.3 Одно падение — одно событие, подтверждённое на живой выкатке. — `todo`
