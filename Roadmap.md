...
## Этап 269. Запуск отчётов отделён от чтения аналитики — `in_progress`
...
- 269.1 Перевести `create_report_ai_job`, `confirm_report_ai_job_candidate`
  и `start_report_export` с `analytics.read` на право отчётов. — `todo`
- 269.2 Синхронизировать карту REST-путей: `/report-ai-job` и
  `/report/StartReport` требуют того же права. — `todo`
- 269.3 Тесты: «Регистратура», «Врач» и «Только чтение» получают отказ на
  запуск отчёта; «Аналитик» и полный доступ — проходят. — `todo`
