...
## Этап 225. Диагнозы ломают update_medical_card — `todo`

Источник: живой прогон 23.08.2026 после выкатки этапа 245. Первый барьер снят —
«Patient does not exist» больше нет, — но на стенде `devslon67` вылез второй,
независимый:
...
Проверено сырым REST на трёх картах: **любая карта с проставленным диагнозом
отдаёт 500**, и обновиться могут только карты, где `diagnos = '0'`. Передача
`diagnoses: [19]` или `diagnoses: [{"id": 19}]` не помогает — падает так же.
Единственное, что снимает ошибку, — `diagnos: ""`, но это **стирает диагноз**,
поэтому обходным путём считаться не может.

Природа ясна: легаси-колонка `diagnos` хранит число строкой, а новый код
присваивает её типизированному свойству `array $diagnoses`. Дефект на стороне
Ветменеджера, но для клиники это выглядит как наш неработающий инструмент.

Важно, что на `devtr6` та же запись проходит с `201`. Значит стенды на разных
сборках, и на новой сборке инструмент нерабочий для реальных карт — а карта без
диагноза как раз нетипична.

- 225.1 Сообщить в Ветменеджер: воспроизведение, тело запроса, три карты. — `todo`
- 225.2 Понять, есть ли формат, который сохраняет диагноз и не падает; UI как-то
  же сохраняет карты. — `todo`
