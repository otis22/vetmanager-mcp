...
## Этап 271. Список инструментов не фильтруется по правам — `in_progress`
...
- 271.1 Фильтровать `tools/list` по скоупам токена. — `todo`
- 271.2 Не ломать инструменты, доступные без прав (`report_problem`,
  `get_report_ai_prompt_helper`), и путь без HTTP-запроса. — `todo`
- 271.3 Тесты на состав списка для каждой группы. — `todo`
