...
async def validate_oauth_authorize_request(session: AsyncSession, params) -> dict:
    ...
    try:
        ...
        raise OAuthRequestError("invalid_scope", str(exc)) from exc
    # Этап 287.2. Здесь стояло сравнение запрошенных прав с заявленными клиентом
    # при регистрации. Регистрация открытая — клиент сам присылает `scope`, и
    # владелец в этот момент не участвует, — поэтому сравнение шло с
    # собственным заявлением клиента: тому, кому нужны широкие права, хватало
    # заявить их при регистрации (на бою так сделали 14 клиентов из 27), а
    # отклонялся ровно тот, кто попросил честно и явно. Границей доступа это не
    # было: выданное всё равно равно пресету владельца
    # (`narrow_oauth_authorize_request_scope`). Проверка снята, а вместе с ней —
    # расчёт запрошенных tool scopes: он остался бы мёртвым, но лежал бы рядом
    # с величинами, которые на доступ влияют (finding внешнего ревью).
