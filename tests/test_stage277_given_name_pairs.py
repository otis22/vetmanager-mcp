...
def _clean(row_count: int) -> float:
    """Секунды на очистку отчёта заданного размера."""
    rows = [
        {
...
def test_cleaning_cost_grows_with_the_report_not_faster():
    """Этап 304. Раньше здесь стоял порог в секундах, и он падал от загрузки
    машины: 06.09.2026 набор дал 4.28 с при пороге 3 с, а тот же тест в одиночку
    — около секунды, три прогона подряд. Порог в секундах отвечает на вопрос
    «сколько сейчас соседей по машине», а не «не деградировала ли очистка».

    Сравниваются два замера **в одном прогоне**: загрузка действует на оба
    одинаково и из отношения уходит. Настоящий риск здесь — квадратичный обход
    (поиск пар имён по всему отчёту), и порог проверен на нём, а не выведен из
    головы: 06.09.2026 замерено, что честная квадратичная работа даёт отношение
    **129** при десятикратном объёме, а нынешняя очистка — **10.5**. Порог 30
    стоит между ними с почти трёхкратным запасом от текущего значения.

    Устойчивость проверена и с другой стороны: тест зелёный, когда рядом
    молотят четыре параллельных прогона, — то есть ровно в тех условиях, в
    которых прежний порог в секундах и падал.
    """
    small = _clean(1_000)
    large = _clean(10_000)

    # Замер меньше миллисекунды — шум таймера, а не результат: сравнивать нечего.
    assert small > 0.001, f"замер на 1000 строк слишком мал для сравнения: {small:.5f}s"

    ratio = large / small
    assert ratio < 30, (
        f"очистка растёт быстрее объёма: 1000 строк — {small:.3f}s, "
        f"10000 — {large:.3f}s, во сколько раз дороже: {ratio:.1f} при десятикратном объёме"
    )


def test_cleaning_a_full_report_stays_within_a_crude_ceiling():
    """Грубый предохранитель с большим запасом: он ловит остановку, а не
    замедление, и потому не зависит от того, чем ещё занята машина."""
    assert _clean(10_000) < 60
